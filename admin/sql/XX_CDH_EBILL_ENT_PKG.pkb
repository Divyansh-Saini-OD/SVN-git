...
AS
 ...
 BEGIN
    XX_CDH_CUST_UTIL_BO_PVT.log_msg(0, '(+)XX_CDH_EBILL_ENT_PKG.insert_epdf_entities(+)');

    x_retcode := 0;


    SELECT NVL(FND_PROFILE.VALUE ('XXOD_EBL_EMAIL_STD_SUB_CONSOLI'),'Your Electronic Billing for the period ' || lc_amp || ' DATEFROM to  ' || lc_amp || ' DATETO for account  ' || lc_amp || ' AOPSNUMBER.')
        INTO l_subject_cons
    FROM DUAL;

    SELECT NVL(FND_PROFILE.VALUE ('XXOD_EBL_EMAIL_STD_SUB_STAND'),'Your Electronic Billing for the period  ' || lc_amp || ' DATEFROM to  ' || lc_amp || ' DATETO for account  ' || lc_amp || ' AOPSNUMBER.')
    ...
    SELECT NVL(FND_PROFILE.VALUE ('XXOD_EBL_EMAIL_STD_MSG'),'Dear Customer,<br><br>Attached is your electronic billing for  ' || lc_amp || ' DATEFROM to  ' || lc_amp || ' DATETO.<br>For questions regarding billing format, please contact electronicbilling@officedepot.com.<br>For account related questions, please call 1-800-721-6592.')
 ...
 FOR l in missing_epdf LOOP
   ...
   -- Insert Data into EBIL Main
    ...
    XX_CDH_EBL_MAIN_PKG.insert_row(
      ...
      XX_CDH_CUST_UTIL_BO_PVT.log_msg(0, '(=)XX_CDH_EBILL_ENT_PKG.insert_epdf_entities, after XX_CDH_EBL_MAIN_PKG.insert_row(=)');
    ...
    END;
        ...
        ELSE
            ...
            l_subject := 'Your Electronic Billing for the period  ' || lc_amp || ' DATEFROM to  ' || lc_amp || ' DATETO for account  ' || lc_amp || ' AOPSNUMBER.';
            l_field_cons_inv  := l_field_invoice;
      ...
      XX_CDH_EBL_TRANS_DTL_PKG.insert_row(
        ...
        XX_CDH_CUST_UTIL_BO_PVT.log_msg(0, '(=)XX_CDH_EBILL_ENT_PKG.insert_epdf_entities, after XX_CDH_EBL_TRANS_DTL_PKG.insert_row(=)');
        ...
        XX_CDH_EBL_FILE_NAME_DTL_PKG.insert_row(
         ...
         XX_CDH_CUST_UTIL_BO_PVT.log_msg(0, '(=)XX_CDH_EBILL_ENT_PKG.insert_epdf_entities, after XX_CDH_EBL_FILE_NAME_DTL_PKG.insert_row, ln_ebl_file_name_id: ' || ln_ebl_file_name_id);
    ...
    XX_CDH_CUST_UTIL_BO_PVT.log_msg(0, 'XX_CDH_EBILL_ENT_PKG.insert_epdf_entities, Success, cust_account_id:,'|| l.cust_acc_id);

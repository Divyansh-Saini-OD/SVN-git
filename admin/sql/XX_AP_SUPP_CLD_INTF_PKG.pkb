...
--|                                                                            |
--+============================================================================+
FUNCTION xx_custom_sup_traits ( p_sup_trait IN VARCHAR2,
...
BEGIN
  ...
  LOOP
	...
	END IF;
	/* 
    IF v_error_flag='N' AND v_buss_Flag = 'Y' THEN
       UPDATE xx_ap_cld_supp_bcls_stg
          SET bcls_process_Flag = 7,
              vendor_id           = cur.vendor_id,
              process_Flag        ='Y'
...
--|                                                                            |
--+============================================================================+
PROCEDURE update_supplier(x_ret_code OUT NUMBER ,
...
BEGIN
  ...
  LOOP
	...
	IF lr_existing_vendor_rec.vendor_id IS NOT NULL THEN
        ...
        lr_vendor_rec.vendor_type_lookup_code:=UPPER(c_sup.vendor_type_lookup_code);
	    lr_vendor_rec.organization_type_lookup_code:=UPPER(c_sup.organization_type);
...
IS 
  ...
  IS
    SELECT *
     ...
     WHERE xas.create_flag  = 'N'
       ...
       AND xas.request_id           = gn_request_id
       AND cont_target              ='EBS';
  CURSOR c_contact_infor(v_vendor_id      NUMBER,
  ...
  CURSOR c_cont_title (v_vendor_id      NUMBER,
                       v_vendor_site_id NUMBER,
					   v_first_name     VARCHAR2,
					   v_last_name      VARCHAR2)
  IS
...
BEGIN
  ...
  LOOP
    FOR r_cont_info IN c_contact_infor(r_cont.vendor_id , 
	                                   r_cont.vendor_site_id ,
									   r_cont.first_name,
									   r_cont.last_name )
    LOOP
      BEGIN
	  ...
	  THEN
        ...
        ap_vendor_pub_pkg.update_vendor_contact_public (p_api_version => 1.0, 
														...
														x_msg_data => x_msg_data );
        COMMIT;
        print_debug_msg(p_message=> l_program_step||'X_RETURN_STATUS ap_vendor_pub_pkg = ' || x_return_status, 
		                p_force  => TRUE);
        print_debug_msg(p_message=> l_program_step||'fnd_api.g_ret_sts_success ap_vendor_pub_pkg ' || fnd_api.g_ret_sts_success , 
		                p_force  => TRUE);
        print_debug_msg(p_message=> l_program_step||'X_MSG_COUNT ap_vendor_pub_pkg = ' || x_msg_count,
                  		p_force  => TRUE);
        IF x_return_status <>'S' AND x_msg_count > 0 
		THEN
          print_debug_msg(p_message=> l_program_step||'x_return_status ap_vendor_pub_pkg' || x_return_status , 
		                  p_force  => TRUE);
          print_debug_msg(p_message=> l_program_step||'fnd_api.g_ret_sts_success ap_vendor_pub_pkg ' || fnd_api.g_ret_sts_success ,
          ...
          LOOP
            ...
            print_debug_msg(p_message=> l_program_step||'The API call failed with error ap_vendor_pub_pkg ' || l_msg ,
               			    p_force  => TRUE);
          END LOOP;
          l_process_flag:='E';
          ------Update for Tiebacking
        ELSE
          print_debug_msg(p_message=> l_program_step||'The API call ap_vendor_pub_pkg ended with SUCESSS status', 
		                  p_force  => TRUE);
          l_process_flag := 'Y';
          l_msg          := '';
          -----------------------Update the status if API successfully updated the record.
    ...
    LOOP
	  ...
	  THEN
          ...
          lv_contact_title_rec.job_title      := r_cont.title;
          fnd_msg_pub.initialize; --to make msg_count 0
          x_return_status := NULL;
          x_msg_count     := NULL;
          x_msg_data      := NULL;
        hz_party_contact_v2pub.update_org_contact ( p_init_msg_list   => fnd_api.g_false, 
													...
													x_msg_data => x_msg_data );
        COMMIT;
        print_debug_msg(p_message=> l_program_step||'X_RETURN_STATUS HZ_PARTY_CONTACT_V2PUB = ' || x_return_status, 
		                p_force  => TRUE);
        print_debug_msg(p_message=> l_program_step||'fnd_api.g_ret_sts_success HZ_PARTY_CONTACT_V2PUB ' || fnd_api.g_ret_sts_success , 
		                p_force  => TRUE);
        print_debug_msg(p_message=> l_program_step||'X_MSG_COUNT HZ_PARTY_CONTACT_V2PUB = ' || x_msg_count,
                 		p_force  => TRUE);
        IF x_return_status <>'S' AND x_msg_count > 0 
		THEN
          print_debug_msg(p_message=> l_program_step||'x_return_status  HZ_PARTY_CONTACT_V2PUB' || x_return_status , 
          ...
          LOOP
            fnd_msg_pub.get ( p_msg_index => i , 
							  ...
							  p_data => l_msg , 
							  p_msg_index_out => ln_msg_index_num );
            print_debug_msg(p_message=> l_program_step||'The API call failed with error ' || l_msg , 
			                p_force  => TRUE);
          END LOOP;
          l_process_flag:='E';
          ------Update for Tiebacking
        ELSE
          print_debug_msg(p_message=> l_program_step||'The API call HZ_PARTY_CONTACT_V2PUB ended with SUCESSS status', 
		                  p_force  => TRUE);
          l_process_flag:='Y';
          l_msg         :='';
          -----------------------Update the status if API successfully updated the record.
    ...
    WHEN OTHERS 
	THEN
      print_debug_msg(p_message=> l_program_step||'In Exception to update records'||SQLERRM, 
	                  p_force  => TRUE);
    END ;
  END LOOP;
EXCEPTION
...
--|                                                                               |
--+===============================================================================+
PROCEDURE validate_supplier_records(x_ret_code OUT NUMBER,
									x_return_status OUT VARCHAR2,
									x_err_buf OUT VARCHAR2)
IS
...
--==========================================================================================
TYPE l_sup_tab IS TABLE OF XX_AP_CLD_SUPPLIERS_STG%rowtype INDEX BY binary_integer;
  ...
  WHEN c_dup_supplier_chk_stg%notfound;
    ...
    UPDATE xx_ap_cld_suppliers_stg
       SET supp_process_flag = gn_process_status_error ,
		   ...
		   error_msg           = l_stg_sup_dup_cnt||' records exist for Supplier Name '||l_stg_sup_name||' in the staging table.'
...
--|                                                                            |
--+============================================================================+
PROCEDURE validate_supplier_site_records(x_ret_code OUT NUMBER,
										 x_return_status OUT VARCHAR2,
										 x_err_buf OUT VARCHAR2
										)
IS
  ...
  -- Variables Declaration used for getting the data into PL/SQL Table for Processing the API
  --==========================================================================================
TYPE l_sup_site_and_add_tab IS TABLE OF XX_AP_CLD_SUPP_SITES_STG%rowtype INDEX BY binary_integer;
...
BEGIN
  ...
  LOOP
    ...
    FETCH c_get_country_code 
	 INTO l_site_country_code;
...
IS
  ...
  -- Variables Declaration used for getting the data into PL/SQL Table for Processing the API
  --==========================================================================================
TYPE l_sup_cont_tab IS TABLE OF xx_ap_cld_supp_contact_stg%ROWTYPE INDEX BY BINARY_INTEGER;
l_sup_cont l_sup_cont_tab;
  ...
  -- Cursor Declarations for Duplicate check of Suppliers in Interface table
  --=========================================================================
  CURSOR c_dup_supplier_chk_int(c_first_name      VARCHAR2,
                                c_last_name       VARCHAR2,
                                c_vendor_id       NUMBER,
  ...
  CURSOR c_sup_contact_exist (c_vendor_ID        NUMBER ,
                              c_vendor_site_id   NUMBER,
							  c_first_name       VARCHAR2,
							  c_last_name        VARCHAR2)
  IS
    SELECT COUNT(1)
  ...
  l_msg_count                  NUMBER        := 0;
  l_trans_count                NUMBER        := 0;
  l_inval_records              NUMBER        := 0;
  l_val_records                NUMBER        := 0;
...
BEGIN
  ...
  LOOP
    ...
    gc_step                   := 'SITE_CONT';
    gc_error_msg              := '';
    OPEN c_dup_supplier_chk_int(
	                            TRIM(UPPER(l_sup_site_cont_type.first_name)),
	                            TRIM(UPPER(l_sup_site_cont_type.last_name)),
								l_sup_site_cont_type.supp_id,
								l_sup_site_cont_type.supp_site_id
							   );
    FETCH c_dup_supplier_chk_int INTO l_int_first_name, l_int_last_name;
    CLOSE c_dup_supplier_chk_int;
	
    IF l_int_first_name IS NOT NULL 
	THEN
      gc_error_site_status_flag := 'Y';
      print_debug_msg(p_message => l_program_step||' : ERROR: XXOD_SUP_EXISTS_IN_INT : Suppiler '|| l_sup_site_cont_type.supplier_name
	                                            ||' already exist in   Interface table with ' ,
					  p_force   => TRUE);
      insert_error (
	                p_program_step   => gc_step ,
	                p_primary_key    => l_sup_site_cont_type.supplier_name ,
					p_error_code     => 'XXOD_SUP_EXISTS_IN_INT' ,
					p_error_message  => 'Suppiler ' ||l_sup_site_cont_type.supplier_name||' already exist in Interface table ' ||' .' ,
					p_stage_col1     => 'SUPPLIER_NAME' ,
					p_stage_val1     => l_sup_site_cont_type.supplier_name ,
					p_stage_col2     => NULL ,
					p_stage_val2     => NULL , 
					p_table_name     => g_sup_cont_table 
				   );
    END IF;
    --====================================================================
    -- Note Required
    --====================================================================
    IF l_sup_site_cont_type.first_name IS NULL 
	THEN
        gc_error_site_status_flag        := 'Y';
        print_debug_msg(p_message => gc_step||' ERROR: FIRST_NAME:'||l_sup_site_cont_type.first_name|| ': XXOD_FIRST_NAME_NULL:FIRST_NAME cannot be NULL',
                		p_force   => FALSE);
        insert_error (
		              p_program_step    => gc_step ,
		              p_primary_key     => l_sup_site_cont_type.supplier_name ,
					  p_error_code      => 'XXOD_FIRST_NAME_NULL' ,
					  p_error_message   => 'First Name cannot be NULL' ,
					  p_stage_col1      => 'LAST_NAME_PREFIX' ,
					  p_stage_val1      => l_sup_site_cont_type.first_name ,
					  p_table_name      => g_sup_cont_table 
					 );
    END IF;
    IF l_sup_site_cont_type.last_name IS NULL 
	THEN
        gc_error_site_status_flag       := 'Y';
        ...
        insert_error (p_program_step   => gc_step ,
		              p_primary_key    => l_sup_site_cont_type.supplier_name ,
					  p_error_code     => 'XXOD_LAST_NAME_NULL' ,
					  p_error_message  => 'Last Name cannot be NULL' ,
					  p_stage_col1     => 'LAST_NAME_PREFIX' ,
					  p_stage_val1     => l_sup_site_cont_type.last_name ,
					  p_table_name     => g_sup_cont_table 
					 );
    END IF;
	...
	THEN
      print_debug_msg(p_message=> gc_step||' l_sup_site_cont_type.update_flag is '||l_sup_site_cont_type.CREATE_FLAG ,
	                  p_force  => FALSE);
      print_debug_msg(p_message=> gc_step||' l_sup_site_cont_type.supplier_name is '||UPPER(l_sup_site_cont_type.supplier_name) ,
	                  p_force  => FALSE);
      print_debug_msg(p_message=> gc_step||' l_sup_site_cont_type.supplier_number is '||UPPER(l_sup_site_cont_type.supplier_number) ,
	                  p_force  => FALSE);
      print_debug_msg(p_message=> gc_step||' l_sup_site_cont_type.vendor_site_code is '||UPPER(l_sup_site_cont_type.vendor_site_code) ,
	                  p_force  => FALSE);					  
      l_sup_cont_exist_cnt := 0;
      OPEN c_sup_contact_exist(
	                           l_sup_site_cont_type.supp_id,
	                           l_sup_site_cont_type.supp_site_id,
							   l_sup_site_cont_type.first_name,
							   l_sup_site_cont_type.last_name
							  );
      FETCH c_sup_contact_exist INTO l_sup_cont_exist_cnt;
      CLOSE c_sup_contact_exist;
      IF l_sup_cont_exist_cnt           > 0 
	  THEN
         l_sup_CREATE_FLAG                := 'N';--update the supplier
    ...
    print_debug_msg(p_message=> gc_step||' After supplier site existence check - gc_error_site_status_flag is '||gc_error_site_status_flag ,
	                p_force  => FALSE);
    print_debug_msg(p_message=> gc_step||' After supplier site existence check - l_sup_create_flag is '||l_sup_CREATE_FLAG ,
	                p_force  => FALSE);
    set_step('Supplier Contact Existence Check Completed');
    IF gc_error_site_status_flag = 'N' 
	THEN -- After Supplier Site Existence Check Completed
      --===============================================================================================
      -- Validating the Supplier Site - Address Details - Phone area code
      --===============================================================================================
      IF l_sup_site_cont_type.area_code IS NOT NULL 
	  THEN
        IF (NOT (isnumeric(l_sup_site_cont_type.area_code)) OR (LENGTH(l_sup_site_cont_type.area_code) <> 3 )) 
		THEN
          gc_error_site_status_flag := 'Y';
          print_debug_msg(p_message=> gc_step||' ERROR: PHONE_AREA_CODE:'||l_sup_site_cont_type.area_code||': XXOD_PHONE_AREA_CODE_INVALID: Phone Area Code '                                   ||l_sup_site_cont_type.area_code||' should be numeric and 3 digits.' ,
		                  p_force  => TRUE);
          insert_error (p_program_step   => gc_step ,
		                p_primary_key    => l_sup_site_cont_type.supplier_name , 
						p_error_code     => 'XXOD_PHONE_AREA_CODE_INVALID' ,
						p_error_message  => 'Phone Area Code '||l_sup_site_cont_type.area_code||' should be numeric and 3 digits.' ,
						p_stage_col1     => 'PHONE_AREA_CODE' ,
						p_stage_val1     => l_sup_site_cont_type.area_code ,
						p_stage_col2     => NULL ,
						p_stage_val2     => NULL ,
						p_table_name     => g_sup_cont_table );
        END IF; -- IF (NOT (isNumeric(l_sup_site_type.PHONE_AREA_CODE))
      ...
      -- Validating the Supplier Site - Address Details - Phone Number
      --===============================================================================================
      IF l_sup_site_cont_type.phone IS NOT NULL 
	  THEN
        IF (LENGTH(l_sup_site_cont_type.phone) NOT IN (7,8) ) 
		THEN -- Phone Number length is 7 and 1 digit count for '-'
          gc_error_site_status_flag := 'Y';
          print_debug_msg(p_message=> gc_step||' ERROR: PHONE_NUMBER:'||l_sup_site_cont_type.phone||': XXOD_PHONE_NUMBER_INVALID: Phone Number '
		                                     ||l_sup_site_cont_type.phone||' should be 7 digits.' ,
		                  p_force  => TRUE);
          insert_error (p_program_step  => gc_step ,
		                p_primary_key   => l_sup_site_cont_type.supplier_name , 
						p_error_code    => 'XXOD_PHONE_NUMBER_INVALID' ,
						p_error_message => 'Phone Number '||l_sup_site_cont_type.phone||' should be 7 digits.' ,
						p_stage_col1    => 'PHONE_NUMBER' ,
						p_stage_val1    => l_sup_site_cont_type.phone ,
						p_stage_col2    => NULL ,
						p_stage_val2    => NULL ,
						p_table_name    => g_sup_cont_table );
        END IF; -- IF (NOT (isNumeric(l_sup_site_type.PHONE_NUMBER))
      ...
      -- Validating the Supplier Site - Address Details - Fax area code
      --===============================================================================================
      IF l_sup_site_cont_type.fax_area_code IS NOT NULL 
	  THEN
        IF (NOT (isnumeric(l_sup_site_cont_type.fax_area_code)) OR (LENGTH(l_sup_site_cont_type.fax_area_code) <> 3 )) 
		THEN
          gc_error_site_status_flag := 'Y';
          print_debug_msg(p_message=> gc_step||' ERROR: FAX_AREA_CODE:'||l_sup_site_cont_type.fax_area_code||': XXOD_FAX_AREA_CODE_INVALID: Fax Area Code '
		                                     ||l_sup_site_cont_type.fax_area_code||' should be numeric and 3 digits.' ,
		                  p_force=> true);
          insert_error (p_program_step  => gc_step ,
		                p_primary_key   => l_sup_site_cont_type.supplier_name , 
						p_error_code    => 'XXOD_FAX_AREA_CODE_INVALID' ,
						p_error_message => 'Fax Area Code '||l_sup_site_cont_type.fax_area_code||' should be numeric and 3 digits.' ,
						p_stage_col1    => 'FAX_AREA_CODE' ,
						p_stage_val1    => l_sup_site_cont_type.fax_area_code ,
						p_stage_col2    => NULL ,
						p_stage_val2    => NULL ,
						p_table_name    => g_sup_cont_table );
        END IF; -- IF (NOT (isNumeric(l_sup_site_type.FAX_AREA_CODE))
      ...
      -- Validating the Supplier Site - Address Details - Fax Number/FAX updated by Priyam
      --===============================================================================================
      IF l_sup_site_cont_type.fax IS NOT NULL 
	  THEN
        IF (LENGTH(l_sup_site_cont_type.fax) NOT IN (7,8) ) 
		THEN -- Fax Number length is 7 and 1 digit count for '-'
          gc_error_site_status_flag := 'Y';
          print_debug_msg(p_message=> gc_step||' ERROR: FAX_NUMBER:'||l_sup_site_cont_type.fax||': XXOD_FAX_NUMBER_INVALID: Fax Number '
		                                     ||l_sup_site_cont_type.fax||' should be 7 digits.' ,
		                  p_force=> true);
          insert_error (
		                p_program_step   => gc_step ,
		                p_primary_key    => l_sup_site_cont_type.supplier_name , 
						p_error_code     => 'XXOD_FAX_NUMBER_INVALID' ,
						p_error_message  => 'Fax Number '||l_sup_site_cont_type.fax||' should be 7 digits.' ,
						p_stage_col1     => 'FAX_NUMBER' ,
						p_stage_val1     => l_sup_site_cont_type.fax ,
						p_stage_col2     => NULL ,
						p_stage_val2     => NULL ,
						p_table_name     => g_sup_cont_table 
						);
  ...
  print_debug_msg(p_message=> gc_step ||' List of the contact failed prefixes is '||l_error_prefix_list ,
                  p_force  => TRUE);
  l_program_step := '';
  print_debug_msg(p_message=> l_program_step||': Do Bulk Update for all Contact Records ' ,
                  p_force  => TRUE);
  print_debug_msg(p_message=> l_program_step||'l_sup_cont.COUNT '||l_sup_cont.COUNT ,
                  p_force  => TRUE);
  IF l_sup_cont.COUNT > 0 
  THEN
    BEGIN
      print_debug_msg(p_message=> l_program_step||'gn_request_id ' ||gn_request_id ,
	                  p_force  => TRUE);
	...
	THEN
      ...
      -- To Insert into Common Error Table
      --============================================================================
      insert_error (p_program_step  => 'SITE_CONT' ,
	                p_primary_key   => NULL ,
					p_error_code    => 'XXOD_BULK_UPD_Contact' ,
...
THEN
  l_err_buff      := SQLCODE || ' - '|| SUBSTR (sqlerrm,1,3500);
  print_debug_msg(p_message => 'ERROR: Exception in validate_Supplier_Contact_records() API - '|| l_err_buff ,
                  p_force => TRUE);
  x_ret_code      := '2';
  x_return_status := 'E';
  x_err_buf       := l_err_buff;
...
--|                                                                            |
--+============================================================================+
PROCEDURE load_supplier_interface(x_processed_records OUT NUMBER ,
...
BEGIN
  ...
  IF l_sup_val_load_cnt >0 THEN
     ...
     IF l_rept_req_id != 0 THEN
        ...
        l_bflag:=fnd_concurrent.wait_for_request(l_rept_req_id,5,0,l_phas_out,l_status_out ,
												 l_dev_phase_out,l_dev_status_out,l_message_out 
												);
        BEGIN
...
--|                                                                            |
--+============================================================================+
PROCEDURE load_supplier_site_interface(x_ret_code OUT NUMBER ,
									   x_return_status OUT VARCHAR2 ,
									   x_err_buf OUT VARCHAR2 
									  )
IS
  ...
  -- Variables Declaration used for getting the data into PL/SQL Table for processing
  --=========================================================================================
TYPE l_sup_site_add_tab IS TABLE OF XX_AP_CLD_SUPP_SITES_STG%rowtype INDEX BY binary_integer;
...
--|                                                                            |
--+============================================================================+
PROCEDURE load_supplier_cont_interface(x_ret_code OUT NUMBER ,
									   x_return_status OUT VARCHAR2 ,
									   x_err_buf OUT VARCHAR2 
									  )
IS
...
BEGIN
  ...
  IF l_sup_val_load_cnt >0 THEN
     ...
     fnd_global.apps_initialize ( user_id => l_user_id ,resp_id => l_resp_id ,resp_appl_id => l_resp_appl_id );
     l_rept_req_id := fnd_request.submit_request (application => 'SQLAP' ,program => 'APXSCIMP',
												  description => '' , start_time => sysdate ,
												  sub_request => false ,argument1 => 'NEW' ,
												  argument2 => 1000 ,argument3 => 'N' ,
												  argument4 => 'N' ,argument5 => 'N'
												 );
     COMMIT;
     IF l_rept_req_id != 0 THEN

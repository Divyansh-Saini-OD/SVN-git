CREATE OR REPLACE PACKAGE BODY xx_ap_supp_cld_intf_pkg
  ...
  -- | 4.2     12-May-2020    Shanti Sethuraj    Modified for jira NAIT-126421                   |
  -- |===========================================================================================+
AS
  /*********************************************************************
...
--|                                                                            |
--+============================================================================+
PROCEDURE get_receiving_details(
  ...
  LOOP
    ...
    IF (cur.attribute8 LIKE 'TR%' AND cur.attribute8 NOT LIKE '%RTV%' ) THEN -- Added as per Version 2.2
      ...
      IF ln_tol_count = 0 THEN
        v_tol_flag   := xx_custom_tolerance(cur.vendor_id, cur.vendor_site_id, cur.org_id, 'Y', TO_NUMBER(cur.favourable_price_pct) , TO_NUMBER(cur.max_price_amt) , TO_NUMBER(cur.min_chargeback_amt) , TO_NUMBER(cur.max_freight_amt) , TO_NUMBER(cur.dist_var_neg_amt) , TO_NUMBER(cur.dist_var_pos_amt) );
      ELSE
        v_tol_flag := xx_custom_tolerance(cur.vendor_id, cur.vendor_site_id, cur.org_id, 'N', TO_NUMBER(cur.favourable_price_pct) , TO_NUMBER(cur.max_price_amt) , TO_NUMBER(cur.min_chargeback_amt) , TO_NUMBER(cur.max_freight_amt) , TO_NUMBER(cur.dist_var_neg_amt) , TO_NUMBER(cur.dist_var_pos_amt) );
      END IF;
      COMMIT;
    END IF;
  END LOOP;
...
BEGIN
  ...
  LOOP
    ...
    get_terms_id(p_terms_name => lc_terms_name, o_terms_id => ln_terms_id);
    BEGIN
      SELECT *
    ...
    IF lr_existing_vendor_rec.vendor_id IS NOT NULL THEN
      ...
      get_receiving_details(p_supplier_num => c_sup.segment1 , o_inspection_required_flag => lc_inspection_required_flag , o_receipt_required_flag => lc_receipt_required_flag , o_qty_rcv_tolerance => ln_qty_rcv_tolerance , o_qty_rcv_exception_code => lc_qty_rcv_exception_code , o_enforce_ship_to_loc_code => lc_enforce_ship_to_loc_code , o_days_early_receipt_allowed => ln_days_early_receipt_allowed , o_days_late_receipt_allowed => ln_days_late_receipt_allowed , o_receipt_days_exception_code => lc_receipt_days_exception_code , o_receiving_routing_id => ln_receiving_routing_id , o_allow_substitute_rcpts_flag => lc_allow_substitute_rcpts_flag , o_allow_unordered_rcpts_flag => lc_allow_unordered_rcpts_flag );
      ...
      IF lc_inspection_required_flag                 IS NOT NULL OR lc_receipt_required_flag IS NOT NULL OR ln_qty_rcv_tolerance IS NOT NULL OR lc_qty_rcv_exception_code IS NOT NULL OR lc_enforce_ship_to_loc_code IS NOT NULL OR ln_days_early_receipt_allowed IS NOT NULL OR ln_days_late_receipt_allowed IS NOT NULL OR lc_receipt_days_exception_code IS NOT NULL OR ln_receiving_routing_id IS NOT NULL OR lc_allow_substitute_rcpts_flag IS NOT NULL OR lc_allow_unordered_rcpts_flag IS NOT NULL THEN
...
BEGIN
  ...
  LOOP
    ...
    CLOSE c_get_tolerance_name;
    -- To get the FOB Code
    IF c_sup_site.fob_lookup_code    = 'ORIGIN' THEN
      lc_fob_value                  := 'SHIPPING';
    ELSIF c_sup_site.fob_lookup_code = 'DESTINATION' THEN
    ...
    END IF;*/
    -- commented by Shanti for jira NAIT-118027
    ...
    x_msg_data     :=NULL;
    -------------------------Calling Site API
    ap_vendor_pub_pkg.update_vendor_site_public(p_api_version => p_api_version, p_init_msg_list => p_init_msg_list, p_commit => p_commit, p_validation_level => p_validation_level, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, p_vendor_site_rec => lr_vendor_site_rec, p_vendor_site_id => lr_vendor_site_rec.vendor_site_id, p_calling_prog => p_calling_prog );
    COMMIT;
    ...
    BEGIN
      ...
      WHERE user_id=
        (SELECT user_id FROM fnd_user WHERE user_name='ODCDH'
        )
    ...
    END IF;
    FND_GLOBAL.apps_initialize( ln_user_id, ln_responsibility_id, ln_responsibility_appl_id );
    lc_return_status := NULL;
    ln_msg_count     := NULL;
    lc_msg_data      := NULL;
    -------------------------Calling Party Site API
    print_debug_msg(p_message=> l_program_step||' Start of Calling Update Party Site API', p_force=>true);
    hz_party_site_v2pub.update_party_site( p_init_msg_list => fnd_api.g_false , p_party_site_rec => l_party_site_rec , p_object_version_number => ln_obj_num , x_return_status => lc_return_status , x_msg_count => ln_msg_count , x_msg_data => lc_msg_data ) ;
    COMMIT;
    ...
    IF lc_return_status = fnd_api.g_ret_sts_success THEN
      ...
      hz_location_v2pub.update_location(p_init_msg_list => fnd_api.g_true, p_location_rec => lr_location_rec, p_object_version_number => p_object_version_number, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data );
      COMMIT;
      ...
      ELSE
        ...
        IF TRIM(c_sup_site.inactive_date) IS NULL THEN
          ...
          LOOP
            ...
            hz_party_site_v2pub.update_party_site_use( p_init_msg_list => FND_API.G_FALSE, p_party_site_use_rec => p_party_site_use_rec, p_object_version_number => p_object_version_number1, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data );
...
BEGIN
  ...
  LOOP
    ...
    lc_error_status2:='N';
    lc_error_mesg   :=NULL;
    print_debug_msg(p_message=> 'Vendor Site/First/Last Name  : '|| r_cont.vendor_site_code||'/'||r_cont.first_name||'/'||r_cont.last_name, p_force=>true);
    ...
    LOOP
      ...
      WHEN OTHERS THEN
        l_email_address := NULL;
      END;
      IF NVL(l_phone_number,'X')                <> NVL(r_cont.phone,'X') OR NVL(l_email_address,'X') <> NVL(r_cont.email_address,'X') OR NVL(l_fax_number,'X') <> NVL(r_cont.fax,'X') OR NVL(l_phone_area_code,'X') <> NVL(r_cont.area_code,'X') OR NVL(l_fax_area_code,'X') <> NVL(r_cont.fax_area_code,'X') OR NVL(r_cont_info.hz_first_name,'X') <> NVL(r_cont.first_name,'X') OR NVL(r_cont_info.hz_last_name,'X') <> NVL(r_cont.last_name,'X') THEN
    ...
    LOOP
      IF NVL(r_cont_title.job_title,'X') <> NVL(r_cont.title,'X') THEN
        ...
        x_msg_count     := NULL;
        x_msg_data      := NULL;
        hz_party_contact_v2pub.update_org_contact ( p_init_msg_list => fnd_api.g_false, p_org_contact_rec => lv_contact_title_rec, p_cont_object_version_number => r_cont_title.cont_object_version_number, p_rel_object_version_number => r_cont_title.rel_object_version_number, p_party_object_version_number=>r_cont_title.party_object_version_number, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data );
        COMMIT;
...
--| Returns       : N/A                                                        |
--+============================================================================+
PROCEDURE update_address_contact(
    x_ret_code OUT NUMBER ,
    x_return_status OUT VARCHAR2 ,
    x_err_buf OUT VARCHAR2 )
IS
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      IF c_sup_site.email_address IS NOT NULL THEN
        print_debug_msg(p_message=> 'c_sup_site.email_address: '|| c_sup_site.email_address, p_force=>true);
        IF c_sup_contact.email_cont_point_id IS NOT NULL THEN
          ...
          hz_contact_point_v2pub.update_contact_point ( p_init_msg_list => fnd_api.g_false, p_contact_point_rec => lr_contact_point_rec, p_edi_rec => lr_edi_rec, p_email_rec => lr_email_rec, p_phone_rec => lr_phone_rec, p_telex_rec => lr_telex_rec, p_web_rec => lr_web_rec, p_object_version_number => ln_object_version_number, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data );
          print_debug_msg(p_message=> 'API Return Status / Msg Count : ' || x_return_status||' / '||x_msg_count, p_force=>true);
          IF x_return_status = 'S' THEN
            COMMIT;
        ...
        ELSE
          ...
          x_msg_data         := NULL;
          -------------------------Calling Address Contact API
          hz_contact_point_v2pub.create_contact_point ( p_init_msg_list => fnd_api.g_false, p_contact_point_rec => lr_contact_point_rec, p_edi_rec => lr_edi_rec, p_email_rec => lr_email_rec, p_phone_rec => lr_phone_rec, p_telex_rec => lr_telex_rec, p_web_rec => lr_web_rec, x_contact_point_id => x_contact_point_id, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data );
          print_debug_msg(p_message=> 'API Return Status / Msg Count : ' || x_return_status||' / '||x_msg_count, p_force=>true);
          IF x_return_status = 'S' THEN
            COMMIT;
      ...
      IF c_sup_site.phone_number IS NOT NULL THEN
        print_debug_msg(p_message=> 'c_sup_site.phone_number: '|| c_sup_site.phone_number, p_force=>true);
        IF c_sup_contact.phone_cont_point_id IS NOT NULL THEN
      ...
      -- Update FAX Address
      IF c_sup_site.fax IS NOT NULL THEN
        print_debug_msg(p_message=> 'c_sup_site.fax: '|| c_sup_site.fax, p_force=>true);
        IF c_sup_contact.fax_cont_point_id IS NOT NULL THEN
...
--|                                                                            |
--+============================================================================+
PROCEDURE update_remittance_email(
    x_ret_code OUT NUMBER ,
    x_return_status OUT VARCHAR2 ,
    x_err_buf OUT VARCHAR2 )
IS
  ...
  IS
    ...
    AND request_id        = gn_request_id
    AND site_process_flag = 7;
  CURSOR c_site_remit_email(p_vendor_id NUMBER, p_vend_site_id NUMBER)
  IS
...
BEGIN
  ...
  END;
  fnd_global.apps_initialize (user_id => ln_user_id, resp_id => ln_responsibility_id, resp_appl_id => ln_responsibility_appl_id);
  ...
  LOOP
    ...
    FOR cr IN c_site_remit_email(cur.vendor_id, cur.vendor_site_id)
    LOOP
      ...
      iby_disbursement_setup_pub.update_external_payee (p_api_version => 1.0, p_init_msg_list => 'T', p_ext_payee_tab => p_external_payee_tab_type, p_ext_payee_id_tab => p_ext_payee_id_tab_type, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, x_ext_payee_status_tab => l_payee_upd_status );
      COMMIT;
...
IS
  ...
  --==========================================================================================
  CURSOR c_sup_site_email (c_supplier_number VARCHAR2, c_vendor_site_code VARCHAR2)
  IS
...
BEGIN
  ...
  LOOP
    ...
    print_debug_msg(p_message=>'Supplier No/Name/Site : '||UPPER(l_sup_site_type.supplier_number)||'/'|| UPPER(l_sup_site_type.supplier_name)||'/'|| UPPER(l_sup_site_type.vendor_site_code), p_force=> false );
    SELECT COUNT(1)
    INTO ln_cnt
    FROM ap_supplier_sites_int xasi
    WHERE xasi.status            ='NEW'
    ...
    ELSE     ---  IF  gc_error_site_status_flag = 'N' THEN
      ...
      l_sup_site_type.create_flag:=l_sup_create_flag;
    END IF;
    print_debug_msg(p_message=> 'After supplier site existence check Error Status/Create Flag : '||gc_error_site_status_flag||'/'|| l_sup_create_flag, p_force=> false );
    --==============================================================================================================
    ...
    IF l_sup_site_type.fob_lookup_code     IS NOT NULL THEN -- Derive the FOB Code
      l_fob_code_cnt                       := 0;
      IF l_sup_site_type.fob_lookup_code    = 'ORIGIN' THEN
        lc_fob_value                       := 'SHIPPING';
      ELSIF l_sup_site_type.fob_lookup_code = 'DESTINATION' THEN
        lc_fob_value                       := 'RECEIVING';
      ELSE
...
BEGIN
  ...
  BEGIN
    ...
    SET A.contact_process_flag = 3 ,
      ...
      ||',Supplier Site does not exist in the System',
      A.process_flag         = 'Y'
    WHERE REQUEST_ID         = gn_request_id
  ...
  LOOP
    ...
    gc_error_msg              := '';
    l_sup_create_flag         :=NULL;
    print_debug_msg(p_message=> 'Supplier/Site/Contact : '||l_sup_site_cont_type.supplier_number||'/'|| UPPER(l_sup_site_cont_type.supplier_name)||'/'|| l_sup_site_cont_type.vendor_site_code||'/'|| l_sup_site_cont_type.first_name||'/'||l_sup_site_cont_type.last_name , p_force => FALSE );
    /*
    ...
    */
    ...
    print_debug_msg(p_message=>'Supplier site existence check status/Create Flag :'||gc_error_site_status_flag||'/'||l_sup_CREATE_FLAG , p_force => FALSE);
...
BEGIN
  ...
  LOOP
    ...
    WHEN OTHERS THEN
      ...
      print_debug_msg(p_message=> l_program_step||'Error- Get supp_site_id and supp_party_site_id' || SQLCODE || sqlerrm, p_force=>true);
    END;
    IF lv_supp_site_id         IS NOT NULL THEN
      p_payee.supplier_site_id := lv_supp_site_id;
      p_payee.party_id         := lv_acct_owner_party_id;
      p_payee.party_site_id    := lv_supp_party_site_id;
      p_payee.payment_function := 'PAYABLES_DISB';
      ...
      IF r_sup_bank.end_date          IS NOT NULL THEN
    ...
    IF l_supplier_type.count > 0 THEN
      ...
      LOOP
        ...
        IF l_supplier_type (l_idx).create_flag = 'Y' THEN
          ...
          get_receiving_details(p_supplier_num => l_supplier_type (l_idx).segment1 , o_inspection_required_flag => lc_inspection_required_flag , o_receipt_required_flag => lc_receipt_required_flag , o_qty_rcv_tolerance => ln_qty_rcv_tolerance , o_qty_rcv_exception_code => lc_qty_rcv_exception_code , o_enforce_ship_to_loc_code => lc_enforce_ship_to_loc_code , o_days_early_receipt_allowed => ln_days_early_receipt_allowed , o_days_late_receipt_allowed => ln_days_late_receipt_allowed , o_receipt_days_exception_code => lc_receipt_days_exception_code , o_receiving_routing_id => ln_receiving_routing_id , o_allow_substitute_rcpts_flag => lc_allow_substitute_rcpts_flag , o_allow_unordered_rcpts_flag => lc_allow_unordered_rcpts_flag );
          UPDATE xx_ap_cld_suppliers_stg
          SET inspection_required_flag     = lc_inspection_required_flag,
            receipt_required_flag          = lc_receipt_required_flag,
            qty_rcv_tolerance              = ln_qty_rcv_tolerance,
  ...
  IF lc_intf_ins_flag='Y' THEN
    ...
    IF l_rept_req_id != 0 THEN
      ...
      SET
        ...
        =
        (SELECT vendor_id,party_id FROM ap_suppliers WHERE segment1=a.segment1
        )
      WHERE request_id=gn_request_id
      AND create_flag ='Y';
      COMMIT;
    ...
    IF l_sup_site_type.count > 0 THEN
      ...
      LOOP
        ...
        IF l_process_site_error_flag='N' THEN
          ...
          CLOSE c_get_tolerance_name;
          -- To get the FOB Code
          IF l_sup_site_type(l_idx).fob_lookup_code    = 'ORIGIN' THEN
            lc_fob_value                              := 'SHIPPING';
          ELSIF l_sup_site_type(l_idx).fob_lookup_code = 'DESTINATION' THEN
            lc_fob_value                              := 'RECEIVING';
          ELSE
      ...
      LOOP
        ...
        IF l_supplier_cont_type (l_idx).CREATE_FLAG = 'Y' THEN
          ...
          IF l_cont_process_error_flag = 'N' THEN
            ...
            l_msg_data      := NULL;
            lc_error_mesg   := NULL;
            pos_vendor_pub_pkg.create_vendor_contact( p_vendor_contact_rec => l_vendor_contact_rec, x_return_status => l_return_status, x_msg_count => l_msg_count, x_msg_data => l_msg_data, x_vendor_contact_id => l_vendor_contact_id, x_per_party_id => l_per_party_id, x_rel_party_id => l_rel_party_id, x_rel_id => l_rel_id, x_org_contact_id => l_org_contact_id, x_party_site_id => l_party_site_id );
            COMMIT;
...
--|                                                                            |
--+============================================================================+
PROCEDURE main_prc_address_contact(
    x_errbuf OUT nocopy  VARCHAR2 ,
    x_retcode OUT nocopy NUMBER )
IS
  --================================================================
...
--|                                                                            |
--+============================================================================+
PROCEDURE main_prc_remittance_email(
    x_errbuf OUT nocopy  VARCHAR2 ,
    x_retcode OUT nocopy NUMBER )
IS
  --================================================================
...
IS
  ...
  ln_request_id_1           NUMBER;  --added by Shanti for jira NAIT-126421
  lb_complete_1             BOOLEAN; --added by Shanti for jira NAIT-126421
...
BEGIN
  ...
  */
  ...
  BEGIN
    ...
    FROM fnd_user_resp_groups
    WHERE user_id=
      (SELECT user_id FROM fnd_user WHERE user_name = 'SVC_ESP_FIN'
      )
    AND responsibility_id=
      (SELECT responsibility_id
      FROM fnd_responsibility
  ...
  fnd_global.apps_initialize( ln_user_id, ln_responsibility_id, ln_responsibility_appl_id );
  fnd_global.set_nls_context('AMERICAN');
  --Start - added by Shanti for jira NAIT-126421
  print_debug_msg(p_message => 'Submitting the Cloud to EBS supplier file details Program to send the PDF File', p_force => true);
  lb_layout       := fnd_request.add_layout('XXFIN', 'XXCLDSUPFILE', 'en', 'US', 'PDF' );
  ln_request_id_1 := fnd_request.submit_request ( application => 'XXFIN', program => 'XXCLDSUPFILE', description => NULL, start_time => SYSDATE, sub_request => FALSE );
  fnd_file.put_line(fnd_file.log,'Sup file request_id'||ln_request_id_1);
  IF ln_request_id_1 > 0 THEN
    COMMIT;
    ...
    lb_complete_1 :=fnd_concurrent.wait_for_request ( request_id => ln_request_id_1, interval => 15, max_wait => 0, phase => lc_phase, status => lc_status, dev_phase => lc_dev_phase, dev_status => lc_dev_status, MESSAGE => lc_message );
  ELSE
    print_debug_msg(p_message => 'Failed to submit the Report Program to generate the output file - ' || SQLERRM , p_force => true);
  END IF;
  IF ((lc_dev_phase = 'COMPLETE') AND (lc_dev_status = 'NORMAL' OR lc_dev_status = 'WARNING')) THEN
    UPDATE xxfin.xx_cloud_supp_file_status
    SET sent          ='Yes' ,
      last_updated_by =fnd_global.user_id,
      last_update_date=sysdate,
      update_request  =ln_request_id_1
    WHERE sent        ='No';
    --AND run_date BETWEEN sysdate-1/24 AND sysdate;
    fnd_file.put_line(fnd_file.log, 'Total number of records updated - '||SQL%ROWCOUNT);
    COMMIT;
  END IF;
  --end of code jira NAIT-126421
  print_debug_msg(p_message => '                                                                             ' , p_force => true);
  ...
  ln_request_id   := fnd_request.submit_request ( application => 'XXFIN', program => 'XXAPCLDINTR', description => NULL, start_time => SYSDATE, sub_request => FALSE, argument1 => gn_request_id );
  IF ln_request_id > 0 THEN
    COMMIT;
    print_debug_msg(p_message => 'Able to submit the Report Program', p_force => true);
    print_debug_msg(p_message => 'While Waiting Report Request to Finish');
    -- wait for request to finish
    lb_complete :=fnd_concurrent.wait_for_request ( request_id => ln_request_id, interval => 15, max_wait => 0, phase => lc_phase, status => lc_status, dev_phase => lc_dev_phase, dev_status => lc_dev_status, MESSAGE => lc_message );
  ELSE
    print_debug_msg(p_message => 'Failed to submit the Report Program to generate the output file - ' || SQLERRM , p_force => true);
  END IF;

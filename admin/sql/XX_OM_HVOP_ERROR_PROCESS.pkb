...
-- |DRAFT1J 31-OCT-2019 Sairam Guddati     Modified for Defect#NAIT-112425   |
-- |DRAFT1K 07-MAR-2020 Abhishek Kumar     Modified for Defect#NAIT-125720   |
-- +=========================================================================+
...
WHERE
    ...
    AND er.ORIGINAL_SYS_DOCUMENT_REF = oh.orig_sys_document_ref
    and oh.request_id is not null
    and oh.error_flag is not null
    and oh.ship_to_org_id is not null
    and oh.ship_to_org_id = hzu.site_use_id
    and oh.sold_to_org_id = hza.cust_account_id
    and hza.status='A'   
    );
	FND_FILE.PUT_LINE(FND_FILE.LOG,'Total records updated: '||SQL%ROWCOUNT);
	
--NAIT-125720 - Added below update statement to pick the records stuck in interface which doesn't have a record in oe_processing_msgs

BEGIN
    UPDATE oe_headers_iface_all h
    SET error_flag='N', request_id= NULL 
    WHERE error_flag='Y'
    AND NOT EXISTS(SELECT 1 
                   FROM oe_processing_msgs m 
                   WHERE h.orig_sys_document_ref = m.original_sys_document_ref --and length(h.orig_sys_document_ref)=12
                   AND H.Order_Source_Id       = M.Order_Source_Id);

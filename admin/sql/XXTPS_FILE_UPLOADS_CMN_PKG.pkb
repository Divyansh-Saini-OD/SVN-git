CREATE OR REPLACE
PACKAGE BODY XXTPS_FILE_UPLOADS_CMN_PKG
-- +==========================================================================+
...
-- |                  Office Depot - Project Simplify                  |
-- +===================================================================+
-- | Name             :  XXTPS_INITIATE_FILE_UPLOAD                    |
...
BEGIN
  ln_req_id := FND_REQUEST.SUBMIT_REQUEST(
                   ...
                   ,program    => 'XXTPS_FILE_UPLOAD' -- CR769
                   ,start_time => sysdate
                   ,sub_request => false
                   ,argument1  => p_file_upload_id );
   COMMIT;
  IF ln_req_id = 0 THEN
    UPDATE xxtps_file_uploads
...
END XXTPS_INITIATE_FILE_UPLOAD;
...
AS
  ...
  lc_error_clob         CLOB;
  lc_clob_code          xxtps_file_uploads.clob_code%TYPE;
  lc_attributes         xxtps_template_file_uploads.attributes%TYPE;
  lc_func_name          xxtps_template_file_uploads.statement%TYPE;
 ...
 lc_single             VARCHAR2(20) := '''';   
  ...
  lc_pre_processing_function  xxtps_template_file_uploads.pre_processing_function%TYPE;
  lc_post_processing_function xxtps_template_file_uploads.post_processing_function%TYPE;
  ...
  IS
  SELECT *
  FROM   xxtps_staging_file_uploads
  WHERE  file_upload_id = p_file_upload_id;
BEGIN
  BEGIN
  ...
  BEGIN
    ...
    FROM   xxtps_template_file_uploads
  ...
  LOOP
    ...
    IF lt_tab_tokens.count > 0 THEN
      lc_insert_str1 := 'INSERT INTO xxtps_staging_file_uploads (staging_file_upload_id,'
                                                              || 'file_upload_id,'
                                                              || 'program,'
                                                              || 'created_by,'
                                                              || 'creation_date,'
    ...
    IF lt_tab_tokens.count > 0 THEN
      ...
      lc_insert_str3 := 'VALUES (xxtps_staging_file_upload_id_s.NEXTVAL, '
                              || p_file_upload_id
                              || ', ''xxtps_file_uploads_pkg.file_upload'', '
                              || '-1, '
                              || 'SYSDATE, '
                              || '-1, '
                              || 'SYSDATE, '
  ...
  IF lt_template_attr.COUNT > 0 THEN
    ...
    WHERE  file_upload_id = p_file_upload_id;
      ...
      -- Will execute the function which is mentioned in xxtps_template_file_uploads table.
      -- ----------------------------------------------------------------------
	...
	-- Defect 9592 End
      ...
      BEGIN
        ...
        THEN
          ...
          fnd_file.PUT_LINE(fnd_file.LOG,   ' ----->>> Before calling XXTPS_BULK_TEMPLATES_PKG');
...
--          fnd_file.PUT_LINE(fnd_file.LOG,'BEGIN ' || ':1 := ' || lc_func_name || '(' || lc_function_str || '); END;');
      ...
      WHEN OTHERS THEN
        ...
        fnd_file.PUT_LINE(fnd_file.LOG,lc_error_str);
      END;
--      fnd_file.PUT_LINE(fnd_file.LOG,   ' ----->>> After calling XXTPS_BULK_TEMPLATES_PKG');
...
-- +==========================================================================+
  ...
  BEGIN
    ...
    fnd_file.PUT_LINE(fnd_file.LOG, '----------- Begin XXTPS_FILE_UPLOADS_CMN_PKG.TRUNCATE_TABLE Procedure -----------');
    ...
    fnd_file.PUT_LINE(fnd_file.LOG, '----------- End XXTPS_FILE_UPLOADS_CMN_PKG.TRUNCATE_TABLE Procedure -----------');
...
-- +==========================================================================+
  ...
  BEGIN
    ...
    fnd_file.PUT_LINE(fnd_file.LOG, '----------- Begin XXTPS_FILE_UPLOADS_CMN_PKG.ANALYZE_TABLE Procedure -----------');
    ...
    fnd_file.PUT_LINE(fnd_file.LOG, '----------- End XXTPS_FILE_UPLOADS_CMN_PKG.ANALYZE_TABLE Procedure -----------');

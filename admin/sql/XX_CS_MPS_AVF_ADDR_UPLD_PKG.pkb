...
as
  ...
  PROCEDURE upd_devices_addr( errbuf      OUT NOCOPY VARCHAR2
	...
	is
	select *
	from   XXOM.XX_CS_MPS_AVF_LOAD_STG
  ...
  PROCEDURE send_avf_mail( p_party_id        IN   NUMBER
    ...
    CURSOR c_ship_to (p_party_id NUMBER) IS
      SELECT DISTINCT 'A'                        action_code
           ...
           , ''                                  province
           , ''                                  country_code
        FROM xxom.xx_cs_mps_device_b d
           , hz_cust_accounts        a
           , hz_parties              p
  ...
  IS
    ...
    select  int.BATCH_ID           
		   ...
		   ,int.LAST_UPDATED_BY 
		   ,dev.PARTY_ID
    from   XXOM.XX_CS_MPS_AVF_LOAD_INT  int,
           apps.XX_CS_MPS_DEVICE_B      dev
	...
	is
	SELECT   
      (select orig_system_reference from apps.hz_cust_site_uses_all where site_use_id = mb.ship_site_id and rownum <2) as device_site_sequence
    ...
    FROM 
	   ...
	   , XXOM.XX_CS_MPS_AVF_LOAD_STG stg
       , APPS.XX_CS_MPS_DEVICE_B MB
    WHERE 
  ...
  BEGIN
	...
	loop
      ...
      insert into XXOM.XX_CS_MPS_AVF_LOAD_STG
      (
         BATCH_ID                 
        ,SERIAL_NO           
        ,IP_ADDRESS          
	  ...
	  update  XXOM.XX_CS_MPS_AVF_LOAD_STG

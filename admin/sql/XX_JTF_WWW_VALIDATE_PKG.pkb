...
-- +=======================================================================+
-- | Name             :XX_JTF_WWW_VALIDATE_PKG.pkb                         |
...
create or replace package body XX_JTF_WWW_VALIDATE_PKG
AS 
...
IS
    ...
    lc_process_name         varchar2(32) := 'JTF WWW VALIDATE';
    lc_source_system        varchar2(10) := 'WWW';
    lc_package_name         varchar2(32) := 'XX_JTF_WWW_VALIDATE_PKG';
    lc_procedure_name       varchar2(32) := 'VALIDATE DATA';
    lc_staging_table_name   varchar2(32) := 'XX_JTF_WWW_PROSPECTIMAGE';
    ...
    Cursor c_API is
      SELECT lpad(A.INTERNID,10,'0') AS internid
            ...
            ,A.REV_BAND
            ,X.country  
        FROM XXCRM.XX_JTF_WWW_NEW_PROSPECTS A
...
BEGIN    
    ...
    fnd_file.put_line(fnd_file.LOG,
      lpad('OD: JTF WWW validate input data',69,' '));
    ...
    fnd_file.put_line(fnd_file.LOG,
      ...
      ' XXCRM.XX_JTF_WWW_PROSPECTIMAGE...');
    DELETE FROM XXCRM.XX_JTF_WWW_PROSPECTIMAGE;
    fnd_file.put_line(fnd_file.LOG,
    ...
    fnd_file.put_line(fnd_file.LOG,
      ...
      ' XXCRM.XX_JTF_WWW_PROSPECTIMAGE...');
    INSERT INTO XXCRM.XX_JTF_WWW_PROSPECTIMAGE
       (INTERNID	
       ,ID	     
       ,NAME	     
       ,ADDR1	     
    ...
    fnd_file.put_line(fnd_file.LOG,
      ' Started deleting existing records from the table' ||  
      ' XXCRM.XX_JTF_WWW_NEW_NEW_PROSPECTS...');
    DELETE FROM XXCRM.XX_JTF_WWW_NEW_PROSPECTS;
    fnd_file.put_line(fnd_file.LOG,
      ' Successfully deleted all the records from the table' || 
      ' XXCRM.XX_JTF_WWW_NEW_PROSPECTS...');
    ...
    fnd_file.put_line(fnd_file.LOG,
      ' Started inserting data into the table' ||
      ' XXCRM.XX_JTF_WWW_NEW_PROSPECTS...');
    INSERT INTO XXCRM.XX_JTF_WWW_NEW_PROSPECTS   
       (INTERNID	
       ,LOAD_DATE     
       ,ORGANIZATION_NAME
       ,FNAME	     
       ...
       ,VALIDATION_BATCH_ID)
    SELECT INTERNID	
          ,TO_DATE(ID,'YYYYMMDD') AS LOAD_DATE
    ...
    FROM XXCRM.XX_JTF_WWW_PROSPECTIMAGE
    WHERE INTERNID NOT IN (SELECT distinct INTERNID 
                           FROM XXCRM.XX_JTF_WWW_PROSPECT_ARCHIVE);
    ...
    LOOP
        ...
        update XXCRM.XX_JTF_WWW_NEW_PROSPECTS
    ...
    fnd_file.put_line(fnd_file.LOG,
      ' Started inserting data into the table' ||
      ' XXCRM.XX_JTF_WWW_PROSPECT_ARCHIVE...');
    INSERT INTO XXCRM.XX_JTF_WWW_PROSPECT_ARCHIVE
       (INTERNID	
       ,LOAD_DATE     
       ,ORGANIZATION_NAME
       ,FNAME	     
    ...
    fnd_file.put_line(fnd_file.LOG,
      ' Started deleting existing records from the table' ||  
      ' XXCRM.XX_JTF_WWW_VALIDATE_EXCEPTIONS...');
    DELETE FROM XXCRM.XX_JTF_WWW_VALIDATE_EXCEPTIONS;
    fnd_file.put_line(fnd_file.LOG,
    ...
    fnd_file.put_line(fnd_file.LOG,
      ' Started inserting data into the table' ||
      ' XXCRM.XX_JTF_WWW_VALIDATE_EXCEPTIONS...');
    INSERT INTO XXCRM.XX_JTF_WWW_VALIDATE_EXCEPTIONS
       (BATCH_ID
       ,EXCEPTION_ID  
       ,LOG_DATE       
       ,PACKAGE_NAME         
    ...
    SELECT BATCH_ID
          ...
          ,SOURCE_SYSTEM_CODE            
          ,ORACLE_ERROR_MSG
      FROM XXCRM.XX_JTF_WWW_VALIDATE_EXCEPTIONS;
    fnd_file.put_line(fnd_file.LOG,
      ' Successfully inserted data into table' || 
      ' XX_COM_EXCEPTIONS_LOG_CONV...');
...
-- if no exceptions are found, a NO RECORDS FOUND row is inserted for reporting purposes
    ...
    lc_program_name := 'XX_JTF_WWW_EXCEPTION_RPT';
    fnd_file.put_line(fnd_file.LOG,

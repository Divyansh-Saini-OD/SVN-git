--REM dbdrv: sql ~PROD ~PATH ~FILE none none none package and phase=plb checkfile(120.95.12010100.21=120.116)(120.73.12010000.12=120.85):~PROD:~PATH:~FILE
create or replace 
PACKAGE BODY XX_PO_INTERFACE_PKG AS
-- +================================================================================+
...
-- |                                 Updating the version 120.73.12010000.100 changes|
-- |                                 of package PO_INTERFACE_B.pks               	  |
-- |                                 of package PO_INTERFACE_S.pks               	  |
-- |           V4.0     08-04-2016   Radhika Patnala   Retrofit for R12.2.5 updating  |
-- |                                                    the version 120.2.12020000.5  |
-- |											Changes of Package PO_INTERFACE_S.pks |
-- |          V5.0     20-Oct-2016   Madhu Bolli   Retrofit for R12.2.5 patch#23318272 updating the version 
-- |                                                  120.121.12020000.60 Changes of Package PO_INTERFACE_B.pkb |

-- +==================================================================================+


/**              (c) Copyright O 2013,2015 Oracle and/or its affiliates.
**                       All Rights Reserved
** ============================================================================
**
**   NAME
...
**      PO_LINES_INTERFACE and PO_DISTRIBUTIONS_INTERFACE tables
**
**   USAGE
**      To install:
**          start POXBWP1B.pls
...
**      09/05/95  K Powell  Continued development.
**      09/27/96  W Lau           bug 404191 taxable_flag defaulting
**                                      enhancement
**      04/30/97        ECSO            Bug 482648 handle quantity/amount
**                                      when adding to foreign currency PO
**      05/15/97        ECSO            Bug 486563 unit_price/price override
**                                      discrepency when combining req lines
**      08/29/97        ECSO            Kanban Enchancement
**      09/02/97  F Purves  Use new tax defaulting API
**      09/10/97  ECSO    Add OE callback for maintaining
**                                      so_drop_ship_sources
**      09/24/97        ECSO            Bug 493106 Update revised date
**                                      when revision is incremented
**      06/10/97        SVAIDYAN        Bug # 630638
**                                      If the price break type is non-cum.
**                                      and the interface line is added to an
**                                      existing shipment, then add the
**                                      interface quantity to the shipment qty
**                                      to find the correct break price.
**
**     08/18/98       sarunach          Changed the insertion logic of receipt
**                                      required flag into po_line_locations.
**                                      Also changed the clubbing logic
**                                      of requisitions while trying to add to
**                                      an exisiting po.
**                                      Bug no:667512.
**                                      Changes made to 110.21
**
**    10/05/98        svaidyan          Bug # 661570: Round the distribution
**                                      qty using nvl(x_ext_precision,5) if
**                                      the order type lookup code is not
**                                      quantity.
**    10/14/98        sarunach
**                       Bug no:714303.Procedure:setup_interface_tables
...
**    12/1/98
**                    dkfchan
**                       Pay on code in included when creating PO and Release.
**
**    12/8/98       frkhan
**       Tax changes
**    12/10/98
**                    bgu
**                       Invoice match flag included when creating PO and Release.
**  02/12/99         sarunach
**                       Bug no 781929
**                       Last accept date is also inserted into
**                       po_line_locations table.
**                       Inserted using the following logic.
**                       last_accept_date = interface.need_by_date
**                                          + rc.days_late_receipt_allowed.
**  04/13/2000      aaggarwa
**      Added award_id changes.. label OGM_0.0
**  /09/27/2000     togeorge
**      Bug# 1433282 Modified for oke integration and note
**           to reciever functionality.
**  11/17/2000      togeorge
**      Bug# 1369049  Added logic to default tax_name in po_lines
**  12/14/2001      sktiwari
**                      Bug# 2082757 Added function SOURCE_BLANKET_LINE.
**  03/12/2002      rschaub
**                      VMI Patchset H
**  09/29/2002      tpoon
**                      FPI UTF8: changed declaration of x_ship_org_name in
**                      PO_INTERFACE_S.create_line to
**                      hr_all_organization_units.name%TYPE.
**  06/02/2009      sjanakir  Modified for Bug#8446396
**  04/13/2015      shikapoo Modified for Rate Card ER
** ============================================================================
*/
...
/* $Header: POXBWP1B.pls 120.121.12020000.60 2015/07/23 07:06:15 linlilin ship $*/

g_pkg_name    CONSTANT VARCHAR2(30) := 'XX_PO_INTERFACE_PKG';  --<SharedProc FPJ> --(V4.0 -R12.2.5)
g_log_head    CONSTANT VARCHAR2(30) := 'po.plsql.XX_PO_INTERFACE_PKG.';     -- (V4.0 -R12.2.5)
...
-- end of 1548597
        ...
        phi.org_id                                      -- <R12 MOAC>
            ...
            , pli.catalog_name        catalog_name,
              --<Unified Catalog R12 End>
			 phi.attribute_category, --added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07  --(V4.0 -R12.2.5)
             phi.attribute1,         --added by Paul DSouza,29-Aug-07							--(V4.0 -R12.2.5)
             phi.attribute6,         --added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07  --(V4.0 -R12.2.5)
             phi.attribute7,         --added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07  --(V4.0 -R12.2.5)
             phi.attribute8,         --added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07	--(V4.0 -R12.2.5)
             phi.attribute9          --added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07  --(V4.0 -R12.2.5)
...
-- bug 18652325 update_accrue_on_receipt_flag
PROCEDURE update_accrue_on_receipt_flag
(p_document_id IN NUMBER
);
...
l_api_name                  VARCHAR2(30) := 'create_documents(CLM wrapper)';
...
BEGIN 
    ...
    /*CLM Phase 2Autocreate grouping
    Initialising variable*/
    x_error_code_tbl := PO_TBL_VARCHAR2000();
    PO_AUTOCREATE_PARAMS.g_error_code_tbl := PO_TBL_VARCHAR2000();
    /*CLM Phase 2Autocreate grouping*/
      ...
      create_documents (
         ...
         l_progress := '030';
         x_draft_id := po_autocreate_params.g_draft_id;
         
         /*CLM Phase 2 Autocreate Grouping.
         Error handling : initialize the error table
         */
         
          x_error_code_tbl :=  PO_AUTOCREATE_PARAMS.g_error_code_tbl;
        ...
        /*CLM Phase 2 Ends*/
         ...
         IF g_debug_stmt THEN
                        PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                            p_token    => l_progress,
                                            p_message  =>   'x_draft_id : ' || x_draft_id
                                                          ||' x_document_number : '||x_document_number
                                                          ||' x_return_status : '|| x_return_status
                                                          ||' x_document_id : '||x_document_id
                                                          ||' x_number_lines : '|| x_number_lines
    ...
    WHEN OTHERS THEN
        IF g_debug_unexp THEN
            PO_DEBUG.debug_exc(p_log_head => g_log_head||l_api_name,
                               p_progress => l_progress);
        END IF;
        PO_MESSAGE_S.sql_error (routine    => 'CREATE_DOCUMENTS_CLM',
                                location   => l_progress,
                                error_code => SQLCODE
        );
        RAISE;  
...
--<Shared Proc FPJ End>
    ...
    -- CLM Autocreate
    l_style_id                      PO_HEADERS_INTERFACE.style_id%TYPE;
    l_clm_enabled_flag              VARCHAR2(1):= 'N';
    l_change_process_type           PO_DOC_STYLE_HEADERS.change_process_type%TYPE;
    l_is_clm_flow                   VARCHAR2(1):= 'N';

    l_proc_plan_prod_exists NUMBER;   --17016107
	l_auction_header_id NUMBER; --Bug 19261272
	l_po_line_id	  NUMBER; /*V6*/
	l_upd_stmt      VARCHAR2(1000); /*V6*/
BEGIN
    --<Shared Proc FPJ Start>
    ...
    /* Getting the style_id of the target document */
    SELECT phi.style_id
      INTO l_style_id
      FROM po_headers_interface phi
   ...
   IF g_debug_stmt THEN
                        PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                            p_token    => l_progress,
                                            p_message  => 'Before checking for CLM Flow : Doc Style Id : '||l_style_id);
   END IF;
   ...
   -- Determines which flow to use
    IF l_style_id IS NOT NULL THEN
         ...
         /* Check if the doc style is clm enabled */
         ...
         SELECT nvl(pdsh.clm_flag,'N'),
          ...
          IF g_debug_stmt THEN
                                PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                                    p_token    => l_progress,
                                                    p_message  => 'Inside checking for CLM Flow : l_clm_enabled_flag :'||l_clm_enabled_flag);

                                PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                                    p_token    => l_progress,
                                                    p_message  => 'Inside checking for CLM Flow : l_change_process_type :'||l_change_process_type);
          END IF;


        -- CLM Flow/ Modification Flow
        IF l_clm_enabled_flag = 'Y' THEN
                 l_is_clm_flow := 'Y';
                 ...
                 IF g_debug_stmt THEN
                                      PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                                          p_token    => l_progress,
                                                          p_message  => 'Is CLM Flow :'||l_is_clm_flow);
                 END IF;
        END IF;
    END IF;


   -- CLM AutoCreate changes
   -- CLM Flow check
    IF l_is_clm_flow = 'Y' THEN

                   PO_AUTOCREATE_PARAMS.g_is_clm_po := l_clm_enabled_flag;
                   PO_AUTOCREATE_PVT.create_po(x_interface_header_id
                                              ,x_document_id
                   ...
                   x_number_lines := PO_AUTOCREATE_PARAMS.g_number_records_processed;
                   g_document_subtype := PO_AUTOCREATE_PARAMS.g_document_subtype;
                   g_interface_source_code := PO_AUTOCREATE_PARAMS.g_interface_source_code;
                   l_original_operating_unit_id := PO_AUTOCREATE_PARAMS.g_original_operating_unit_id;
                   l_org_context_changed := PO_AUTOCREATE_PARAMS.g_org_context_changed;
                   
                   l_progress := '018';
                   IF g_debug_stmt THEN
                                          PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                                              p_token    => l_progress,
                                                              p_message  => 'After create_po in CLM flow : Number of lines processed : '||x_number_lines);
                   END IF;

                   COMMIT;

    ELSE    -- Non CLM/ Non Modification Flow
                    ...
                    -- Bug 18180225 : Derive l_original_operating_unit_id before using it
                    ...
                    IF(g_document_type IN ('PO','PA') AND g_document_subtype IN ('STANDARD', 'BLANKET')) THEN 
                         
                      IF x_document_id IS NULL AND l_mode = 'ADD' THEN 
                         SELECT po_header_id INTO x_document_id
                         FROM po_headers_all
                         WHERE segment1 = l_segment1
						 AND type_lookup_code = g_document_subtype --Bug 18405112
    ...
    IF g_interface_source_code = 'SOURCING' AND g_document_subtype IN ('BLANKET', 'CONTRACT') THEN
      ...
      END IF;
          ...
          USING IN  interface.auction_header_id, IN x_document_id, OUT x_return_status;
...
/* Starts added for V6 */	
	...
	-- Start Procurement Plan Update Source Of Supply  <17016107>
 	     IF g_interface_source_code = 'SOURCING' AND g_document_subtype IN ('BLANKET', 'CONTRACT', 'STANDARD') THEN
		   ...
		   SELECT auction_header_id, po_line_id INTO
 	       l_auction_header_id, l_po_line_id
 	       FROM po_lines_all
 	       WHERE
 	       po_header_id = x_document_id
 	       AND rownum<2; --auction header id from base instead of interface Bugs 19261272,19007330
 	 
 	       IF l_proc_plan_prod_exists =1 THEN

 	         if (g_document_subtype = 'STANDARD') then
			 
 	           EXECUTE IMMEDIATE 'BEGIN po_proc_plan_pub.update_proc_plan_for_fps(:p_auction_header_id,:p_po_header_id,:x_return_status); END; '
 	           ...
 	           IF g_debug_stmt THEN
 	               PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
 	                                   p_token    => l_progress,
 	                                   p_message  => 'After update_proc_plan_for_fps:'||x_return_status);
 	           END IF;
...
--<Bug 13542989 :START, Included Bug Fix:14080332 >
...
--<Bug 13542989 :END, Included Bug Fix:14080332>


-- Bug#20221798  start
l_req_header_id      PO_REQUISITION_HEADERS_ALL.requisition_header_id%TYPE;

CURSOR c_get_req_header_id (c_req_line_id    NUMBER)
  IS
...
/*Bug No.1793703:The decode statement below for updating the revision_num
        ...
        END IF;
	/*bug 12674183 While trying to add a req line to Blanket Release in Incomplete status
	The Approved flag in po_releases all was having null value.
...
--DPCARD}
     ...
     ,tax_attribute_update_code   --<eTax Integration R12>
            ...
            , clm_document_number
			,attribute_category                  ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)  --(V4.0-R12.2.5)
            ,attribute1                          ---added by Paul DSouza,29-Aug-07(V2.0)						   --(V4.0-R12.2.5)
            ,attribute6                          ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)  --(V4.0-R12.2.5)
            ,attribute7                          ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)  --(V4.0-R12.2.5)
            ,attribute8                          ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)  --(V4.0-R12.2.5)
            ,attribute9)
...
--DPCARD}
       ...
       ,nvl2(g_calculate_tax_flag, 'CREATE', null)  --<eTax Integration R12>
            ...
            , interface.document_num
			,interface.attribute_category   ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)
            ,interface.attribute1           ---added by Paul DSouza,OD CUSTOMIZATION,29-Aug-07(V2.0)
            ,interface.attribute6           ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)
            ,interface.attribute7           ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)
            ,interface.attribute8           ---added by Sandeep Gorla,E0216,OD CUSTOMIZATION,10-Apr-07(V2.0)
            ,interface.attribute9);
    EXCEPTION
    ...
    /* Bug#13542989: Start : Get Default document transimmision information and
...
*/
       ...
       VALUES (g_po_release_id,
        ...
        -- Bug#20221798 - populate cbc_accounting_date for Release
        -- if environment is encumbrance enabled

        IF (NVL(params.po_encumbrance_flag, 'N') = 'Y') THEN
          l_progress := '295';

          IF g_debug_stmt THEN    --< Bug 3210331: use proper debugging >
            PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                p_token    => l_progress,
                                p_message  => 'calling populate_cbc_accounting_date:');
          END IF;

          OPEN c_get_req_header_id (interface.requisition_line_id);
          FETCH c_get_req_header_id INTO l_req_header_id;
          CLOSE c_get_req_header_id;

          populate_cbc_accounting_date(
              p_req_header_id        => l_req_header_id
            , p_document_id          => g_po_release_id
          ...
          IF g_debug_stmt THEN    --< Bug 3210331: use proper debugging >
            PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
                                p_token    => l_progress,
                                p_message  => 'l_return_status= ' || l_return_status);
          END IF;
        END IF;     -- Bug#20221798  end
    ...
    IF (interface.line_type_id IS NOT NULL) THEN
      l_progress := '010';
    ...
    IF (((g_interface_source_code ='SOURCING') AND (g_document_subtype = 'BLANKET'))
         OR (interface.document_subtype='STANDARD' AND l_order_type_lookup_code = 'RATE'
              AND l_purchase_basis = 'TEMP LABOR' ))
    THEN
...
--end of add for 1805397
    ...
    IF (g_document_subtype in ('STANDARD', 'PLANNED')) THEN
       ...
       PO_TAX_INTERFACE_PVT.calculate_tax( x_return_status     => l_return_status,
                                           ...
                                           p_calling_program   => 'AUTOCREATE'); --PDOI Enhancement Bug#17063664
       IF l_return_status <> FND_API.G_RET_STS_SUCCESS THEN
         l_progress := '414';
         IF g_interface_source_code = 'CONSUMPTION_ADVICE' THEN
...
/* ============================================================================
     NAME: POPULATE_CBC_ACCOUNTING_DATE 
     DESC: Stamp cbc_accounting_date from  PO_REQUISITION_HEADERS_ALL
           to Standare PO or Release document 
     ARGS: IN : p_req_header_id       IN number
                p_document_id         IN NUMBER
...
IS
  ...
  l_cbc_accounting_date                DATE;

  l_api_name CONSTANT VARCHAR2(30) := 'populate_cbc_accounting_date';
  d_mod CONSTANT VARCHAR2(255) := g_log_head||l_api_name;
  l_progress                           VARCHAR2(3);
  
  CURSOR c_get_cbc_accounting_date
    (c_req_header_id    PO_REQUISITION_HEADERS_ALL.requisition_header_id%TYPE)
  IS
...
BEGIN
  ...
  IF g_debug_stmt THEN    
    PO_DEBUG.debug_begin(p_log_head => d_mod);

         PO_DEBUG.debug_var (p_log_head => g_log_head||l_api_name,
                             p_progress => l_progress,
                             p_name     => 'p_req_header_id',
                             p_value    => p_req_header_id);

         PO_DEBUG.debug_var (p_log_head => g_log_head||l_api_name,
                             p_progress => l_progress,
                             p_name     => 'p_document_id',
                             p_value    => p_document_id);


         PO_DEBUG.debug_var (p_log_head => g_log_head||l_api_name,
                             p_progress => l_progress,
                             p_name     => 'p_document_subtype',
                             p_value    => p_document_subtype);
  ...
  IF p_req_header_id IS NULL THEN
    IF g_debug_stmt THEN
      PO_DEBUG.debug_stmt(d_mod,l_progress,'p_req_header_id IS NULL,not need to check');
    END IF;

    RETURN;
  END IF;

  -- first check if cbc is enabled 
  IGC_CBC_PO_GRP.is_cbc_enabled
    ...
    --
    -- calling IGC_CBC_PO_GRP.is_cbc_enabled returned success
    -- checking the value of l_cbc_enabled
    --
    IF g_debug_stmt THEN 
      PO_DEBUG.debug_stmt(d_mod,l_progress,'l_cbc_enabled = ' || l_cbc_enabled);
    END IF;

    IF l_cbc_enabled = 'Y' THEN

      --
      -- get cbc_accounting_date from requisition header and
      -- stamp it on po/release header
      --
      OPEN c_get_cbc_accounting_date (p_req_header_id);
      FETCH c_get_cbc_accounting_date INTO l_cbc_accounting_date;
      CLOSE c_get_cbc_accounting_date;

      IF g_debug_stmt THEN
        PO_DEBUG.debug_stmt(d_mod,l_progress,'After fetching cbc_accounting_date');
        PO_DEBUG.debug_var (p_log_head => g_log_head||l_api_name,
                            p_progress => l_progress,
                            p_name     => 'l_cbc_accounting_date',
                            p_value    => l_cbc_accounting_date);

      END IF;

      l_progress       := '030';

      IF l_cbc_accounting_date IS NOT NULL THEN
        IF (p_document_subtype = 'STANDARD') THEN
          IF g_debug_stmt THEN
            PO_DEBUG.debug_stmt(d_mod,l_progress,'Stamp cbc_accounting_date to STANDARD PO');
          END IF;

          UPDATE po_headers_all
             SET cbc_accounting_date = l_cbc_accounting_date
           WHERE po_header_id = p_document_id;

        ELSIF (p_document_subtype = 'RELEASE') THEN
          IF g_debug_stmt THEN
            PO_DEBUG.debug_stmt(d_mod,l_progress,'Stamp cbc_accounting_date to RELEASE');
          END IF;

          UPDATE po_releases_all
             SET cbc_accounting_date = l_cbc_accounting_date
           WHERE po_release_id = p_document_id;
        END IF;       -- update PO/Release

      END IF;         -- l_cbc_accounting_date is not NULL
    END IF;           -- l_cbc_enabled  = 'Y'

EXCEPTION
    WHEN FND_API.g_exc_error THEN
      IF g_debug_stmt THEN
        PO_DEBUG.debug_stmt(d_mod,l_progress,'Encounter error from IGC_CBC_PO_GRP.is_cbc_enabled ' );
      END IF;

    WHEN FND_API.g_exc_unexpected_error THEN
      IF g_debug_stmt THEN
        PO_DEBUG.debug_stmt(d_mod,l_progress,'Encounter unexpected error from IGC_CBC_PO_GRP.is_cbc_enabled ' );
      END IF;
...
END populate_cbc_accounting_date;
...
/*Bug 1391523 . Added market price to the INSERT statement */
     ...
     ,tax_attribute_update_code --<eTax Integration R12>
			...
			, attribute_category -- OD CUSTOMIZATION Changed by Paul DSouza(V2.0) --(V4.0 -R12.2.5)
      )
    VALUES (x_po_line_id,
                        interface.last_update_date,
                        interface.last_updated_by,
...
--<INVCONV R12 END>
          ...
          ,nvl2(g_calculate_tax_flag, 'CREATE', null) --<eTax Integration R12>
			...
			, interface.attribute_category  --OD CUSTOMIZATION Changed by Paul DSouza(V2.0) --(V4.0 -R12.2.5)
      );
...
*/
   ...
   fnd_attached_documents2_pkg.
      ...
      'PO_LINES',
      x_po_line_id,
      interface.requisition_line_id,  --bug21179962
      '',
      '',
      '',
      '',
   ...
   THEN
	    ...
	    --Revoke the change for 17967195 as bringing up bug 18949737. And change fda.description 
            ...
            AND    fad.pk2_value = to_char(interface.requisition_line_id);  --bug 21179962
  ...
  -- Bug 20391864 : Rate Card ER
  IF ((g_document_subtype = 'BLANKET'  AND g_interface_source_code = 'SOURCING') 
       OR (interface.document_subtype='STANDARD' AND x_order_type_lookup_code = 'RATE'
              AND l_purchase_basis = 'TEMP LABOR' ))
  THEN
          ...
          x_po_line_id,
                      x_cumulative_flag,
                            interface.need_by_date,
 	                             p_req_line_price => INTERFACE.unit_price); --bug 8845486
  /* <TIMEPHASED FPI> */
      ...
      IF (g_document_type = 'PO') THEN
       IF (g_mode = 'ADD') THEN
        IF (g_document_type = 'PO') THEN
         ...
         END IF;
	/*bug 12673501 starts: the call to po_actions.po_close is a redundant call
	in the above if conditions it is called with the right parameters.
	This call raises an exception hence commenting it.*/
         /*IF NOT po_actions.close_po(interface.po_header_id,
                    x_doctype,
                    g_document_subtype,
                    x_po_line_id,
...
-- Bug 3472140: Changed precisions to 15
        ...
        -- Bug 20212998, add do_currency_conversion call for fix price order type
        -- to convert currency for blanket release
	ELSIF ( x_order_type_lookup_code = 'FIXED PRICE') THEN
	     XX_PO_INTERFACE_PKG.do_currency_conversion(
                      p_order_type_lookup_code => 'FIXED PRICE',
                      p_interface_source_code  => g_interface_source_code,
...
-- end of 1548597
        ...
        ,tax_attribute_update_code --<eTax Integration R12>
		, attribute_category  --OD CUSTOMIZATION Added by Paul DSouza(V2.0) --(V4.0 -R12.2.5)
              )
       VALUES (x_line_location_id,
               ...
               0,
               0,
               NVL(x_amount,interface.amount),    -- amount                 -- <SERVICES FPJ> --Bug 20212998
...
-- end of 1548597
        ...
        , nvl2(g_calculate_tax_flag, 'CREATE', null) --<eTax Integration R12>
		, interface.attribute_category     --(V4.0 -R12.2.5)
);
    l_progress := '260';
    IF g_debug_stmt THEN    --< Bug 3210331: use proper debugging >
        PO_DEBUG.debug_stmt(p_log_head => g_log_head||l_api_name,
...
/* Bug 18053781 : End : To address the currency conversion from req to PO*/  
    ...
    XX_PO_INTERFACE_PKG.calibrate_last_dist_quantity(x_line_location_id);
...
--Bug 20229912 Start: Should use the same type as PO_HEADERS_ALL.segment1.
--Bug 18759905 Start
x_segment1 PO_HEADERS_ALL.segment1%TYPE := NULL;
--Bug 18759905 End
--Bug 20229912 End
                ...
                AND pol2.line_num =
                         (
                           ...
                           AND nvl(pol.vendor_product_num,-1)=
                                     nvl(x_vendor_product_num,-1)	--bug 18682940
                         )
                FOR UPDATE OF QUANTITY;
                            ...
                            AND   (pol.transaction_reason_code IS NULL
         					...
         					-- Bug 9745707 starts
 	                              ...
 	                              AND ((---  CASE 1 : REQ IS NT SOURCED TO ANY DOC
                                    ...
                                    -- Bug#20930010: either need-by-date of the requisition or creation_date of
                                    -- the document  is within the expiration_date of the blanket
                                    ...
                                    AND ( TRUNC(NVL(POL.EXPIRATION_DATE,SYSDATE + 1)) >=
                                          TRUNC(DECODE(POL.EXPIRATION_DATE,NULL,SYSDATE,NVL(PRL.NEED_BY_DATE,SYSDATE)))
                                          OR TRUNC(POL.EXPIRATION_DATE) >= TRUNC(SYSDATE)
 	                                     )
                                    )
 	                                     OR
...
BEGIN
	...
	BEGIN
	  ...
	  SELECT PLI.requisition_line_id, PLI.ip_category_id
	    INTO p_req_line_id, p_ip_cat_id
		FROM po_lines_interface PLI, por_attribute_values PAV
		WHERE PLI.interface_header_id = p_interface_header_id
		   AND PLI.requisition_line_id = PAV.req_line_id;
    ...
    IF p_req_line_id IS NOT NULL 
      AND p_ip_cat_id IS NOT NULL
      THEN
         
    BEGIN

    IF g_debug_stmt THEN
        PO_DEBUG.debug_begin(p_log_head => g_log_head||'Before insert into po_attr_values_interface');
		PO_DEBUG.debug_begin(p_log_head => g_log_head||'p_req_line_id = '||p_req_line_id);
		PO_DEBUG.debug_begin(p_log_head => g_log_head||'p_ip_cat_id = '||p_ip_cat_id);
    END IF;

	INSERT INTO po_attr_values_interface
      (interface_attr_values_id, interface_header_id,
       interface_line_id, process_code, action,
       po_line_id, req_template_name,
       inventory_item_id, org_id, ip_category_id, 
     ...
     SELECT po_attr_values_interface_s.NEXTVAL, PLI.INTERFACE_HEADER_ID,
            PLI.INTERFACE_LINE_ID, PLI.PROCESS_CODE, PLI.ACTION,
            PLI.PO_LINE_ID, PLI.TEMPLATE_NAME, PLI.ITEM_ID, 
            PLI.ORGANIZATION_ID, PLI.IP_CATEGORY_ID, 
             PAV.text_base_attribute1, PAV.text_base_attribute2, PAV.text_base_attribute3,
             PAV.text_base_attribute4, PAV.text_base_attribute5, PAV.text_base_attribute6,
             PAV.text_base_attribute7, PAV.text_base_attribute8, PAV.text_base_attribute9,
             PAV.text_base_attribute10, PAV.text_base_attribute11, PAV.text_base_attribute12,
             PAV.text_base_attribute13, PAV.text_base_attribute14, PAV.text_base_attribute15,
             PAV.text_base_attribute16, PAV.text_base_attribute17, PAV.text_base_attribute18,
             PAV.text_base_attribute19, PAV.text_base_attribute20, PAV.text_base_attribute21,
             PAV.text_base_attribute22, PAV.text_base_attribute23, PAV.text_base_attribute24,
             PAV.text_base_attribute25, PAV.text_base_attribute26, PAV.text_base_attribute27,
             PAV.text_base_attribute28, PAV.text_base_attribute29, PAV.text_base_attribute30,
             PAV.text_base_attribute31, PAV.text_base_attribute32, PAV.text_base_attribute33,
             PAV.text_base_attribute34, PAV.text_base_attribute35, PAV.text_base_attribute36,
             PAV.text_base_attribute37, PAV.text_base_attribute38, PAV.text_base_attribute39,
             PAV.text_base_attribute40, PAV.text_base_attribute41, PAV.text_base_attribute42,
             PAV.text_base_attribute43, PAV.text_base_attribute44, PAV.text_base_attribute45,
             PAV.text_base_attribute46, PAV.text_base_attribute47, PAV.text_base_attribute48,
             PAV.text_base_attribute49, PAV.text_base_attribute50, PAV.text_base_attribute51,
             PAV.text_base_attribute52, PAV.text_base_attribute53, PAV.text_base_attribute54,
             PAV.text_base_attribute55, PAV.text_base_attribute56, PAV.text_base_attribute57,
             PAV.text_base_attribute58, PAV.text_base_attribute59, PAV.text_base_attribute60,
             PAV.text_base_attribute61, PAV.text_base_attribute62, PAV.text_base_attribute63,
             PAV.text_base_attribute64, PAV.text_base_attribute65, PAV.text_base_attribute66,
             PAV.text_base_attribute67, PAV.text_base_attribute68, PAV.text_base_attribute69,
             PAV.text_base_attribute70, PAV.text_base_attribute71, PAV.text_base_attribute72,
             PAV.text_base_attribute73, PAV.text_base_attribute74, PAV.text_base_attribute75,
             PAV.text_base_attribute76, PAV.text_base_attribute77, PAV.text_base_attribute78,
             PAV.text_base_attribute79, PAV.text_base_attribute80, PAV.text_base_attribute81,
             PAV.text_base_attribute82, PAV.text_base_attribute83, PAV.text_base_attribute84,
             PAV.text_base_attribute85, PAV.text_base_attribute86, PAV.text_base_attribute87,
             PAV.text_base_attribute88, PAV.text_base_attribute89, PAV.text_base_attribute90,
             PAV.text_base_attribute91, PAV.text_base_attribute92, PAV.text_base_attribute93,
             PAV.text_base_attribute94, PAV.text_base_attribute95, PAV.text_base_attribute96,
             PAV.text_base_attribute97, PAV.text_base_attribute98, PAV.text_base_attribute99,
             PAV.text_base_attribute100, PAV.num_base_attribute1, PAV.num_base_attribute2,
             PAV.num_base_attribute3, PAV.num_base_attribute4, PAV.num_base_attribute5,
             PAV.num_base_attribute6, PAV.num_base_attribute7, PAV.num_base_attribute8,
             PAV.num_base_attribute9, PAV.num_base_attribute10, PAV.num_base_attribute11,
             PAV.num_base_attribute12, PAV.num_base_attribute13, PAV.num_base_attribute14,
             PAV.num_base_attribute15, PAV.num_base_attribute16, PAV.num_base_attribute17,
             PAV.num_base_attribute18, PAV.num_base_attribute19, PAV.num_base_attribute20,
             PAV.num_base_attribute21, PAV.num_base_attribute22, PAV.num_base_attribute23,
             PAV.num_base_attribute24, PAV.num_base_attribute25, PAV.num_base_attribute26,
             PAV.num_base_attribute27, PAV.num_base_attribute28, PAV.num_base_attribute29,
             PAV.num_base_attribute30, PAV.num_base_attribute31, PAV.num_base_attribute32,
             PAV.num_base_attribute33, PAV.num_base_attribute34, PAV.num_base_attribute35,
             PAV.num_base_attribute36, PAV.num_base_attribute37, PAV.num_base_attribute38,
             PAV.num_base_attribute39, PAV.num_base_attribute40, PAV.num_base_attribute41,
             PAV.num_base_attribute42, PAV.num_base_attribute43, PAV.num_base_attribute44,
             PAV.num_base_attribute45, PAV.num_base_attribute46, PAV.num_base_attribute47,
             PAV.num_base_attribute48, PAV.num_base_attribute49, PAV.num_base_attribute50,
             PAV.num_base_attribute51, PAV.num_base_attribute52, PAV.num_base_attribute53,
             PAV.num_base_attribute54, PAV.num_base_attribute55, PAV.num_base_attribute56,
             PAV.num_base_attribute57, PAV.num_base_attribute58, PAV.num_base_attribute59,
             PAV.num_base_attribute60, PAV.num_base_attribute61, PAV.num_base_attribute62,
             PAV.num_base_attribute63, PAV.num_base_attribute64, PAV.num_base_attribute65,
             PAV.num_base_attribute66, PAV.num_base_attribute67, PAV.num_base_attribute68,
             PAV.num_base_attribute69, PAV.num_base_attribute70, PAV.num_base_attribute71,
             PAV.num_base_attribute72, PAV.num_base_attribute73, PAV.num_base_attribute74,
             PAV.num_base_attribute75, PAV.num_base_attribute76, PAV.num_base_attribute77,
             PAV.num_base_attribute78, PAV.num_base_attribute79, PAV.num_base_attribute80,
             PAV.num_base_attribute81, PAV.num_base_attribute82, PAV.num_base_attribute83,
             PAV.num_base_attribute84, PAV.num_base_attribute85, PAV.num_base_attribute86,
             PAV.num_base_attribute87, PAV.num_base_attribute88, PAV.num_base_attribute89,
             PAV.num_base_attribute90, PAV.num_base_attribute91, PAV.num_base_attribute92,
             PAV.num_base_attribute93, PAV.num_base_attribute94, PAV.num_base_attribute95,
             PAV.num_base_attribute96, PAV.num_base_attribute97, PAV.num_base_attribute98,
             PAV.num_base_attribute99, PAV.num_base_attribute100, PAV.text_cat_attribute1,
             PAV.text_cat_attribute2, PAV.text_cat_attribute3, PAV.text_cat_attribute4,
             PAV.text_cat_attribute5, PAV.text_cat_attribute6, PAV.text_cat_attribute7,
             PAV.text_cat_attribute8, PAV.text_cat_attribute9, PAV.text_cat_attribute10,
             PAV.text_cat_attribute11, PAV.text_cat_attribute12, PAV.text_cat_attribute13,
             PAV.text_cat_attribute14, PAV.text_cat_attribute15, PAV.text_cat_attribute16,
             PAV.text_cat_attribute17, PAV.text_cat_attribute18, PAV.text_cat_attribute19,
             PAV.text_cat_attribute20, PAV.text_cat_attribute21, PAV.text_cat_attribute22,
             PAV.text_cat_attribute23, PAV.text_cat_attribute24, PAV.text_cat_attribute25,
             PAV.text_cat_attribute26, PAV.text_cat_attribute27, PAV.text_cat_attribute28,
             PAV.text_cat_attribute29, PAV.text_cat_attribute30, PAV.text_cat_attribute31,
             PAV.text_cat_attribute32, PAV.text_cat_attribute33, PAV.text_cat_attribute34,
             PAV.text_cat_attribute35, PAV.text_cat_attribute36, PAV.text_cat_attribute37,
             PAV.text_cat_attribute38, PAV.text_cat_attribute39, PAV.text_cat_attribute40,
             PAV.text_cat_attribute41, PAV.text_cat_attribute42, PAV.text_cat_attribute43,
             PAV.text_cat_attribute44, PAV.text_cat_attribute45, PAV.text_cat_attribute46,
             PAV.text_cat_attribute47, PAV.text_cat_attribute48, PAV.text_cat_attribute49,
             PAV.text_cat_attribute50, PAV.num_cat_attribute1, PAV.num_cat_attribute2,
             PAV.num_cat_attribute3, PAV.num_cat_attribute4, PAV.num_cat_attribute5,
             PAV.num_cat_attribute6, PAV.num_cat_attribute7, PAV.num_cat_attribute8,
             PAV.num_cat_attribute9, PAV.num_cat_attribute10, PAV.num_cat_attribute11,
             PAV.num_cat_attribute12, PAV.num_cat_attribute13, PAV.num_cat_attribute14,
             PAV.num_cat_attribute15, PAV.num_cat_attribute16, PAV.num_cat_attribute17,
             PAV.num_cat_attribute18, PAV.num_cat_attribute19, PAV.num_cat_attribute20,
             PAV.num_cat_attribute21, PAV.num_cat_attribute22, PAV.num_cat_attribute23,
             PAV.num_cat_attribute24, PAV.num_cat_attribute25, PAV.num_cat_attribute26,
             PAV.num_cat_attribute27, PAV.num_cat_attribute28, PAV.num_cat_attribute29,
             PAV.num_cat_attribute30, PAV.num_cat_attribute31, PAV.num_cat_attribute32,
             PAV.num_cat_attribute33, PAV.num_cat_attribute34, PAV.num_cat_attribute35,
             PAV.num_cat_attribute36, PAV.num_cat_attribute37, PAV.num_cat_attribute38,
             PAV.num_cat_attribute39, PAV.num_cat_attribute40, PAV.num_cat_attribute41,
             PAV.num_cat_attribute42, PAV.num_cat_attribute43, PAV.num_cat_attribute44,
             PAV.num_cat_attribute45, PAV.num_cat_attribute46, PAV.num_cat_attribute47,
             PAV.num_cat_attribute48, PAV.num_cat_attribute49, PAV.num_cat_attribute50 
	...
	SELECT PLI.requisition_line_id, PLI.ip_category_id
	  INTO p_req_line_id, p_ip_cat_id
	  FROM po_lines_interface PLI, por_attribute_values_tlp PAVT
	  WHERE PLI.interface_header_id = p_interface_header_id
	    AND PLI.requisition_line_id = PAVT.req_line_id;
	...
	IF g_debug_stmt THEN
        PO_DEBUG.debug_begin(p_log_head => g_log_head||'Before insert into po_attr_values_tlp_interface');
		PO_DEBUG.debug_begin(p_log_head => g_log_head||'p_req_line_id = '||p_req_line_id);
		PO_DEBUG.debug_begin(p_log_head => g_log_head||'p_ip_cat_id = '||p_ip_cat_id);
    END IF;
    ...
    INSERT INTO po_attr_values_tlp_interface
      ...
      SELECT po_attr_values_tlp_interface_s.NEXTVAL, PLI.INTERFACE_HEADER_ID,
            PLI.INTERFACE_LINE_ID, PLI.ACTION, PLI.PROCESS_CODE,
            PLI.PO_LINE_ID, PLI.TEMPLATE_NAME, PLI.ITEM_ID, 
            PLI.ORGANIZATION_ID, PAVT.LANGUAGE, PLI.IP_CATEGORY_ID, 
             PAVT.tl_text_base_attribute1, PAVT.tl_text_base_attribute2, PAVT.tl_text_base_attribute3,
             PAVT.tl_text_base_attribute4, PAVT.tl_text_base_attribute5, PAVT.tl_text_base_attribute6,
             PAVT.tl_text_base_attribute7, PAVT.tl_text_base_attribute8, PAVT.tl_text_base_attribute9,
             PAVT.tl_text_base_attribute10, PAVT.tl_text_base_attribute11, PAVT.tl_text_base_attribute12,
             PAVT.tl_text_base_attribute13, PAVT.tl_text_base_attribute14, PAVT.tl_text_base_attribute15,
             PAVT.tl_text_base_attribute16, PAVT.tl_text_base_attribute17, PAVT.tl_text_base_attribute18,
             PAVT.tl_text_base_attribute19, PAVT.tl_text_base_attribute20, PAVT.tl_text_base_attribute21,
             PAVT.tl_text_base_attribute22, PAVT.tl_text_base_attribute23, PAVT.tl_text_base_attribute24,
             PAVT.tl_text_base_attribute25, PAVT.tl_text_base_attribute26, PAVT.tl_text_base_attribute27,
             PAVT.tl_text_base_attribute28, PAVT.tl_text_base_attribute29, PAVT.tl_text_base_attribute30,
             PAVT.tl_text_base_attribute31, PAVT.tl_text_base_attribute32, PAVT.tl_text_base_attribute33,
             PAVT.tl_text_base_attribute34, PAVT.tl_text_base_attribute35, PAVT.tl_text_base_attribute36,
             PAVT.tl_text_base_attribute37, PAVT.tl_text_base_attribute38, PAVT.tl_text_base_attribute39,
             PAVT.tl_text_base_attribute40, PAVT.tl_text_base_attribute41, PAVT.tl_text_base_attribute42,
             PAVT.tl_text_base_attribute43, PAVT.tl_text_base_attribute44, PAVT.tl_text_base_attribute45,
             PAVT.tl_text_base_attribute46, PAVT.tl_text_base_attribute47, PAVT.tl_text_base_attribute48,
             PAVT.tl_text_base_attribute49, PAVT.tl_text_base_attribute50, PAVT.tl_text_base_attribute51,
             PAVT.tl_text_base_attribute52, PAVT.tl_text_base_attribute53, PAVT.tl_text_base_attribute54,
             PAVT.tl_text_base_attribute55, PAVT.tl_text_base_attribute56, PAVT.tl_text_base_attribute57,
             PAVT.tl_text_base_attribute58, PAVT.tl_text_base_attribute59, PAVT.tl_text_base_attribute60,
             PAVT.tl_text_base_attribute61, PAVT.tl_text_base_attribute62, PAVT.tl_text_base_attribute63,
             PAVT.tl_text_base_attribute64, PAVT.tl_text_base_attribute65, PAVT.tl_text_base_attribute66,
             PAVT.tl_text_base_attribute67, PAVT.tl_text_base_attribute68, PAVT.tl_text_base_attribute69,
             PAVT.tl_text_base_attribute70, PAVT.tl_text_base_attribute71, PAVT.tl_text_base_attribute72,
             PAVT.tl_text_base_attribute73, PAVT.tl_text_base_attribute74, PAVT.tl_text_base_attribute75,
             PAVT.tl_text_base_attribute76, PAVT.tl_text_base_attribute77, PAVT.tl_text_base_attribute78,
             PAVT.tl_text_base_attribute79, PAVT.tl_text_base_attribute80, PAVT.tl_text_base_attribute81,
             PAVT.tl_text_base_attribute82, PAVT.tl_text_base_attribute83, PAVT.tl_text_base_attribute84,
             PAVT.tl_text_base_attribute85, PAVT.tl_text_base_attribute86, PAVT.tl_text_base_attribute87,
             PAVT.tl_text_base_attribute88, PAVT.tl_text_base_attribute89, PAVT.tl_text_base_attribute90,
             PAVT.tl_text_base_attribute91, PAVT.tl_text_base_attribute92, PAVT.tl_text_base_attribute93,
             PAVT.tl_text_base_attribute94, PAVT.tl_text_base_attribute95, PAVT.tl_text_base_attribute96,
             PAVT.tl_text_base_attribute97, PAVT.tl_text_base_attribute98, PAVT.tl_text_base_attribute99,
             PAVT.tl_text_base_attribute100, PAVT.tl_text_cat_attribute1,
             PAVT.tl_text_cat_attribute2, PAVT.tl_text_cat_attribute3, PAVT.tl_text_cat_attribute4,
             PAVT.tl_text_cat_attribute5, PAVT.tl_text_cat_attribute6, PAVT.tl_text_cat_attribute7,
             PAVT.tl_text_cat_attribute8, PAVT.tl_text_cat_attribute9, PAVT.tl_text_cat_attribute10,
             PAVT.tl_text_cat_attribute11, PAVT.tl_text_cat_attribute12, PAVT.tl_text_cat_attribute13,
             PAVT.tl_text_cat_attribute14, PAVT.tl_text_cat_attribute15, PAVT.tl_text_cat_attribute16,
             PAVT.tl_text_cat_attribute17, PAVT.tl_text_cat_attribute18, PAVT.tl_text_cat_attribute19,
             PAVT.tl_text_cat_attribute20, PAVT.tl_text_cat_attribute21, PAVT.tl_text_cat_attribute22,
             PAVT.tl_text_cat_attribute23, PAVT.tl_text_cat_attribute24, PAVT.tl_text_cat_attribute25,
             PAVT.tl_text_cat_attribute26, PAVT.tl_text_cat_attribute27, PAVT.tl_text_cat_attribute28,
             PAVT.tl_text_cat_attribute29, PAVT.tl_text_cat_attribute30, PAVT.tl_text_cat_attribute31,
             PAVT.tl_text_cat_attribute32, PAVT.tl_text_cat_attribute33, PAVT.tl_text_cat_attribute34,
             PAVT.tl_text_cat_attribute35, PAVT.tl_text_cat_attribute36, PAVT.tl_text_cat_attribute37,
             PAVT.tl_text_cat_attribute38, PAVT.tl_text_cat_attribute39, PAVT.tl_text_cat_attribute40,
             PAVT.tl_text_cat_attribute41, PAVT.tl_text_cat_attribute42, PAVT.tl_text_cat_attribute43,
             PAVT.tl_text_cat_attribute44, PAVT.tl_text_cat_attribute45, PAVT.tl_text_cat_attribute46,
             PAVT.tl_text_cat_attribute47, PAVT.tl_text_cat_attribute48, PAVT.tl_text_cat_attribute49,
...
BEGIN 
    ...
    END IF;
     ...
     IF x_document_id IS NOT NULL THEN 
       ...
       l_progress := '100';
       --bug 18652325 update_accrue_on_receipt_flag
       update_accrue_on_receipt_flag(p_document_id => x_document_id);
...
l_attch_suppress_flag VARCHAR2(1);  --Bug20735890
...
BEGIN
  ...
  LOOP 
   ...
   --Bug 20735890 Start: The sourcing attachments should not be suppressed
   -- if the line action is ADD. 
   l_attch_suppress_flag := null;
   IF c_line_dtls.action = 'ADD' THEN
     l_attch_suppress_flag := 'N';   
   END IF;
   --Bug 20735890 End   
   ...
   l_is_complex_work_po := PO_COMPLEX_WORK_PVT.is_complex_work_po(p_document_id);
    ...
    ( x_auction_header_id      =>  c_line_dtls.auction_header_id,
      ...
      x_column1		       =>  l_column1,
      x_attch_suppress_flag    =>  l_attch_suppress_flag,
      X_created_by 	       =>  FND_GLOBAL.user_id,
      X_last_update_login      =>  FND_GLOBAL.login_id
     );
...
p_interface_header_id IN NUMBER
  ...
  WHERE poll.po_header_id = p_document_id
    ...
    AND poll.line_location_id   = plli.line_location_id  --Bug#17063664
...
--Start of Comments
--Bug 18652325 start
--Name: update_accrue_on_receipt_flag
--Procedure:
--  This procedure update the accrue_on_receipt_flag to match the 
--  destination_type_code on distribution level.
--  This is to fix the accrue_on_receipt_flag problem after PDOI ER BUG#17063664
--Parameters:
--IN:
-- p_document_id
--IN OUT:
...
(p_document_id IN NUMBER) IS
  ...
  l_api_name    CONSTANT VARCHAR2(30) := 'update_accrue_on_receipt_flag';
  l_progress VARCHAR2(3) := '000';
...
BEGIN
 ...
 LOOP
    ...
    IF p_doc_dtls.item_id IS NULL AND
    p_doc_dtls.destination_type_code = 'SHOP FLOOR' 
    THEN

      UPDATE po_line_locations_all
      SET accrue_on_receipt_flag = 'Y'
      WHERE line_location_id = p_doc_dtls.line_location_id;

      UPDATE po_distributions_all
      SET accrue_on_receipt_flag = 'Y'
      WHERE po_distribution_id = p_doc_dtls.po_distribution_id;
...
EXCEPTION
  WHEN OTHERS THEN
     ...
     END IF;
 RAISE;
END update_accrue_on_receipt_flag;
--Bug 18652325 end

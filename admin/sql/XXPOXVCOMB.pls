...
REM dbdrv: checkfile(120.49.12000000.77=120.61.12010000.61)(115.77=120.0):~PROD:~PATH:~FILE
...
/* $Header: POXVCOMB.pls 120.61.12010000.73 2013/05/03 10:41:22 srpantha ship $ */
...
-- SQL Join:
    ...
    PO_COMMUNICATION_PVT.g_with_canceled_lines := p_with_canceled_lines;
    ...
    || l_xml_message_query || ',' || l_head_short_attachment_query || ',' || l_head_url_attachment_query || ',' || l_head_file_attachment_query ||'
      FROM PO_HEADERS_XML phx WHERE phx.PO_HEADER_ID = PO_COMMUNICATION_PVT.getDocumentId() AND phx.revision_num = PO_COMMUNICATION_PVT.getRevisionNum()';
        ...
        IF(p_document_subtype <> 'CONTRACTS') THEN -- contracts will have only headers
          ...
          SELECT count(*) into l_count FROM po_lines_all 
           WHERE po_header_id = p_document_id
             AND nvl(cancel_flag, 'N') = decode(p_with_canceled_lines, 'N', 'N', nvl(cancel_flag, 'N')); --Bug#6138794
	 ...
	 --Bug#6138794, add flag to control if includes the cancelled lines or not, default will include
            ...
            l_xml_message_query || ',' || l_head_short_attachment_query || ',' || l_head_url_attachment_query || ',' || l_head_file_attachment_query ||',
              ...
              WHERE  plx.po_header_id = phx.po_header_id and nvl(plx.cancel_flag, ''N'') = decode(PO_COMMUNICATION_PVT.getWithCanceledLines(), ''N'', ''N'', nvl(plx.cancel_flag, ''N'')) order by plx.line_num) AS lines
              FROM PO_HEADERS_XML phx WHERE phx.PO_HEADER_ID = PO_COMMUNICATION_PVT.getDocumentId() AND phx.revision_num = PO_COMMUNICATION_PVT.getRevisionNum()';
	    ...
	    --Bug#6138794, add flag to control if includes the cancelled lines or not, default will include
	      ...
	      shows canceled lines with latest revision only **************/
              ...
              FROM po_lines_xml plx WHERE plx.po_header_id = phx.po_header_id and nvl(plx.cancel_flag, ''N'') = decode(PO_COMMUNICATION_PVT.getWithCanceledLines(), ''N'', ''N'', nvl(plx.cancel_flag, ''N'')) order by plx.line_num ) AS lines
  ...
  /*  Bug#3574748: Added the condition SHIPMENT_TYPE in ('BLANKET','STANDARD') in shipment query. */
    ...
    FROM PO_HEADERS_ARCHIVE_XML phx WHERE phx.PO_HEADER_ID = PO_COMMUNICATION_PVT.getDocumentId() AND phx.revision_num = PO_COMMUNICATION_PVT.getRevisionNum()';
	 ...
	 --Bug#6138794, add flag to control if includes the cancelled lines or not, default will include
          ...
          and pla.revision_num <= pcgt.revision_number ) and nvl(plx.cancel_flag, ''N'') = decode(PO_COMMUNICATION_PVT.getWithCanceledLines(), ''N'', ''N'', nvl(plx.cancel_flag, ''N'')) order by plx.line_num ) AS lines

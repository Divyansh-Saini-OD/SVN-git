...
CREATE OR REPLACE VIEW APPS.XXBI_USER_SITE_DTL_MGR_FCT_V AS
...
where  sdtl.rsd_user_id = nvl(xxbi_utility_pkg.get_rsd_user_id(FND_GLOBAL.USER_ID), 9999999999)
  and  (sdtl.user_id   in (select map_user_id
                           from   xxcrm.XXBI_GROUP_MBR_USER_ID_MV m 
                           where  m.user_id =FND_GLOBAL.USER_ID
                          )
       ) 

create or replace
PACKAGE BODY XX_CDH_NEW_CONV_MASTER_PKG
-- +===================================================================+
...
-- |========  =========== ================== ==========================|
-- |1.0       01-Mar-2010 Sreedhar Mohan     Copied from Conv Master   |
-- |                                                                   |
-- +===================================================================+
AS
...
BEGIN
   ...
   IF p_submit_bulk = 'Y' THEN
      ...
      -------------------------------------------------------------------
      --Defect 29919 - run batch activation only for US jobs
...
-- |========  =========== ================== ==========================|
-- |1.0       01-Mar-2010 Sreedhar Mohan     Copied from Conv Master   |
-- |1.1       24-Sep-2012 Deepti S           Added 5 conc programs in  |
-- |                                         the request stage 20      |
-- +===================================================================+
...
ln_ebs_batch_id                  NUMBER := p_ebs_batch_id;
lb_success                       BOOLEAN;
ln_req_id                        NUMBER;
...
BEGIN
   ...
   ----------------------------------------------------------
   -- Set the context for the request set XX_CDH_NEW_LOAD_CV_TCA
   ----------------------------------------------------------
   lb_success := fnd_submit.set_request_set('XXCNV', 'XX_CDH_NEW_LOAD_CV_TCA');
  ...
  /* ---------------------------------------------------------------------------------
      -- Submit program OD: CDH Load Oracle INT to STG Process which is in stage STAGE20
      ----------------------------------------------------------------------------------

      lb_success := fnd_submit.submit_program
                       (  application => 'XXCNV',
                          program     => 'XX_CDH_CONV_LOAD_INT_STG_PKG',
                          stage       => 'XX_CDH_LOAD_CVSTG',
                          argument1   => ln_ebs_batch_id
                       );
      IF ( NOT lb_success ) THEN
         RAISE le_submit_failed;
...
where  batch_id between p_from_ebs_batch_id
and    p_to_ebs_batch_id;
...
BEGIN
   ...
   LOOP
      ...
      ----------------------------------------------------------
      -- Set the context for the request set XX_CDH_NEW_CONV_AOPS_RS
      ----------------------------------------------------------
      lb_success := fnd_submit.set_request_set('XXCNV', 'XX_CDH_NEW_CONV_AOPS_RS');

      IF ( lb_success ) THEN
         ------------------------------------------------------------------------------------------
         -- Submit program OD: CDH Submit AOPS Conversion Request Set which is in stage STAGE10
         ------------------------------------------------------------------------------------------
         lb_success := fnd_submit.submit_program
                          (    application => 'XXCNV'
                              ,program     => 'XX_CDH_NEW_SUBMIT_AOPS_RS'
                              ,stage       => 'XX_CDH_NEW_SUBMIT_AOPS_RS'
                              ,argument1   => i.batch_id
                              ,argument2   => p_submit_update
                              ,argument3   => p_sbmt_load_INT_to_STG
         ...
         ELSE
            fnd_file.put_line (fnd_file.log, ' ');
            fnd_file.put_line (fnd_file.log, 'AOPS Conversion Request Set submitted with request id: '|| TO_CHAR( ln_req_id )||' for AOPS batch : '||i.batch_id);
            COMMIT;
         END IF;
      END IF;
   END LOOP;
...
lv_ebs_table_name               VARCHAR2(2000);
...
BEGIN
   ...
   LOOP
      ...
      ELSE
         IF lt_ebs_batch_rec.ebs_batch_id = (ln_end_ebs_batch + 1) THEN
            ln_end_ebs_batch := lt_ebs_batch_rec.ebs_batch_id;
         ELSE
            ---------------------
            -- Submit AOPS Master
            ...
            lt_conc_request_id := FND_REQUEST.submit_request
                                          (   application => 'XXCNV',
                                              program     => 'XX_CDH_NEW_AOPS_CONV_MASTER',
                                              description => NULL,
                                              start_time  => NULL,
                                              sub_request => FALSE,
                                              ...
                                              argument5   => p_sbmt_load_INT_to_STG,
            ...
            ELSE
               ...
               fnd_file.put_line (fnd_file.log, 'Start AOPS EBS Batch Id : '|| ln_start_ebs_batch);
               fnd_file.put_line (fnd_file.log, 'End   AOPS EBS Batch Id : '|| ln_end_ebs_batch);
               COMMIT;
            END IF;
         END IF;
      END IF;
   ...
   IF lv_batch_submitted = 'N' AND
      ln_start_ebs_batch IS NOT NULL AND
      ln_end_ebs_batch   IS NOT NULL THEN

...
AS
  ...
  --   6.8       08/02/2019  Jitendra           AR Refund & POS mailcheck interface changes to accommodate using Cloud numbering when Supplier Master in in Cloud
  -- =========================================================================================================================
  -------------------------------------------------
  --Start of changes for Defect #3340
  -------------------------------------------------
TYPE receipt_writeoff_record
IS
  RECORD
  (
...
TYPE receipt_writeoff_type
IS
  TABLE OF receipt_writeoff_record INDEX BY BINARY_INTEGER;
...
IS
  ...
  IS
    ...
    WHERE (refund_request IN('Send Refund', 'Send Refund Alt')
    OR(refund_request     IS NULL
    AND check_cust         = 0))
    ORDER BY customer_id;
  -- Added for QC Defect 17501 - End
  CURSOR id_refund_trx_escheat( i_trx_date_from IN DATE, i_trx_date_to IN DATE, p_escheat_inact_days IN NUMBER) -- Added for CR 697/698 R1.2
  IS
    ...
    OR refund_status                     = 'Declined')
    AND ( (NVL(p_only_pre_selected, 'N') = 'Y'
    AND refund_request                  IN('Declined', 'Escheat'))
    OR (NVL(p_only_pre_selected, 'N')    = 'N'))
    AND amount_due_remaining BETWEEN NVL( -1 * p_amount_to, -9999999999999) AND NVL( -1 * p_amount_from, -0.000001)
    AND trx_date BETWEEN NVL(i_trx_date_from, trx_date) AND NVL(i_trx_date_to, trx_date)
    AND cash_receipt_status IN('UNAPP', 'UNID') -- Added by Rama Krishna K for V 3.4
    ...
    OR refund_request                                                                             IS NULL
    AND check_cust(p_no_activity_in, p_escheat_inact_days, xatv.customer_id, xatv.cash_receipt_id) = 0)
    ORDER BY customer_id;
  ...
  CURSOR id_refund_trx_sel( i_trx_date_from IN DATE, i_trx_date_to IN DATE)
  IS
    SELECT
      /*+ LEADING(@"SEL$D9D7977F_2" "RCT"@"SEL$D9D7977F_2") LEADING(@"SEL$53D7279B_1" "ACR"@"SEL$2") */
    ...
    WHERE org_id               = p_org_id
    AND refund_request        IN('Send Refund', 'Send Refund Alt')
    AND ( (p_only_for_user_id IS NOT NULL
    AND last_updated_by        = p_only_for_user_id)
    OR (p_only_for_user_id    IS NULL))
      --   AND ( refund_status IS NULL OR refund_status = 'Declined' )  Commented for CR 697 / 698
    AND amount_due_remaining BETWEEN NVL( -1 * p_amount_to, -9999999999999) AND NVL( -1 * p_amount_from, -0.000001)
    AND trx_date BETWEEN NVL(i_trx_date_from, trx_date) AND NVL(i_trx_date_to, trx_date);
  CURSOR id_refund_trx_om( i_trx_date_from IN DATE, i_trx_date_to IN DATE)
  IS
    SELECT SOURCE,
      customer_id,
      NULL customer_number,
...
TYPE idtrxtab
IS
  TABLE OF idtrxrec INDEX BY BINARY_INTEGER;
  vidtrxtab idtrxtab;
...
BEGIN
  ...
  INTO xx_ar_refund_itm
    (SELECT SOURCE,
      ...
      OR refund_status    = 'Declined')
      AND amount_due_remaining BETWEEN NVL( -1 * p_amount_to, -9999999999999) AND NVL( -1 * p_amount_from, -0.000001)
      AND trx_date BETWEEN NVL(lp_date_from, trx_date) AND NVL(lp_date_to, trx_date)
      AND ( (NVL(p_only_pre_selected, 'N') = 'Y'
      AND refund_request                  IN('Send Refund', 'Send Refund Alt'))
      OR (NVL(p_only_pre_selected, 'N')    = 'N'))
  ...
  BEGIN
    ...
    LOOP
      ln_check_cust   := 1;
      ln_check_cust   := check_cust(p_no_activity_in, ln_refund_inact_days, i.customer_id, i.cash_receipt_id);
      IF ln_check_cust = 0 THEN
...
EXCEPTION
WHEN OTHERS THEN
  fnd_file.put_line(fnd_file.LOG, 'Exception in PROCEDURE insert_refund_itm : ' || SQLERRM);
...
IS
BEGIN
  fnd_file.put_line(fnd_file.LOG, 'fetching lookups');
  SELECT meaning
  ...
  IF (p_process_type   = 'OM') THEN
    OPEN id_refund_trx_om(lp_date_from, lp_date_to);
  ELSIF(p_process_type = 'E') THEN
    --OPEN id_refund_trx_escheat(lp_date_from, lp_date_to); Commented for CR 697/698 R1.2
    OPEN id_refund_trx_escheat(lp_date_from, lp_date_to, ln_escheat_inact_days); -- Added for CR 697/698 R1.2
  ELSE                                                                           -- IF (lp_process_type = 'R') THEN
    IF (p_only_pre_selected = 'Y') THEN
      OPEN id_refund_trx_sel(lp_date_from, lp_date_to);
    ELSE
      ...
      -- Added for QC Defect 17501 - Start
      insert_refund_itm(lp_date_from, lp_date_to, ln_refund_inact_days);
...
BEGIN
  IF (p_process_type = 'OM') THEN
    FETCH id_refund_trx_om BULK COLLECT INTO vidtrxtab LIMIT p_limit_size;
    --Changed the limit size hardcoded value to parameter  for defect 4901 on 05-APR-10
  ELSIF(p_process_type = 'E') THEN
    FETCH id_refund_trx_escheat BULK COLLECT INTO vidtrxtab LIMIT p_limit_size;
    --Changed the limit size hardcoded value to parameter  for defect 4901 on 05-APR-10
  ELSE -- IF (p_process_type = 'R') THEN
    IF (p_only_pre_selected = 'Y') THEN
      FETCH id_refund_trx_sel BULK COLLECT INTO vidtrxtab LIMIT p_limit_size;
      --Changed the limit size hardcoded value to parameter  for defect 4901 on 05-APR-10
    ELSE
      FETCH id_refund_trx_ar BULK COLLECT INTO vidtrxtab LIMIT p_limit_size;
      --Changed the limit size hardcoded value to parameter  for defect 4901 on 05-APR-10
    END IF;
  END IF;
END;
...
IS
BEGIN
  IF (id_refund_trx_om%ISOPEN) THEN
    CLOSE id_refund_trx_om;
  END IF;
  IF (id_refund_trx_escheat%ISOPEN) THEN
    CLOSE id_refund_trx_escheat;
  END IF;
  IF (id_refund_trx_sel%ISOPEN) THEN
    CLOSE id_refund_trx_sel;
  END IF;
  IF (id_refund_trx_ar%ISOPEN) THEN
...
BEGIN
  ...
  ln_id_count   := 0;
  ln_ins_count  := 0;
  mo_global.set_policy_context('S', p_org_id); --Added For R12 Upgrade Retrofit
  BEGIN
  ...
  EXCEPTION
  WHEN OTHERS THEN
    od_message('O', 'Set of Books not found!');
    od_message('M', 'Set of Books not found!');
    RAISE;
  END;
  od_message('O', ' ');
  od_message('O', 'Parameters: ');
  od_message('O', '----------- ');
  od_message('O', ' ');
  od_message('O', 'Identify:' || p_process_type || ' with no activity in:' || NVL(p_no_activity_in, 0) || ' days');
  IF p_amount_from IS NOT NULL OR p_amount_to IS NOT NULL THEN
    od_message('O', '   Transaction Amounts between :' || p_amount_from || ' and ' || p_amount_to);
  END IF;
  ...
  IF p_trx_date_to IS NOT NULL THEN
    ...
    od_message('O', '   Transactions between :' || ld_date_from || ' and ' || ld_date_to);
  END IF;
  IF NVL(p_only_pre_selected, 'N') = 'Y' THEN
    od_message('O', '   Identify Only Pre-Selected Transactions:' || p_only_pre_selected);
    IF p_only_for_user_id IS NOT NULL THEN
      BEGIN
        SELECT user_name
        INTO lc_user_name
        FROM fnd_user
        WHERE user_id = p_only_for_user_id;
        od_message('O', '   Identify Pre-Selected Transactions for only User: ' || lc_user_name);
      EXCEPTION
      WHEN OTHERS THEN
        od_message('M', ' Identify Pre-Selected Transactions for only User: User Not Found');
      END;
    END IF;
  END IF;
  od_message('O', ' ');
  od_message('O', g_print_line);
  od_message('O', ' ');
  od_message('O', ' ');
  -- based on the parameters, open the corresponding cursor
  open_cursor(lp_date_from => ld_date_from, lp_date_to => ld_date_to);
  LOOP
    -- fetch next set of records for the related cursor
    fnd_file.put_line(fnd_file.LOG, 'Fetch start' || TO_CHAR(SYSDATE, 'HH24:MI:SS'));
    FETCH_RECORDS();
    fnd_file.put_line(fnd_file.LOG, 'Fetch end' || TO_CHAR(SYSDATE, 'HH24:MI:SS'));
    EXIT
  WHEN vidtrxtab.COUNT = 0;
    ln_id_count       := ln_id_count + vidtrxtab.COUNT;
    od_message('M', 'Processing...' || vidtrxtab.COUNT || ' transactions identified...');
    fnd_file.put_line(fnd_file.LOG, p_process_type || ' ' || p_no_activity_in || ' ' || ln_refund_inact_days || ' ' || ln_escheat_inact_days);
    IF vidtrxtab.COUNT > 0 THEN
      ...
      LOOP
        ...
        FROM DUAL;
        IF (vidtrxtab(i).customer_id IS NOT NULL) THEN
          BEGIN
            SELECT account_number,
              party_name,
              SUBSTR(hca.orig_system_reference, 1, 8) aops_customer_number -- Added for R1.2 CR714(Defect# 2532)
          ...
          EXCEPTION
          WHEN OTHERS THEN
            od_message('M', 'Errors while fetching customer information. ' || SQLERRM);
          END;
        ELSE
          -- if customer is not defined (unidentified), and refunding the customer,
          --   then through an error message
          IF (lc_ident_type = 'R') THEN
            od_message('M', 'Processing a refund for a transaction without a customer.');
          END IF;
        END IF;
        -- defect 11109 - set party_name to store_customer_name if OM/SAS mailcheck
        IF (vidtrxtab(i).store_customer_name IS NOT NULL) THEN
          vidtrxtab(i).party_name            := vidtrxtab(i).store_customer_name;
        END IF;
        IF p_process_type IN('E') AND vidtrxtab(i).SOURCE != 'OM' THEN
          ...
          lc_write_off_only      := 'N';
          lc_dff1                := 'Escheat';
          IF lc_sob_name          = 'US_USD_P' THEN
            IF vidtrxtab(i).CLASS = 'PMT' THEN
              lc_activity_type   := 'US_ESCHEAT_REC_WRITEOFF_OD';
            ELSE
              lc_activity_type := 'US_ESCHEAT_CM_WRITEOFF_OD';
            END IF;
          ELSIF lc_sob_name       = 'CA_CAD_P' THEN
            IF vidtrxtab(i).CLASS = 'PMT' THEN
          ...
          END IF;
        ELSIF p_process_type IN('OM') AND vidtrxtab(i).SOURCE = 'OM' AND vidtrxtab(i).CLASS = 'PMT' THEN
          -- OM Mail-Check Refunds.
          -- Check the Hold/Delete status to determine if this is
          -- an escheat trx
          IF (vidtrxtab(i).om_hold_status = 'P' AND vidtrxtab(i).om_delete_status = 'N') THEN
            -- Pay Customer;
            lc_write_off_only  := 'N';
            lc_escheat_flag    := 'N';
            IF lc_sob_name      = 'US_USD_P' THEN
          ...
          ELSIF(vidtrxtab(i).om_delete_status IN('A', 'E')) THEN
            --Writeoff_As_escheat;
            lc_escheat_flag    := 'Y';
            lc_write_off_only  := 'N';
            IF lc_sob_name      = 'US_USD_P' THEN
          ...
          ELSE
            ...
            lc_escheat_flag                       := 'N';
            IF lc_sob_name                         = 'US_USD_P' THEN
              IF (vidtrxtab(i).om_delete_status    = 'S') THEN
                lc_activity_type                  := 'US_MAILCK_REV_' || vidtrxtab(i).om_store_number || '_OD';
              ELSIF((vidtrxtab(i).om_delete_status = 'O')
                -- V4.0, Added OR Condition for delete_status 'M'
                OR(vidtrxtab(i).om_delete_status = 'M')) THEN
                lc_activity_type                := 'US_MAILCK_O/S_' || vidtrxtab(i).om_store_number || '_OD';
              ELSE
            ...
            ELSIF lc_sob_name                      = 'CA_CAD_P' THEN
              IF (vidtrxtab(i).om_delete_status    = 'S') THEN
                lc_activity_type                  := 'CA_MAILCK_REV_' || vidtrxtab(i).om_store_number || '_OD';
              ELSIF((vidtrxtab(i).om_delete_status = 'O')
                -- V4.0, Added OR Condition for delete_status 'M'
                OR(vidtrxtab(i).om_delete_status = 'M')) THEN
                lc_activity_type                := 'CA_MAILCK_O/S_' || vidtrxtab(i).om_store_number || '_OD';
              ELSE
                -- V4.0, For any other combinations, display error message
                GOTO NEXT_RECORD;
            ...
            END IF;
          END IF;
          IF NVL(lc_escheat_flag, 'N') = 'Y' THEN
            lc_dff1                   := 'Escheat';
          ELSE
        ...
        ELSE
          ...
          ELSE
            RAISE exp_invalid_sob;
          END IF;
          IF (vidtrxtab(i).refund_request   = 'Send Refund Alt') THEN
            lc_escheat_flag                := 'N';
            lc_selected_flag               := 'N';
            lc_refund_alt_flag             := 'Y';
            lc_dff1                        := 'Send Refund Alt';
          ELSIF(vidtrxtab(i).refund_request = 'Send Refund') THEN
            lc_escheat_flag                := 'N';
            lc_selected_flag               := 'N';
            lc_refund_alt_flag             := 'N';
            lc_dff1                        := 'Send Refund';
          ELSIF(vidtrxtab(i).refund_request = 'Escheat') THEN
        ...
        ELSE -- 'NON-OM' Source.
          IF NVL(lc_refund_alt_flag, 'N') = 'Y' THEN
            ln_primary_bill_loc_id       := vidtrxtab(i).location_id;
            lc_address1                  := NULL;
            lc_address2                  := NULL;
            lc_address3                  := NULL;
          ...
          ELSE
            ...
            od_message('M', 'Get primary bill Site for Customer# ' || vidtrxtab(i).customer_number || ' - ' || vidtrxtab(i).party_name);
            IF (vidtrxtab(i).customer_id IS NOT NULL) THEN
              BEGIN
        ...
        BEGIN
          ...
          INTO xx_ar_refund_trx_tmp
            ...
            (
              ...
              ,
              DECODE(vidtrxtab(i).CLASS, 'PMT', 'R', 'CM', 'C', 'INV', 'I') -- Added for CR 697/698 R1.2
              ,
              vidtrxtab(i).trx_number,
              vidtrxtab(i).trx_currency_code,
              -1 * vidtrxtab(i).refund_amount,
              'N',
              lc_selected_flag
              ...
              ,
              DECODE(lc_ident_type, 'OM', 'US_MAILCHECK_OD', lc_payment_method_name) -- v5.0
              ,
          ...
          END IF;
          ln_ins_count           := NVL(ln_ins_count, 0)  + 1;
          ln_refund_tot          := NVL(ln_refund_tot, 0) + vidtrxtab(i).refund_amount * -1;
          IF NVL(ln_ins_count, 0) = 1 THEN
            od_message('O', g_print_line);
            -- Customer # (10)  Customer Name (30) Type (12)
            -- Transaction Number (15)  Refund Amount (15)
            -- Status (7)  Escheat(8) Refund Alt (10)
            od_message('O', 'Customer# ' || 'Payee                         ' || 'Trx Type    ' || 'Trx Number     ' || ' Refund Amount '
            -- || 'Status '
            || 'Escheat ' || 'Refund Alt');
            od_message('O', g_print_line);
            od_message('O', '');
          ...
          IF vidtrxtab(i).CLASS    = 'PMT' THEN
            lc_trx_type           := 'Receipt';
          ELSIF vidtrxtab(i).CLASS = 'CM' THEN -- 697/698 Change
            lc_trx_type           := 'Credit Memo';
          ELSE
            lc_trx_type := 'Invoice';
          END IF;
          od_message('O', RPAD(SUBSTR(vidtrxtab(i).customer_number, 1, 10), 10, ' ') || RPAD(SUBSTR(vidtrxtab(i).party_name, 1, 30), 30, ' ') || RPAD(lc_trx_type, 12, ' ') || RPAD(SUBSTR(vidtrxtab(i).trx_number, 1, 15), 15, ' ') || LPAD(TO_CHAR( vidtrxtab(i).refund_amount * -1, '99G999G990D00PR'), 15, ' ') || '   ' || lc_escheat_flag || '    ' || '    ' || lc_refund_alt_flag || '    ');
        EXCEPTION
        WHEN OTHERS THEN
          od_message('M', SQLCODE || ': ' || SQLERRM || ' When saving Trx ' || vidtrxtab(i).CLASS || ':' || vidtrxtab(i).trx_number);
        END;
        -- V4.0, Ignore the record, in case of wrong Combination of Delete_Status and Hold_Status
        <<NEXT_RECORD>> -- V4.
        od_message('E', 'Invalid Combination of Delete_Status and Hold_Status');
      END LOOP;
    END IF;
  END LOOP;
  IF ln_id_count > 0 THEN
    od_message('O', ' ');
    od_message('O', g_print_line);
    IF p_process_type = 'R' THEN
      od_message('M', 'Identified ' || ln_id_count || ' transactions to Review Form');
      od_message('M', 'Inserted ' || ln_ins_count || ' transactions to Review Form');
      od_message('O', RPAD(' ', 10, ' ') || RPAD(' ', 30, ' ') || LPAD('Total Refunds Identified:', 27, ' ') || LPAD(TO_CHAR(ln_refund_tot, '99G999G990D00PR'), 15, ' '));
    ELSIF p_process_type = 'E' THEN
      od_message('O', RPAD(' ', 10, ' ') || RPAD(' ', 30, ' ') || LPAD('Escheats Identified:', 27, ' ') || LPAD(TO_CHAR(ln_refund_tot, '99G999G990D00PR'), 15, ' '));
    ELSIF p_process_type = 'OM' THEN
      od_message('O', RPAD(' ', 10, ' ') || LPAD('Total Store Refunds/Escheats Identified:', 57, ' ') || LPAD(TO_CHAR(ln_refund_tot, '99G999G990D00PR'), 15, ' '));
    ELSE
      od_message('O', RPAD(' ', 10, ' ') || RPAD(' ', 30, ' ') || LPAD('Total :', 27, ' ') || LPAD(TO_CHAR(ln_refund_tot, '99G999G990D00PR'), 15, ' '));
      od_message('M', 'Identified ' || ln_id_count || ' transactions.');
      od_message('M', 'Inserted ' || ln_ins_count || ' transactions.');
    END IF;
    od_message('O', g_print_line);
    od_message('O', ' ');
  ELSE -- No records Identified.
    od_message('M', 'No Transactions Identified for Review');
  END IF;
  close_cursors();
  COMMIT;
EXCEPTION
WHEN exp_invalid_sob THEN
  close_cursors();
  od_message('E', 'Error Identifying Refund Transactions.', 'Invalid Set of Books');
WHEN OTHERS THEN
  close_cursors();
  od_message('M', 'Error Identifying Refund Transactions.' || SQLCODE || ':' || SQLERRM);
  od_message('E', 'Error Identifying Refund Transactions.');
...
IS
  CURSOR c_refund_hdr( p_org_id IN NUMBER)
  IS
    ...
    AND status                     = 'W'
    AND selected_flag              = 'Y'
    AND ( (NVL(p_om_escheats, 'N') = 'Y'
    AND(identification_type        = 'OM'
    AND escheat_flag               = 'Y'))
    OR (NVL(p_om_escheats, 'N')    = 'N'
    AND NOT(identification_type    = 'OM'
    AND escheat_flag               = 'Y')))
    AND org_id                     = p_org_id
    AND last_updated_by            = NVL(p_user_id, last_updated_by) --Added for the defect 8304
    ...
    ORDER BY customer_number,
      trx_type;
  CURSOR c_trx( p_customer_id IN NUMBER, p_org_id IN NUMBER, p_refund_header_id IN NUMBER)
  IS
    -- defect 8298 - B.Looman - added hint for performance enhancements
    SELECT
      /*+ INDEX (XX_AR_REFUND_TRX_TMP XX_AR_REFUND_TRX_TMP_U1) */
      *
    FROM xx_ar_refund_trx_tmp
    ...
    AND selected_flag    = 'Y'
    AND org_id           = p_org_id
    AND refund_header_id = p_refund_header_id FOR UPDATE OF adjustment_number,
      adj_created_flag,
      adj_creation_date
    ORDER BY customer_number,
      trx_number;
  CURSOR c_trx_to_unapply( p_cash_receipt_id IN NUMBER)
  IS
  ...
  CURSOR get_addr( p_org_id IN NUMBER)
  IS
    SELECT COUNT(*),
      customer_id,
      org_id,
    ...
    OR alt_city                     IS NULL
    OR NVL(alt_state, alt_province) IS NULL)
...
BEGIN
  ...
  od_message('O', '');
  od_message('O', 'Set of Books:' || fnd_profile.VALUE('GL_SET_OF_BKS_NAME') || '                 OD: Refunds - Create Refunds                        Date:' || TRUNC(SYSDATE));
  od_message('O', '');
  od_message('O', '');
  ...
  BEGIN
    ...
    LOOP
      BEGIN
        ...
        OR NVL(alt_state, alt_province) IS NULL)
        AND identification_type          = 'R';
        od_message('M', 'Updated ' || SQL%ROWCOUNT || ' rows.');
        od_message('M', 'Found Primary Bill_To for Customer#:' || v_get_addr.customer_number || ' in org:' || v_get_addr.org_id);
      EXCEPTION
      WHEN OTHERS THEN
        od_message('M', '*** Error getting Primary Bill_To for Customer#:' || v_get_addr.customer_number || ' in org:' || v_get_addr.org_id || '  Error:' || SQLCODE || ':' || SQLERRM);
      END;
    END LOOP;
  EXCEPTION
  WHEN OTHERS THEN
  ...
  LOOP
    ...
    IF lc_trx_type = 'C' OR lc_trx_type = 'I' THEN -- Added for CR 697/698 R1.2
      lc_trx_type := 'Credit Memo';
    ELSE
      lc_trx_type := 'Receipt';
    END IF;
    od_message('M', '----------------------------------------------------------------------');
    FOR v_trx IN c_trx(v_refund_hdr.customer_id, ln_org_id, v_refund_hdr.refund_header_id)
    LOOP
      lc_err          := 'N';
      lc_adj_warning  := 'N';
      lc_adj_num      := NULL;
      ...
      ln_mail_check := 0;
      od_message('M', 'Customer#:' || v_refund_hdr.customer_number || ' Type:' || v_trx.trx_type || '  Trx#:' || v_trx.trx_number || ' Refund Amount:' || v_refund_hdr.trx_currency_code || ' ' || v_refund_hdr.refund_amount);
      ...
      BEGIN
        SELECT payment_schedule_id,
          DECODE(ps.CLASS, 'PMT', 'Receipt', 'Credit Memo'),
          -1 * amount_due_remaining
        INTO ln_ps_id,
          lc_trx_type,
          ln_amt_rem
        FROM ar_payment_schedules ps
        WHERE ps.customer_id   = v_refund_hdr.customer_id
        AND ( (v_trx.trx_type  = 'R'
        AND ps.cash_receipt_id = v_trx.trx_id)
        ...
        IF v_trx.trx_type      = 'R' THEN
          ...
          WHEN OTHERS THEN
            ln_onacct_amt := 0;
          END;
          od_message('M', 'Amt_Remaining:' || ln_amt_rem || ' On Acct Amt:' || ln_onacct_amt);
          ln_amt_rem := NVL(ln_amt_rem, 0) - NVL(ln_onacct_amt, 0);
        END IF;
      EXCEPTION
      WHEN OTHERS THEN
        lc_err := 'Y';
      ...
      IF ln_amt_rem <> v_trx.refund_amount AND NVL(v_trx.ref_mailcheck_id, 0) = 0 THEN
        lc_err      := 'Y';
        od_message('M', '*** Remaining Amount:' || ln_amt_rem || ' does not match Refund Amount:' || v_trx.refund_amount);
        IF v_trx.trx_type = 'R' AND ln_onacct_amt <> 0 THEN
          UPDATE xx_ar_refund_trx_tmp
          SET error_flag         = 'Y',
            last_update_date     = SYSDATE,
            last_updated_by      = fnd_profile.VALUE('USER_ID'),
      ...
      IF NVL(v_trx.escheat_flag, 'N')      = 'N' AND TRIM(v_trx.alt_address1) IS NULL AND TRIM(v_trx.alt_city) IS NULL AND (TRIM(v_trx.alt_state) IS NULL OR TRIM(v_trx.alt_province) IS NULL) THEN
        IF NVL(v_trx.refund_alt_flag, 'N') = 'Y' OR v_trx.primary_bill_loc_id IS NULL THEN
          ...
          IF NOT( v_trx.identification_type = 'OM' AND v_trx.om_hold_status != 'P' AND v_trx.om_delete_status != 'N') THEN
            lc_err                         := 'Y';
            UPDATE xx_ar_refund_trx_tmp
            SET error_flag         = 'Y',
              last_update_date     = SYSDATE,
            ...
            INTO xx_ar_refund_error_log
              ...
              (
                ...
                lc_trx_type,
                v_trx.trx_number,
                DECODE(v_trx.refund_alt_flag, 'Y', 'Alternate', 'Primary Bill-To')
              );
          END IF;
        END IF;
      END IF;
      IF v_trx.activity_type IS NULL THEN
        lc_err               := 'Y';
        UPDATE xx_ar_refund_trx_tmp
        SET error_flag         = 'Y',
          last_update_date     = SYSDATE,
      ...
      IF lc_err = 'N' THEN
        ...
        IF v_trx.trx_type                      = 'C' OR v_trx.trx_type = 'I' THEN -- Added for CR 697/698 R1.2
          IF NVL(v_trx.escheat_flag, 'N')      = 'N' THEN
            IF NVL(v_trx.refund_alt_flag, 'N') = 'Y' THEN
              lc_reason_code                  := 'REFUND ALT';
              lc_comments                     := ';' || v_trx.alt_address1 || ' ' || v_trx.alt_address2 || ' ' || v_trx.alt_address3 || ' ' || v_trx.alt_city || ' ' || NVL(v_trx.alt_state, v_trx.alt_province) || ' ' || v_trx.alt_postal_code || ' ' || v_trx.alt_country;
            ELSE
          ...
          x_msg_data      := NULL;
          x_msg_count     := NULL;
          create_cm_adjustment(ln_ps_id, v_trx.trx_id, v_refund_hdr.customer_number, v_trx.refund_amount, ln_org_id, v_trx.activity_type --lc_cm_adj_name
          , lc_reason_code, lc_comments, lc_adj_num, x_return_status, x_msg_count, x_msg_data);
          od_message('M', 'After Create CM Adjustment for:' || v_trx.trx_number || ' for :' || v_trx.trx_currency_code || ' ' || v_trx.refund_amount || ' Status:' || x_return_status || ' Message:' || x_msg_count || ':' || x_msg_data);
          ...
          IF NVL(x_return_status, fnd_api.g_ret_sts_success) = fnd_api.g_ret_sts_success THEN
            ...
            od_message('M', 'Adj Api returned Success. Msg Count:' || x_msg_count);
            IF NVL(x_msg_count, 0) > 0 THEN
              ...
              INTO xx_ar_refund_error_log
                ...
                (
                  ...
                  lc_trx_type,
                  v_trx.trx_number,
                  SUBSTR(x_msg_data, 1, 1000)
                );
            END IF;
          ELSIF NVL(x_return_status, fnd_api.g_ret_sts_success) <> fnd_api.g_ret_sts_success THEN
            lc_err                                              := 'Y';
            IF NVL(x_msg_data, 'x') IN('R0007') THEN
              UPDATE xx_ar_refund_trx_tmp
              SET error_flag         = 'Y',
                last_update_date     = SYSDATE,
                last_updated_by      = fnd_profile.VALUE('USER_ID'),
            ...
            END IF;
          END IF;
          IF NVL(lc_err, 'N') = 'N' THEN
            od_message('M', 'Success creating CM Adjustment ' || lc_adj_num);
            UPDATE xx_ar_refund_trx_tmp
            SET adjustment_number = lc_adj_num,
              adj_created_flag    = 'Y',
              adj_creation_date   = SYSDATE,
              error_flag          = NVL(lc_adj_warning, 'N'),
              last_update_date    = SYSDATE,
              last_updated_by     = fnd_profile.VALUE('USER_ID'),
              last_update_login   = fnd_profile.VALUE('LOGIN_ID')
          ...
          ELSE -- Lc_error='Y'
            ...
            od_message('M', '*** Error - Refund could not be created for TRX:' || v_trx.trx_type || '/' || v_trx.trx_number);
          END IF;
        ELSE -- Receipt.
          IF NVL(v_trx.escheat_flag, 'N') = 'Y' THEN
            FOR v_unapp_trx IN c_trx_to_unapply(v_trx.trx_id)
            LOOP
              IF v_unapp_trx.trx_number = 'On Account' THEN
                od_message('M', 'Unapply On Account of ' || v_unapp_trx.amount_applied || ' for receipt:' || v_trx.trx_number);
                x_return_status := NULL;
                x_msg_data      := NULL;
                x_msg_count     := NULL;
                unapply_on_account(v_unapp_trx.receivable_application_id, x_return_status, x_msg_count, x_msg_data);
              ELSIF v_unapp_trx.trx_number = 'Prepayment' THEN
                od_message('M', 'Unapply Prepayment of ' || v_unapp_trx.amount_applied || ' for receipt:' || v_trx.trx_number || ' - recv_appl_id = ' || v_unapp_trx.receivable_application_id);
                x_return_status := NULL;
                x_msg_data      := NULL;
                x_msg_count     := NULL;
                unapply_prepayment(v_unapp_trx.receivable_application_id, x_return_status, x_msg_count, x_msg_data);
              ELSE
                od_message('M', '*** Warning: Non Prepayment/On-Account application' || ' found for trx:' || v_trx.trx_number);
              END IF;
            END LOOP;
          ELSE -- Non-Escheats.
            IF NVL(v_trx.refund_alt_flag, 'N') = 'Y' THEN
          ...
          IF NVL(v_trx.identification_type, 'x') = 'OM' AND INSTR(NVL(v_trx.payment_method_name, 'XXX'), 'MAILCHECK') > 0 THEN
            ln_mail_check                       := 1;
          END IF;
          ...
          IF NVL(x_return_status, fnd_api.g_ret_sts_success) = fnd_api.g_ret_sts_success
            ...
            , v_trx.trx_id, v_refund_hdr.customer_number, v_trx.refund_amount, ln_org_id, v_trx.activity_type, lc_reason_code, lc_comments, NVL(v_trx.escheat_flag, 'N'), lc_adj_num, x_return_status, x_msg_count, x_msg_data);
            IF NVL(x_return_status, fnd_api.g_ret_sts_success) <> fnd_api.g_ret_sts_success
              --Receipt Write-off  was not successful
              THEN
              lc_err := 'Y';
              IF NVL(x_msg_data, 'x') IN('R0012') THEN
                UPDATE xx_ar_refund_trx_tmp
                SET error_flag         = 'Y',
                  last_update_date     = SYSDATE,
                  last_updated_by      = fnd_profile.VALUE('USER_ID'),
                ...
                INTO xx_ar_refund_error_log
                  ...
                  (
                    ln_conc_request_id,
                    SUBSTR(x_msg_data, 1, 5),
                    v_refund_hdr.customer_number,
                    lc_trx_type,
                    v_trx.trx_number,
                    v_trx.activity_type
                  );
              ELSIF x_msg_data IN('R0013') THEN
                UPDATE xx_ar_refund_trx_tmp
                SET error_flag         = 'Y',
                  last_update_date     = SYSDATE,
                  last_updated_by      = fnd_profile.VALUE('USER_ID'),
              ...
              END IF;
              od_message('M', '**Error Creating Receipt Write-Off: ' || x_msg_data);
            ELSE
              od_message('M', 'After Create Receipt Write-off for:' || v_trx.trx_number || ' for :' || v_trx.trx_currency_code || ' ' || v_trx.refund_amount || ' Status:' || x_return_status || ' Message:' || x_msg_count || ':' || x_msg_data);
              od_message('M', 'Receivable_Application_ID for Receipt write-off:' || lc_adj_num);
            END IF;
          ELSIF ln_mail_check = 0 THEN -- V4.0, Added as it doesn't go thru Write Off Process
            --On-Account/Prepayment application error
            lc_err := 'Y';
            IF NVL(x_msg_data, 'x') IN('R0010') THEN
              UPDATE xx_ar_refund_trx_tmp
              SET error_flag         = 'Y',
                last_update_date     = SYSDATE,
                last_updated_by      = fnd_profile.VALUE('USER_ID'),
        ...
        END IF;
        IF lc_err                  = 'N' THEN
          IF ln_mail_check         = 0 THEN -- V4.0, Added to not count the adjustment/wrote off for store mailcheck
            ln_refunds_count      := NVL(ln_refunds_count, 0)      + 1;
            ln_refund_amounts_tot := NVL(ln_refund_amounts_tot, 0) + v_trx.refund_amount;
          END IF; -- v4.0
          ...
          BEGIN
            ...
            SET adjustment_number = lc_adj_num,
              ...
              last_update_login   = fnd_profile.VALUE('LOGIN_ID')
            WHERE CURRENT OF c_trx;
            IF ln_refunds_count = 1 THEN
              od_message('O', 'Adjustments/Write-off Created for the following Transactions');
              od_message('O', ' ');
              od_message('O', g_print_line);
              -- Customer # (10)  Customer Name (25) Type (12)
              -- Transaction Number (15)  Refund Amount (15)
              -- Adjustment# (20)   Status (15)  E(1)
              od_message('O', 'Customer# ' || 'Payee                    ' || 'Trx Type    ' || 'Trx Number     ' || ' Refund Amount ' || 'Adjustment#         ' || 'Status       ' || 'Escheat');
              od_message('O', g_print_line);
            END IF;
            od_message('O', RPAD(SUBSTR(v_refund_hdr.customer_number, 1, 10), 10, ' ') || RPAD(SUBSTR(v_trx.payee_name, 1, 25), 25, ' ') || RPAD(lc_trx_type, 12, ' ') || RPAD(SUBSTR(v_trx.trx_number, 1, 15), 15, ' ') || LPAD(TO_CHAR(v_trx.refund_amount, '99G999G990D00PR'), 15, ' ') || RPAD(SUBSTR(lc_adj_num, 1, 20), 20, ' ') || '   ' || RPAD(v_trx.status, 11, ' ') || '  ' || v_trx.escheat_flag);
            BEGIN
              ...
              IF v_trx.identification_type = 'OM' AND v_trx.trx_type = 'R' THEN
                UPDATE ar_cash_receipts_all acr
                ...
                AND NVL(aops_order_number, pos_transaction_number) =
                  (SELECT attribute7
            ...
            EXCEPTION
            WHEN OTHERS THEN
              od_message('M', 'Warning: Could not update DFF on Cash Receipt.' || SQLCODE || ':' || SQLERRM);
            END;
          EXCEPTION
          WHEN OTHERS THEN
            od_message('M', '*** Error updating adjustment created flag and' || ' status after creating adjustment. Error:' || SQLCODE || ':' || SQLERRM);
          END;
        END IF;
      ELSE -- if pre adjust/write-off error = 'N'
        od_message('M', '*** Error (Pre-Loop)- Refund could not be created for TRX:' || v_trx.trx_type || '/' || v_trx.trx_number);
      END IF; -- if err = 'N'
      IF lc_err          = 'Y' THEN
        ln_errors_count := NVL(ln_errors_count, 0) + 1;
      END IF;
  ...
  IF(p_user_id IS NULL) -- Defect#37226 - Added IF condition to restrict procedure call for manually submitted requests
    ...
    update_dffs;
  END IF;
  IF ln_refunds_count > 0 THEN
    od_message('O', ' ');
    od_message('O', g_print_line);
    od_message('O', RPAD(' ', 10, ' ') || RPAD(' ', 25, ' ') || RPAD(' ', 10, ' ') || LPAD('    Total:  ', 15, ' ') || ' ' || LPAD(TO_CHAR(ln_refund_amounts_tot, '9G999G990D00PR'), 16, ' '));
    od_message('O', ' ');
  END IF;
  ----------------------------------------------
  --Start of Changes for the Defect #3340
  ----------------------------------------------
  IF (gn_tot_receipt_writeoff > 0) THEN
    od_message('O', '');
    od_message('O', '');
    od_message('O', '');
    od_message('O', g_print_line);
    od_message('O', '                                                 Refunds to be Reclassified ');
    ...
    od_message('O', g_print_line); --Modified on 28-DEC-09 -- Moved out of LOOP
    FOR cntr IN 1 .. gn_count - 1
    LOOP
      ...
      fnd_file.put_line(fnd_file.output, gt_receipt_writeoff(cntr).account_dr                                                                                                                            -- Modified for defect # 4348 on 4/29/2010
      || '|' || gt_receipt_writeoff(cntr).refund_amount || '||'                                                                                                                                          -- Modified for defect # 4348 on 4/29/2010
      || gt_receipt_writeoff(cntr).receipt_date || '|' || gt_receipt_writeoff(cntr).description || '|' || gt_receipt_writeoff(cntr).location_description || '|' || gt_receipt_writeoff(cntr).location_dr -- Modified for 4348 on 4/29/2010
      || '|' || gt_receipt_writeoff(cntr).meaning_debit || '|' || gt_receipt_writeoff(cntr).account_seg_dr);
      --Added for defect #4348
      fnd_file.put_line(fnd_file.output, gt_receipt_writeoff(cntr).account_cr                                                                                                                                                                              -- Modified for defect # 4348 on 4/29/2010
      || '||'                                                                                                                                                                                                                                              -- Modified for defect # 4348 on 4/29/2010
      || gt_receipt_writeoff(cntr).refund_amount || '|' || gt_receipt_writeoff(cntr).receipt_date || '|' || gt_receipt_writeoff(cntr).description || '|' || gt_receipt_writeoff(cntr).location_description || '|' || gt_receipt_writeoff(cntr).location_cr -- Modified for 4348 on 4/29/2010
      || '|' || gt_receipt_writeoff(cntr).meaning_credit || '|' || gt_receipt_writeoff(cntr).account_seg_cr);
    END LOOP;
  ELSE
    od_message('O', '');
    od_message('O', '');
    ...
    od_message('O', '                                                 Refunds to be Reclassified ');
    od_message('O', g_print_line);
    od_message('O', '*** No Receipt Write OFF done using Generic Receivable Activity ***');
  END IF;
  ----------------------------------------------
  --End of Changes for the Defect #3340
  ----------------------------------------------
  IF ln_errors_count > 0 THEN
  ...
  od_message('O', '');
  od_message('O', g_print_line);
  od_message('O', '                                                 Process Summary ');
  od_message('O', g_print_line);
  od_message('O', 'Number of refunds successfully Created          :' || ln_refunds_count);
  od_message('O', 'Total Amount of refunds successfully Created    :' || ln_refund_amounts_tot);
  od_message('O', 'Number of errors when creating refunds          :' || ln_errors_count);
EXCEPTION
WHEN OTHERS THEN
...
BEGIN
  ...
  WHERE TRIM(r.NAME) = p_adj_name
  AND org_id         = p_org_id
  AND status         = 'A';
  od_message('M', 'CM Adjustment Name: ' || p_adj_name);
  o_adj_num                      := NULL;
  ...
  lr_adj_rec.comments            := p_reason_code || p_comments;
  ...
  fnd_file.put_line(fnd_file.LOG, 'ln_activity_id: ' || ln_activity_id);
  fnd_file.put_line(fnd_file.LOG, 'p_reason_code: ' || p_reason_code);
  fnd_file.put_line(fnd_file.LOG, 'p_reason_code || p_comments: ' || p_reason_code || p_comments);
  ar_adjust_pub.create_adjustment(p_api_name => 'AR_ADJUST_PUB', p_api_version => 1.0, p_init_msg_list => fnd_api.g_true, p_msg_count => x_msg_count, p_msg_data => x_msg_data, p_return_status => x_return_status, p_adj_rec => lr_adj_rec, p_new_adjust_number => lc_adj_num, p_new_adjust_id => ln_adj_id
  ...
  IF NVL(x_return_status, 'x') = fnd_api.g_ret_sts_success THEN
    od_message('M', 'Before Approve:' || x_msg_data);
    od_message('M', 'Status: ' || x_return_status || ' Count:' || x_msg_count || 'Data:' || x_msg_data);
    x_return_status := NULL;
    x_msg_count     := NULL;
    x_msg_data      := NULL;
    IF lc_adj_num   IS NOT NULL THEN
      BEGIN
        SELECT NVL(status, 'X')
        INTO lc_adj_status
        FROM ar_adjustments_all
        WHERE adjustment_number = lc_adj_num;
        IF lc_adj_status       != 'A' THEN
          ar_adjust_pub.approve_adjustment(p_api_name => 'AR_ADJUST_PUB', p_api_version => 1.0, p_msg_count => x_msg_count, p_msg_data => x_msg_data, p_return_status => x_return_status, p_adj_rec => NULL, p_chk_approval_limits => fnd_api.g_false, p_old_adjust_id => ln_adj_id);
        END IF;
      EXCEPTION
      WHEN NO_DATA_FOUND THEN
        od_message('M', 'Error Getting Status for Adjustment:' || lc_adj_num);
      END;
    END IF;
    od_message('M', 'After Approve:' || x_msg_data);
    od_message('M', 'Status: ' || x_return_status || ' Count:' || x_msg_count || 'Data:' || x_msg_data);
  END IF;
  IF NVL(x_msg_count, 0) > 0 THEN
    ...
    LOOP
      ...
      od_message('M', '*** ' || i || ' of ' || x_msg_count || ': ' || NVL(lc_api_err_msg, x_msg_data));
    END LOOP;
  END IF;
EXCEPTION
WHEN NO_DATA_FOUND THEN
  od_message('M', '*** CM Adjustment: ' || p_adj_name || ' not Defined for org_id:' || p_org_id);
  x_return_status := 'E';
  x_msg_data      := 'R0007';
WHEN OTHERS THEN
  od_message('M', '*** CM Adjustment: ' || p_adj_name || ' for org_id:' || p_org_id || ' Other error-' || SQLERRM);
  x_return_status := 'E';
  x_msg_data      := 'R0007';
END;
...
BEGIN
  BEGIN
    ...
    BEGIN
      ...
      od_message('M', 'Receipt Write-Off Name: ' || p_wo_name);
      ar_receipt_api_pub.activity_application (p_api_version => 1.0, p_init_msg_list => fnd_api.g_true, p_commit => fnd_api.g_false, p_validation_level => fnd_api.g_valid_level_full, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, p_cash_receipt_id => p_cash_receipt_id, p_amount_applied => p_amount, p_applied_payment_schedule_id => ln_applied_payment_schedule_id, p_receivables_trx_id => ln_activity_id, p_comments => p_reason_code || p_comments, p_apply_date => TRUNC(SYSDATE), p_application_ref_type => lc_application_ref_type, p_application_ref_id => ln_application_ref_id, p_application_ref_num => lc_application_ref_num, p_secondary_application_ref_id => ln_secondary_appln_ref_id, p_receivable_application_id => ln_receivable_application_id, p_called_from => 'OD Refunds Process');
      o_receivable_application_id := 'Rcv Appl ID:' || TO_CHAR(ln_receivable_application_id);
      IF NVL(x_msg_count, 0)       > 0 THEN
        FOR i IN 1 .. x_msg_count
        LOOP
        ...
        WHEN OTHERS THEN
          ln_activity_id := NULL;
          od_message ('M', '*** Receipt Write-off error @OD_AR_REF_GENERIC_ACTIVTY translation derivation:' || p_wo_name || ' -Other Err - ' || SQLERRM);
        END;
        IF (ln_activity_id IS NOT NULL) THEN
          od_message('M', 'Generic Receivable Activity ID: ' || ln_activity_id);
          ar_receipt_api_pub.activity_application (p_api_version => 1.0, p_init_msg_list => fnd_api.g_true, p_commit => fnd_api.g_false, p_validation_level => fnd_api.g_valid_level_full, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, p_cash_receipt_id => p_cash_receipt_id, p_amount_applied => p_amount, p_applied_payment_schedule_id => ln_applied_payment_schedule_id, p_receivables_trx_id => ln_activity_id, p_comments => p_reason_code || p_comments, p_apply_date => TRUNC(SYSDATE), p_application_ref_type => lc_application_ref_type, p_application_ref_id => ln_application_ref_id, p_application_ref_num => lc_application_ref_num, p_secondary_application_ref_id => ln_secondary_appln_ref_id, p_receivable_application_id => ln_receivable_application_id, p_called_from => 'OD Refunds Process');
          o_receivable_application_id := 'Rcv Appl ID:' || TO_CHAR(ln_receivable_application_id);
          ----------------------------------------------------------------
          ...
          BEGIN
            SELECT receipt_number
              ...
              ,
              TO_CHAR(receipt_date, 'RRRR-MM-DD') --Added for defect 4348
              ,
              NVL(attribute1, attribute2),
              currency_code
            ...
            gn_tot_receipt_writeoff                     := gn_tot_receipt_writeoff + 1;
            ----------------------------------------------------------
            ...
            SET original_activity_type = p_wo_name,
              ...
              account_orig_dr          = gt_receipt_writeoff(gn_count).account_dr
              -- Modified for 4348 on 4/29/2010
              ,
              account_generic_cr = gt_receipt_writeoff(gn_count).account_cr
              --  Modified for 4348 on 4/29/2010
              ,
              last_update_date     = SYSDATE,
              last_updated_by      = fnd_profile.VALUE('USER_ID'),
          ...
          EXCEPTION
          WHEN OTHERS THEN
            od_message ('M', '*** Receipt Write-off error @Reclassification output derivation section:' || SQLCODE || ' -Other Err - ' || SQLERRM);
          END; --Added Exception on 28-DEC-09
        ELSE
          od_message ('M', 'Generic Receivable Activity does not exist for the Original receivable Activity: ' || p_wo_name);
        END IF;
        ----------------------------------------------------------
        --End of Changes for the Defect #3340
        ----------------------------------------------------------
    ...
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      od_message('M', '*** Receipt Write-Off: ' || p_wo_name || 'not Defined for org_id:' || p_org_id);
      x_return_status := 'E';
      x_msg_data      := 'R0012';
    WHEN OTHERS THEN
      od_message('M', '*** Receipt Write-off:' || p_wo_name || ' -Other Err1 - ' || SQLERRM);
      x_return_status := 'E';
      x_msg_data      := 'R0012';
    END;
  EXCEPTION
  WHEN NO_DATA_FOUND THEN
    od_message('M', '*** CRW: Receipt Write-Off: Payment Schedule not Defined');
    x_return_status := 'E';
    x_msg_data      := 'R0013';
  WHEN OTHERS THEN
    od_message('M', '*** CRW: Receipt Write-Off: Payment Schedule - Other Err2 - ' || SQLERRM);
    x_return_status := 'E';
    x_msg_data      := 'R0013';
  END;
END;
...
BEGIN
  ar_receipt_api_pub.unapply_other_account(p_api_version => 1.0, p_init_msg_list => fnd_api.g_true, p_commit => fnd_api.g_false, p_validation_level => fnd_api.g_valid_level_full, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, p_receivable_application_id => p_receivable_application_id);
  IF NVL(x_msg_count, 0) > 0 THEN
    FOR i IN 1 .. x_msg_count
    LOOP
      lc_err_msg := fnd_msg_pub.get(p_encoded => fnd_api.g_false);
      od_message('M', '*** ' || i || '.' || SUBSTR(lc_err_msg, 1, 255));
      IF x_msg_data IS NOT NULL THEN
        x_msg_data  := SUBSTR( x_msg_data || '/' || i || '.' || lc_err_msg, 1, 2000);
      ELSE
        x_msg_data := SUBSTR( i || '.' || lc_err_msg, 1, 2000);
      END IF;
    END LOOP;
  END IF;
  IF x_return_status <> 'S' THEN
    od_message('M', '*** Error Un-Applying Pre-Payment');
  END IF;
EXCEPTION
WHEN OTHERS THEN
  od_message('M', '*** Error Un-Applying Pre-Payment: ' || SQLERRM);
  x_return_status := 'E';
  x_msg_data      := 'R0010' || ':' || SQLCODE || ':' || SQLERRM;
END;
--------------------------------------------------------------------------------
...
IS
  lc_err_msg VARCHAR2(2000);
BEGIN
  od_message('M', 'Before Calling API for Unapplying On Account.');
  ar_receipt_api_pub.unapply_on_account(p_api_version => 1.0, p_init_msg_list => fnd_api.g_true, p_commit => fnd_api.g_false, p_validation_level => fnd_api.g_valid_level_full, x_return_status => x_return_status, x_msg_count => x_msg_count, x_msg_data => x_msg_data, p_receivable_application_id => p_receivable_application_id);
  od_message('M', 'After Calling API for Unapplying On Account-Error Count:' || x_msg_count || ' Error Status:' || x_return_status || '  Msg Data:' || x_msg_data);
  IF NVL(x_msg_count, 0) > 0 THEN
    FOR i IN 1 .. x_msg_count
    LOOP
      lc_err_msg := fnd_msg_pub.get(p_encoded => fnd_api.g_false);
  ...
  END IF;
  IF x_return_status <> 'S' THEN
    od_message('M', 'Error Un-Applying On Account');
  END IF;
EXCEPTION
WHEN OTHERS THEN
  od_message('M', '*** Error Un-Applying On Account: ' || SQLERRM);
  x_return_status := 'E';
  x_msg_data      := 'R0010' || ':' || SQLCODE || ':' || SQLERRM;
END;
--------------------------------------------------------------------------------
...
BEGIN
  ...
  INTO x_sitecode
  FROM DUAL;
  lc_site_category                                := NULL;  
  IF NVL(p_refund_hdr_rec.identification_type,'x') = 'OM' THEN
...
BEGIN
  ...
  INTO ap_invoices_interface
    ...
    (
      ...
      ,
      DECODE(p_inv_amt, 0, 'US_OD_CLEARING_TREASURY', 'US_OD_EXP_NON_DISC') --decode added FOR defect 12225 v5.3 Gaurav
    );
...
BEGIN
  ...
  WHEN OTHERS THEN
    ln_dist_ccid := NULL;
  END;
  IF p_refund_trx_rec.identification_type = 'OM' THEN
  ...
  lc_lob      := '90'; --'04';
  lc_location := '010000';
  od_message('M', 'COA ID:' || ln_coa_id || '  Identification Type:' || p_refund_trx_rec.identification_type || ' Location:' || lc_location);
  IF lc_location IS NOT NULL THEN
    -- AP INVOICE DISTRIBUTION.
    BEGIN
      -- Fix for Defect 2313
      -- Call to the overloaded version of function derive_company_from_location
      -- with the parameter ORG_ID
      lc_company := xx_gl_translate_utl_pkg.derive_company_from_location(p_location => lc_location, p_org_id => ln_org_id);
    EXCEPTION
    WHEN OTHERS THEN
      od_message('M', 'Company/Lob could not be determined for Location/Cost-Center' || lc_location);
      RAISE exep_company;
    END;
  ELSE
    od_message('M', 'Location / Store Number not found!');
    RAISE exep_location;
  END IF;
  IF lc_company IS NOT NULL THEN
    lc_dist_cc  := lc_company || '.' || lc_cost_center || '.' || lc_account || '.' || lc_location || '.' || lc_intercompany || '.' || lc_lob || '.' || lc_future;
  ELSE
    od_message('M', 'Company could not be determined for location:' || lc_location);
    RAISE exep_company;
  END IF;
  od_message('M', 'Concatenated Segments:' || lc_dist_cc);
  ln_dist_ccid := fnd_flex_ext.get_ccid('SQLGL', 'GL#', ln_coa_id, NULL, lc_dist_cc);
  -- v 5.0 Starts
  IF p_refund_trx_rec.identification_type   = 'OM' AND p_line = 2 THEN
    IF (p_refund_trx_rec.om_delete_status   = 'M' OR p_refund_trx_rec.om_delete_status = 'O') THEN
      lc_act_type                          := xx_ar_prepayments_pkg.get_country_prefix(ln_org_id) || '_MAILCK_O/S' || '_' || p_refund_trx_rec.om_store_number;
    ELSIF(p_refund_trx_rec.om_delete_status = 'A' OR p_refund_trx_rec.om_delete_status = 'E') THEN
      lc_act_type                          := xx_ar_prepayments_pkg.get_country_prefix(ln_org_id) || '_ESCHEAT_REC_WRITEOFF_OD';
    ELSIF(p_refund_trx_rec.om_delete_status = 'S') THEN
      lc_act_type                          := xx_ar_prepayments_pkg.get_country_prefix(ln_org_id) || '_MAILCK_REV' || '_' || p_refund_trx_rec.om_store_number;
    END IF;
  END IF;
  IF lc_act_type IS NOT NULL THEN
    BEGIN
      SELECT code_combination_id
      INTO ln_dist_ccid
      FROM ar_receivables_trx_all
      WHERE INSTR(NAME, lc_act_type) > 0;
    EXCEPTION
    WHEN OTHERS THEN
      ln_dist_ccid := NULL;
    END;
  END IF;
  -- v 5.0 ENDS
  IF ln_dist_ccid = 0 OR ln_dist_ccid = -1 THEN
    ln_dist_ccid := NULL;
    x_err_mesg   := 'Error creating Code Combination (' || lc_dist_cc || ') for Trx: ' || p_refund_trx_rec.trx_number || '. Error:' || fnd_flex_ext.GET_MESSAGE;
  ELSE
    INSERT
    INTO ap_invoice_lines_interface
      ...
      (
        ...
        || DECODE(p_refund_trx_rec.trx_type, 'R', 'Receipt', 'Credit Memo ') -- Added for CR 697/698 R1.2
        || ': '
  ...
  END IF;
EXCEPTION
WHEN exep_location THEN
  x_err_mesg        := 'Location / Store Number not found.';
  o_invoice_line_id := 0;
WHEN exep_company THEN
  x_err_mesg        := 'Company segment could not be determined for location:' || lc_location;
  o_invoice_line_id := 0;
WHEN OTHERS THEN
  o_invoice_line_id := 0;
  x_err_mesg        := SQLCODE || SQLERRM;
...
BEGIN
  ...
  WHERE TRIM(UPPER(vendor_name)) = TRIM(UPPER(p_name))
  AND ROWNUM                     = 1;
EXCEPTION
WHEN OTHERS THEN
  o_vendor_id := NULL;
...
BEGIN
  ...
  AND TRIM(UPPER(NVL(address_line2, 'X'))) = TRIM(UPPER(NVL(p_address2, 'X')))
  AND TRIM(UPPER(NVL(address_line3, 'X'))) = TRIM(UPPER(NVL(p_address3, 'X')))
  AND TRIM(UPPER(city))                    = TRIM(UPPER(p_city))
  AND TRIM(UPPER(NVL(state, 'X')))         = TRIM(UPPER(NVL(p_state, 'X')))
  AND TRIM(UPPER(NVL(province, 'X')))      = TRIM(UPPER(NVL(p_province, 'X')))
  AND TRIM(UPPER(NVL(country, 'X')))       = TRIM(UPPER(NVL(p_country, 'X')))
...
BEGIN
  ...
  WHERE DECODE(p_vendor_id, NULL, vendor_interface_id, vendor_id) = DECODE(p_vendor_id, NULL, p_interface_id, p_vendor_id)
...
IS
  ...
  IS
    ...
    AND xartt.om_write_off_only = DECODE(xartt.identification_type, 'OM', DECODE(INSTR(NVL(payment_method_name, 'XXX'), 'MAILCHECK'), 0, 'N', om_write_off_only), 'N')
    AND xartt.escheat_flag      = DECODE(xartt.identification_type, 'OM', DECODE(INSTR(NVL(payment_method_name, 'XXX'), 'MAILCHECK'), 0, 'N', xartt.escheat_flag), 'N')
    AND org_id                  = p_org_id;
  CURSOR c_trx( p_refund_header_id IN NUMBER)
  IS
    SELECT *
    FROM xx_ar_refund_trx_tmp
    WHERE refund_header_id = p_refund_header_id
      ...
      --AND escheat_flag = 'N'
      FOR UPDATE OF ap_invoice_number,
      inv_created_flag,
      ap_inv_creation_date;
...
BEGIN
  ...
  od_message('O', ln_conc_request_id);
  od_message('O', fnd_profile.VALUE('GL_SET_OF_BKS_NAME') || '                 OD: Refunds - Create Invoices for Refunds                    Date:' || TRUNC(SYSDATE));
  od_message('O', '');
  SELECT gsb.short_name
  INTO lc_sob_name
  ...
  AND hru.organization_id   = ln_org_id;
  ...
  LOOP
    BEGIN
      ...
      lc_savepoint            := 'SAVEPOINT-XXARRFNDC' || v_refund_hdr.refund_header_id;
      od_message('M', ' ');
      od_message('M', g_print_line);
      SAVEPOINT lc_savepoint;
      od_message('M', 'Set Savepoint:' || lc_savepoint);
      IF v_refund_hdr.trx_type = 'R' THEN
        lc_trx_type           := 'Receipt';
      ELSE
        lc_trx_type := 'Credit Memo';
      END IF;
      ...
      IF NVL(v_refund_hdr.identification_type, 'x') = 'OM' AND INSTR(NVL(v_refund_hdr.payment_method_name, 'XXX'), 'MAILCHECK') > 0 THEN
        IF lc_sob_name                              = 'US_USD_P' AND NVL(v_refund_hdr.activity_type, 'XXX') <> 'US_MAILCK_CLR_OD' THEN
          ln_invoice_amount                        := 0;
          lc_payee_name                            := 'INTERNAL SUPPLIER MAILCHECK';
        END IF;
        IF lc_sob_name       = 'CA_USD_P' AND NVL(v_refund_hdr.activity_type, 'XXX') <> 'CA_MAILCK_CLR_OD' THEN
          ln_invoice_amount := 0;
          lc_payee_name     := 'INTERNAL SUPPLIER MAILCHECK';
        END IF;
      END IF;
      ...
      get_vendor_id(lc_payee_name, ln_vendor_id);
      lc_comn_err_loc := 'After get_vendor_id';
      -- If record exists ....
      IF ln_vendor_id    IS NOT NULL THEN
        lc_create_vendor := 'N';
        ...
        IF lc_payee_name = 'INTERNAL SUPPLIER MAILCHECK' AND ln_invoice_amount = 0 THEN
          BEGIN
            IF lc_sob_name = 'US_USD_P' THEN
              SELECT vendor_site_id,
                vendor_site_code
              ...
              AND ROWNUM           = 1;
            ELSIF lc_sob_name      = 'CA_USD_P' THEN
              SELECT vendor_site_id,
                vendor_site_code
              INTO ln_vendor_site_id,
                lc_sitecode
        ...
        IF ln_vendor_site_id IS NULL THEN
          get_vendor_site_id(ln_vendor_id, v_refund_hdr.alt_address1, v_refund_hdr.alt_address2, v_refund_hdr.alt_address3, v_refund_hdr.alt_city, v_refund_hdr.alt_state, v_refund_hdr.alt_province, v_refund_hdr.alt_country, ln_vendor_site_id, lc_sitecode);
        ...
        ELSE
          ...
          get_vendor_site_int_id(ln_vendor_id, NULL, v_refund_hdr.alt_address1, v_refund_hdr.alt_address2, v_refund_hdr.alt_address3, v_refund_hdr.alt_city, v_refund_hdr.alt_state, v_refund_hdr.alt_province, v_refund_hdr.alt_country, lc_sitecode);
          lc_comn_err_loc := 'After Vend Site ID 2';
          -- If record does not exist
          IF lc_sitecode   IS NULL THEN
            lc_create_site := 'Y';
          ELSE
            lc_create_site := 'N';
          END IF;
      ...
      ELSE
        ...
        IF ln_vendor_interface_id IS NOT NULL THEN
          lc_create_vendor        := 'N';
        ELSE
          lc_create_vendor := 'Y';
        END IF;
        -- Check if record exists in ap_supplier_sites_int table for the vendor_interface_id and address
        get_vendor_site_int_id(NULL, ln_vendor_interface_id, v_refund_hdr.alt_address1, v_refund_hdr.alt_address2, v_refund_hdr.alt_address3, v_refund_hdr.alt_city, v_refund_hdr.alt_state, v_refund_hdr.alt_province, v_refund_hdr.alt_country, lc_sitecode);
        -- If record does not exist
        IF lc_sitecode   IS NULL THEN
          lc_create_site := 'Y';
        ELSE
      ...
      IF lc_create_vendor = 'Y' THEN
        -- Insert a record into ap_suppliers_int table
        insert_supplier_interface(v_refund_hdr, lc_sob_name, ln_vendor_interface_id, lc_err_mesg);
        IF lc_err_mesg            IS NOT NULL THEN
          lc_err_mesg             := 'Vendor Interface Insert Error: ' || lc_err_mesg;
        ...
        ELSE
          ln_vendor_cnt := NVL(ln_vendor_cnt, 0) + 1;
        END IF;
      END IF;
      -- If vendor site is to be created
      IF lc_create_site = 'Y' THEN
        -- If vendor is to be created
        IF lc_create_vendor = 'Y' THEN
          -- Insert a record into ap_supplier_sites_int table with
          -- the vendor_interface_id as opposed to with the vendor_id
          insert_supplier_site_int(v_refund_hdr, ln_vendor_interface_id, NULL, lc_sob_name, lc_sitecode, lc_err_mesg);
        ELSE
          ...
          ELSE
            ...
            insert_supplier_site_int(v_refund_hdr, NULL, ln_vendor_id, lc_sob_name, lc_sitecode, lc_err_mesg);
          END IF;
        END IF;
        IF lc_err_mesg            IS NOT NULL THEN
          lc_err_mesg             := 'Vendor Site Interface Insert Error:' || lc_err_mesg;
          lc_comn_err_loc         := 'XX_AR_REFUNDS_PKG.Create_AP_Invoice-Vendor Site Intf';
          lc_insert_error_sup_site:='SS'; -- Defect 30123
          RAISE intf_insert_error;
        ELSE
          ln_site_cnt := NVL(ln_site_cnt, 0) + 1;
        END IF;
      ...
      EXCEPTION
      WHEN OTHERS THEN
        od_message ('M', '*** Error Updating Refunds Staging Table after creating vendor for Refund_Header_ID:' || v_refund_hdr.refund_header_id || ' Trx:' || v_refund_hdr.trx_number);
      END;
      /* Disabled Code
      ...
      */
      ...
      IF NVL(v_refund_hdr.identification_type, 'x') = 'OM' THEN
      ...
      IF NVL(v_refund_hdr.identification_type, 'x') = 'OM' THEN
        BEGIN
          SELECT SUBSTR(xamch.pos_transaction_number, 9, 2)
            || '/'
            || SUBSTR(xamch.pos_transaction_number, 11, 2)
            || '/'
            || SUBSTR(xamch.pos_transaction_number, 5, 4),
            xamch.aops_order_number,
            NVL(xamch.aops_order_number, xamch.pos_transaction_number)
          INTO lc_sale_return_date,
            lc_aops_order_number,
            lc_in_invoice_number
          FROM xx_ar_mail_check_holds xamch
          WHERE ref_mailcheck_id = v_refund_hdr.ref_mailcheck_id
          AND ROWNUM             = 1;
        EXCEPTION
        WHEN OTHERS THEN
        ...
        END;
        IF lc_aops_order_number IS NOT NULL THEN
          lc_description        := 'Order ' || lc_aops_order_number;
        ELSE
          lc_description := 'Store Refund ' || lc_sale_return_date;
        END IF;
      ELSE
        ...
        IF xx_lc_description IS NULL THEN --Code Change for defect 22806
          lc_description     := 'REF CUST#' || v_refund_hdr.customer_number || '/' || v_refund_hdr.aops_customer_number;
      ...
      , lc_sitecode, v_refund_hdr.trx_currency_code, lc_sob_name
      -- V4.0, Not required, as passed description directly as a parameter
      --, v_refund_hdr.customer_number
      , lc_in_invoice_number -- V4.0, v_refund_hdr.trx_number
      , lc_invoice_source
      -- V4.0, Passed description as a parameter
      , lc_description, ln_invoice_id, lc_invoice_num, lc_err_mesg);
      lc_comn_err_loc   := 'After Insert invoice intf call';
      IF lc_err_mesg    IS NOT NULL OR ln_invoice_id = 0 THEN
        lc_err_mesg     := 'Invoice Interface Insert Error: ' || lc_err_mesg;
      ...
      LOOP
        ...
        insert_invoice_lines_int(ln_invoice_id, ln_line_num, v_refund_hdr.customer_number, v_trx, ln_invoice_line_id, lc_err_mesg, 1);
        -- V4.0, for $0 invoice, need to insert 2 dist lines one with +ve and another with -ve amt
        IF ln_invoice_amount   = 0 THEN
          v_trx.refund_amount := -1 * v_trx.refund_amount;
          ln_line_num         := 2;
          insert_invoice_lines_int(ln_invoice_id, ln_line_num, v_refund_hdr.customer_number, v_trx, ln_invoice_line_id, lc_err_mesg, 2);
        END IF;
        -- V4.0
        IF lc_err_mesg IS NOT NULL OR NVL(ln_invoice_line_id, 0) = 0 THEN
          od_message('M', 'Error after call to Insert Invoice lines Int');
          lc_err_mesg     := 'Invoice Line Interface Insert Error: ' || lc_err_mesg;
        ...
        EXCEPTION
        WHEN OTHERS THEN
          od_message('M', '*** Error updating AP invoice creation status for trx:' || v_trx.trx_number || ' for Customer' || v_trx.customer_number);
        END;
        ln_line_num := NVL(ln_line_num, 0) + 1;
      END LOOP;
      ...
      ELSE
        ...
        WHERE customer_trx_id = v_refund_hdr.trx_id;
      END IF;
      ln_inv_count := NVL(ln_inv_count, 0) + 1;
    EXCEPTION
    WHEN intf_insert_error THEN
      /*od_message('E'
      ...
      od_message('M', '***Error at:' || lc_comn_err_loc || '. Rolling back to savepoint:' || lc_savepoint);
      od_message('M', lc_err_mesg);
      ...
      */
      ...
      ln_inv_err_count := NVL(ln_inv_err_count, 0) + 1;
    ...
    WHEN OTHERS THEN
      od_message('E', 'Error creating AP invoice for refund - Trx:' || v_refund_hdr.trx_number || ' for Customer' || v_refund_hdr.customer_number || 'Error:' || SQLCODE || ':' || SQLERRM, lc_comn_err_loc);
      od_message('M', '***Error at:' || lc_comn_err_loc || '. Rolling back to savepoint:' || lc_savepoint);
      ROLLBACK TO lc_savepoint;
      ln_inv_err_count := NVL(ln_inv_err_count, 0) + 1;
      INSERT
  ...
  BEGIN
    ...
    IF NVL(ln_vendor_cnt, 0) > 0 THEN
      ...
      IF (ln_req_id = 0) THEN
        od_message('M', 'Error Submitting "Supplier Open Interface Import"');
        --fnd_message.retrieve;
        fnd_message.raise_error;
      ELSE
        COMMIT;
        -- Wait for request to complete
        lb_wait := fnd_concurrent.wait_for_request(ln_req_id, 20, 0, lc_phase, lc_status, lc_dev_phase, lc_dev_status, lc_message);
        BEGIN
          FOR vend_err_rec IN
          (SELECT asi.reject_code,
            ...
            AND xart.org_id           = fnd_global.org_id
          )--defect#89061
          LOOP
            IF vend_err_rec.status = 'PROCESSED' THEN
              UPDATE xx_ar_refund_trx_tmp
              SET error_flag          = 'N',
                status                = 'S',
            ...
            ELSIF vend_err_rec.status = 'REJECTED' THEN
              UPDATE xx_ar_refund_trx_tmp
              SET error_flag         = 'Y',
                status               = 'X',
              ...
              WHERE refund_header_id = vend_err_rec.refund_header_id;
              -- Update Invoice Created Status on Transaction
              IF vend_err_rec.trx_type = 'R' THEN
                UPDATE ar_cash_receipts_all acr
                SET attribute10    = 'Sent to AP',
                  last_update_date = SYSDATE -- Defect #13458
                  ,
              ...
              INTO xx_ar_refund_error_log
                ...
                (
                  ...
                  DECODE(vend_err_rec.trx_type, 'R', 'Receipt', 'Credit Memo'),
        ...
        DELETE
        FROM ap_suppliers_int
        WHERE import_request_id = ln_req_id
        AND status              = 'PROCESSED';
      END IF;
    END IF;
    -- If records inserted in ap_supplier_sites_int table
    IF NVL(ln_site_cnt, 0) > 0 THEN
      ...
      IF (ln_req_id = 0) THEN
        od_message('M', '*** Error Submitting "Supplier Sites Open Interface Import"');
        --fnd_message.retrieve;
        fnd_message.raise_error;
      ELSE
        COMMIT;
        ...
        lb_wait := fnd_concurrent.wait_for_request(ln_req_id, 20, 0, lc_phase, lc_status, lc_dev_phase, lc_dev_status, lc_message);
        BEGIN
          FOR vend_sites_err_rec IN
          (SELECT xart.refund_header_id,
            xart.customer_number,
            xart.trx_id,
            xart.trx_type,
            xart.trx_number,
          ...
          LOOP
            IF vend_sites_err_rec.status = 'PROCESSED' THEN
              -- Update Invoice Created Status on Transaction
              IF vend_sites_err_rec.trx_type = 'R' THEN
                UPDATE ar_cash_receipts_all acr
                SET attribute10    = 'Vendor Created/Matched',
                  last_update_date = SYSDATE -- Defect #13458
                  ,
            ...
            ELSIF vend_sites_err_rec.status = 'REJECTED' THEN
              -- Update Invoice Created Status on Transaction
              IF vend_sites_err_rec.trx_type = 'R' THEN
                UPDATE ar_cash_receipts_all acr
                SET attribute10    = 'Sent to AP',
              ...
              INTO xx_ar_refund_error_log
                ...
                (
                  ...
                  DECODE(vend_sites_err_rec.trx_type, 'R', 'Receipt', 'Credit Memo'),
                  ...
                  || ' '
                  || NVL(vend_sites_err_rec.state, vend_sites_err_rec.province)
                  || ' '
        ...
        EXCEPTION
        WHEN OTHERS THEN
          od_message('M', '*** Error updating Staus on Supplier Site Creation failure');
        END;
        ...
        DELETE
        FROM ap_supplier_sites_int s
        WHERE import_request_id = ln_req_id
        AND s.status            = 'PROCESSED';
      END IF;
    END IF;
    IF NVL(ln_inv_count, 0) > 0 THEN
      ...
      , CHR(0)                                                   --argument3
      , 'REFUND' || TO_CHAR(SYSDATE, 'DD-MON-YY')                --argument4
      , CHR(0)                                                   --argument5
      , CHR(0)                                                   --argument6
      , CHR(0)                                                   --argument7
      , 'N'                                                      --argument8
      ...
      IF (ln_req_id = 0) THEN
        od_message('M', '*** Error Submitting "Payables Open Interface Import" Source:US_OD_AR_REFUND');
        --fnd_message.retrieve;
        fnd_message.raise_error;
      ELSE
        COMMIT;
      ...
      ln_req_id2      := fnd_request.submit_request('SQLAP' --application
      , 'APXIIMPT'                                          --program
      , 'Payables Open Interface Import'                    --description
      , SYSDATE                                             --start_time
      , FALSE                                               --sub_request
      ...
      ELSE
        COMMIT;
        -- Wait for request to complete
        lb_wait := fnd_concurrent.wait_for_request(ln_req_id2, 20, 0, lc_phase, lc_status, lc_dev_phase, lc_dev_status, lc_message);
      ...
      LOOP
        IF inv_recs.status = 'PROCESSED' THEN
          UPDATE xx_ar_refund_trx_tmp
          SET status              = 'V',
            inv_created_flag      = 'Y',
          ...
          EXCEPTION
          WHEN OTHERS THEN
            od_message('M', '*** Error Updating Status after successfully creating' || ' AP Invoice Number:' || inv_recs.invoice_num);
          END;
        ELSIF inv_recs.status = 'REJECTED' THEN
          UPDATE xx_ar_refund_trx_tmp
          ...
          BEGIN
            SELECT trx_id,
              DECODE(trx_type, 'R', 'Receipt', 'Credit Memo'),
              customer_number
            INTO ln_trx_id,
              lc_trx_type,
              lc_cust_number
            ...
            ELSE
              ...
              WHERE customer_trx_id = ln_trx_id;
            END IF;
            od_message('M', 'Checking Intf errs for Inv_id:' || inv_recs.invoice_id || ' for conc req id:' || ln_conc_request_id);
            FOR err_invs IN
            (SELECT       *
            ...
            LOOP
              od_message('M', '*** Error AP INV INTF ERR: Vendor:' || lc_cust_number || ' Inv#' || err_invs.invoice_num || ' Msg:' || err_invs.description);
              INSERT
              INTO xx_ar_refund_error_log
                (
                  conc_request_id,
          ...
          EXCEPTION
          WHEN OTHERS THEN
            od_message('M', '*** Error Updating Status for invoices rejected ' || 'in Payables Open Interface' || ' AP Invoice Number:' || inv_recs.invoice_num);
          END;
        ELSE
          NULL;
        END IF;
    ...
    od_message('O', g_print_line);
    od_message('O', '');
    od_message('O', 'Number of Transactions Processed             :' || ln_proc_count);
    od_message('O', 'Number of New Vendors created                :' || ln_vendor_cnt);
    od_message('O', 'Number of New Vendors Sites created          :' || ln_site_cnt);
    od_message('O', 'Number of Refund Invoices Created            :' || ln_inv_count);
    od_message('O', 'Number of Invoices not created due to errors :' || ln_inv_err_count);
    od_message('O', '');
    IF ln_inv_err_count > 0 THEN
      print_errors(ln_conc_request_id);
    END IF;
    update_dffs;
  EXCEPTION
  WHEN OTHERS THEN
    ...
    BEGIN
      xx_com_error_log_pub.log_error(p_program_type => 'CONCURRENT PROGRAM', p_program_name => 'XXARRFNDI', p_program_id => fnd_profile.VALUE('CONC_REQUEST_ID'), p_module_name => 'xxfin', p_error_location => lc_comn_err_loc, p_error_message_count => 1, p_error_message_code => 'E', p_error_message => SQLCODE || ':' || SQLERRM, p_error_message_severity => 'FATAL', p_notify_flag => 'N', p_object_type => 'OD Refunds: AP Supplier/Invoice Interface', p_object_id => NULL, p_return_code => l_return_code, p_msg_count => l_msg_count);
      COMMIT;
    END;
  END;
  update_dffs;
...
IS
  ...
  */
  ...
  AND EXISTS
    ...
    FROM ap_invoices api,
      ap_vendor_sites_v avs
    WHERE ( NVL(amount_paid, 0) + NVL(discount_amount_taken, 0)) = invoice_amount
...
BEGIN
  ...
  LOOP
    ...
    IF declined_recs.trx_type = 'R' THEN
      UPDATE ar_cash_receipts_all acr
  ...
  LOOP
    ...
    -- Update Paid Status on Transaction.
    IF inv_created_rec.trx_type = 'R' THEN
      UPDATE ar_cash_receipts_all acr
      SET attribute10    = 'Invoice Created',
        last_update_date = SYSDATE -- Defect #13458
        ,
      ...
      BEGIN
        -- Update OM DFF Status to show status.
        IF inv_created_rec.identification_type = 'OM' THEN
          UPDATE ar_cash_receipts_all acr
          SET attribute13 = 'AP INVOICE CREATED|'
            || TO_CHAR(SYSDATE, 'RRRR/MM/DD HH24:MI:SS')
          WHERE cash_receipt_id = inv_created_rec.trx_id
          AND attribute13 LIKE 'ON HOLD|%';
          UPDATE xx_ar_mail_check_holds
          SET ap_invoice_id =
      ...
      EXCEPTION
      WHEN OTHERS THEN
        od_message('M', 'Warning: Problem updating INVOICED status on OM Receipt DFF');
      END;
    ELSE
      UPDATE ra_customer_trx_all
      SET attribute10    = 'Invoice Created',
  ...
  LOOP
    ...
    IF paid_rec.trx_type = 'R' THEN
      ...
      BEGIN
        -- Update OM DFF Status to show status.
        IF paid_rec.identification_type = 'OM' THEN
          UPDATE ar_cash_receipts_all acr
      ...
      EXCEPTION
      WHEN OTHERS THEN
        od_message('M', 'Warning: Problem updating PAID status on OM Receipt DFF');
      END;
    ELSE
      UPDATE ra_customer_trx_all
      SET attribute10    = 'Paid',
...
EXCEPTION
WHEN OTHERS THEN
  od_message('M', 'Error Updating DFFs. ' || SQLCODE || ':' || SQLERRM);
END;
...
BEGIN
  ...
  od_message('O', ' ');
  od_message('O', g_print_line);
  od_message('O', '                                               Error Summary');
  od_message('O', g_print_line);
  od_message('O', ' ');
  -- Customer # (10) Type (12) Transaction Number (18)
  od_message('O', 'Customer# ' || 'Trx Type    ' || 'Trx Number        ' || 'Error');
  od_message('O', g_print_line);
  FOR v_err_recs IN
  (SELECT DISTINCT ec.err_code err_code,
    el.customer_number,
    el.trx_type,
    el.trx_number,
    REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ec.err_msg, '<A1>', el.attribute1), '<A2>', el.attribute2), '<A3>', el.attribute3), '<A4>', el.attribute4), '<A5>', el.attribute5) err_msg
  FROM xx_ar_refund_error_log el,
    xx_ar_refund_err_codes ec
  WHERE ec.err_code      = el.err_code
  AND el.conc_request_id = NVL(p_request_id, fnd_profile.VALUE('CONC_REQUEST_ID'))
  ORDER BY 1
  )
  LOOP
    od_message('O', RPAD(SUBSTR(v_err_recs.customer_number, 1, 10), 10, ' ') || RPAD(SUBSTR(v_err_recs.trx_type, 1, 12), 12, ' ') || RPAD(SUBSTR(v_err_recs.trx_number, 1, 18), 18, ' ') || v_err_recs.err_code || ':' || v_err_recs.err_msg, NULL, 80);
  END LOOP;
END;
PROCEDURE od_message(
    p_msg_type       IN VARCHAR2,
  ...
  ELSIF p_msg_type = 'O' THEN
    ...
    IF NVL(LENGTH(p_msg), 0) > 120 THEN
      ...
      LOOP
        ...
        ELSE
          fnd_file.put_line(fnd_file.output, LPAD(' ', 120 - p_addnl_line_len, ' ') || SUBSTR(LTRIM(p_msg), ln_char_count + 1, p_addnl_line_len));
...
BEGIN
  BEGIN
    ...
    fnd_file.put_line(fnd_file.LOG, 'Truncate Ends for xx_ar_open_trans_itm table at ' || TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI:SS')); --Added for defect 4901 on 05-APR-10
  ...
  BEGIN
    ...
    fnd_file.put_line(fnd_file.LOG, 'Truncate Ends for xx_ar_open_credits_itm table at ' || TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI:SS'));
  ...
  BEGIN
    ...
    INSERT
    INTO xx_ar_open_trans_itm
      (SELECT
          /*+PARALLEL(APS,8) FULL(APS)*/
          *
        FROM ar_payment_schedules_all aps
        WHERE aps.status = 'OP'
      );
    ln_open_trans_count := SQL%ROWCOUNT;
    fnd_file.put_line(fnd_file.LOG, 'Inserted in xx_ar_open_trans_itm table at ' || TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI:SS')); --Added for defect 4901 on 05-APR-10
    --        FND_FILE.PUT_LINE (fnd_file.LOG,'');                                                                                                --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, 'Total number of records inserted in xx_ar_open_trans_itm ' || ln_open_trans_count || ' rows'); --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, '');                                                                                            --Added for defect 4901 on 05-APR-10
  EXCEPTION
  WHEN OTHERS THEN
    fnd_file.put_line(fnd_file.LOG, 'Insertion failed in xx_ar_open_trans_itm'); --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, '');                                         --Added for defect 4901 on 05-APR-10
  ...
  BEGIN
    ...
    INSERT
      /*+ PARALLEL(XAOTI2,8) */
    INTO xx_ar_open_credits_itm xaoti2
      (SELECT
          /*+ FULL(XAOTI) PARALLEL(XAOTI,8) */
          'NON-OM' SOURCE,
          xaoti.customer_id,
        ...
        AND NOT EXISTS
          (SELECT 1
          FROM xx_ar_refund_trx_tmp
          WHERE trx_id = xaoti.cash_receipt_id
          AND trx_type = 'R'
          AND status  != 'D'
          AND ROWNUM   = 1
          )
        AND NOT EXISTS
          (SELECT
            ...
            1
          FROM hz_cust_accounts hca
          WHERE hca.cust_account_id = xaoti.customer_id
          AND customer_type         = 'I'
          AND ROWNUM                = 1
          )
    ...
    fnd_file.put_line(fnd_file.LOG, 'Inserted in xx_ar_open_credits_itm table at ' || TO_CHAR(SYSDATE, 'DD/MON/YYYY HH24:MI:SS')); --Added for defect 4901 on 05-APR-10
    --      FND_FILE.PUT_LINE (fnd_file.LOG,'');                                                                                                 --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, 'Total number of records inserted in xx_ar_open_credits_itm ' || ln_open_credits_count || ' rows'); --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, '');                                                                                                --Added for defect 4901 on 05-APR-10
  EXCEPTION
  WHEN OTHERS THEN
    fnd_file.put_line(fnd_file.LOG, 'Insertion failed in xx_ar_open_credits_itm'); --Added for defect 4901 on 05-APR-10
    fnd_file.put_line(fnd_file.LOG, '');                                           --Added for defect 4901 on 05-APR-10
    --ROLLBACK to SAVEPOINT sp4;     --Commented for defect 4901 on 05-APR-10
  END;
  COMMIT;
  ln_total_records := ln_open_trans_count + ln_open_credits_count;
  fnd_file.put_line(fnd_file.LOG, 'Total number of records for both the tables ' || ln_total_records || ' rows'); --Added for defect 4901 on 05-APR-10
  fnd_file.put_line(fnd_file.LOG, '');                                                                            --Added for defect 4901 on 05-APR-10
...
IS
  ln_check NUMBER := 0;
BEGIN
  IF (gn_cust_id = p_customer_id) THEN
    RETURN gn_check;
  ELSE
    gn_cust_id := p_customer_id;
    SELECT COUNT(1)
    ...
    IF ln_check <> 1 THEN
      ...
      WHERE EXISTS
        (SELECT
          /*+ INDEX (APS AR_PAYMENT_SCHEDULES_N6) */
          1
        FROM ar_payment_schedules_all aps
...
-- |Version   Date          Author              Remarks                      |
-- |=======   ==========   =============   ==================================|
-- |   1      7-AUG-19     Jitendra A     Initial version--Added for #6.8    |

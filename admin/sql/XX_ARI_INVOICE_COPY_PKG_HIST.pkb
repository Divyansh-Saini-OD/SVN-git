CREATE OR REPLACE PACKAGE BODY XXAPPS_HISTORY_QUERY.XX_ARI_INVOICE_COPY_PKG AS
-- +============================================================================================+
...
-- | 2.0         19-Jul-2016  Suresh Naragam   Version Compiled in History Schema (Defect#2157) |
-- |                                           Added Wallet Location query for SSL/TSL Upgrade  |
-- | 2.1 		 14-MAY-2018  Dinesh N         Retrofit OM tables with Views -NAIT-37762		|
-- +============================================================================================+
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    PUT_ERR_LINE('Error in the function get_unreprintable_trxs'||SQLERRM);
...
BEGIN
  ...
  put_log_line( 'Submit the Copy File Program (XXFIN, XXCOMFILCOPY)' );
  ...
  n_conc_request_id :=
    ...
    ( application    => 'XXFIN',                     -- application short name
      ...
      description    => 'OD: Copy Invoice XML File', -- additional request description
      start_time     => NULL,                        -- request submit time
...
*/
  ...
  n_conc_request_id := XX_XDO_REQUESTS_API_PKG.process_request
...
BEGIN
  ...
  n_conc_request_id :=
    ...
    ( application    => 'XXFIN',                 -- application short name
      ...
      argument2      => p_invoice_trx_list,      -- Invoice Trx List
      argument3      => p_cons_bill_list,        -- Consolidated Bill List
...
IS
  ...
  v_status_code            VARCHAR2(30)   := NULL;
  v_status_desc            VARCHAR2(80)   := NULL;
  lc_current_schema        VARCHAR2(100)  := NULL;
  lc_session_user          VARCHAR2(100)  := NULL;
  lc_current_user          VARCHAR2(100)  := NULL;
BEGIN

  x_url := NULL;
  x_request_id := -1;
  PUT_ERR_LINE('In the procedure run_prog_to_get_invoice '||p_customer_trx_id);
  
  SELECT sys_context('userenv','current_schema'),
         sys_Context('userenv','session_user'),
         sys_Context('userenv','current_user')
  INTO lc_current_schema,
       lc_session_user,
       lc_current_user
  FROM dual;
  PUT_ERR_LINE('Current Schema '||lc_current_schema);
  PUT_ERR_LINE('Session User '||lc_session_user);
  PUT_ERR_LINE('Current User '||lc_current_user);
  ...
  EXCEPTION    
  WHEN OTHERS THEN
    PUT_ERR_LINE('In the procedure run_prog_to_get_invoice Exception '||SQLERRM);
  END;
  PUT_ERR_LINE('In the procedure run_prog_to_get_invoice - Calling get_unreprintable_trxs');
  IF get_unreprintable_trxs(n_cust_account_id, s_invoice_trx) IS NOT NULL THEN
    PUT_ERR_LINE('In the procedure run_prog_to_get_invoice - reprint request aborted');
  ...
  b_success :=
    ...
    ( template_appl_name    => FND_PROFILE.value('XX_AR_INV_COPY_TEMPLATE_APP'),
      ...
      template_territory    => v_user_territory,
      output_format         => GC_XDO_TEMPLATE_FORMAT );
  PUT_ERR_LINE('In the procedure run_prog_to_get_invoice - Submitting the request');
  n_conc_request_id :=
    FND_REQUEST.submit_request
    ( application    => 'XXFIN',                 -- application short name
      program        => 'XXARINVIND',            -- concurrent program name
      ...
      argument9      => 'HED' );                 -- Source (default "Hedberg")
  PUT_ERR_LINE('In the procedure run_prog_to_get_invoice - requet id : '||n_conc_request_id);
  IF (n_conc_request_id > 0) THEN
    COMMIT; -- must commit work so that the concurrent manager polls the request
  ELSE
  ...
  IF NOT FND_CONCURRENT.wait_for_request
    ( request_id    => n_conc_request_id,
      interval      => 5,                      -- check every 5 secs
      max_wait      => 60*25,                   -- check for max of 5 minutes
      phase         => v_phase_desc,
      status        => v_status_desc,
      dev_phase     => v_phase_code,
      dev_status    => v_status_code,
  ...
  ELSE
    x_url := FND_WEBFILE.GET_URL(FND_WEBFILE.REQUEST_OUT, n_conc_request_id, null, null, 10, null, null, null, null );
    PUT_ERR_LINE('x_url from FND_WEBFILE.GET_URL -- '||x_url);
    IF (NOT fnd_profile.defined('APPS_CGI_AGENT')) THEN -- external iRec server not accessible inside firewall; need to target internal server
      SELECT substr(V.profile_option_value, 1,  instr(V.profile_option_value, '/', 1, 3)-1) || substr(x_url, instr(x_url, '/', 1, 3))
      INTO x_url
      FROM FND_PROFILE_OPTIONS O JOIN FND_PROFILE_OPTION_VALUES V
      ...
      WHERE level_id=10001 AND level_value=0 AND rownum=1;
    END IF;
    PUT_ERR_LINE('x_url -- '||x_url);
    x_request_id := n_conc_request_id;
  END IF;
  EXCEPTION WHEN OTHERS THEN
    PUT_ERR_LINE('Exception when others '||SQLCODE||' - '||SQLERRM);
...
--  PUT_ERR_LINE(fnd_global.user_id,p_customer_trx_id);
  PUT_ERR_LINE('In the procedure get Invoice - 1');
  ...
  EXCEPTION WHEN NO_DATA_FOUND THEN BEGIN
    PUT_ERR_LINE('In the procedure get Invoice - 1 no data found'||' - '||p_customer_trx_id);
    IF p_customer_trx_id>0 THEN -- Individual Transaction
      PUT_ERR_LINE('In the procedure get Invoice - Calling run_prog_to_get_invoice');
      run_prog_to_get_invoice  ( p_customer_trx_id, sURL, n_request_id);
      PUT_ERR_LINE('p_customer_trx_id - '||p_customer_trx_id||' sURL '||sURL||' n_request_id '||n_request_id);
    ELSE                        -- Consolidated Bill
      PUT_ERR_LINE('In the procedure get Invoice - Calling run_prog_to_get_cons_bill');
      run_prog_to_get_cons_bill(-p_customer_trx_id, sURL, n_request_id);
    END IF;

    IF n_request_id>0 AND length(sURL)>0 THEN BEGIN
      PUT_ERR_LINE('In the procedure get Invoice - DBMS_LOB.createtemporary');
      ...
      PUT_ERR_LINE('In the procedure get Invoice - UTL_HTTP.begin_request');
      l_http_request  := UTL_HTTP.begin_request(sURL);
--      UTL_HTTP.set_transfer_timeout(300); -- this is in seconds.  default is 60
      
      PUT_ERR_LINE('In the procedure get Invoice - UTL_HTTP.get_response');
      l_http_response := UTL_HTTP.get_response(l_http_request);
      
      PUT_ERR_LINE('In the procedure get Invoice - Loop read_raw');
      BEGIN
        LOOP
         UTL_HTTP.read_raw(l_http_response, l_raw, 32767);
         DBMS_LOB.writeappend (x_blob, UTL_RAW.length(l_raw), l_raw);
         PUT_ERR_LINE('In the Loop');
        END LOOP;
      EXCEPTION
        WHEN UTL_HTTP.end_of_body THEN
          UTL_HTTP.end_response(l_http_response);
          PUT_ERR_LINE('Exception at end_of_body');
      END;
      lgh_file := dbms_lob.getlength(x_blob);
      PUT_ERR_LINE('Length of the BLOB'||lgh_file);
      PUT_ERR_LINE('In the procedure get Invoice - UTL_RAW.cast_to_varchar2'||UTL_RAW.cast_to_varchar2(dbms_lob.substr(x_blob,5,1)));
      IF UTL_RAW.cast_to_varchar2(dbms_lob.substr(x_blob,5,1))<>'%PDF-' THEN -- verify PDF doc prefix
          PUT_ERR_LINE('In the procedure get Invoice - DBMS_LOB.freetemporary');
          DBMS_LOB.freetemporary(x_blob);
          PUT_ERR_LINE('In the procedure get Invoice - DBMS_LOB.createtemporary');
          DBMS_LOB.createtemporary(x_blob, FALSE); -- return empty blob because not valid pdf.
          PUT_ERR_LINE('In the procedure get Invoice - DBMS_LOB.createtemporary - End');
      ELSE
        PUT_ERR_LINE('In the procedure get Invoice - Else Condition');
        BEGIN
          PUT_ERR_LINE('In the procedure get Invoice - update/insert');
        ...
        END;
      END IF;
      PUT_ERR_LINE('In the procedure get Invoice - End before exception');
      EXCEPTION WHEN OTHERS THEN

CREATE OR REPLACE PROCEDURE xx_ar_cbi_sum_subt2 
                                                (
...
END;
 ...
 IF SUBSTR(p_sort_list ,1 ,2) !='B1' THEN 
  ...
  -- ================   
  -- Third Insert for the first field in the sort order where the value is blank...
  -- ================  
BEGIN
  source_cur := dbms_sql.open_cursor;
...
END;
 ...
 ELSE
   lv_sql_stmnt5 :='SELECT SUM(TOTAL_AMOUNT) TOTAL_AMOUNT'
                 ||lv_enter
                 ||'FROM xx_ar_cbi_softheaders_hdr_v '
                 ||'WHERE cons_inv_id ='||p_cons_inv_id;

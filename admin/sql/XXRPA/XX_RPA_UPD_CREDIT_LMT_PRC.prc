CREATE OR REPLACE PROCEDURE XX_RPA_UPD_CREDIT_LMT_PRC ( P_ACCOUNT_NUMBER  IN hz_cust_accounts_all.account_number%TYPE, -- MANDATORY PARAMETER
                                                        P_OVERALL_CRE_LMT IN hz_cust_profile_amts.overall_credit_limit%TYPE,  -- OPTIONAL PARAMETER
                                                        P_TEMP_CRE_LMT    IN hz_cust_profile_amts.overall_credit_limit%TYPE,  -- OPTIONAL PARAMETER
...
BEGIN         
  ...
  ----------------------------------------------------------------------
  IF P_TEMP_CRE_LMT IS NOT NULL THEN
  l_cust_prof_amt_rec.trx_credit_limit := P_TEMP_CRE_LMT;
  END IF;
  ----------------------------------------------------------------------
  IF P_OVERALL_CRE_LMT IS NOT NULL THEN

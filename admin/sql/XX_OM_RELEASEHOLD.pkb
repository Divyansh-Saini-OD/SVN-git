...
-- | 1.8    15-DEC-2017  Venkata Battu     Added Book Order Procedure 43798      |
-- | 1.9    15-APR-2018  Theja Rajula      Retrofit NAIT 34134      			 |
-- +=============================================================================+
   ...
   AS
      ...
      IS
         SELECT h.header_id, h.order_number, h.transactional_curr_code
           FROM xx_oe_order_headers_v h,  --Retrofit NAIT 34134
                xx_oe_payments_v i, 	--Retrofit NAIT 34134
	        ...
	        /*End of code changes for Defect# 27876     */
               ...
               LOOP            
                  ...
                  THEN
					 ...
					 SELECT SUM (prepaid_amount)
                        ...
                        FROM xx_oe_payments_v 	--Retrofit NAIT 34134
                        WHERE header_id = ln_header_id
					...
					--Defect#39944
                     ...
                     BEGIN
                        ...
                        INSERT INTO oe_payments									--Assuming Retrofit not required NAIT 34134
			 ...
			 /* Added for defect #27876*/
               ...
               SELECT LENGTH (orig_sys_document_ref)
                 INTO ln_osr_length
                 FROM xx_oe_order_headers_v			--Retrofit NAIT 34134
                WHERE header_id = ln_header_id;

               SELECT ROUND (order_total, 2) order_total
                 INTO ln_order_total
                 FROM xx_om_header_attributes_v		--Retrofit NAIT 34134
                WHERE header_id = ln_header_id;
               ...
               THEN
                 ...
                 /* wf_engine.completeactivityinternalname
                  ...
                  COMMIT; */
				-- Calling Book Order API  
                  book_order (p_header_id  => l_header_rec.header_id(1)
               ...
               ELSE
                  ...
                  THEN
					...
					put_log_line (p_debug_flag, 'N', ' ');
			        put_log_line (p_debug_flag, 'N', 'Apply Hold Status :'||lc_return_status);		
                  END IF;
               END IF;
            END IF;
      ...
      IS
         SELECT   *
             FROM (SELECT imp_file_name,
                          ...
                          DECODE ((SELECT 1
                                     FROM xx_oe_payments_v o	--Retrofit NAIT 34134
                          ...
                          (SELECT DECODE
                             ...
                             FROM xx_om_legacy_deposits a,
                                  xx_oe_order_headers_v c,		--Retrofit NAIT 34134
                          ...
                          DECODE
                             ((SELECT 1
                                 FROM xx_oe_payments_v i,		--Retrofit NAIT 34134
                     ...
                     FROM oe_order_holds_all oh,
                          xx_oe_order_headers_v h,		--Retrofit NAIT 34134
                          oe_hold_sources_all hs,
                          oe_hold_definitions hd,
                          xx_om_header_attributes_v	x		--Retrofit NAIT 34134
                    WHERE oh.hold_source_id = hs.hold_source_id
...
-- BEGIN
                     SELECT ot.NAME
                       INTO lc_so_type
                       FROM xx_oe_order_headers_v h,		--Retrofit NAIT 34134
                            xx_oe_order_lines_v l,			--Retrofit NAIT 34134
...
-- Calling the API to to Book an Existing Order --
   ...
   THEN
         put_log_line (p_debug_flag,
                       'N',
                       ' calling book order API for Header Id :  ' ||p_header_id 
                      );
   END IF;
...
IF v_return_status = fnd_api.g_ret_sts_success THEN
   ...
   THEN
         put_log_line (p_debug_flag,
                       'N',
                       'Order has been booked successfully for header :' ||p_header_id 
                      );
   END IF;        
...
ELSE
   ...
   THEN
         put_log_line (p_debug_flag,
                       'N',
                       'Order booking has failed for Header :' ||p_header_id 
                      );
   END IF;
...
WHEN OTHERS
THEN
     put_log_line (p_debug_flag,'N','Exception in book order Procedure :' ||SQLERRM); 
END;   

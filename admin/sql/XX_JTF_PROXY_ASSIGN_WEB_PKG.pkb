...
create or replace
PACKAGE BODY XX_JTF_PROXY_ASSIGN_WEB_PKG AS
...
PROXY_ROLE_CODE constant varchar2(20) := 'OD_NA_SA_PRXY_ROL';

-- Exceptions
LOCK_RECORD_EXCEPTION           EXCEPTION;
ASSIGN_ROLE_EXCEPTION           EXCEPTION;
CREATE_ROLE_RELATE_EXCEPTION    EXCEPTION;
UNKNOWK_ERROR_EXCEPTION         EXCEPTION;
CREATE_GRP_MEMBRSHP_EXCEPTION   EXCEPTION;
-- ============================================================================
  ...
  if (lc_return_status = fnd_api.G_RET_STS_SUCCESS) then
    ...
    if (p_role_relate_id is null) then
      ...
      fnd_message.set_token('DETAILS',
                            ...
                            '  p_role_resource_type=' || p_role_resource_type);
      raise CREATE_ROLE_RELATE_EXCEPTION;
    end if;
  ...
  if (rec.role_relate_id is not null) then
    ...
    if (p_role_assign_start_date between rec.start_date_active
      ...
      update_row (x_role_relate_id        => rec.role_relate_id,
                  ...
                  x_object_version_number => rec.object_version_number,
                  x_commit                => FND_API.G_FALSE,
                  x_msg_count             => ln_msg_cnt,
                  x_msg_data              => lc_msg_data);
                  
      if(ln_msg_cnt > 0) then
        fnd_message.set_name('XXCRM', 'XX_CRM_0090_API_ERROR');
        fnd_message.set_token('API_ERROR', lc_msg_data);
        raise ASSIGN_ROLE_EXCEPTION;
      end if;
...
procedure create_proxy_role(x_resource_id           in  number,
 ...
 cursor c_roles is select role_id from apps.jtf_rs_roles_vl where role_code = 'OD_NA_SA_PRXY_ROL';
...
begin
  ...
  if( nvl(x_end_date_active,x_start_date_active)  < x_start_date_active) then
    x_msg_count := x_msg_count + 1;
    x_msg_data := 'The end date cannot be earlier than the start date';
  end if;
  ...
  if(x_msg_count = 0) then
      ...
      for rec in  c_roles
      loop
        n_role_id := rec.role_id;
        exit;
      end loop;
      
      if( n_role_id <> -1) then
                   insert_row(x_resource_id       => x_resource_id,
                          x_resource_start_date   => x_resource_start_date,
                          x_group_id              => x_group_id,
                          x_role_resource_type    => 'RS_GROUP_MEMBER',
                          x_role_id               => n_role_id,
                          x_role_code             => 'PROXY_ROLE_CODE',
        ...
        x_msg_data := 'No proxy role found';
        x_msg_count := x_msg_count + 1;
      end if;
  
  end if;

end create_proxy_role;
...
begin
  ...
  if (ln_group_member_id is null) then
    ...
    if (lc_return_status = fnd_api.G_RET_STS_SUCCESS) then
      ...
      fnd_message.set_name('XXCRM', 'XX_CRM_0090_API_ERROR');
      fnd_message.set_token('API_ERROR', lc_msg);
      raise CREATE_GRP_MEMBRSHP_EXCEPTION;
    end if;
  ...
  select object_version_number
   ...
   rollback;
    ...
    if (x_msg_data is null or x_msg_data='') then
      x_msg_data := 'An unknown error was received';
    end if;
    x_msg_count := x_msg_count + 1;
end insert_row;
-- ============================================================================
...
begin
  ...
  if(x_commit = 'Y') then
    lc_temp_flag := fnd_api.G_TRUE;
  else 
    lc_temp_flag := fnd_api.G_FALSE;
  end if;

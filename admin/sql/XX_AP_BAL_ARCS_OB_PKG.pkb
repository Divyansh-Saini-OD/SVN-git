...
PROCEDURE populate_ap_out_file(
  ...
  SELECT gcc.segment1 Company,
         ...
         gcc.segment6 LOB,
         gcc.segment7 future,
         MAX(NVL(gab.beginning_balance,0)) ytd_beginning_bal,
         SUM(NVL(xll.entered_dr,0)) ptd_net_dr,
         SUM(NVL(xll.entered_cr,0)) ptd_net_cr,
         MAX(NVL(gab.beginning_balance,0) ) + (SUM(NVL(xll.entered_dr,0)) - SUM(NVL( xll.entered_cr,0)))  ytd_balance,
         gld.currency_code,
         gld.ledger_id,
         per.period_name,
         ...
         ,gab.period_name gab_period_name
    FROM xla_ae_lines xll,
         ...
         gl_balances gb,
         gl_periods per,
         xx_gl_arcs_balances gab
 WHERE 1 = 1
   AND gab.period_name(+)                = per.period_name
   AND gab.company(+)                    = gcc.segment1
   AND gab.account(+)                    = gcc.segment3
   AND gab.location(+)                   = gcc.segment4
   AND gab.lob(+)                        = gcc.segment6
   ...
   AND per.period_name                   = c_period_name
   ...
   AND gcc.segment3                        IN ('20101000', '20204000')
  ...
  TYPE LC_GL_DATA_TAB IS TABLE OF c_get_gl_data%ROWTYPE INDEX BY PLS_INTEGER;
  ...
  lc_next_period_name          VARCHAR2(10);

  le_no_balance_found EXCEPTION;
...
BEGIN
  ...
  lc_instance_name:= SUBSTR(LOWER(SYS_CONTEXT('USERENV', 'INSTANCE_NAME') ), 1, 8);
  ...
  SELECT period_name
    INTO lc_next_period_name
    FROM gl_periods per
   WHERE start_date = (SELECT end_date+1
                         FROM gl_periods per
                        WHERE period_name = p_period_name
                       );
  print_debug_msg('Next Period Name:'|| lc_next_period_name, TRUE);
  ...
  print_debug_msg('lc_gl_data.COUNT:'|| LC_GL_DATA.COUNT, TRUE);

  IF LC_GL_DATA.COUNT > 0 THEN
    FOR l_rec IN LC_GL_DATA.FIRST..LC_GL_DATA.LAST
    LOOP

       IF lc_gl_data(l_rec).ytd_beginning_bal <> 0 
          OR lc_gl_data(l_rec).ptd_net_dr <> 0
          OR lc_gl_data(l_rec).ptd_net_cr <> 0
          OR lc_gl_data(l_rec).ytd_balance <> 0 THEN
          lv_line_count := lv_line_count + 1;
          utl_file.put_line(lc_file_handle, lc_gl_data(l_rec).company||','|| lc_gl_data(l_rec).cost_center||','||
                            lc_gl_data(l_rec).account||','|| lc_gl_data(l_rec).location||','|| lc_gl_data(l_rec).intercompany||','||
                            lc_gl_data(l_rec).lob||','|| lc_gl_data(l_rec).future||','|| lc_gl_data(l_rec).ytd_beginning_bal||','||
                            lc_gl_data(l_rec).ptd_net_dr||','|| lc_gl_data(l_rec).ptd_net_cr||','|| lc_gl_data(l_rec).ytd_balance||','||
                            lc_gl_data(l_rec).currency_code||','|| lc_gl_data(l_rec).ledger_id||','|| lc_gl_data(l_rec).period_name||','||
                            lc_gl_data(l_rec).code_combination_id||','|| lc_gl_data(l_rec).balance_type||','|| lc_gl_data(l_rec).account_description
                           );
          UPDATE xx_gl_arcs_balances
             SET  currency_code       = lc_gl_data(l_rec).currency_code
                 ,sum_ptd_net_dr      = lc_gl_data(l_rec).ptd_net_dr
                 ,sum_ptd_net_cr      = lc_gl_data(l_rec).ptd_net_cr
                 ,ending_balance      = lc_gl_data(l_rec).ytd_balance
                 ,updated_date        = systimestamp
                 ,updated_by          = user
            WHERE period_name = lc_gl_data(l_rec).period_name
              AND company     = lc_gl_data(l_rec).company
              AND account     = lc_gl_data(l_rec).account
              AND location    = lc_gl_data(l_rec).location
              AND lob         = lc_gl_data(l_rec).lob;

            IF sql%ROWCOUNT = 0 THEN
               print_debug_msg('Inserting missed Period Name:'|| p_period_name, TRUE);

               INSERT
                 INTO xx_gl_arcs_balances
                      (period_name        
                      ,company            
                      ,account
                      ,location
               ...
               VALUES (p_period_name
                      ...
                      ,lc_gl_data(l_rec).ptd_net_dr
                      ,lc_gl_data(l_rec).ptd_net_cr
          ...
          BEGIN
            ...
            UPDATE xx_gl_arcs_balances
               SET currency_code       = lc_gl_data(l_rec).currency_code
                  ,beginning_balance   = lc_gl_data(l_rec).ytd_balance
                  ,updated_date        = systimestamp
                  ,updated_by          = user
             WHERE period_name = lc_next_period_name
               AND company     = lc_gl_data(l_rec).company
               AND account     = lc_gl_data(l_rec).account
               AND location    = lc_gl_data(l_rec).location
               AND lob         = lc_gl_data(l_rec).lob;
            ...
            IF sql%ROWCOUNT = 0 THEN
               print_debug_msg('Inserting Next Period Name:'|| lc_next_period_name, TRUE);
               INSERT
                 INTO xx_gl_arcs_balances
                      (period_name        
                      ,company            
    ...
    SELECT lc_next_period_name
           ,cp.company
           ,cp.account
           ,cp.location
           ,cp.lob
           ,cp.currency_code
           ,cp.beginning_balance
      FROM xx_gl_arcs_balances cp
          ,xx_gl_arcs_balances np
     WHERE cp.period_name = p_period_name
       AND np.period_name(+) = lc_next_period_name
       AND cp.company = np.company(+)
       AND cp.account = np.account(+)
       AND cp.location = np.location(+)
       AND cp.lob = np.lob(+)
       AND np.account IS NULL;

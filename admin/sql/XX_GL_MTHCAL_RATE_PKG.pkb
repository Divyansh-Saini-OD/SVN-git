...
***********************************************************************************/
PROCEDURE xx_archive_file_DIMS(
...
BEGIN
  ...
  END;
  lc_source_file_name   := lc_source_dir_path||'/'||p_file;
  IF p_request_type     <>'DAILY' THEN
    lc_source_file_name := '/app/ebs/ct' || lc_instance_name || '/xxfin/ftp/out/hyperion/'||p_file;
    lc_dest_file_name   := lc_target_dir_path||'/'||p_file;
  ELSE
    lc_source_file_name := '/app/ebs/ct' || lc_instance_name || '/xxfin/ftp/out/rates/'||p_file||'.txt';
  ...
  IF ln_request_id > 0 THEN
    ...
    lb_complete := fnd_concurrent.wait_for_request(request_id => ln_request_id, interval => 10, max_wait => 0, phase => lc_phase, status => lc_status, dev_phase => lc_dev_phase, dev_status => lc_dev_status, MESSAGE => lc_message );
  END IF;
  COMMIT;
EXCEPTION
WHEN OTHERS THEN
...
BEGIN
  ...
  IF p_request_type     <>'DAILY' THEN
    ...
    lc_dest_file_name   := lc_target_dir_path||'/'||p_file;
  ELSE
    lc_source_file_name := '/app/ebs/ct' || lc_instance_name || '/xxfin/ftp/out/rates/'||p_file||'.zip';
...
***********************************************************************************/
PROCEDURE xx_file_copy_dims(
    p_request_type IN VARCHAR2,
    p_copy_file    IN VARCHAR2)
IS
  lc_dest_file_name   VARCHAR2(200);
  lc_source_file_name VARCHAR2(200);
  lc_source_dir_path  VARCHAR2(4000);
...
BEGIN
  ...
  lc_zip_destination_dir :='/app/ebs/ct' || lc_instance_name || '/xxfin/ftp/out/rates';
...
IS
  ...
  BEGIN
    SELECT LAST_DAY(ADD_MONTHS(p_date,-1))+1 INTO ld_from_date FROM DUAL;
    SELECT LAST_DAY(p_date) INTO ld_to_date FROM DUAL;
    IF P_DATE = LD_TO_DATE THEN --NAIT-132879
      FOR rec IN cr_calendar_avg_rates(ld_from_date,ld_to_date)
      LOOP
        BEGIN
          INSERT
      ...
      WHEN OTHERS THEN
        ...
        RETURN;
      END;
      FOR rec IN cr_calendar_end_rates
      (
        ld_to_date
      )
      LOOP
	...
	l_filehandle := UTL_FILE.fopen(gc_file_path,lc_file_name ,'w',ln_buffer);
	  ...
	  FND_FILE.PUT_LINE(FND_FILE.LOG,'Conversion Date: '|| LD_TO_DATE); --NAIT-132879
	  FND_FILE.PUT_LINE(FND_FILE.LOG,'File Name: '|| lc_file_name);     --NAIT-132879
    ...
    ELSE
	    ...
	    BEGIN
		  SELECT MAX(conversion_Date) INTO ln_conv_date
			FROM gl_daily_rates
	  ...
	  SELECT TO_CHAR(ln_conv_date,'Mon') INTO lc_filename_part1 FROM DUAL;
      SELECT TO_CHAR(ln_conv_date,'RR') INTO lc_filename_part2 FROM DUAL;
      ...
      FOR rec IN cr_calendar_rates(ln_conv_date)
      LOOP
        l_data:=rec.from_currency||lc_delimeter||rec.to_currency||lc_delimeter||rec.ratetype||lc_delimeter||rec.conversion_rate;
        UTL_FILE.PUT_LINE(l_filehandle,l_data);
      END LOOP;
  ...
  --NAIT-132879 start
  WHEN ex_conv_date_exception THEN

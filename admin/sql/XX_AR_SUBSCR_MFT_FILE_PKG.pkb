 ...
 ***********************************************/
  ...
  **********************************************************/
  ...
  AS
    ...
    IS
       ...
       lc_procedure_name       CONSTANT VARCHAR2(61) := gc_package_name || '.' || 'mft_generate_file';
    ...
    logit(p_message => '---------------------------------------------------',
          p_debug   => TRUE);
    logit(p_message => 'Starting MFT File Creation routine. ',
          p_debug   => TRUE);
    logit(p_message => '---------------------------------------------------',
          p_debug   => TRUE);
    ...
    BEGIN
      ...
      WHERE directory_name = 'XXFIN_VANTIV_SBS';
    EXCEPTION
      WHEN OTHERS THEN
      lc_file_path := NULL;
    END;

    lt_file_name    := 'ODVANTIV_SBS'||'_'||TO_CHAR (SYSDATE,'DDMONYYYYHH24MISS')||'.txt';

    logit(p_message => '---------------------------------------------------',
          p_debug   => TRUE);
    logit(p_message => 'File Name is: '||lt_file_name||' and File Path is: '||lc_file_path,
          p_debug   => TRUE);
    logit(p_message => '---------------------------------------------------',
          p_debug   => TRUE);

    lt_file_handle := UTL_FILE.fopen('XXFIN_VANTIV_SBS',lt_file_name,'W',ln_max_linesize);

    OPEN c_get_mft_data;
    FETCH c_get_mft_data BULK COLLECT INTO LC_MFT_DATA;
    ...
    IF LC_MFT_DATA.COUNT > 0 THEN
      ...
      ***************************************************************************************/
               logit(p_message => '--------------------------------------------------------------------------------------------------------------------------------',
                     p_debug   => TRUE);
               logit(p_message => 'START of inserting Record for Contract #: ' || LC_MFT_DATA(l_rec).contract_number,
                     p_debug   => TRUE);
               logit(p_message => '--------------------------------------------------------------------------------------------------------------------------------',
                     p_debug   => TRUE);
                ...
                UTL_FILE.put_line(lt_file_handle,
                                  RPAD(SUBSTR(LC_MFT_DATA(l_rec).contract_number,1,8),10)|| 
                                  RPAD(SUBSTR(LC_MFT_DATA(l_rec).contract_id,1,18),20)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).initial_order_number,' '),1,23),25)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).contract_name,' '),1,23),25)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).card_type,' '),1,13),15)|| 
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).card_token,' '),1,53),55)|| 
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).card_expiration_date,' '),1,4),6)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).card_encryption_label,' '),1,28),30)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).card_encryption_hash,' '),1,43),45)|| 
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).auth_status,' '),1,8),10)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).auth_message,' '),1,98),100)||
                                  RPAD(SUBSTR(NVL(LC_MFT_DATA(l_rec).error_date,' '),1,8),10)
                                );
      END LOOP;
      UTL_FILE.put_line(lt_file_handle,CHR(10)||'Total Number Of Records : '||ln_rec_cnt);
      UTL_FILE.fclose(lt_file_handle);
    END IF;
 EXCEPTION
 WHEN UTL_FILE.ACCESS_DENIED THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' access_denied :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
 ...
 WHEN UTL_FILE.DELETE_FAILED THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' delete_failed :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
 ...
 WHEN UTL_FILE.FILE_OPEN THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' file_open :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
 ...
 WHEN UTL_FILE.INTERNAL_ERROR THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' internal_error :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
 ...
 WHEN UTL_FILE.INVALID_FILEHANDLE then
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_filehandle :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
 ...
 WHEN UTL_FILE.INVALID_FILENAME then
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_filename :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
 ...
 WHEN UTL_FILE.INVALID_MAXLINESIZE then
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_maxlinesize :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.invalid_mode THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_mode :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.invalid_offset THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_offset :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.invalid_operation THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_operation :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.invalid_path THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' invalid_path :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.read_error THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' read_error :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.rename_failed THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' rename_failed :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
...
WHEN UTL_FILE.write_error THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' write_error :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  logit (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lt_file_handle := UTL_FILE.fopen (lc_file_path, lt_file_name, 'W', 32767);
  UTL_FILE.fclose(lt_file_handle);
  ...
  p_retcode := 2;
WHEN OTHERS THEN
  lc_errormsg := ( 'AR Subscription Generate MFT File Program Errored :- ' || ' Others Exception :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );

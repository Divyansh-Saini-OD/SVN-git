...
CREATE OR REPLACE
PACKAGE BODY xxod_fin_reports_pkg AS
...
-- |DRAFT 1B 23-MAR-2016  Vasu R           Removed schema References   |
-- |                                       for R.12.2                  |
-- +===================================================================+
...
-- +===================================================================+
   IS
      ...
      l_global_ultimate_party   hz_parties.party_id%TYPE;
      l_root                    NUMBER;
   BEGIN
      l_root := p_party_id;
...
IS
 ...
 BEGIN
	SELECT NVL(SUM(FDD.deprn_reserve),0)
  ...
  BEGIN
    IF(p_transaction_type_code='ACTIVE') THEN
		...
		AND transaction_header_id_in = (
         ...
         AND FTH.date_effective <= p_period_end_date
...
BEGIN
      ...
      FROM  jtf_rs_roles_vl JRRV
   	...
   	AND JRRV.manager_flag ='Y'
  	   AND JRGMRV.group_id IN (SELECT JRGM.group_id
                            FROM jtf_rs_salesreps      JRS
                                  ,jtf_rs_group_members        JRGM 
                            WHERE JRS.salesrep_id=p_salesrep_id
                            AND   JRS.resource_id = JRGM.resource_id
                            AND  TRUNC(SYSDATE) BETWEEN NVL(JRS.start_date_active,SYSDATE-1) 
                            							        AND NVL(JRS.end_date_active,SYSDATE+1)
                          )
					 ...
					 BEGIN
							...
							SELECT JRRE.source_name
 							INTO lc_rsd_name
  							...
  							AND JRGR.group_id IN (SELECT JRGM.group_id
                             					FROM jtf_rs_salesreps      JRS
                                  				 ,jtf_rs_group_members        JRGM 
                             					WHERE JRS.salesrep_id=p_salesrep_id
                             					AND   JRS.resource_id = JRGM.resource_id

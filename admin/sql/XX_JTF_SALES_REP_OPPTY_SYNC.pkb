...
PROMPT
PROMPT Creating XX_JTF_SALES_REP_OPPTY_SYNC package body
PROMPT

CREATE OR REPLACE PACKAGE BODY XX_JTF_SALES_REP_OPPTY_SYNC 
-- +===================================================================================+
...
EXCEPTION
   WHEN OTHERS THEN
       ...
       XX_COM_ERROR_LOG_PUB.log_error_crm(
                                          ...
                                          , p_program_name           => 'XX_JTF_SALES_REP_OPPTY_SYNC.WRITE_LOG'
                                          , p_program_id             => gn_program_id
                                          , p_module_name            => G_MODULE_NAME
                                          , p_error_location         => 'XX_JTF_SALES_REP_OPPTY_SYNC.WRITE_LOG'
...
EXCEPTION
   WHEN OTHERS THEN
       ...
       XX_COM_ERROR_LOG_PUB.log_error_crm(
                                          ...
                                          , p_application_name       => G_APPLICATION_NAME
                                          , p_program_type           => G_PROGRAM_TYPE
                                          , p_program_name           => 'XX_JTF_SALES_REP_OPPTY_SYNC.WRITE_OUT'
                                          , p_program_id             => gn_program_id
                                          , p_module_name            => G_MODULE_NAME
                                          , p_error_location         => 'XX_JTF_SALES_REP_OPPTY_SYNC.WRITE_OUT'
                                          , p_error_message_code     => 'XX_TM_0007_UNEXPECTED_ERR'
                                          , p_error_message          => lc_error_message
                                          , p_error_message_severity => G_MEDIUM_ERROR_MSG_SEVERTY
                                          , p_error_status           => G_ERROR_STATUS_FLAG
...
lc_oppty_sync_success   VARCHAR2(03);
ln_oppty_updt_success   PLS_INTEGER := 0;
...
FROM   xx_tm_nam_terr_curr_assign_v 
...
--------------------------------
TYPE oppty_id_tbl_type IS TABLE OF xx_tm_nam_terr_curr_assign_v.entity_id%TYPE INDEX BY BINARY_INTEGER;
lt_oppty_id oppty_id_tbl_type;
...
lt_named_acct_oppty named_acct_tbl_type;
...
BEGIN
   ...
   WRITE_LOG(RPAD(' ',90,'-'));
   WRITE_LOG(LPAD('OD: Synchronize Opportunity Named Account',62));
   WRITE_LOG(RPAD(' ',90,'-'));
   WRITE_LOG('');
   
   WRITE_OUT(RPAD('Office Depot',65)||LPAD('Date: '||TRUNC(sysdate),25));
   WRITE_OUT(RPAD(' ',90,'-'));
   WRITE_OUT(LPAD('OD: Synchronize Opportunity Named Account',62));
   WRITE_OUT(RPAD(' ',90,'-'));
   WRITE_OUT('');
   WRITE_OUT(RPAD(' ',90,'-'));
   ...
   LOOP
       ...
       FETCH lcu_oppty_id BULK COLLECT INTO lt_oppty_id LIMIT G_LIMIT;
               
       IF lt_oppty_id.COUNT <> 0 THEN
         
          FOR i IN lt_oppty_id.FIRST .. lt_oppty_id.LAST
          LOOP
              ...
              BEGIN
                 ...
                 lc_oppty_sync_success := NULL;
                 lt_named_acct_oppty.DELETE;
                 
                 -- To count the number of records read
                                                      
                 ln_total_count := ln_total_count + 1;
                                                                     
                 ln_oppty_id := lt_oppty_id(i);
                                                      
                 WRITE_LOG(RPAD(' ',80,'-'));
                 WRITE_LOG('Processing for the opportunity id: '||ln_oppty_id);
                 
                 -- Fetch the address_id for that lead_id from the table AS_LEADS_ALL
                                              
                 BEGIN
                                                   
                      SELECT ALA.address_id
                      INTO   ln_address_id
                      FROM   as_leads_all ALA
                      WHERE  ALA.lead_id =  ln_oppty_id;
                 ...
                 EXCEPTION
                    WHEN OTHERS THEN
                        ...
                        XX_COM_ERROR_LOG_PUB.log_error_crm(
                                                           ...
                                                           , p_application_name       => G_APPLICATION_NAME
                                                           , p_program_type           => G_PROGRAM_TYPE
                                                           , p_program_name           => 'XX_JTF_SALES_REP_OPPTY_SYNC.UPDATE_OPPTY'
                                                           , p_program_id             => gn_program_id 
                                                           , p_module_name            => G_MODULE_NAME
                                                           , p_error_location         => 'XX_JTF_SALES_REP_OPPTY_SYNC.UPDATE_OPPTY'
                 ...
                 -- is different from the Named_Acct_Terr_ID (associated to the OPPORTUNITY)
                 
                 OPEN lcu_named_acct_terr_id(
                                             p_entity_type1   => 'PARTY_SITE'                
                                             , p_entity_id1   => ln_address_id
                                             , p_entity_type2 => 'OPPORTUNITY'
                                             , p_entity_id2   => ln_oppty_id
                                            );
                 ...
                 IF lt_named_acct_site.COUNT <> 0 THEN
                   ...
                   LOOP
                       ...
                       -- Create a new entity record entity_type = 'OPPORTUNITY'
                       ...
                       ELSE
                           ...
                           ln_oppty_updt_success := ln_oppty_updt_success + 1;
                 ...
                 -- Validate whether the Named_Acct_Terr_ID (associated to the OPPORTUNITY) derived
                 -- is different from the Named_Acct_Terr_ID (associated to the PARTY SITE)
                                  
                 OPEN lcu_named_acct_terr_id(
                                             p_entity_type1   => 'OPPORTUNITY'
                                             , p_entity_id1   => ln_oppty_id 
                                             , p_entity_type2 => 'PARTY_SITE'
                                             , p_entity_id2   => ln_address_id
                                            );
                 FETCH lcu_named_acct_terr_id BULK COLLECT INTO lt_named_acct_oppty;
                 CLOSE lcu_named_acct_terr_id; 
                 
                 -- If any record is found, end date that record in the custom assignments table
                                                   
                 IF lt_named_acct_oppty.COUNT <> 0 THEN
                                    
                   lc_record_exists       := 'N';
                   lc_oppty_sync_success  := 'Y';                   
                                    
                   FOR l IN lt_named_acct_oppty.FIRST .. lt_named_acct_oppty.LAST
                   LOOP
                       ...
                       -- End Date the entity record entity_type = 'OPPORTUNITY'
                                                         
                       XX_JTF_RS_NAMED_ACC_TERR.update_row(
                                                           p_api_version            => 1.0
                                                           , p_start_date_active    => NULL
                                                           , p_end_date_active      => SYSDATE
                                                           , p_named_acct_terr_id   => lt_named_acct_oppty(l)
                                                           , p_entity_type          => 'OPPORTUNITY'
                                                           , p_entity_id            => ln_oppty_id
                                                           , x_return_status        => lc_return_status
                                                           , x_msg_count            => ln_msg_count
                   ...
                   END LOOP; -- lt_named_acct_oppty.FIRST .. lt_named_acct_oppty.LAST
                                                   
                 END IF; -- lt_named_acct_lead.COUNT <> 0
                 
              EXCEPTION
                 WHEN EX_OPPTY_SYNC THEN
                     FND_MESSAGE.SET_NAME('XXCRM','XX_TM_0149_OPPTY_SYNC_FAILED');
                     FND_MESSAGE.SET_TOKEN('P_LEAD_ID', ln_oppty_id);
                     lc_error_message := FND_MESSAGE.GET;
                     lc_oppty_sync_success := 'N';
                     lc_record_exists      := 'N';
                     WRITE_LOG(lc_error_message);
                     XX_COM_ERROR_LOG_PUB.log_error_crm(
                                                        p_return_code              => FND_API.G_RET_STS_ERROR 
                                                        ...
                                                        , p_module_name            => G_MODULE_NAME
                                                        , p_error_location         => 'XX_JTF_SALES_REP_OPPTY_SYNC.UPDATE_OPPTY'
                                                        , p_error_message_code     => 'XX_TM_0149_OPPTY_SYNC_FAILED'
                                                        , p_error_message          => lc_error_message
                                                        , p_error_message_severity => G_MEDIUM_ERROR_MSG_SEVERTY
                                                        , p_error_status           => G_ERROR_STATUS_FLAG
                                                       );
                 WHEN OTHERS THEN
                     FND_MESSAGE.SET_NAME('XXCRM','XX_TM_0007_UNEXPECTED_ERR');
                     lc_set_message     :=  'Unexpected Error while synchronization of oppty_id : '||ln_oppty_id;
                     FND_MESSAGE.SET_TOKEN('PROCEDURE_NAME', lc_set_message);
                     FND_MESSAGE.SET_TOKEN('ERROR_CODE', SQLCODE);
                     FND_MESSAGE.SET_TOKEN('ERROR_MESSAGE', SQLERRM);
                     lc_error_message := FND_MESSAGE.GET;
              ...
              IF lc_oppty_sync_success = 'Y' THEN
                
                ln_success_count := ln_success_count + 1;
              
              ELSIF lc_oppty_sync_success = 'N' THEN
                   
                   ln_error_count := ln_error_count + 1;
              
              END IF; -- lc_oppty_sync_success = 'Y'
          ...
          END LOOP; -- lt_oppty_id.FIRST .. lt_oppty_id.LAST
        
       END IF; -- lt_oppty_id.COUNT <> 0
   ...
   IF ln_total_count = 0 THEN
     ...
     FND_MESSAGE.SET_TOKEN('P_ENTITY_TYPE', 'OPPORTUNITY');
     lc_error_message := FND_MESSAGE.GET;
     WRITE_LOG(lc_error_message);
     XX_COM_ERROR_LOG_PUB.log_error_crm(
                                        p_return_code               => FND_API.G_RET_STS_ERROR 
   ...
   FND_MESSAGE.SET_TOKEN('P_RECORDS_UPDATED', ln_oppty_updt_success);
...
EXCEPTION
   WHEN OTHERS THEN
       FND_MESSAGE.SET_NAME('XXCRM','XX_TM_0007_UNEXPECTED_ERR');
       lc_set_message     :=  'Unexpected Error while synchronization of opportunity';
       FND_MESSAGE.SET_TOKEN('PROCEDURE_NAME', lc_set_message);
       FND_MESSAGE.SET_TOKEN('ERROR_CODE', SQLCODE);
       FND_MESSAGE.SET_TOKEN('ERROR_MESSAGE', SQLERRM);
       lc_error_message := FND_MESSAGE.GET;

-- $Id: XXCDH_AR_ABL_CUSTOMER_PKG.pkb 112379 2010-08-31 15:36:11Z Srinivas Cherukuri $
-- $Rev: 112379 $
-- $HeadURL: https://svn.na.odcorp.net/od/crm/branches/10.3/xxcrm/admin/sql/XXCDH_AR_ABL_CUSTOMER_PKG.pkb $
-- $Author: Srinivas Cherukuri $
-- $Date: 2010-08-31 11:36:11 -0400 (Tue, 31 Aug 2010) $
SET SHOW          OFF;
SET VERIFY        OFF;
SET ECHO          OFF;
SET TAB           OFF;
...
CREATE OR REPLACE PACKAGE BODY XXCDH_AR_ABL_CUSTOMER_PKG
-- +=====================================================================================+
...
-- |                                     Added the hint LEADING on the 02  Functions.    |
-- |                                     Removed Parallel from the cursor lcu_abl_cust.  |
-- |					 Moved up the hz_customer_profile and ra_terms as|
-- |					 inline view.                                    |
-- |                                     			                         |
-- |V1.1      22-Feb-10   nabarun Ghosh	 Vladimir's suggestion as below:                 |
-- |                                     Added rel.OBJECT_TABLE_NAME = 'HZ_PARTIES'      |
...
-- |                                     			                         |
-- |1.3       22-JUN-2010 Srini          ABL Customer File Phone field enhancement.      |
-- |                                     QC# 5127		                         |
-- |                                     - If first character is alpha.                  |
-- |                                     - If data length is greater than 12.            |
-- |                                     			                         |
-- |1.4       11-Dec-2015  Vasu Raparla  Removed Schema References for R.12.2            |                                     			                         |
-- |                                     			                         |
-- +=====================================================================================+
AS
...
BEGIN
	  ...
	  FROM   hz_cust_accounts      role_acct,
...
RETURN XXCDH_AR_ABL_CUSTOMER_PKG.lt_subq_func
PIPELINED
  ...
  IS
            WITH cust_profile AS
              (SELECT /*+ NO_MERGE */
               ...
               FROM    hz_customer_profiles   cust_prof
...
BEGIN
        ...
        IF lt_subq_cur_rec.COUNT > 0 THEN
         ...
         LOOP
          ...
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.party_id         :=  lt_subq_cur_rec(i).party_id          ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.party_name       :=  lt_subq_cur_rec(i).party_name        ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.account_number   :=  lt_subq_cur_rec(i).account_number    ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.party_site_id    :=  lt_subq_cur_rec(i).party_site_id     ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.address1         :=  lt_subq_cur_rec(i).address1          ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.address2         :=  lt_subq_cur_rec(i).address2          ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.city             :=  lt_subq_cur_rec(i).city              ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.state            :=  lt_subq_cur_rec(i).state             ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.province         :=  lt_subq_cur_rec(i).province          ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.postal_code      :=  lt_subq_cur_rec(i).postal_code       ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.country          :=  lt_subq_cur_rec(i).country           ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.country_code     :=  lt_subq_cur_rec(i).country_code      ;
          XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out.cust_account_id  :=  lt_subq_cur_rec(i).cust_account_id	 ;	
          														 
          PIPE ROW (XXCDH_AR_ABL_CUSTOMER_PKG.lrec_subq_func_out);							
...
AS
    ...
    FROM(  
    	   ...
    	   FROM  TABLE(XXCDH_AR_ABL_CUSTOMER_PKG.Get_abl_cust_Details) subq_func,
         ...
         LOOP 
          ...
          IF lc_phone_number IS NULL THEN 
            ...
            BEGIN
                   ...
                   SELECT REGEXP_REPLACE(cont_point.phone_area_code||cont_point.phone_number,'([[:digit:]]{3})([[:digit:]]{3})([[:digit:]]{4})','\1-\2-\3') phone
	           INTO    lc_phone_number
          ...
          ---------------------------------------------------------------------
          -- Change by Srini for Defect# 5127.
          ---------------------------------------------------------------------
          
          lc_phone_number := CASE WHEN ASCII(SUBSTR(lc_phone_number, 1, 1)) BETWEEN 48 AND 57 
                                  THEN SUBSTR(lc_phone_number, 1, 12) 
                                  ELSE NULL END;
                                  
          ---------------------------------------------------------------------
          -- End - Change by Srini for Defect# 5127.
          ---------------------------------------------------------------------

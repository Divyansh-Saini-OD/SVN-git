CREATE OR REPLACE PROCEDURE xx_ar_cbi_sum_subt4 
                                                (
...
END;
 ...
 ELSE
   lv_sql_stmnt9 :='SELECT SUM(TOTAL_AMOUNT) TOTAL_AMOUNT'
                 ||lv_enter
                 ||'FROM xx_ar_cbi_softheaders_hdr_v '
                 ||'WHERE cons_inv_id ='||p_cons_inv_id;
...
BEGIN  
  source_cur := dbms_sql.open_cursor;
  dbms_sql.parse(source_cur, lv_sql_stmnt9, dbms_sql.NATIVE);  

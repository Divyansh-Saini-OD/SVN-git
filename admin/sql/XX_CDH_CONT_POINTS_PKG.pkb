...
/*$Header: ARH2CPSB.pls 120.41.12020000.5 2012/10/19 08:38:48 rgokavar ship $ */
-- +===================================================================+
-- |                  Office Depot - Project Simplify                  |
-- |                                                                   |
-- +===================================================================+
-- | Name  : XX_CDH_CONT_POINTS_PKG                                    |
-- | Description: E0096 Seeded package ARH2CPSB.pls customized  to call|
-- |                    custom validate logic for email address        |
-- |                                                                   |
-- |Change Record:                                                     |
-- |===============                                                    |
-- |Version    Date          Author           Remarks                  |
-- |=======    ==========    =============    ======================== |
-- |DRAFT 1A   12-DEC-2013   Sreedhar Mohan   R12 Reftrofit            |
-- |      1.1  31-AUG-2016   Vasu Raparla     Retrofitted for R12.2.5  |
-- |                                          upgrade                  |
-- +===================================================================+
  ...
  */
  ...
  --   16-NOV-2011    Sudhir Gokavarapu o Bug13363490
  --                                      considering 'https://' also before
  --                                      appending 'http://' to the URL
  --
  --
    ...
    IF (l_web_type = 'HTTP') THEN
      IF (l_url IS NOT NULL) THEN
        ...
        IF (l_http_position = 0) THEN
		    l_http_position := INSTR(l_url,'https://');
			   --Bug13363490
			   IF (l_http_position = 0) THEN
			   ...
			   ELSIF (l_http_position > 1) THEN
				  ...
				  l_url := 'https://'||REPLACE(l_url,'https://','');
			   END IF;
        ELSIF (l_http_position > 1) THEN
          -- remove from other location and append it in front
...
--End of Bug No: 3131865.
    ...
    --Bug13117445
           --Added DQM Sync call
 	     hz_dqm_sync.sync_contact_point(x_contact_point_id, 'C');
    ...
    THEN
      ...
      THEN
        do_unset_primary_by_purpose(l_owner_table_name,
                                     ...
                                     p_contact_point_rec.contact_point_id);
      --Bug#12584582
      ELSIF (p_contact_point_rec.primary_by_purpose = 'N' AND
             NVL(l_primary_by_purpose,'N') = 'Y') THEN
           p_contact_point_rec.primary_by_purpose := 'N';
	  --End of Bug#12584582 changes.
...
--  Bug 4693719 : pass NULL if the secure data is not updated
  ...
  --  18-OCT-2012   Sudhir G            Bug14698590: Removed c_contact_point and raising
  --                                    Events for all Contact Point Ids, which are in update statement.
  PROCEDURE do_unset_prim_contact_point (
    p_owner_table_name                      IN     VARCHAR2,
    p_owner_table_id                        IN     NUMBER,
    p_contact_point_type                    IN     VARCHAR2,
...
*/
	...
	l_contact_point_rec                     contact_point_rec_type;
    l_edi_rec                   edi_rec_type ;
    l_email_rec                 email_rec_type ;
    l_phone_rec                 phone_rec_type ;
    l_telex_rec                 telex_rec_type ;
	...
	TYPE contactpointidtab IS TABLE OF hz_contact_points.contact_point_id%TYPE;
    l_contactpointidtab contactpointidtab;
	I NUMBER;
  ...
  */
	...
	RETURNING
    contact_point_id
    BULK COLLECT INTO l_contactpointidtab;

--Bug14698590
--Direct Update is missing Business event call, added event call.

  I := 1;
	IF l_contactpointidtab.COUNT > 0 AND
	(HZ_UTILITY_V2PUB.G_EXECUTE_API_CALLOUTS in ('Y', 'EVENTS_ENABLED')) THEN
    LOOP

	l_contact_point_rec.contact_point_id := l_contactpointidtab(i);

		-- Debug info.
		IF fnd_log.level_procedure>=fnd_log.g_current_runtime_level THEN
			hz_utility_v2pub.debug(p_message=>'Raising Business Event for Contact Point Id : '||l_contact_point_rec.contact_point_id,
								   p_prefix=>l_debug_prefix,
								   p_msg_level=>fnd_log.level_procedure);
		END IF;
      ...
      IF(HZ_UTILITY_V2PUB.G_EXECUTE_API_CALLOUTS in ('EVENTS_ENABLED', 'BO_EVENTS_ENABLED')) THEN
        ...
        HZ_POPULATE_BOT_PKG.pop_hz_contact_points(
          p_operation     => p_mode,
          p_contact_point_id => l_contactpointidtab(i));
      END IF;

		 IF I = l_contactpointidtab.COUNT THEN
            EXIT;
         END IF;
	i := i + 1;
	END LOOP;
  ...
  --  18-OCT-2012   Sudhir G            Bug14698590: Raising  Events for all Contact Point Ids,
  --                                    which are in update statement.
  --
  --
  ...
  BEGIN
    ...
    */
    ...
    THEN
      ...
      --Bug 5370799
      --Bug 13117445
      --Commented sync_contact_point call as it will be called in do_create_contact_point just after contact_point insertion

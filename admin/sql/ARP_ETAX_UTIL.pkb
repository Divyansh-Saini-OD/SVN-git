create or replace
PACKAGE BODY ARP_ETAX_UTIL AS
/* $Header: AREBTUTB.pls 120.92.12020000.20 2015/07/10 09:50:22 aravipud ship $ */
...
-- |=======    ==========        =============       ========================                |
-- |1.0       28-NOV-2016         Arun Gannarapu     Commented piece of code to fix the      |
-- |                                                 defect 39804 as part of 12.2 upgrade    |
-- +=========================================================================================+
...
/*21111858 Start*/
PROCEDURE set_default_tax_class(p_customer_trx_id IN NUMBER,
...
IS
  ...
  CURSOR line_to_tax_class(p_customer_trx_id NUMBER, p_phase VARCHAR2) IS
     ...
     and     tl.customer_trx_id = p_customer_trx_id
     and     ctt.cust_trx_type_id = t.cust_trx_type_id
     and     NVL(t.previous_customer_trx_id, -99) =
             DECODE(ctt.type ,'CM', nvl(t.previous_customer_trx_id,-99) , -99)
     ...
     and     nvl(tl.tax_classification_code, -99) <> zx.output_tax_classification_code;

BEGIN
  IF PG_DEBUG = 'Y' THEN
        debug( 'arp_etax_util.set_default_tax_class()+' );
  END IF;

      OPEN line_to_tax_class(p_customer_trx_id, p_phase);
...
BEGIN
   ...
   SELECT
      ...
      tax_gt.interface_line_context,
      tax_gt.interface_line_attribute1 ,
      tax_gt.interface_line_attribute2 ,
      tax_gt.interface_line_attribute3 ,
      tax_gt.interface_line_attribute4 ,
      tax_gt.interface_line_attribute5 ,
      tax_gt.interface_line_attribute6 ,
      tax_gt.interface_line_attribute7 ,
      tax_gt.interface_line_attribute8 ,
      tax_gt.interface_line_attribute9 ,
      tax_gt.interface_line_attribute10 ,
      tax_gt.interface_line_attribute11 ,
      tax_gt.interface_line_attribute12 ,
      tax_gt.interface_line_attribute13 ,
      tax_gt.interface_line_attribute14,
      tax_gt.interface_line_attribute15
   FROM   ZX_LINES               zxt,
          RA_CUSTOMER_TRX_LINES  plin,
          ZX_LINES               inv_zxt,
          RA_CUSTOMER_TRX_LINES  inv_lin,
          AR_TAX_LINE_ATTRIBUTES_GT tax_gt
   WHERE
   ...
   AND    zxt.tax_line_id = tax_gt.tax_line_id(+)
   and    zxt.trx_line_id = tax_gt.trx_line_id(+)
   and    nvl(zxt.manually_entered_flag,'N') = tax_gt.manually_entered_flag(+);
   ...
   /* default the tax classification code from zx * //*21111858 Start*/
   set_default_tax_class(p_customer_trx_id);
   /*21111858 end*/
...
insert into AR_TAX_LINE_ATTRIBUTES_GT (
Customer_trx_id ,
tax_line_number ,
tax_line_id,
trx_line_id ,
...
zxt.trx_line_id,
ctl_tax.request_id ,
zxt.manually_entered_flag,
ctl_tax.interface_line_context,
ctl_tax.interface_line_attribute1 ,
ctl_tax.interface_line_attribute2 ,
ctl_tax.interface_line_attribute3 ,
ctl_tax.interface_line_attribute4 ,
ctl_tax.interface_line_attribute5 ,
ctl_tax.interface_line_attribute6 ,
ctl_tax.interface_line_attribute7 ,
ctl_tax.interface_line_attribute8 ,
ctl_tax.interface_line_attribute9 ,
ctl_tax.interface_line_attribute10 ,
ctl_tax.interface_line_attribute11 ,
ctl_tax.interface_line_attribute12 ,
ctl_tax.interface_line_attribute13 ,
ctl_tax.interface_line_attribute14,
ctl_tax.interface_line_attribute15
from
ZX_LINES               zxt,
RA_CUSTOMER_TRX_LINES  ctl,
RA_CUSTOMER_TRX_LINES  ctl_tax
WHERE   zxt.application_id = 222
AND  zxt.manually_entered_flag = 'Y'
AND  ctl.request_id is not null
AND  zxt.entity_code = 'TRANSACTIONS'
AND  zxt.event_class_code in ('INVOICE','DEBIT_MEMO','CREDIT_MEMO')
AND  zxt.trx_id = ctl.customer_trx_id
AND  zxt.trx_level_type = 'LINE'
AND  zxt.trx_line_id = ctl.customer_trx_line_id
AND  ctl.customer_trx_line_id = ctl_tax.link_to_cust_trx_line_id
AND  ctl_tax.tax_line_id = zxt.tax_line_id
AND  ctl_tax.line_type = 'TAX'
AND  ctl_tax.customer_trx_id = ctl.customer_trx_id
AND  ctl.customer_trx_id =p_customer_trx_id;

   IF PG_DEBUG in ('Y', 'C') THEN
      l_rows := SQL%ROWCOUNT;
      debug(' Number of tax rows inserted into gt = ' || l_rows);
   END IF;
...
IS
  ...
  CURSOR line_to_tax_class(p_request_id NUMBER, p_phase VARCHAR2) IS
     ...
     and     NVL(t.previous_customer_trx_id, -99) =
             DECODE(p_phase, 'INV', -99,'DM',-99, NVL(t.previous_customer_trx_id,-99))/*21417336*/
     and     zx.application_id = 222
     and     zx.entity_code = 'TRANSACTIONS'
     and     zx.event_class_code in ('INVOICE','DEBIT_MEMO','CREDIT_MEMO')
     and     zx.trx_id = tl.customer_trx_id
...
/* Added parameter to hold values properly*/
   ...
   /* 21213472:  */
   t_prorated_amt_line_index number_table_type;
           ...
           l_prorated_tax := c_tax_lines.tax_amt;
                   ...
                   IF t_prorated_amt(c_tax_lines.adjusted_doc_line_id) <> 0 or l_prorated_tax <> 0 THEN
                        l_prorated_line := t_prorated_amt(c_tax_lines.adjusted_doc_line_id)
                                    - l_prorated_tax;
           /* 21213472 : t_prorated_amt_line_index which holds the ids of the c_tax_lines.adjusted_doc_line_id
              which were processed and have tax. */

           t_prorated_amt_line_index(c_tax_lines.adjusted_doc_line_id) := t_prorated_amt(c_tax_lines.adjusted_doc_line_id);
...
/* 21213472: Flow quot mode 'N' For lines for which tax is not prorated adjust the tax amount of those line
             accordingly for prorate. Same fix is carried in quot mode Y which is unused code as of now  */
			 
			-- Commented below code by AG per defect 39804 
        /* FOR j in t_prorated_amt.first..t_prorated_amt.last
        LOOP
          IF  t_prorated_amt_line_index.exists(j) THEN  -- Now add the recoverable amount for line to p_prorated_line 
            null;
          ELSE
            l_prorated_tax := 0;
            l_prorated_line := t_prorated_amt(j);
            IF p_mode in ('APP_ED', 'APP_UED') THEN
              -- for receipt applications, we switched the sign
              --  at the beginning.. now we have to switch it back 
              l_prorated_tax := l_prorated_tax * -1;
              l_prorated_line := l_prorated_line * -1;
          ...
          END IF;
        END LOOP;  */
          -- Comment end by AG per defect 39804

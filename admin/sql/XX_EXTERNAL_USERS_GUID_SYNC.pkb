...
CREATE OR REPLACE PACKAGE BODY XX_EXTERNAL_USERS_GUID_SYNC
-- +===================================================================+
-- |                  Office Depot - Project Simplify                  |
-- |                                                                   |
-- +===================================================================+
-- | Name        :  XX_EXTERNAL_USERS_GUID_SYNC.pkb                           |
...
AS
  ...
  BEGIN
      XX_COM_ERROR_LOG_PUB.log_error
        ...
        ,p_attribute15             => 'XX_EXTERNAL_USERS_GUID_SYNC'          --------index exists on attribute15
        ,p_program_id              => 0                    
  ...
  begin
    ...
    $if $$enable_debug
    $then
      log('XX_EXTERNAL_USERS_GUID_SYNC.sync_guid(+)');
    $end
    ...
    loop
      ...
      $if $$enable_debug
      $then
        log('After call to FND_LDAP_USER.LINK_USER, X_USER_GUID: ' || X_USER_GUID || ', X_RESULT: ' || X_RESULT);
        log('Before call to FND_USER_PKG.UPDATEUSER');
      $end      
  ...
  exception
    when others then
      log('Exception in XX_EXTERNAL_USERS_GUID_SYNC.sync_guid: ' || SQLERRM);

...
BEGIN
  ...
  LOOP
	...
	ln_count              := ln_count+1;
    --FND_FILE.PUT_LINE(FND_FILE.LOG,'lc_dts_service_url: '||lc_dts_service_url);
    BEGIN
	
		BEGIN
		  --Setting of http headers 
		  --FND_FILE.PUT_LINE(FND_FILE.LOG,'Current system time is - setting http headers '|| TO_CHAR(SYSDATE,'DD-MON-YYYY HH24:MI:SS'));
		  ...
		  COMMIT;
		  --FND_FILE.PUT_LINE(FND_FILE.LOG,'Response : '||lc_response.status_code);
		EXCEPTION
		WHEN UTL_HTTP.BAD_URL THEN
		...
		WHEN OTHERS THEN
		  UTL_HTTP.END_RESPONSE(lc_response);
		  FND_FILE.PUT_LINE(FND_FILE.LOG, 'Error while setting the DTS http request new tst : '||SQLERRM);
		  lc_user_exception_msg := 'Other Error:'||UTL_HTTP.GET_DETAILED_SQLCODE || UTL_HTTP.GET_DETAILED_SQLERRM;
		  RAISE le_dts_srvc_url_error;
		END;
		--FND_FILE.PUT_LINE(FND_FILE.LOG,'Current system time is -response time '|| TO_CHAR(SYSDATE,'DD-MON-YYYY HH24:MI:SS'));
		IF lc_response.status_code = 200 THEN 
		  BEGIN
			...
			WHEN OTHERS THEN
			  ...
			  RAISE le_insert_error;
			END;
			--IF lc_error_message IS NULL AND lc_pod_response IS NOT NULL THEN
			IF lc_order_status IN ('50', '51', '52', '07' ) THEN
			  
			  IF lc_pod_response IS NOT NULL THEN
	...
	WHEN le_dts_srvc_url_error THEN
	 populate_pod_errors( NULL, lc_response.status_code, UTL_HTTP.GET_DETAILED_SQLCODE || UTL_HTTP.GET_DETAILED_SQLERRM, lc_user_exception_msg , fetch_elig_trans_rec.order_number , fetch_elig_trans_rec.customer_trx_id , fetch_elig_trans_rec.account_name, 'c66f342d-98da' , ln_conc_request_id, ln_user_id ); 
    END;
  END LOOP;
  ...
  FUNCTION GET_POD_MSG (p_cust_account_id IN NUMBER , p_customer_trx_id IN NUMBER , p_cust_doc_id IN NUMBER )   
	...
	Begin	
	 ...
	 ln_pay_doc      := 0; 
		...
		OR Shipping_Event_Status IS NOT NULL
    ...
    IF ln_pod_cnt >= 1 AND ln_pod_tab_cnt = 0 AND ln_pay_doc = 1 THEN 
	    lc_pod_blurb_msg:= 'The order has been shipped but not been delivered yet, please refer to order history for POD.';
    ELSE 

...
CREATE OR REPLACE PACKAGE BODY XX_AP_VENDOR_PORTAL_PKG
IS
  -- +=====================================================================================================+
  -- |                              Office Depot                                                           |
  -- +=====================================================================================================+
  ...
  -- | 6.4      15-Sep-2019  Shanti Sethuraj  Modified query in RTV_LINE_DETAILS, NAIT-105660              |
  -- +=====================================================================================================+
...
IS
  ...
  lc_user_name FND_USER.USER_NAME%TYPE     := FND_GLOBAL.USER_NAME;
BEGIN
  XX_COM_ERROR_LOG_PUB.log_error ( p_return_code => FND_API.G_RET_STS_SUCCESS ,p_msg_count => 1 ,p_application_name => 'XXFIN' ,p_program_type => 'ERROR' ,p_attribute15 => 'XX_AP_VENDOR_PORTAL_PKG' ,p_attribute16 => g_proc ,p_program_id => 0 ,p_module_name => 'AP' ,p_error_message => p_error_msg ,p_error_message_severity => 'MAJOR' ,p_error_status => 'ACTIVE' ,p_created_by => ln_user_id ,p_last_updated_by => ln_user_id ,p_last_update_login => ln_login );
...
-- |                   p_po_number                                     |
-- +===================================================================+
PROCEDURE invoice_payment_status(
  ...
  IS
    ...
    SELECT xx_ap_inv_pymt_status_rec_type((
      CASE
        ...
        THEN 'CAN'
        ELSE 'USA'
      END), NVL(pvsa.attribute9,NVL(pvsa.vendor_site_code_alt,pvsa.vendor_site_id)) , ai.invoice_num, ai.invoice_date, ai.amount_applicable_to_discount, (ai.amount_applicable_to_discount-ai.invoice_amount) , ai.discount_amount_taken, (ai.amount_applicable_to_discount-ai.discount_amount_taken -(ai.amount_applicable_to_discount-ai.invoice_amount)) , apsa.due_date, NVL(aca.check_number,0) , NVL(aca.amount,0) , aca.check_date,
      ...
      ai.attribute11
      ||'-'
      ||LPAD(XX_AP_VENDOR_PORTAL_PKG.get_po_loc(NVL(ai.po_header_id,ai.quick_po_header_id)),4,0) , ai.voucher_num)
    ...
    OR ai.invoice_num LIKE p_invoice_number)
    ...
    SELECT 1
    FROM dual
    WHERE (ai.po_header_id   IS NULL
    OR ai.quick_po_header_id IS NULL)
    ...
    SELECT xx_ap_inv_pymt_status_rec_type((
      CASE
        ...
        THEN 'CAN'
        ELSE 'USA'
      END), NVL(pvsa.attribute9,NVL(pvsa.vendor_site_code_alt,pvsa.vendor_site_id)) , ai.invoice_num, ai.invoice_date, ai.amount_applicable_to_discount, (ai.amount_applicable_to_discount-ai.invoice_amount), ai.discount_amount_taken, (ai.amount_applicable_to_discount-ai.discount_amount_taken -(ai.amount_applicable_to_discount-ai.invoice_amount)), apsa.due_date, 0 , 0 , --NVL(aca.amount,0),
      NULL ,                                                                                                                                                                                                                                                                                                                                                                       --aca.check_date,
      -- SUBSTR(ai.description,9),
      LPAD(XX_AP_VENDOR_PORTAL_PKG.get_po_loc(NVL(ai.po_header_id,ai.quick_po_header_id)),4,0) ,
      -- get_po_number(NVL(ai.po_header_id,ai.quick_po_header_id)),
    ...
    OR xev.event_type_code      = 'DEBIT MEMO VALIDATED'
    OR xev.event_type_code      = 'CREDIT MEMO VALIDATED')
...
IS
  CURSOR check_info_cur
  IS
    --Below query added to display only check information at header level for NAIT-49748 by Ragni Gupta, 12-Jul-18
    SELECT xx_chk_inf_inquiry_rec_type((
      CASE
        WHEN aca.org_id = 403
        THEN 'CAN'
        ELSE 'USA'
      END), NVL(pvsa.attribute9,NVL(pvsa.vendor_site_code_alt,pvsa.vendor_site_id)), NVL(aca.check_number,0), aca.check_date, aca.status_lookup_code, NVL(aca.amount,0), NULL,-- ai.discount_amount_taken,
      NULL,                                                                                                                                                                   -- ai.invoice_num,
      NULL,                                                                                                                                                                   --ai. amount,
      aca.vendor_name, aca.address_line1, aca.address_line2, aca.city, aca.state, aca.zip)
...
IS
  CURSOR check_details_cur
  IS
    SELECT xx_ap_check_details_rec_type(ai.invoice_num, ai.invoice_date, ai.amount_applicable_to_discount, (ai.amount_applicable_to_discount-ai.invoice_amount), ai.discount_amount_taken, (ai.amount_applicable_to_discount-ai.discount_amount_taken-(ai.amount_applicable_to_discount-ai.invoice_amount)), ai.description )
    FROM ap_invoices_all ai ,
      ap_supplier_sites_all pvsa ,
      ap_payment_schedules_all apsa ,
      ap_invoice_payments_all aipa ,
...
IS
  CURSOR chargeback_details_cur
  IS
    SELECT XX_AP_CHRGBK_DETAILS_REC_TYPE(
      CASE
        WHEN aildm.description LIKE 'QTY%'
        THEN 'SH-SHORTAGE'
        WHEN aildm.description LIKE 'Price%'
        THEN 'PO-PRICING'
        ELSE NULL
      END,-- chargeback_type,
...
IS
  ...
  IS
    SELECT XX_AP_RTV_DTL_REC_TYPE('USA', vendor_num, rtv_number, creation_date, freight_bill_num1
      ||freight_bill_num2
      ||freight_bill_num3
      || freight_bill_num4
      ||freight_bill_num5
      ...
      ||freight_bill_num8
      ||freight_bill_num9
      || freight_bill_num10, location, carrier_name)
    ...
    AND ((p_freight_bill_number IS NULL)
    OR (freight_bill_num1        = p_freight_bill_number
    OR freight_bill_num2         = p_freight_bill_number
    OR freight_bill_num3         = p_freight_bill_number
    OR freight_bill_num4         = p_freight_bill_number
    OR freight_bill_num5         = p_freight_bill_number
    OR freight_bill_num6         = p_freight_bill_number
    OR freight_bill_num7         = p_freight_bill_number
    OR freight_bill_num8         = p_freight_bill_number
    OR freight_bill_num9         = p_freight_bill_number
    OR freight_bill_num10        = p_freight_bill_number));
  indx          NUMBER;
  f_indx        NUMBER;
  lc_debug_flag VARCHAR2(1);
...
IS
  CURSOR rtv_details_cur
  IS
    SELECT XX_AP_RTV_DETAILS_REC_TYPE(xrh.location, xrl.sku, xrl.vendor_product_code, xrl.item_description, xrl.qty, xrl.cost, xrl.line_amount, NVL(xrl.adjusted_qty,'0'), NVL(xrl.adjusted_cost,'0.00'), NVL(xrl.adjusted_line_amount,'0.00'), xrl.worksheet_num, xrl.rga_number, xrh.carrier_name, xrh.freight_bill_num1, xrh.freight_bill_num2, xrh.freight_bill_num3, xrh.freight_bill_num4, xrh.freight_bill_num5)
    FROM xx_ap_rtv_hdr_attr xrh,
      xx_ap_rtv_lines_attr xrl
    WHERE xrh.invoice_num = p_document_number
    AND xrh.rtv_number    =xrl.rtv_number -- Added by Shanti for Jira NAIT-105660
...
-- |                   p_po_number                                     |
-- +===================================================================+
PROCEDURE inv_pay_status_mul_vend(
    p_country            IN VARCHAR2,
...
IS
  ...
  -- |                   p_check_number                                  |
  -- +===================================================================+
PROCEDURE check_info_inquiry_mul_vend(
    p_country            IN VARCHAR2,
    p_vendor_number_list IN STRINGS_ARRAY,
    p_invoice_number     IN VARCHAR2,
    p_check_date_from    IN VARCHAR2,

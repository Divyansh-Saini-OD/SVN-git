...
CREATE OR REPLACE PACKAGE BODY XX_CDH_CUSTOMER_MASTER_BO_PUB
-- +=========================================================================================+
-- |                  Office Depot                                                           |
-- +=========================================================================================+
...
BEGIN
  ...
  g_bpel_process_id := p_bpel_process_id;
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  ...
  from   dual;
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(l_bo_process_id, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.raise_wf_business_event(+)');
  ...
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(-)XX_CDH_CUSTOMER_BO_WRAP_PUB.raise_wf_business_event(-)');
...
EXCEPTION
    WHEN OTHERS THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_package_name           =>  'XX_CDH_CUSTOMER_MASTER_BO_PUB'            
          ...
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.raise_wf_business_event '  || SQLERRM      
...
BEGIN
  ...
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.create_org_cust_bo(+)');
  ...
  */
  ...
  IF((l_account_objs IS NOT NULL) AND
   ...
   XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(=)XX_CDH_CUSTOMER_BO_WRAP_PUB.create_org_cust_bo, After save_accounts, x_cust_acct_id: ' || x_cust_acct_id);   
    IF l_return_status <> FND_API.G_RET_STS_SUCCESS THEN
      RAISE fnd_api.g_exc_error;
    END IF;
  END IF;
    ...
    LOOP
      ...
      IF l_return_status <> FND_API.G_RET_STS_SUCCESS THEN
        ...
        XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
            ...
            , p_orig_system            =>  null
            , p_orig_system_reference  =>  null
            , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.create_org_cust_bo while calling process_account_record'  || x_errbuf || ', ' || SQLERRM      
            , p_oracle_error_code      =>  SQLCODE    
            , p_oracle_error_msg       =>  SQLERRM 
        );
      END IF;	
  ...
  --Call commit process
  do_commit(l_account_objs(1).attribute18, l_create_update_flag);
...
EXCEPTION
    WHEN fnd_api.g_exc_error THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.create_org_cust_bo '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      );  
      X_RETURN_STATUS := 'E';    
      X_ERRBUF        := 'Error in XX_CDH_CUSTOMER_MASTER_BO_PUB.create_org_cust_bo'     || SQLERRM;                                          
    WHEN OTHERS THEN
      ROLLBACK to create_org_cust_bo;
      --call exception process
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
...
BEGIN
  ...
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.update_org_cust_bo(+)');
  ...
  --Create Party layer objects
  /*
  XX_CDH_PARTY_BO_WRAP_PVT.save_organization(
    p_organization_obj   => l_organization_obj,    
    p_bo_process_id      => g_bo_process_id,    
    p_bpel_process_id    => p_bpel_process_id,    
    p_created_by_module  => p_created_by_module,    
  ...
  */
  ...
  IF((l_account_objs IS NOT NULL) AND
   ...
   XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(=)XX_CDH_CUSTOMER_BO_WRAP_PUB.update_org_cust_bo, After save_accounts, x_cust_acct_id: ' || x_cust_acct_id);   
  ...
  END IF;
  --Call commit process
  do_commit(l_account_objs(1).customer_type, l_create_update_flag);
...
EXCEPTION
    WHEN fnd_api.g_exc_error THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.update_org_cust_bo '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      );  
      X_RETURN_STATUS := 'E';    
      X_ERRBUF        := 'Error in XX_CDH_CUSTOMER_MASTER_BO_PUB.update_org_cust_bo'     || SQLERRM;                                          
    WHEN OTHERS THEN
      ROLLBACK to update_org_cust_bo;
      --call exception process
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
...
BEGIN
  ...
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_account(+), start_time: ' || to_char(sysdate,'DD-MON-YYYY HH24:MI:SS'));
  ...
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(-)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_account(-), complete_time: ' || to_char(sysdate,'DD-MON-YYYY HH24:MI:SS'));
...
EXCEPTION
  WHEN fnd_api.g_exc_error THEN
    ...
    XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
        ...
        , p_orig_system            =>  null
        , p_orig_system_reference  =>  null
        , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_account '  || SQLERRM      
        , p_oracle_error_code      =>  SQLCODE    
        , p_oracle_error_msg       =>  SQLERRM 
    );  
    X_RETURN_STATUS := 'E';    
    X_ERRBUF        := 'Error in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_account'     || SQLERRM;                                          
  WHEN OTHERS THEN
    --call exception process
    XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          p_bo_process_id          =>  G_BO_PROCESS_ID        
...
IS
  x_account_osr     VARCHAR2(255) := null;
  x_account_id      NUMBER := null;
  x_party_id        NUMBER := null;
...
BEGIN
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_direct_customer(+)');

  process_direct_customer(
    p_xx_cdh_customer_bo   => p_xx_cdh_customer_bo,
    p_xx_cdh_ext_objs      => p_xx_cdh_ext_objs   ,
    p_bpel_process_id      => p_bpel_process_id   ,
    p_cust_prof_cls_name   => p_cust_prof_cls_name,
...
EXCEPTION
    WHEN fnd_api.g_exc_error THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_logged_by              =>  FND_GLOBAL.user_id                    
          , p_package_name           =>  'XX_CDH_CUSTOMER_MASTER_BO_PUB'            
          , p_procedure_name         =>  'process_direct_customer'              
          , p_bo_table_name          =>  'HZ_CUST_ACCOUNTS'        
          , p_bo_column_name         =>  'CUST_ACCOUNT_ID'       
          , p_bo_column_value        =>  null       
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_direct_customer '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      );  
      --X_RETURN_STATUS := 'E';    
      --X_ERRBUF        := 'Error in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_direct_customer'     || SQLERRM;                                          
    WHEN OTHERS THEN
      --call exception process
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
            p_bo_process_id          =>  G_BO_PROCESS_ID        
...
IS
  ...
  l_create_update_flag     VARCHAR2(1) := null;
  l_os_owner_table_id      number := 0;  
  l_party_id               NUMBER;
  x_cust_account_id        NUMBER;
  --customer profile id out
...
BEGIN
  ...
  --Call commit process
  do_commit(p_xx_cdh_customer_bo.account_objs(1).customer_type, l_create_update_flag);
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(-)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_direct_customer(-)');
EXCEPTION
    WHEN fnd_api.g_exc_error THEN
...
IS
  ...
  --Copy all nested collection objects from PARTY LAYER into local collection objects
  l_organization_obj                HZ_ORGANIZATION_BO := P_XX_CDH_CUSTOMER_BO.organization_obj;
  l_account_obj                     HZ_CUST_ACCT_BO := P_XX_CDH_CUSTOMER_BO.account_objs(1);
  l_customer_profile_obj            HZ_CUSTOMER_PROFILE_BO;
...
BEGIN
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_contract_customer(+)');
  ...
  /*
  --Create Org Party
  XX_CDH_PARTY_BO_PVT.DO_CREATE_ORGANIZATION_PARTY(
    p_organization_obj  => l_organization_obj,
    p_bo_process_id     => g_bo_process_id,
    p_bpel_process_id   => p_bpel_process_id,
    p_created_by_module => p_created_by_module,
  ...
  --Create or Save Relationships
  --save party sites
  for i in 1..l_party_site_objs.COUNT
  LOOP
    --Create Party Site
    XX_CDH_PARTY_BO_PVT.DO_SAVE_PARTY_SITE_BO(
      p_bo_process_id      => g_bo_process_id,
      p_bpel_process_id    => p_bpel_process_id,
      p_validate_bo_flag   => FND_API.G_FALSE,
      p_party_site_obj     => l_party_site_objs(i),
      p_created_by_module  => p_created_by_module,
      p_obj_source         => NULL,
      p_return_obj_flag    => l_return_obj_flag,
      p_create_update_flag => 'C',
      x_return_status      => x_return_status,
      ...
      px_parent_id         => l_party_id,
      px_parent_os         => l_organization_obj.orig_system,
      px_parent_osr        => l_organization_obj.orig_system_reference,
      px_parent_obj_type   => l_organization_obj.organization_type
    );
  END LOOP;
  
  for j in 1..l_contact_objs.COUNT
  LOOP
  --Create Org Contact
    XX_CDH_PARTY_BO_PVT.DO_SAVE_ORG_CONTACTS(
      p_oc_objs               => l_contact_objs,
      p_bo_process_id         => G_BO_PROCESS_ID,
      p_bpel_process_id       => P_BPEL_PROCESS_ID,
      p_created_by_module     => p_created_by_module,
      p_create_update_flag    => 'C',
      p_organization_id       => l_party_id,
      p_orig_system           => l_organization_obj.orig_system,
      p_orig_system_reference => l_organization_obj.orig_system_reference
    );
  END LOOP;
  --Create Classification
  XX_CDH_PARTY_BO_PVT.save_classifications(
  ...
  --Create Account
  XX_CDH_ACCOUNT_BO_WRAP_PVT.do_create_cust_account (
    p_cust_acct_obj       => l_account_obj,
    p_bo_process_id       => g_bo_process_id,
    p_bpel_process_id     => p_bpel_process_id,
    p_created_by_module   => p_created_by_module,
    x_cust_account_id     => x_cust_account_id,
    x_cust_acct_os        => l_organization_obj.orig_system,
    x_cust_acct_osr       => l_organization_obj.orig_system_reference,
    x_party_id            => l_party_id
  );
  
  x_account_osr := l_account_obj.orig_system_reference;
  ...
  IF((l_account_obj.cust_acct_site_objs IS NOT NULL) AND
     (l_account_obj.cust_acct_site_objs.COUNT > 0)) THEN
    HZ_CUST_ACCT_SITE_BO_PVT.save_cust_acct_sites(
      p_cas_objs           => l_account_obj.cust_acct_site_objs,
      p_create_update_flag => 'C',
      p_obj_source         => null,
      x_return_status      => l_return_status,
      x_msg_count          => l_msg_count,
      x_msg_data           => l_msg_data,
      p_parent_acct_id     => x_cust_account_id,
      p_parent_acct_os     => l_account_obj.orig_system,
      p_parent_acct_osr    => l_account_obj.orig_system_reference
    );
  END IF;
  
  --Create Account Profile (with CREDIT CARD)
    ...
    l_customer_profile_obj := HZ_CUSTOMER_PROFILE_BO.create_object(
      p_status                  => 'A',
      p_attribute3              => p_ab_flag,
      p_attribute4              => p_reactivated_flag
    );
    
    XX_CDH_ACCOUNT_BO_WRAP_PVT.create_cust_profile(
      p_cp_obj                  => l_customer_profile_obj,
      p_ca_id                   => x_cust_account_id,
      p_casu_id                 => null,
      x_cp_id                   => x_cp_id,
      p_acct_osr                => l_account_obj.ORIG_SYSTEM_REFERENCE,
      p_cust_prof_cls_name      => p_cust_prof_cls_name,
      p_ab_flag                 => p_ab_flag,         
      p_reactivated_flag        => p_reactivated_flag,
      p_cust_type               => l_account_obj.attribute18,
      p_bo_process_id           => g_bo_process_id,
      p_bpel_process_id         => p_bpel_process_id,
      p_created_by_module       => p_created_by_module,
      x_return_status           => l_return_status, 
  ...
  --create Account Extensibles
  
  --Call commit process
  do_commit(l_account_obj.customer_type, l_create_update_flag);
  */
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(-)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_contract_customer(-)');
EXCEPTION
    WHEN fnd_api.g_exc_error THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_logged_by              =>  FND_GLOBAL.user_id                    
          , p_package_name           =>  'XX_CDH_CUSTOMER_MASTER_BO_PUB'            
          , p_procedure_name         =>  'process_contract_customer'              
          , p_bo_table_name          =>  'HZ_CUST_ACCOUNTS'        
          , p_bo_column_name         =>  'CUST_ACCOUNT_ID'       
          , p_bo_column_value        =>  null       
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_contract_customer '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      );  
      X_RETURN_STATUS := 'E';    
      X_ERRBUF        := 'Error in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_contract_customer'     || SQLERRM;                                          
    WHEN OTHERS THEN
      --call exception process
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
            p_bo_process_id          =>  G_BO_PROCESS_ID        
...
BEGIN
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.process_customer_data(+)');
...
EXCEPTION
    WHEN OTHERS THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.process_customer_data '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      ); 
...
BEGIN
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(+)XX_CDH_CUSTOMER_BO_WRAP_PUB.sync_customer(+)');

  l_hz_org_cust_bo_payload     := XMLTYPE(p_xx_cdh_customer_bo);
  l_xx_cdh_acct_ext_bo_payload := XMLTYPE(p_xx_cdh_acct_ext_bo);

  --Dump the payload into database
  ...
  commit;
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(=)XX_CDH_CUSTOMER_BO_WRAP_PUB.sync_customer, x_errbuf: ' || x_errbuf);
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(=)XX_CDH_CUSTOMER_BO_WRAP_PUB.sync_customer, x_retcode: ' || x_retcode);
  --debug msg
  XX_CDH_CUST_UTIL_BO_PVT.log_msg(G_BO_PROCESS_ID, '(-)XX_CDH_CUSTOMER_BO_WRAP_PUB.sync_customer(-)');
...
EXCEPTION
    WHEN OTHERS THEN
      ...
      XX_CDH_CUST_UTIL_BO_PVT.LOG_EXCEPTION (
          ...
          , p_orig_system            =>  null
          , p_orig_system_reference  =>  null
          , p_exception_log          =>  'Exception in XX_CDH_CUSTOMER_MASTER_BO_PUB.sync_customer '  || SQLERRM      
          , p_oracle_error_code      =>  SQLCODE    
          , p_oracle_error_msg       =>  SQLERRM 
      ); 
END sync_customer;   

...
-- |                                       selection API.              |
-- | 1.2     08-Jan-2008  Bibiana Penski   Modified API Signature      |
-- |                                       using new parameter record. |
-- +===================================================================+
...
BEGIN
 --Check if Country code is NULL
  ...
  IF p_plselection_rec.country_code IS NULL then
      ...
      lc_entity_ref    := 'COUNTRY_CODE-'||p_plselection_rec.country_code;
...
BEGIN
 ...
 IF p_plselection_rec.zip_code IS NOT NULL AND p_plselection_rec.country_code IS NOT NULL THEN
     ...
     --NA operation may send zip code like '99999-1234' for US. Canada will not have '-' in the zipcode so it will not be affected.Need to truncate first 5 characters.
      lc_zip_code := p_plselection_rec.zip_code;
        ...
        -- dbms_output.put_line ('In zip code');
         SELECT rt_par.zone
           ...
           FROM wsh_regions_tl RT
               ...
               ,wsh_regions R_PAR
          WHERE rt.postal_code_from = p_plselection_rec.zip_code
            AND r.country_code      = p_plselection_rec.country_code
   ...
   ELSIF p_plselection_rec.city IS NOT NULL AND p_plselection_rec.state_code IS NOT NULL
                                           AND p_plselection_rec.country_code IS NOT NULL THEN
       ...
       BEGIN
          SELECT rt_par.zone
            ...
            FROM wsh_regions_tl RT
                ...
                ,wsh_regions_tl RT_PAR
                ,wsh_regions R_PAR
           WHERE r.country_code    = p_plselection_rec.country_code
             AND r.state_code      = p_plselection_rec.state_code
             AND rt.city           = p_plselection_rec.city
             AND r.region_id       = rt.region_id
             AND rt.region_id      = z.region_id
             AND rt_par.region_id  = z.parent_region_id
             AND r_par.attribute5  = 'Y'
       ...
       EXCEPTION
          WHEN NO_DATA_FOUND THEN
            ...
            fnd_message.set_name ('XXOM', 'XX_WSH_INVALID_ZONE');
            lc_err_desc      := fnd_message.get;
            lc_entity_ref    := 'CITY-'||p_plselection_rec.city;
            lc_entity_ref_id := -99999; --p_plselection_rec.city;
            RAISE EX_ZONE_PL_ERR;
   ...
   ELSIF p_plselection_rec.state_code IS NOT NULL AND p_plselection_rec.country_code IS NOT NULL THEN
       ...
       BEGIN
        ...
        EXCEPTION
           WHEN NO_DATA_FOUND THEN
             ...
             fnd_message.set_name ('XXOM', 'XX_WSH_INVALID_ZONE');
             lc_err_desc      := fnd_message.get;
             lc_entity_ref    := 'STATE_CODE-'||p_plselection_rec.state_code;
             lc_entity_ref_id := -99999;
             RAISE EX_ZONE_PL_ERR;
   ...
   ELSIF p_plselection_rec.country_code IS NOT NULL THEN
...
-- |                p_plselection_rec   - Parameters record            |
-- |                                                                   |
-- | Returns      :                                                    |
-- |                x_price_list_rec   -- PL record                    |
-- |                x_ret_code                                         |
-- |                x_err_buf                                          |
-- |                                                                   |
-- +===================================================================+
...
--lc_list_name            qp_list_headers_tl.name%type;
    ...
    IF x_ret_code =FND_API.G_RET_STS_SUCCESS then
        ...
        IF p_web_site_key_rec.order_source ='WWW' THEN
         ...
         lc_price_list_id := ln_web_pricelist_id;
                x_price_list_rec.OD_Price_list_type:= XX_QP_LIST_SELECTION_UTIL_PKG.G_PRICE_LIST_TYPE_WZONE;
 
        ELSE
 
         lc_price_list_id := ln_zone_priceist_id ;
                x_price_list_rec.OD_Price_list_type:= XX_QP_LIST_SELECTION_UTIL_PKG.G_PRICE_LIST_TYPE_CZONE;
 
        END IF;
     dbms_output.put_line ('in main process --lc_price_list_id -'||lc_price_list_id);

 BEGIN
            dbms_output.put_line ('in main process --p_price_date -'||p_plselection_rec.ordered_date);

            SELECT  qlb.list_header_id,
                    qlb.list_type_code
                    --,qlt.name         
            INTO x_price_list_rec.price_list_id
                ,x_price_list_rec.price_list_type
                --,lc_list_name
            FROM --qp_list_headers_tl QLT,
                 qp_list_headers_b QLB
            WHERE 1=1
            --AND qlt.list_header_id    = qlb.list_header_id
            AND qlb.list_header_id    = lc_price_list_id
            ...
            AND nvl(qlb.end_date_active,sysdate+1)> p_plselection_rec.ordered_date
            AND qlb.start_date_active <= p_plselection_rec.ordered_date
            ;

...
CREATE OR REPLACE PACKAGE BODY XX_ITEM_CNVN_PKG
-- +====================================================================================+
...
-- |1.2       18-Jul-2007 Arun Andavar       Sleep time and Max wait time are made as   |
-- |                                          parameters.                               |
-- |1.3       19-Jul-2007 Arun Andavar      1)Flags for validation orgs, master and     |
-- |                                          location attributes are added and logic   |
-- |                                          for their reprocessing is also            |
-- |                                          incorporated.                             |
-- |                                        2)CR-If any of the category fails then the  |
-- |                                           item will not be processed.              |
-- |1.4       20-Jul-2007 Susheel Raina      TL Reviewed                                |
-- |1.5       23-Jul-2007 Fajna K.P          Modified update_batch_id to incorporate    |
-- |                                         Location custom attribute reprocessing     |
-- |1.6       23-Jul-2007 Parvez Siddiqui    TL Reviewed                                |
-- |1.7       27-Jul-2007 Fajna K.P          Modified Update Statements of  Category    |
-- |                                         process flags for Success Records          |
-- |1.8       27-Jul-2007 Parvez Siddiqui    TL Reviewed                                |
-- |                                                                                    |
-- |1.9       31-Jul-2007 Fajna K.P          Modified Insert Statements for  Validation |
-- |                                         Organizations to replace Inventory_item_id |
-- |                                         with Segment1                              |
-- |1.10      31-Jul-2007 Parvez Siddiqui    TL Reviewed                                |
-- |1.11      02-SEP-2007 Paddy Sanjeevi	   Modified to run separate master and loc    |
-- +====================================================================================+
AS
----------------------------
...
G_CHILD_PROGRAM             CONSTANT VARCHAR2(50)                                               :=  'XX_INV_ITEM_CNVN_CHILD';
...
BEGIN
    ...
    SET    XSIM.load_batch_id                  = ln_seq
          ...
          --Added by Arun Andavar in ver 1.3 START
          ,XSIM.validation_orgs_status_flag    = (CASE WHEN validation_orgs_status_flag     IS NULL  THEN 'N' ELSE validation_orgs_status_flag   END)
          ,XSIM.master_item_attr_process_flag  = (CASE WHEN master_item_attr_process_flag   IS NULL  THEN  1  ELSE master_item_attr_process_flag END)
          --Added by Arun Andavar in ver 1.3 END
    ...
    SET     XSIL.load_batch_id              = ln_seq
           ,XSIL.location_process_flag      = (CASE WHEN location_process_flag       IS NULL  OR location_process_flag = 1 THEN 2 ELSE location_process_flag      END)
            --Added by Arun Andavar in ver 1.3 START
           ,XSIL.loc_item_attr_process_flag = (CASE WHEN loc_item_attr_process_flag  IS NULL  THEN 1 ELSE loc_item_attr_process_flag END)
           --Added by Arun Andavar in ver 1.3 END
           WHERE   
--	XSIL.item IN  (SELECT item
--                                   FROM   xx_item_master_stg XSIM
--                                   WHERE  XSIM.load_batch_id  =  ln_seq
--                                   AND    XSIM.process_flag   =  1
--                                 )
--    AND 
    XSIL.load_batch_id          IS NULL
    AND     XSIL.process_flag           = 1
    AND     ROWNUM<=gn_batch_size;

    ln_loc_count := SQL%ROWCOUNT;
    ln_batch_size_count := ln_loc_count;
    gn_record_count     := gn_record_count + ln_batch_size_count;
...
-- | Parameters  :                                                      |
-- +====================================================================+
PROCEDURE update_batch_id( p_master   IN VARCHAR2
				  ,x_errbuf   OUT NOCOPY VARCHAR2
                          ,x_retcode  OUT NOCOPY VARCHAR2
                         )
IS
   ...
   IF p_master='Y' THEN  
    ...
    WHERE  XSIM.process_flag                = 1
      AND (
               ...
               --Added by Arun Andavar in ver 1.3 START
               OR  XSIM.validation_orgs_status_flag IN ('N','P','F')
               OR  XSIM.master_item_attr_process_flag = 6
               --Added by Arun Andavar in ver 1.3 END
               )
         );
...
/*
    ...
    WHERE  XSIM.process_flag                = 1
      AND (
               ...
               OR
               (    XSIM.item_process_flag             = 7
               AND  XSIM.inv_category_process_flag     = 7
               AND  XSIM.odpb_category_process_flag    = 7
               AND  XSIM.po_category_process_flag      = 7
               AND  XSIM.atp_category_process_flag     = 7
               --Added by Arun Andavar in ver 1.3 START
               AND  XSIM.validation_orgs_status_flag   = 'S'
               AND  XSIM.master_item_attr_process_flag = 7
               --Added by Arun Andavar in ver 1.3 END
               AND  EXISTS ( SELECT 1
                             FROM   xx_item_loc_stg XSIL
                             WHERE  (XSIL.location_process_flag   IN (2,3,4,6)
                                     OR
                                     XSIL.loc_item_attr_process_flag = 6
                                    )
...
*/
   ELSE
    ...
    SET    XSIL.load_batch_id           = NULL
           --Added by Fajna K.P in ver 1.5 -START
          ,XSIL.location_process_flag   = (CASE WHEN location_process_flag          <>7    THEN 1 ELSE location_process_flag            END)
           --Added by Fajna K.P in ver 1.5 -END
    ...
    IF lc_return_status = 'S' THEN
        ...
        LOOP
            ...
            ELSE
               SELECT COUNT(1)
                 INTO ln_current_count
                 FROM xx_item_loc_stg XSIM
                WHERE XSIM.load_batch_id IS NULL
                  AND XSIM.process_flag = 1
			AND ROWNUM<gn_batch_size+1;
		END IF;
		...
		IF p_master='Y' THEN
	      ...
	      ELSE
               display_log('No Data Found in the Table XX_ITEM_LOC_STG');
	      END IF;
            x_retcode := 1;
        ELSE
...
/*
       IF p_master='Y' THEN
          SELECT count(1) INTO ln_processed
		FROM xx_item_master_stg
	     WHERE process_Flag=1 AND load_batch_id IS NOT NULL
	       AND item_process_flag=7
		 AND rownum<2;
       ELSE
          SELECT count(1) INTO ln_processed
		FROM xx_item_loc_stg
	     WHERE process_Flag=1 AND load_batch_id IS NOT NULL
	       AND location_process_flag=7
		 AND rownum<2;
	 END IF;
*/
--       IF ln_processed<>1 THEN
                  ...
                  IF gn_inv_structure_id IS NOT NULL THEN
                      ...
                      FETCH lcu_inv_category INTO ln_inv_category_id;
                          IF  lcu_inv_category%NOTFOUND THEN
                              ...
                              --Modified by Arun Andavar in ver 1.3 START
                              lt_inv_categoryid(i):= NULL;
                              --Modified by Arun Andavar in ver 1.3 END
                          ...
                          ELSE
                              ...
                              FETCH lcu_odpb_category INTO ln_odbrand_category_id;
                                  IF  lcu_odpb_category%NOTFOUND THEN
                                      lt_odpb_categorypf(i):= 3;
                                      --Modified by Arun Andavar in ver 1.3 SKR START
                                      lt_odpb_categoryid(i):= NULL;
                                      --Modified by Arun Andavar in ver 1.3 END
                                      --Adding error message to stack
    ...
    BEGIN
        INSERT INTO XX_INV_ITEM_MASTER_ATTRIBUTES
                                        ...
                                        SELECT   XIMS.inventory_item_id
                                                ...
                                                ,XIMS.short_desc
                                                ,XIMS.source_system_code
                                                ,XIMS.source_system_ref
                                                ,XIMS.store_ord_mult
                                                ,SYSDATE
                                                ,g_user_id
                                                ,g_user_id
        ...
        --Added by Arun Andavar in ver 1.3 START
        UPDATE xx_item_master_stg XIMS
    ...
    BEGIN
        INSERT INTO XX_INV_ITEM_ORG_ATTRIBUTES
                                   ...
                                   SELECT   XILS.inventory_item_id
                                           ...
                                           ,XILS.source_system_code
                                           ,XILS.source_system_ref
                                           ,SYSDATE
                                           ,g_user_id
                                           ,g_user_id
                                           ,SYSDATE
                                           ,g_user_id
                                   FROM     xx_item_loc_stg XILS
...
---------------------------------------------------------------------------
CURSOR lcu_val_org_error(p_batch_id IN NUMBER)
IS
SELECT CASE COUNT(MSII.segment1) WHEN 0                THEN 'S'
                                 WHEN gn_val_org_count THEN 'F'
                                 ELSE                       'P'
       END CASE
      ,MSII.segment1
      ,XIMS.ROWID
FROM   mtl_system_items_interface MSII
      ,xx_item_master_stg         XIMS
WHERE  XIMS.load_batch_id               = p_batch_id
AND    XIMS.item_process_flag           IN (4,5,6,7)
AND    XIMS.validation_orgs_status_flag <> 'S'
AND    MSII.set_process_id              = XIMS.load_batch_id
AND    MSII.organization_id IN (  SELECT  HOU.organization_id
                                  FROM    hr_organization_units HOU
                                  WHERE   HOU.type        = 'VAL'
                                  AND     SYSDATE BETWEEN NVL(HOU.date_from,SYSDATE-1)
                                  AND     NVL(HOU.date_to,SYSDATE+1)
                               )
AND   MSII.segment1                 = XIMS.item
...
IF  gc_master_setup_status='Y' THEN
    ...
    IF  gc_valorg_setup_status='Y' THEN
        ...
        LOOP
            BEGIN
                ...
                FROM    xx_item_master_stg XSIM
                WHERE   XSIM.load_batch_id = p_batch_id
                --Modified by Arun Andavar in ver 1.3 -Added '7' START
                AND     XSIM.item_process_flag IN (4,5,6,7)
                --Modified by Arun Andavar in ver 1.3 -Added '7' END
                --Added by Arun Andavar in ver 1.3 START
   ...
   --Added by Arun Andavar in ver 1.3 SKR START
   AND      XSIM.item_process_flag IN (4,5,6,7)
   --Added by Arun Andavar in ver 1.3 START
   ;
   END IF;
    ...
    WHERE  EXISTS (    
                   SELECT 1    
                   --Modified by Fajna K.P in ver 1.7 START
                   FROM   mtl_item_categories MIC     
                         ,mtl_system_items_b MSIB  
                   --Modified by Fajna K.P in ver 1.7 END
    ...
    --Added by Fajna K.P in ver 1.7 START
    AND    XSIM.inv_category_process_flag   IN  (4,5,6)    
    AND    XSIM.load_batch_id               =   p_batch_id    
    --Added by Fajna K.P in ver 1.7 END
   ...
   IF p_master='Y' THEN    
    ...
    ----------------------------------------------------------------------------------
    UPDATE xx_item_master_stg XIMS
    SET    XIMS.validation_orgs_status_flag = 'N'
    WHERE  XIMS.load_batch_id               = p_batch_id
    AND    XIMS.validation_orgs_status_flag IN ('F','P')
    AND    XIMS.item_process_flag           > 3;
    COMMIT;

    OPEN lcu_val_org_error(p_batch_id);
    FETCH lcu_val_org_error BULK COLLECT INTO lt_val_org_status_flag,lt_item_number, lt_row_id;
    CLOSE lcu_val_org_error;
    IF lt_item_number.COUNT > 0 THEN
       FORALL i IN 1 .. lt_item_number.LAST
       UPDATE xx_item_master_stg XIMS
       SET    XIMS.validation_orgs_status_flag = lt_val_org_status_flag(i) -- Posible Values P or F
       WHERE  XIMS.ROWID                       = lt_row_id(i);
    END IF;

    UPDATE xx_item_master_stg XIMS
    SET    XIMS.validation_orgs_status_flag = 'S'
    WHERE  XIMS.load_batch_id               = p_batch_id
    AND    XIMS.validation_orgs_status_flag = 'N'
    AND    XIMS.item_process_flag           > 3;
...
BEGIN
    BEGIN
        ...
        IF  p_validate_only_flag = 'N' THEN
            ...
            FND_FILE.PUT_LINE(FND_FILE.OUTPUT,'BEFORE PROCESS ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            FND_FILE.PUT_LINE(FND_FILE.LOG,'BEFORE PROCESS ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            ...
            FND_FILE.PUT_LINE(FND_FILE.OUTPUT,'AFTER PROCESS ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            FND_FILE.PUT_LINE(FND_FILE.LOG,'AFTER PROCESS ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            ...
            FND_FILE.PUT_LINE(FND_FILE.OUTPUT,'BEFORE INSERT ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            FND_FILE.PUT_LINE(FND_FILE.LOG,'BEFORE INSERT ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            ...
            FND_FILE.PUT_LINE(FND_FILE.OUTPUT,'AFTER INSERT ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
            FND_FILE.PUT_LINE(FND_FILE.LOG,'AFTER INSERT ITEM DATA :'||TO_CHAR(SYSDATE,'DD-MON-RR HH24:MI:SS'));
    ...
    IF ln_request_id IS NOT NULL THEN
	 ...
	 ELSE
        ...
        XX_COM_CONV_ELEMENTS_PKG.upd_control_info_proc(
                                                         ...
                                                         ,p_batch_id                    => p_batch_id
                                                         ,p_conversion_id               => gn_conversion_id
                                                         ,p_num_bus_objs_failed_valid   => ln_locations_invalid
                                                         ,p_num_bus_objs_failed_process => ln_locations_failed
                                                         ,p_num_bus_objs_succ_process   => ln_locations_processed
                                                     );

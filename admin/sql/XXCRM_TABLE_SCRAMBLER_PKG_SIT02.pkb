create or replace PACKAGE BODY XXCRM_TABLE_SCRAMBLER_PKG
-- +===================================================================+
...
----------------------------------------------------------------------
      ...
      lc_exp_account_id     apps.xx_crm_wcelg_cust.cust_account_id%TYPE;
      ...
      IS
         SELECT   a.column_name exportfieldname,
             ...
             FROM all_tab_columns a,
                  apps.xx_fin_translatedefinition b,
                  apps.xx_fin_translatevalues c
...
----------------------------------------------------------------------
      ...
      BEGIN
         ...
         fnd_file.put_line (fnd_file.LOG, ' ');
         lc_destpath :=
              nvl(fnd_profile.VALUE ('XX_CRM_WEBCOLLECT_DEST_PATH'),'/app/ebs/ctgsisit02/xxcrm/ftp/out/webcollect/')
              || lc_file_name;

          lc_archpath :=
                        nvl(fnd_profile.VALUE ('XX_CRM_WEBCOLLECT_ARCH_PATH'),'/app/ebs/ctgsisit02/xxcrm/archive/outbound')              
              ;

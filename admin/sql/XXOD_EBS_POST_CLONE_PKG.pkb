...
--                                 ,p_exception_message  => lc_exception_message);
      ...
      -- V 1.11 --> Added Email , Havish Kasina
		...
		/* Added by Havish Kasina as per Version 1.63 */		
		BEGIN
            -----------------------------------------------------------
            -- AP_NI_22 - Update for OD_PGP_KEYS Translation 
            -----------------------------------------------------------
            lc_identifier          := 'AP_NI_22';
            lc_object_type         := 'Translation';
            lc_object_name         := 'OD_PGP_KEYS';
            lc_action              := 'Update OD_PGP_KEYS';
            ld_start_date_time     := TO_CHAR(SYSDATE, 'MM/DD/YYYY HH24:MI:SS');
            ...
            xx_write_to_log (lc_filehandle,'Start of update for AP_NI_22');
            ...
            UPDATE xx_fin_translatevalues 
             ...
             WHERE 1 = 1
               AND translate_id  = (SELECT translate_id 
                                      FROM xx_fin_translatedefinition 
                                     WHERE translation_name = 'OD_PGP_KEYS'
									   AND enabled_flag     = 'Y'  )
			   AND source_value1 = 'INBOUND_EMAIL_IDS'
			   AND target_value2 = 'Y';

            ln_count := SQL%rowcount;

            xx_write_to_log (lc_filehandle,'No of rows updated for AP_NI_22 is: ' || SQL%rowcount );
            COMMIT;
        ...
        WHEN OTHERS 
		THEN
            xx_write_to_log (lc_filehandle,'Error encountered during AP_NI_22: '||SQLERRM);
            ROLLBACK;
            lc_status              := 'ERROR'; 
            lc_result              := 'Unable to update transalation values';   
            ld_end_date_time       := TO_CHAR(SYSDATE, 'MM/DD/YYYY HH24:MI:SS');
            lc_exception_message   := 'Error encountered during AP_NI_22: '||SQLERRM;

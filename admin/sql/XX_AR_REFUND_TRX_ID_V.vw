...
-- |   9     2010/03/26     Venkatesh B     Modified for the defect#4901                       |
-- +===========================================================================================+
...
( source,
  ...
  last_updated_by )
AS
  (SELECT /*+ FULL(aps) PARALLEL(aps,4) */ 'NON-OM' source,  --Changed for defect#4901
          aps.customer_id,
          aps.cash_receipt_id, 
          aps.customer_trx_id,
          aps.class,
    ...
    WHERE acr.cash_receipt_id = aps.cash_receipt_id
      ...
      /*AND NOT EXISTS                          --Commented for the defect#4901
          (SELECT 1
      ...
      AND aps.status='OP'*/
      AND XX_AR_Get_Customer_ID_Type(aps.customer_id) != 1         --Added for the defect#4901
      AND aps.amount_due_remaining < 0                             --Added for the defect#4901
      AND aps.status||'' ='OP'                                     --Added for the defect#4901
  UNION ALL   
   SELECT /*+ FULL(aps) PARALLEL(aps,4) */ 'NON-OM' source,   --Changed for the defect#4901
        aps.customer_id,
        aps.cash_receipt_id, 
        aps.customer_trx_id,
        aps.class,
  ...
  --AND aps.class = 'CM'  -- Commented for CR 697/698
    ...
    /*AND NOT EXISTS
          (SELECT 1
           ...
           WHERE cust_account_id = aps.customer_id
           AND customer_type = 'I')*/
    AND XX_AR_Get_Customer_ID_Type(aps.customer_id) != 1 --Added for defect#4901
    AND aps.amount_due_remaining < 0 
    AND aps.status='OP');

...
BEGIN
  ...
  IF (i_party_sites) THEN
       /* SYNC_INDEX_REALTIME(l_index_owner || '.hz_stage_party_sites_t1',
        ...
        BEGIN
          ...
          FROM fnd_concurrent_programs cp, fnd_concurrent_requests cr
          WHERE cp.concurrent_program_id=cr.concurrent_program_id 
          AND concurrent_program_name = 'XX_CDH_SYNC_SITE_INDEX'
          AND PHASE_CODE = 'R'
          AND ROWNUM = 1;
  ...
  IF (i_contacts) THEN
      ...
      BEGIN
          ...
          FROM fnd_concurrent_programs cp, fnd_concurrent_requests cr
          WHERE cp.concurrent_program_id=cr.concurrent_program_id 
          AND concurrent_program_name = 'XX_CDH_SYNC_CONTACT_INDEX'
          AND PHASE_CODE = 'R'
          AND ROWNUM = 1;
  ...
  IF (i_contact_points) THEN
      ...
      */
      ...
      BEGIN
          ...
          FROM fnd_concurrent_programs cp, fnd_concurrent_requests cr
          WHERE cp.concurrent_program_id=cr.concurrent_program_id 
          AND concurrent_program_name = 'XX_CDH_SYNC_CNCT_POINT_INDEX'
          AND PHASE_CODE = 'R'
          AND ROWNUM = 1;

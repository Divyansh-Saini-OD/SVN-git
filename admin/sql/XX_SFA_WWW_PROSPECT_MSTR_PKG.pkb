...
-- +=======================================================================+
-- | Name             :XX_SFA_WWW_PROSPECT_MSTR_PKG.pkb                    |
...
create or replace package body XX_SFA_WWW_PROSPECT_MSTR_PKG
AS 
PROCEDURE sfa_prospect_mstr_main 
...
IS
    ...
    lv_process_name         varchar2(32) := 'SFA WWW PROSPECT';
    lc_program_name         VARCHAR2(32);
    lc_stage_name           varchar2(32);
    lc_package_name         varchar2(32) := 'XX_SFA_WWW_PROSPECT_MSTR_PKG';
    ...
    lc_create_ext_attrib    varchar2(1) := 'Y';
...
BEGIN    
    ...
    fnd_file.put_line (fnd_file.log,' ');
    lc_request_set := 'XX_SFA_WWW_PROSPECT_SET';
...
--
    fnd_file.put_line (fnd_file.log,' ');
    Lc_program_name := 'XX_SFA_WWW_PROSPECT';
...
--
   ...
   if lb_success then 
    ...
    ELSE
        fnd_file.put_line(fnd_file.LOG,' ');     
        fnd_file.put_line(fnd_file.LOG,
          'Request Set submitted with request id: ' || to_char(ln_req_id));
        COMMIT;
    END IF;
    --------------------------------------------------------------------
    ...
    Select count(*) into ln_HZ_PARTIES_CNT
     ...
     Where orig_system = 'SX'
       And orig_system_reference 
            in (select lpad(internid,10,'0') || '-00001-SX'
                  from xxcrm.xx_SFA_www_new_prospects)
     ...
     Where orig_system = 'SX'
       And (orig_system_reference 
         ...
         or orig_system_reference 
            in (select lpad(internid,10,'0') || '-00002-SX'
                  from xxcrm.xx_SFA_www_new_prospects) )
    ...
    Select count(*) into ln_hz_contact_points_cnt
     ...
     Where orig_system = 'SX'
       And orig_system_reference 
            in (select lpad(internid,10,'0') || '-PHONE'
                  from xxcrm.xx_SFA_www_new_prospects)
    ...
    if (ln_HZ_PARTy_sites_CNT / 2) <> ln_new_prospects_cnt then 
        fnd_file.put_line (fnd_file.log,
...
END sfa_prospect_mstr_main;

END XX_SFA_WWW_PROSPECT_MSTR_PKG;
/
Show errors
/

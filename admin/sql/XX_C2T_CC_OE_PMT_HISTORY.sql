...
-- +=======================================================================================+
-- | Name        :    XX_C2T_CC_OE_PMT_HISTORY.sql                                         |
-- | Description :    OE Payments - NULL Credit Cards                                      |
-- |                                                                                       |
-- |Change Record:    C0705                                                                |
-- |===============                                                                        |
-- |Version  Date         Author                Remarks                                    |
-- |=======  ===========  ==================    ===========================================|
-- | 1.0     09-30-2015   Havish Kasina         Initial Version                            |
-- | 2.0     10-29-2015   Havish Kasina         Added Schema Name gsi_history to the table |
-- | 3.0     10-30-2015   Havish Kasina         Added SET SERVEROUTPUT ON                  |
-- +=======================================================================================+
SET SERVEROUTPUT ON
DECLARE
...
BEGIN
		...
		------------------------------------------------------------------------
		-- STEP #1 - Drop task XX_C2T_NULL_CC_OE_PMT_HIST                          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #1 - Drop task XX_C2T_NULL_CC_OE_PMT_HIST - '||SYSDATE);   
			
		BEGIN
			gc_error_loc := 'Executing dbms_parallel_execute.drop_task.';
			DBMS_PARALLEL_EXECUTE.DROP_TASK('XX_C2T_NULL_CC_OE_PMT_HIST');
			
			DBMS_OUTPUT.PUT_LINE('XX_C2T_NULL_CC_OE_PMT_HIST was dropped from previous run.');
    EXCEPTION
			WHEN OTHERS 
      THEN 
			   gc_error_loc := 'XX_C2T_NULL_CC_OE_PMT_HIST did not exist.Drop was not required.';
			   DBMS_OUTPUT.PUT_LINE(gc_error_loc);
		END;
			
		------------------------------------------------------------------------
    -- STEP #2 - Create task XX_C2T_NULL_CC_OE_PMT_HIST.                       --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #2 - Create task XX_C2T_NULL_CC_OE_PMT_HIST. - '||SYSDATE); 
			
		gc_error_loc := 'Executing dbms_parallel_execute.create_task.';
		DBMS_PARALLEL_EXECUTE.CREATE_TASK(TASK_NAME => 'XX_C2T_NULL_CC_OE_PMT_HIST');
			   
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_task.');
			
		------------------------------------------------------------------------
		-- STEP #3 - Create Chunks for task XX_C2T_NULL_CC_OE_PMT_HIST.            --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #3 - Create Chunks for task XX_C2T_NULL_CC_OE_PMT_HIST. - '||SYSDATE); 
		...
		gc_chunk_sql := 'SELECT MIN(X.header_id)    MIN_HEADER_ID
	                     ...
	                     FROM (SELECT /*+ full(OE_PMT) parallel(OE_PMT) */ 
				                            OE_PMT.header_id
				                           ,NTILE(128) OVER(ORDER BY OE_PMT.header_id) THREAD_NUM
				                       FROM gsi_history.oe_payments OE_PMT
				                      WHERE payment_type_code =  ''CREDIT_CARD''
				                        AND (attribute3 = ''N'' OR attribute3 IS NULL)
				                        AND (credit_card_number IS NOT NULL OR attribute4 IS NOT NULL)) X
		...
		DBMS_PARALLEL_EXECUTE.CREATE_CHUNKS_BY_SQL('XX_C2T_NULL_CC_OE_PMT_HIST', gc_chunk_sql, FALSE);
			
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_chunks_by_sql.');
			
		------------------------------------------------------------------------
		-- STEP #4 - Run Task XX_C2T_NULL_CC_OE_PMT_HIST.                          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #4 - Run Task XX_C2T_NULL_CC_OE_PMT_HIST. - '||SYSDATE); 
			
    gc_error_loc := 'Generating SQL Statement.';
		gc_sql_stmt  :=  'UPDATE gsi_history.oe_payments 
                       ...
                       WHERE header_id BETWEEN :start_id AND :end_id
                         ...
                         AND (attribute3 = ''N'' OR attribute3 IS NULL)
                         AND (credit_card_number IS NOT NULL OR attribute4 IS NOT NULL)';
			
		gc_error_loc := 'Executing dbms_parallel_execute.run_task using 64 parallel threads';
		DBMS_PARALLEL_EXECUTE.RUN_TASK(task_name => 'XX_C2T_NULL_CC_OE_PMT_HIST',
    ...
    ------------------------------------------------------------------------
    -- STEP #5 - Verify Status of Chunks XX_C2T_NULL_CC_OE_PMT_HIST            --
    ------------------------------------------------------------------------
		gc_error_loc := 'Getting total number of chunks';
		SELECT COUNT(1)
			INTO gc_total_chunks_cnt
			FROM dba_parallel_execute_chunks
		 WHERE task_name = 'XX_C2T_NULL_CC_OE_PMT_HIST';
    ...
    THEN
			 ...
			 DBMS_PARALLEL_EXECUTE.RESUME_TASK ('XX_C2T_NULL_CC_OE_PMT_HIST');
		...
		DBMS_OUTPUT.PUT_LINE('XX_C2T_NULL_CC_OE_PMT_HIST has been completed - '||SYSDATE);

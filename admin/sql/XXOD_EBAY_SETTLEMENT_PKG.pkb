...
AS
  ...
  -- | 1.1         02-Dec-2020  Mayur Palsokar   Modified for NAIT-162646                         |
  -- +============================================================================================+
  -- +============================================================================================+
  -- |  Global variables                                                                          |
  -- +============================================================================================+
...
IS
  ...
  G_CONTENT_TYPE     VARCHAR2(150) := 'application/x-www-form-urlencoded';  
  G_AUTH_URL         VARCHAR2(150) := 'https://api.ebay.com/identity/v1/oauth2/token';
...
WRITE_LOG(GC_DEBUG_FLAG, 'XXOD_GET_ACCESS_TOKEN:', 'Begin');
  ...
  L_REQ      := UTL_HTTP.BEGIN_REQUEST(G_AUTH_URL, 'POST',' HTTP/1.1');
  UTL_HTTP.SET_HEADER(L_REQ, 'Authorization', 'Basic '||L_AUTH_STRING);
  UTL_HTTP.SET_HEADER(L_REQ, 'Content-Type', G_CONTENT_TYPE);
  UTL_HTTP.SET_HEADER(L_REQ, 'Content-Length', LENGTH(LC_CONTENT));
  UTL_HTTP.WRITE_TEXT(L_REQ, LC_CONTENT);
  L_RES := UTL_HTTP.GET_RESPONSE(L_REQ);
...
IS
  L_REQ UTL_HTTP.REQ;
  L_RES UTL_HTTP.RESP;
  L_TRANS_DATE_FROM VARCHAR2(50);
  L_TRANS_DATE_TO VARCHAR2(50);
  L_LOAD_BEGIN_DT VARCHAR2(50);
  L_MAX_CREATION_DATE DATE;
  L_LIMIT            NUMBER       := 200;
  LC_TRANSACTION_EP  VARCHAR2(200);
  LC_TRANSACTION_URL VARCHAR2(200);
  LCLOB_BUFFER CLOB;
  LC_BUFFER CLOB;
  L_TOTAL_TRANS VARCHAR2(10);
  L_NEXT_PAGE   VARCHAR2(200);
  L_LOOP_COUNT  VARCHAR2(10);
  L_LOAD_BEGIN_FLAG VARCHAR2(5):= 'N';
...
BEGIN
 ...
 -- Set load begin date if required
BEGIN 
 SELECT XFTV.TARGET_VALUE20,
    XFTV.TARGET_VALUE21
  INTO L_LOAD_BEGIN_DT,
    L_LOAD_BEGIN_FLAG
  FROM XX_FIN_TRANSLATEDEFINITION XFTD,
    XX_FIN_TRANSLATEVALUES XFTV
  WHERE 1               =1
 ...
 IF L_LOAD_BEGIN_FLAG = 'Y' THEN 
 LC_TRANSACTION_EP  := '/sell/finances/v1/transaction?filter=transactionStatus:{PAYOUT},transactionDate:['||L_LOAD_BEGIN_DT||'T00:00:00.000Z]'||'&'||'limit='||l_limit;
 ELSE 
  SELECT TO_CHAR(SYSDATE,'RRRR-MM-DD'), TO_CHAR(SYSDATE - 1,'RRRR-MM-DD') 
  INTO L_TRANS_DATE_TO, L_TRANS_DATE_FROM 
  FROM DUAL;
  
  LC_TRANSACTION_EP  := '/sell/finances/v1/transaction?filter=transactionStatus:{PAYOUT},transactionDate:['||L_TRANS_DATE_FROM||'T00:00:00.000Z..'||L_TRANS_DATE_TO||'T00:00:00.000Z]'||'&'||'limit='||l_limit;
 END IF;
  ...
  INTO XXOD_EBAY_MPL_TRANS_RES 
     (TRANSACTIONID,
	  ...
	  CREATION_DATE,
	  STATUS)
	  SELECT TRANSACTIONID_1,
      ORDERID_1,
      PAYOUTID_1,
      TRANSACTIONTYPE_1,
      MARKETPLACEFEES_VALUE_1,
      TRANSACTIONDATE_1,
      TRANSACTIONSTATUS_1,
	  SYSDATE,
	  'N'
    FROM DUAL,
      JSON_TABLE ( LCLOB_BUFFER ,'$.transactions[*]' COLUMNS ( TRANSACTIONID_1 VARCHAR2 ( 100 ) PATH '$.transactionId', ORDERID_1 VARCHAR2 ( 100 ) PATH '$.orderId', PAYOUTID_1 VARCHAR2 ( 100 ) PATH '$.payoutId', TRANSACTIONTYPE_1 VARCHAR2 ( 100 ) PATH '$.transactionType', TRANSACTIONDATE_1 VARCHAR2 ( 100 ) PATH '$.transactionDate', NESTED path '$.orderLineItems[*]' COLUMNS ( NESTED path '$.marketplaceFees[*]' COLUMNS( MARKETPLACEFEES_VALUE_1 VARCHAR2 ( 100 ) PATH '$.amount.value' ) ), TRANSACTIONSTATUS_1 VARCHAR2 ( 100 ) PATH '$.transactionStatus'))
   WHERE 1 = 1
   AND NOT EXISTS  (SELECT 1 FROM XXOD_EBAY_MPL_TRANS_RES T
   WHERE T.ORDERID = ORDERID_1);
...
IS
  ...
  LC_ORDERS_EP  VARCHAR2(500);
  LC_ORDERS_URL VARCHAR2(500);
  LCLOB_BUFFER CLOB;
  LC_BUFFER CLOB;
  L_TOTAL_TRANS      VARCHAR2(10);
  L_NEXT_PAGE        VARCHAR2(500);
  L_LOOP_COUNT       VARCHAR2(10);
  L_ORD_DATE_FROM VARCHAR2(50);
  L_ORD_DATE_TO VARCHAR2(50);
  L_LOAD_BEGIN_DT VARCHAR2(50);
  L_MAX_CREATION_DATE VARCHAR2(50);
  L_LOAD_BEGIN_FLAG VARCHAR2(5);
...
BEGIN 
 SELECT XFTV.TARGET_VALUE20,
  ...
  <<rerun_api_call_1>>
  IF L_LOAD_BEGIN_FLAG = 'Y' THEN 
  LC_ORDERS_EP := '/sell/fulfillment/v1/order?filter=orderfulfillmentstatus:%7BFULFILLED%7CIN_PROGRESS%7D,creationdate:%5B'||L_LOAD_BEGIN_DT||'T00:00:00.000Z%5D'||'&'||'limit='||l_limit;
  ELSE 
  SELECT TO_CHAR(SYSDATE,'RRRR-MM-DD'), TO_CHAR(SYSDATE - 1,'RRRR-MM-DD') 
  INTO L_ORD_DATE_TO, L_ORD_DATE_FROM 
  FROM DUAL;
  
  LC_ORDERS_EP := '/sell/fulfillment/v1/order?filter=orderfulfillmentstatus:%7BFULFILLED%7CIN_PROGRESS%7D,creationdate:%5B'||L_ORD_DATE_FROM||'T00:00:00.000Z..'||L_ORD_DATE_TO||'T00:00:00.000Z%5D'||'&'||'limit='||l_limit;
  END IF;
 ...
 -- WHEN UTL_HTTP.TOO_MANY_REQUESTS THEN
  ...
  IF LCLOB_BUFFER LIKE '%errorId%30500%' THEN
  GOTO rerun_api_call_1;
  END IF;

  INSERT INTO XXOD_EBAY_MPL_ORDER_RES(ORDERID,
      LEGACYORDERID,
      PAYSUM_PAYMETHOD,
      ...
      LEGACYORDERID_1,
      PAYSUM_PAYMETHOD_1,
      PAYSUM_PAYMENTSTATUS_1,
      LINEITEMS_SKU_1,
      LINEITEMS_TITLE_1,
      LINEITEMS_LINEITEMCOST_VAL_1,
      LINEITEMS_QUANTITY_1,
      LINEITEMS_DELIVERYCOST_VAL_1,
      LINEITEMS_TAXTYPE_1,
      LINEITEMS_EBAYREMITTAXES_VAL_1,
	  SYSDATE,
	  'N'
    FROM DUAL,
      JSON_TABLE (LCLOB_BUFFER ,'$.orders[*]' COLUMNS ( ORDERID_1 VARCHAR2 ( 100 ) PATH '$.orderId', LEGACYORDERID_1 VARCHAR2 ( 100 ) PATH '$.legacyOrderId', PAYSUM_PAYMETHOD_1 VARCHAR2 ( 100 ) PATH '$.paymentSummary.payments.paymentMethod', PAYSUM_PAYMENTSTATUS_1 VARCHAR2 ( 100 ) PATH '$.paymentSummary.payments.paymentStatus', NESTED path '$.lineItems[*]' COLUMNS ( LINEITEMS_SKU_1 VARCHAR2 ( 100 ) PATH '$.sku', LINEITEMS_TITLE_1 VARCHAR2 ( 100 ) PATH '$.title', LINEITEMS_LINEITEMCOST_VAL_1 VARCHAR2 ( 100 ) PATH '$.lineItemCost.value', LINEITEMS_QUANTITY_1 VARCHAR2 ( 100 ) PATH '$.quantity', LINEITEMS_DELIVERYCOST_VAL_1 VARCHAR2 ( 100 ) PATH '$.deliveryCost.shippingCost.value', LINEITEMS_TAXTYPE_1 VARCHAR2 ( 100 ) PATH '$.ebayCollectAndRemitTaxes.taxType', LINEITEMS_EBAYREMITTAXES_VAL_1 VARCHAR2 ( 100 ) PATH '$.ebayCollectAndRemitTaxes.amount.value' ) ))
WHERE 1 = 1
   AND NOT EXISTS  (SELECT 1 FROM XXOD_EBAY_MPL_ORDER_RES O
   WHERE O.ORDERID = ORDERID_1);
...
IS
  ...
  IS
    ...
    FROM XXOD_EBAY_MPL_TRANS_RES T,
	     XXOD_EBAY_MPL_ORDER_RES O
	WHERE 1               = 1
    AND T.ORDERID = O.ORDERID
	AND T.STATUS = 'N'
	AND O.STATUS = 'N'
	AND T.TRANSACTIONSTATUS = 'PAYOUT'
    AND NOT EXISTS (SELECT 1 FROM XX_CE_MARKETPLACE_PRE_STG
	WHERE  PROCESS_NAME = 'NEW_EBAY_MPL'
	 AND ATTRIBUTE3 = T.ORDERID); 
  ...
  LOOP
      ...
      INTO XX_CE_MARKETPLACE_PRE_STG
        ...
        (
          ...
          I.TRANSACTIONTYPE,
          I.TRANSACTIONDATE,
          I.MARKETPLACEFEES_VALUE,
          I.TRANSACTIONSTATUS,
          I.LEGACYORDERID,
          I.PAYSUM_PAYMETHOD,
          I.PAYSUM_PAYMENTSTATUS,
          I.LINEITEMS_SKU,
          I.LINEITEMS_TITLE,
          I.LINEITEMS_LINEITEMCOST_VALUE,
          I.LINEITEMS_QUANTITY,
          I.LINEITEMS_DELIVERYCOST_VALUE,
          I.LINEITEMS_TAXTYPE,
          I.LINEITEMS_EBAYREMITTAXES_VALUE,
		...
		UPDATE XXOD_EBAY_MPL_TRANS_RES
		SET STATUS = 'P'
		WHERE ORDERID = I.ORDERID;
		
		UPDATE XXOD_EBAY_MPL_ORDER_RES
		SET STATUS = 'P'
		WHERE ORDERID = I.ORDERID;
...
BEGIN
  ...
  --  DBMS_OUTPUT.PUT_LINE(P_LOCATION||': '||P_STATEMENT);
  END IF;
END WRITE_LOG;
...
IS
  ...
  G_FIN_SCOPE        VARCHAR2(150) := 'https://api.ebay.com/oauth/api_scope/sell.finances';
  G_FUL_SCOPE        VARCHAR2(150) := 'https://api.ebay.com/oauth/api_scope/sell.fulfillment';
...
BEGIN
  ...
  LC_ACCESS_TOKEN_FIN := XXOD_GET_ACCESS_TOKEN(G_FIN_SCOPE, LC_AUTH_STRING);
  ...
  LC_ACCESS_TOKEN_FUL := XXOD_GET_ACCESS_TOKEN(G_FUL_SCOPE, LC_AUTH_STRING);

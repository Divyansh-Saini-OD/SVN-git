create or replace 
PACKAGE BODY XX_AR_CBI_RPRN_SUBTOTALS AS
---+========================================================================================================+
...
-- |    2.8             18-APR-2018       Punit Gupta CG     Retrofit OD AR Reprint Summary/                |
...
WHILE (lb_go_fwd)
 LOOP
  IF ln_counter =1 THEN
	...
	lv_prefix :='SELECT /*+ push_pred(oeoh) push_pred(xxomh) */ '||lv_enter||'  '; --  Commented and Changed by Punit on 16-MAY-2018 for Performance Tuning 
  ELSE
    lv_prefix :=lv_enter||' ,';
  END IF;
  lv_sort_idx :=lv_sort_idx +1;
...
,trx.customer_trx_id										customer_trx_id
	  ...
	  ,case when exists (select 1  from xx_oe_order_headers_v ooh -- Commented and Changed by Punit CG on 18-APR-2018 for Defect NAIT-31695
...
from  ra_customer_trx_lines ractl
      ,xx_oe_order_lines_v oeol -- Commented and Changed by Punit CG on 02-APR-2018 for Defect NAIT-31695
      --,oe_order_lines        oeol
...
-- Added the below block for R1.3 CR 733 Defect 1212
       BEGIN
          SELECT -- XOLA.ret_orig_order_num Commented for defect 5846     
		         /*+ opt_param('_optimizer_push_pred_cost_based','false') push_pred(xola) push_pred(oela) push_pred(oeha) */ -- Hint changed by Punit on 17-MAY-2018 for Performance Tuning  
		         --/*+ push_pred(xola) push_pred(oela) push_pred(oeha) */ -- Hint added by Punit on 16-MAY-2018 for Performance Tuning 
...
-- Added the below block for R1.3 CR 733 Defect 1212
       BEGIN
          ...
          FROM  xx_om_line_attributes_v XOLA  -- Commented and Changed by Punit CG on 18-APR-2018 for Defect NAIT-31695 
		        --xx_om_line_attributes_all XOLA
                ,ra_customer_trx_all       RCT
				,xx_oe_order_lines_v OELA -- Commented and Changed by Punit CG on 02-APR-2018 for Defect NAIT-31695
                --,oe_order_lines_all        OELA
				,xx_oe_order_headers_v OEHA -- Commented and Changed by Punit CG on 18-APR-2018 for Defect NAIT-31695
                --,oe_order_headers_all      OEHA-- added for defect 5846
                ,ar_payment_schedules_all  APS

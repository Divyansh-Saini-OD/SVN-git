CREATE OR REPLACE PACKAGE BODY XX_CDH_CREDIT_FLAG_CHK_PKG
AS
-- +===========================================================================+
...
-- |1.1      12-NOV-2015 Havish Kasina Removed the Schema References as per    |
-- |                                   R12.2 Retrofit Changes                  |
-- +===========================================================================+
   PROCEDURE XX_CDH_CREDIT_FLAG_TEMP_LOAD(
...
-- |                         WIPRO/Office Depot                                |
-- +===========================================================================+
-- | Name        : XX_CDH_CREDIT_FLAG_TEMP_LOAD                                 |
-- | Description :                                                             |
...
-- =====================================================================================
    ...
    EXCEPTION
    WHEN OTHERS THEN
    x_retmsg := 'Error - Unexpected Error in XX_CDH_CREDIT_FLAG_CHK_PKG.XX_CDH_CREDIT_FLAG_TEMP_LOAD '||Chr(10)||
                'SQLCODE - ' || SQLCODE || ' SQLERRM - ' || SQLERRM;
               
    FND_FILE.put_line(FND_FILE.log,x_retmsg);
    --lc_status := 'Error';
    x_retcode:=2;
    END XX_CDH_CREDIT_FLAG_TEMP_LOAD;


    PROCEDURE XX_CDH_CREDIT_FLAG_CHK_PROC (
                                          x_retcode   OUT NUMBER,
                                          x_retmsg    OUT VARCHAR2)
-- +===========================================================================+
-- |                  Office Depot - eBilling Project                          |
...
-- +===========================================================================+
  AS
     ...
     CURSOR lcu_relationship_type (ln_party_id IN NUMBER)
     IS
        SELECT NVL((SELECT 'Parent' FROM dual WHERE EXISTS 
                   ( SELECT 1 
                     FROM   hz_relationships HREL 
                      WHERE  HREL.relationship_type = 'OD_FIN_HIER'
                      AND    HREL.subject_id        = ln_party_id
                      AND    HREL.start_date        <= TRUNC(SYSDATE) 
                      AND    HREL.end_date          >= TRUNC(SYSDATE)
                      and    hrel.direction_code = 'P')),'Not Parent') parent_indicator FROM dual;
    ...
    lc_legacy_acc_number        XXCRM_CREDIT_FLAG_TEMP.aops_account_number%TYPE;
    lc_legacy_status            XXCRM_CREDIT_FLAG_TEMP.aops_cust_status%TYPE;
    ...
    lrec_relationship_type      lcu_relationship_type %ROWTYPE; 
    ln_party_id                 NUMBER;
    lc_relationship_type        VARCHAR2(20);
...
-- ===============================================================================================
             SELECT COUNT(1)
             INTO    ln_row_chk
             FROM    XXCRM_CREDIT_FLAG_TEMP XCFT
...
-- ===============================================================================================
             ...
             THEN
                ...
                UPDATE XXCRM_CREDIT_FLAG_TEMP XCFT
...
-- ===============================================================================================
             ...
             OPEN lcu_relationship_type (lt_cust_prof_tab(i).party_id);
               FETCH lcu_relationship_type INTO lrec_relationship_type;
             CLOSE lcu_relationship_type;

/*      BEGIN
        ln_party_id := lt_cust_prof_tab(i).party_id;
        --lrec_relationship_type := NULL;
        lc_relationship_type:='';
        SELECT  DISTINCT CASE WHEN HREL.direction_code = 'P' THEN 'Parent' ELSE 'Not Parent' END direction_code
        INTO    lc_relationship_type
        FROM hz_relationships HREL
        WHERE HREL.relationship_type='OD_FIN_HIER'
        AND HREL.subject_id = ln_party_id
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
            lc_relationship_type := 'Not Parent';
         
         WHEN OTHERS THEN
            lc_relationship_type := 'Error';
...
-- ===============================================================================================
             INSERT
             INTO
             XXCRM_CREDIT_FLAG_TEMP(aops_account_number,aops_cust_type,aops_cust_name,aops_cust_status,process_status,attribute1,process_date)
             ...
             FND_FILE.put_line(FND_FILE.OUTPUT,lt_cust_prof_tab(i).aops_acct_number||','||
                                               ...
                                               '"'||lrec_relationship_type.parent_indicator||'"');
   ...
   EXCEPTION
   WHEN OTHERS THEN
   x_retmsg := 'Error - Unexpected Error in XX_CDH_CREDIT_FLAG_CHK_PKG.XX_CDH_CREDIT_FLAG_CHK_PROC '
               || 'SQLCODE - ' || SQLCODE || ' SQLERRM - ' || SQLERRM;
               
   FND_FILE.put_line(FND_FILE.log,x_retmsg);
   x_retcode := 1;
   END XX_CDH_CREDIT_FLAG_CHK_PROC;
...
-- +===========================================================================+
   AS
       ...
       SELECT      HCP.profile_class_id
                 ...
                 , TERM.name  term_name
                 , NVL((SELECT 'Parent' FROM dual WHERE EXISTS 
                    ( SELECT 1 
                      FROM   hz_relationships HREL 
                      WHERE  HREL.relationship_type = 'OD_FIN_HIER'
                      ...
                      AND    HREL.start_date        <= TRUNC(SYSDATE) 
                      AND    HREL.end_date          >= TRUNC(SYSDATE)
                      and    hrel.direction_code = 'P')),'Not Parent') parent_indicator 
        ...
        AND     HCP.site_use_id IS NULL
        AND     HCA.orig_system_reference = LPAD(lc_aops_account_number,8,0)||'-00001-A0';
...
-- ==================================================================================
        ...
        THEN
          ...
          XX_CDH_CREDIT_FLAG_TEMP_LOAD(lc_full_file_name,x_retcode,x_retmsg);
        ELSE
        BEGIN
...
-- =====================================================================================
          ...
          LOOP
             ...
             LOOP
                INSERT INTO XXCRM_CREDIT_FLAG_TEMP(aops_account_number,aops_cust_type,aops_cust_name,aops_cust_status) 
        ...
        EXCEPTION
        WHEN OTHERS THEN
          x_retmsg := 'Error - Unexpected Error in XX_CDH_CREDIT_FLAG_CHK_PKG.XX_CDH_CREDIT_FLAG_TEMP_LOAD'
               || 'SQLCODE - ' || SQLCODE || ' SQLERRM - ' || SQLERRM;
...
-- ==========================================================================
        ELSE
            ...
            FND_FILE.put_line(FND_FILE.log,x_retmsg);
            BEGIN
            XX_CDH_CREDIT_FLAG_CHK_PROC(ln_retcode,lc_retmsg);
            EXCEPTION
            WHEN OTHERS THEN
...
-- ============================================================================================================
            BEGIN
            FOR lcu_mismatch_update IN (SELECT XCFT.aops_account_number,XCFT.aops_cust_status FROM XXCRM_CREDIT_FLAG_TEMP XCFT WHERE process_status IS NULL)
            LOOP
               ln_credit_lt_usd:=0;
               ln_credit_lt_cad:=0;
            ...
            FND_FILE.put_line(FND_FILE.OUTPUT,lcu_mismatch_update.aops_account_number||','||
                                           ...
                                           '"'||lrec_cust_details.parent_indicator||'"');
            ...
            WHEN OTHERS THEN
             x_retmsg:=('Error in processing the record for : 
                         Oracle Account Number: '||lrec_cust_details.account_number||','||
                        'due to '||SQLCODE||'-'||SQLERRM);
             x_retcode := 1;
             FND_FILE.put_line(FND_FILE.log,x_retmsg);
            END;
...
-- ===================================================================================================
   ...
   EXCEPTION
   WHEN OTHERS THEN
    p_errbuf := 'Error - Unexpected Error in XX_CDH_CREDIT_FLAG_CHK_PKG.XX_CDH_CREDIT_FLAG_CHK_MAIN '
               || 'SQLCODE - ' || SQLCODE || ' SQLERRM - ' || SQLERRM;
               
    FND_FILE.put_line(FND_FILE.log,p_errbuf);
    p_retcode:=2;
   END XX_CDH_CREDIT_FLAG_CHK_MAIN;

...
g_app_id       number := fnd_profile.value('RESP_ID');
g_resp_id      number := fnd_profile.value('RESP_APPL_ID');
...
begin
	 ...
	 ln_item_id                        :=NULL;
	 ln_list_price                     :=NULL;
	 ln_invoice_close_tolerance        :=NULL;
	 ln_receive_close_tolerance        :=NULL;
	 lc_receipt_required_flag          :=NULL;
	 lc_inventory_item_status_code     :=NULL;
     ...
     then
      ...
      debug('master organization item id : '||ln_item_id ||'   '|| 'list price :'||ln_list_price||' ' ||' Invoice Close Tolerance :'||ln_invoice_close_tolerance);
      debug('Receive Close Tolerance : '||ln_receive_close_tolerance ||'   '|| 'Receipt Required Falg :'||lc_receipt_required_flag||' ' ||' Inventory Item status Code :'||lc_inventory_item_status_code);
     ...
     fnd_global.apps_initialize (g_user_id,g_resp_id,g_app_id);
     debug('Step :5 ');
    if item_tbl.count >0
    then
	    debug('Step :6 ');
        ...
        loop
            ...
            l_item_tbl(i).invoice_close_tolerance   := ln_invoice_close_tolerance;
            l_item_tbl(i).receive_close_tolerance   := ln_receive_close_tolerance;
            l_item_tbl(i).receipt_required_flag     := lc_receipt_required_flag;  
            l_item_tbl(i).inventory_item_status_code:= lc_inventory_item_status_code;  
        end loop;
        debug('Step :7 '); 		
        if g_debug_lvl <> g_def_debug
        then

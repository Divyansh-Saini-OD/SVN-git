...
--      END;
 ...
 --NAIT-190252 JIT start
  
  PROCEDURE is_new_ext_user(
      p_fnd_user_name IN VARCHAR2 ,
      x_is_new_ext_user OUT NOCOPY VARCHAR2 ,
      x_new_extuser_rec OUT NOCOPY XX_EXTERNAL_USERS_PVT.external_user_rec_type )
  IS
    l_new_ext_user VARCHAR2(1) := 'N';
    l_ext_user_rec XX_EXTERNAL_USERS_PVT.external_user_rec_type;
    
    CURSOR c_ext_user (p_user_name VARCHAR2)
    IS
      SELECT ROWID ,
        ext_user_id ,
        userid ,
  ...
  BEGIN
    ...
    OPEN c_ext_user(p_fnd_user_name);
    
    FETCH c_ext_user INTO l_ext_user_rec; --what are you doing with this? -park it
    
    IF c_ext_user%NOTFOUND THEN
      l_new_ext_user := 'Y';
    ELSE
      l_new_ext_user := 'N';
    END IF; -- c_ext_user%NOTFOUND
    
    CLOSE c_ext_user;
    
    x_new_extuser_rec := l_ext_user_rec;
    x_is_new_ext_user := l_new_ext_user;
    
  EXCEPTION
  WHEN OTHERS THEN
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(1, 'Error in is_new_ext_user: ' || SQLERRM);
    x_is_new_ext_user := 'N';
  END is_new_ext_user;
  
  PROCEDURE is_new_fnd_user(
      p_fnd_user_name IN VARCHAR2 ,
      x_is_new_fnd_user OUT NOCOPY VARCHAR2 ,
      x_new_fnduser_rec OUT NOCOPY XX_EXTERNAL_USERS_PVT.fnd_user_rec_type )
  IS
    l_new_fnd_user VARCHAR2(1) := 'N';
    l_fnd_user_rec XX_EXTERNAL_USERS_PVT.fnd_user_rec_type;
  ...
  BEGIN
    ...
    OPEN c_fnd_user(p_fnd_user_name);
    FETCH c_fnd_user INTO l_fnd_user_rec; --what are you doing with this rec? -park it
    
    x_new_fnduser_rec := l_fnd_user_rec;
    ...
    x_is_new_fnd_user := l_new_fnd_user;
    
  EXCEPTION
  WHEN OTHERS THEN
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(1, 'Error in is_new_fnd_user: ' || SQLERRM);
    x_is_new_fnd_user := 'N';
  END is_new_fnd_user;
  
  PROCEDURE check_webuser_contact(
  ...
  IS
    ...
    CURSOR c_person (p_person_osr IN VARCHAR2)
    IS
      SELECT party_id
      FROM hz_parties
      WHERE orig_system_reference = p_person_osr
      AND status                  = 'A';
      
    CURSOR c_contact (p_contact_osr IN VARCHAR2)
    IS
      select hoc.ORG_CONTACT_ID, hr.PARTY_ID
      from   hz_org_contacts hoc
            ,hz_relationships hr
      where  1=1
      AND    hoc.orig_system_reference = p_contact_osr
      and    hoc.PARTY_RELATIONSHIP_ID=hr.RELATIONSHIP_ID
      and    hr.subject_type='PERSON'
      AND    hoc.status                = 'A';
      
    CURSOR c_org_party_id (p_AOPS_Customer_ID IN VARCHAR2)
    IS
      SELECT party_id
      FROM hz_cust_accounts
      WHERE orig_system_reference=p_AOPS_Customer_ID
        || '-00001-A0' ;
        
    CURSOR c_org_account_id (p_AOPS_Customer_ID IN VARCHAR2)
    IS
    ...
    CURSOR c_cust_account_roles (p_contact_osr IN VARCHAR2)
    IS
      SELECT CUST_ACCOUNT_ROLE_ID
      FROM hz_cust_account_roles
      WHERE 1                  =1
      AND orig_system_reference=p_contact_osr ;
      
    CURSOR c_first_cont_role (p_cust_account_role_id IN NUMBER)
    IS
      SELECT RESPONSIBILITY_ID
      FROM HZ_ROLE_RESPONSIBILITY
      WHERE 1                 =1
      AND RESPONSIBILITY_TYPE ='FIRST_CONTACT'
      AND CUST_ACCOUNT_ROLE_ID=p_cust_account_role_id ;
      
    CURSOR c_self_serve_cont_role (p_cust_account_role_id IN NUMBER)
    IS
      SELECT RESPONSIBILITY_ID
      FROM HZ_ROLE_RESPONSIBILITY
      WHERE 1                      =1
    ...
    l_person_party_id             NUMBER         :=0;
    l_org_party_id                NUMBER         :=0;
    l_org_account_id              NUMBER         :=0;
    l_new_per_party               VARCHAR2(1)    := 'N';
    lr_person_rec                 hz_party_v2pub.person_rec_type;
    l_party_number                VARCHAR2(15)   := NULL;
    ln_profile_id                 NUMBER         :=0;
    l_return_status               VARCHAR2(1)    := 'N';
    l_msg_count                   NUMBER         := 0;
    l_msg_data                    VARCHAR2(2000) := NULL;
    lc_org_contact_return_status  VARCHAR2(1)    := 'N';
    ln_org_contact_msg_count      NUMBER         := 0;
    lc_org_contact_msg_data       VARCHAR2(2000) := NULL;
    ln_org_contact_id             NUMBER         :=0;
    ln_party_rel_id               NUMBER         :=0;
    ln_relationship_party_id      NUMBER         :=0;
    lc_party_number_org_contact   VARCHAR2(30)   :=NULL;
    l_new_org_contact             VARCHAR2(1)    := 'N';
    l_new_cust_acct_role          VARCHAR2(1)    := 'N';
    l_new_first_cont_role         VARCHAR2(1)    := 'N';
    l_new_self_serve_cont_role    VARCHAR2(1)    := 'N';
    lr_org_contact_rec            HZ_PARTY_CONTACT_V2PUB.org_contact_rec_type :=NULL;
    ln_cust_account_role_id       NUMBER         :=0;
    ln_responsibility_id          NUMBER         :=0;
    lc_cust_acct_role_rtrn_status VARCHAR2(1)    := 'N';
    ln_cust_acct_role_msg_count   NUMBER         :=0;
    lc_cust_acct_role_msg_data    VARCHAR2(2000) := NULL;
    l_cust_account_role_id        NUMBER         :=0;
    ...
    ln_responsibility_id1         NUMBER         := 0;
    ln_RESPONSIBILITY_ID2         NUMBER         := 0;
    lc_role_resp_return_status    VARCHAR2(1)    := 'N';
    ln_role_resp_msg_count        NUMBER         := 0;
    lc_role_resp_msg_data         VARCHAR2(2000) := NULL;
  BEGIN
    OPEN c_person(p_contact_id);
    
    FETCH c_person INTO l_person_party_id;
    
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'in check_webuser_contact    :' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_person_party_id    :' || l_person_party_id );
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_per_party    :' || l_new_per_party );
    
    OPEN c_contact(p_contact_id);
    FETCH c_contact INTO ln_org_contact_id, ln_relationship_party_id;
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_org_contact    :' || l_new_org_contact );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_org_contact_id    :' || ln_org_contact_id );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_relationship_party_id    :' || ln_relationship_party_id );
    
    OPEN c_org_party_id(p_AOPS_Customer_ID);
    FETCH c_org_party_id INTO l_org_party_id;
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_org_party_id    :' || l_org_party_id );
    CLOSE c_org_party_id;
    
    OPEN c_org_account_id(p_AOPS_Customer_ID);
    FETCH c_org_account_id INTO l_org_account_id;
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_org_account_id    :' || l_org_account_id );
    CLOSE c_org_account_id;
    
    IF l_new_per_party                               = 'Y' THEN
      lr_person_rec.created_by_module               := 'XXCONV';
      lr_person_rec.person_first_name               := p_user_first_name;
      lr_person_rec.person_last_name                := p_user_last_name;
      lr_person_rec.party_rec.orig_system           := 'A0';
      lr_person_rec.party_rec.orig_system_reference := p_contact_id;
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'x_person_party_id    :' || x_person_party_id );
    
    IF l_new_org_contact                                   = 'Y' THEN
      lr_org_contact_rec.created_by_module                := 'XXCONV';
      lr_org_contact_rec.orig_system                      := 'A0';
      lr_org_contact_rec.orig_system_reference            := p_contact_id;
      lr_org_contact_rec.party_rel_rec.subject_id         := l_person_party_id;
      lr_org_contact_rec.party_rel_rec.subject_type       := 'PERSON';
      lr_org_contact_rec.party_rel_rec.subject_table_name := 'HZ_PARTIES';
      lr_org_contact_rec.party_rel_rec.object_id          := l_org_party_id;
    ...
    x_org_contact_id        := ln_org_contact_id;
    x_relationship_party_id := ln_relationship_party_id;
    
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'lc_org_contact_return_status    :' || lc_org_contact_return_status );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'x_org_contact_id    :' || x_org_contact_id );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'x_relationship_party_id    :' || x_relationship_party_id );
    
    OPEN c_cust_account_roles(p_contact_id);
    FETCH c_cust_account_roles INTO l_cust_account_role_id;
    
    IF c_cust_account_roles%NOTFOUND THEN
      l_new_cust_acct_role := 'Y';
    ELSE
      l_new_cust_acct_role := 'N';
    END IF; -- c_cust_account_roles%NOTFOUND
    
    CLOSE c_cust_account_roles;

    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'after  c_cust_account_roles    :' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_cust_acct_role    :' || l_new_cust_acct_role );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_cust_account_role_id    :' || l_cust_account_role_id );
    
    
    IF l_new_cust_acct_role = 'Y' THEN
      -- Create the person as CONTACT for the customer site
      lr_cust_account_role_rec.party_id              := ln_relationship_party_id;
      lr_cust_account_role_rec.cust_account_id       := l_org_account_id;
      lr_cust_account_role_rec.orig_system           := 'A0';
      lr_cust_account_role_rec.orig_system_reference := p_contact_id;
      lr_cust_account_role_rec.role_type             := 'CONTACT';
      lr_cust_account_role_rec.created_by_module     := 'XXCONV';
      lr_cust_account_role_rec.primary_flag          := 'N';
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3,'after  create_cust_account_role:' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_cust_account_role_id    :' || l_cust_account_role_id );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'lc_cust_acct_role_rtrn_status    :' || lc_cust_acct_role_rtrn_status );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'lc_cust_acct_role_msg_data    :' || lc_cust_acct_role_msg_data );
    
    OPEN c_first_cont_role(l_cust_account_role_id);
    FETCH c_first_cont_role INTO ln_responsibility_id1;
    
    IF c_first_cont_role%NOTFOUND THEN
      l_new_first_cont_role := 'Y';
    ELSE
      l_new_first_cont_role := 'N';
    END IF; -- c_first_cont_role%NOTFOUND
    
    CLOSE c_first_cont_role;
    
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'after  c_first_cont_role :' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_first_cont_role    :' || l_new_first_cont_role );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_responsibility_id1    :' || ln_responsibility_id1 );
    
    OPEN c_self_serve_cont_role(l_cust_account_role_id);
    
    FETCH c_self_serve_cont_role INTO ln_responsibility_id2;
    
    IF c_self_serve_cont_role%NOTFOUND THEN
      l_new_self_serve_cont_role := 'Y';
    ELSE
      l_new_self_serve_cont_role := 'N';
    END IF; -- c_self_serve_cont_role%NOTFOUND
    
    CLOSE c_self_serve_cont_role;
    
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'after  c_self_serve_cont_role :' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_self_serve_cont_role    :' || l_new_self_serve_cont_role );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_responsibility_id2    :' || ln_responsibility_id2 );
    
    IF l_new_first_cont_role                            = 'Y' THEN
      lr_role_responsibility_rec.cust_account_role_id  := l_cust_account_role_id;
      lr_role_responsibility_rec.responsibility_type   := 'FIRST_CONTACT';
      lr_role_responsibility_rec.created_by_module     := 'XXCONV';
      lr_role_responsibility_rec.primary_flag          := 'N';
      lr_role_responsibility_rec.orig_system_reference := p_contact_id;
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'after FIRST_CONTACT create_role_responsibility :' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'lc_role_resp_return_status    :' || lc_role_resp_return_status );
    
    IF l_new_self_serve_cont_role                       = 'Y' THEN
      lr_role_responsibility_rec.responsibility_type   := 'SELF_SERVICE_USER';
      lr_role_responsibility_rec.cust_account_role_id  := l_cust_account_role_id;
      lr_role_responsibility_rec.created_by_module     := 'XXCONV';
      lr_role_responsibility_rec.primary_flag          := 'N';
      lr_role_responsibility_rec.orig_system_reference := p_contact_id;
      ...
      hz_cust_account_role_v2pub.create_role_responsibility (p_init_msg_list           => 'T'
                                                           ...
                                                           , x_msg_count               => ln_role_resp_msg_count
                                                           , x_msg_data                => lc_role_resp_msg_data );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'after SELF_SERVICE_USER create_role_responsibility :' );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'lc_role_resp_return_status    :' || lc_role_resp_return_status );
    END IF;
    
  EXCEPTION
  WHEN OTHERS THEN
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'Error in CHECK_CONTACT: ' || SQLERRM);
  END check_webuser_contact;
  ...
  IS
    ...
    l_new_ext_user VARCHAR2(1)                                     := 'N';
    l_new_fnd_user VARCHAR2(1)                                     := 'N';
    l_new_extuser_rec XX_EXTERNAL_USERS_PVT.external_user_rec_type := NULL;
    l_cur_extuser_rec XX_EXTERNAL_USERS_PVT.external_user_rec_type := NULL;
    l_fnd_user_rec XX_EXTERNAL_USERS_PVT.fnd_user_rec_type;
    l_return_status          VARCHAR2(1)    := 'N';
    l_msg_count              NUMBER         := 0;
    l_msg_data               VARCHAR2(2000) := NULL;
    ln_org_contact_id        NUMBER         := 0;
    ln_relationship_party_id NUMBER         := 0;
    ln_person_party_id       NUMBER         := 0;
    l_fnd_return_status      VARCHAR2(1)    := 'N';
    l_fnd_msg_count          NUMBER         := 0;
    l_fnd_msg_data           VARCHAR2(2000) := NULL;
    l_org_name               VARCHAR2(2000) := NULL;
    l_new_resp_id            NUMBER         := 0;
    l_new_appl_id            NUMBER         := 0;
    l_new_resp_desc          VARCHAR2(120)  := NULL;
    ln_fnd_user_id           NUMBER         := 0;
    n_rows_updated           NUMBER         :=0;
    LN_RESPONSIBILITY_ID     NUMBER         := 0;
    l_resp_exist             VARCHAR2(1)    := 'N';
    
    CURSOR check_user_resp (p_fnd_user_id IN NUMBER)
    IS
      SELECT RESPONSIBILITY_ID
      FROM fnd_user_resp_groups
      WHERE 1    =1
      AND user_id=p_fnd_user_id
  ...
  BEGIN
    ...
    --$then
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_fnd_user_name  :' || p_fnd_user_name );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_userid         :' || p_userid );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_user_first_name:' || p_user_first_name);
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_user_last_name :' || p_user_last_name );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_access_code    :' || p_access_code );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_email          :' || p_email );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'p_phone_nbr      :' || p_phone_nbr );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'AOPS_Customer_ID :' || AOPS_Customer_ID );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'contact_id       :' || contact_id );
    
    check_webuser_contact( p_contact_id            => contact_id 
                          ,p_AOPS_Customer_ID      => AOPS_Customer_ID 
    ...
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3,'After check_webuser_contact' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_person_party_id    :' || ln_person_party_id );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_org_contact_id    :' || ln_org_contact_id );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_relationship_party_id    :' || ln_relationship_party_id );    
       
    is_new_ext_user( p_fnd_user_name => p_fnd_user_name 
                   , x_is_new_ext_user => l_new_ext_user 
                   , x_new_extuser_rec => l_new_extuser_rec );
                   
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After is_new_ext_user' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_ext_user    :' || l_new_ext_user );
    
    is_new_fnd_user( p_fnd_user_name => p_fnd_user_name 
                   , x_is_new_fnd_user => l_new_fnd_user 
                   , x_new_fnduser_rec => l_fnd_user_rec );
                   
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After is_new_fnd_user' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_fnd_user    :' || l_new_fnd_user );
    
    IF l_new_ext_user    = 'Y' THEN
      l_new_extuser_rec := NULL;
      
      l_new_extuser_rec.fnd_user_name     := p_fnd_user_name;
      l_new_extuser_rec.userid            := p_userid;
      l_new_extuser_rec.password          := '';
      l_new_extuser_rec.person_first_name := p_user_first_name;
      l_new_extuser_rec.person_middle_name:= NULL;
      l_new_extuser_rec.person_last_name  := p_user_last_name;
      l_new_extuser_rec.email             := p_email;
      l_new_extuser_rec.party_id          := ln_relationship_party_id;
      l_new_extuser_rec.status            := 0;
      l_new_extuser_rec.contact_osr       := contact_id;
      l_new_extuser_rec.acct_site_osr     := AOPS_Customer_ID || '-00001-A0'; --at site seq1?
      l_new_extuser_rec.webuser_osr       := SUBSTR(p_fnd_user_name,6,LENGTH(p_fnd_user_name));
      l_new_extuser_rec.access_code       := p_access_code;
      l_new_extuser_rec.permission_flag   := 'M'; --check this??
      l_new_extuser_rec.site_key          := '100100';
    
      save_external_user ( p_site_key        => '100100' 
                         , p_userid          => p_userid 
                         , p_action_type     => l_new_ext_user 
                         , p_new_extuser_rec => l_new_extuser_rec 
      ...
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After save_external_user' );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_return_status    :' || l_return_status );
    END IF;
    
    IF l_new_fnd_user = 'Y' THEN
      ln_fnd_user_id := fnd_user_pkg.CreateUserId (x_user_name                  => p_fnd_user_name
                                                 , x_owner                      => 'CUST'
                                                 , x_unencrypted_password       => 'EXTERNAL1'
                                                 , x_session_number             => userenv('sessionid')
                                                 , x_start_date                 => SYSDATE
                                                 , x_end_date                   => NULL
                                                 , x_last_logon_date            => NULL
                                                 , x_description                => 'FND User for ' || p_email
                                                 ...
                                                 , x_fax                        => NULL
                                                 , x_customer_id                => ln_relationship_party_id
                                                 , x_supplier_id                => NULL
                                                 , x_user_guid                  => NULL
                                                 , x_change_source              => NULL );
    END IF;
    
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After fnd_user_pkg.createuser' );
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'ln_fnd_user_id    :' || ln_fnd_user_id );
    ...
    */
    ...
    IF l_fnd_user_rec.customer_id IS NULL THEN
      UPDATE fnd_user
      SET ENCRYPTED_FOUNDATION_PASSWORD = 'EXTERNAL' ,
        ENCRYPTED_USER_PASSWORD         = 'EXTERNAL' ,
        CUSTOMER_ID                     = ln_relationship_party_id ,
        PERSON_PARTY_ID                 = ln_person_party_id
      WHERE user_name                   =p_fnd_user_name ;
      n_rows_updated                   := SQL%ROWCOUNT;
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After fnd_user update' );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'rows update       :' || n_rows_updated );
    END IF;
    
    OPEN check_user_resp(ln_fnd_user_id);
    FETCH check_user_resp INTO LN_RESPONSIBILITY_ID;
    IF check_user_resp%NOTFOUND THEN
      l_resp_exist := 'N';
    ELSE
      l_resp_exist := 'Y';
    END IF; -- check_user_resp%NOTFOUND
    CLOSE check_user_resp;
    
    -- -----------------------------------------------------------------
    -- Check If user Has access to iRec or iRec with Credit
    -- -----------------------------------------------------------------
    IF l_resp_exist = 'N' THEN
      get_resp_id ( p_orig_system => 'A0' , p_orig_system_access => p_access_code , p_org_name => 'OU_US' , x_resp_id => l_new_resp_id , x_appl_id => l_new_appl_id , x_responsibility_name => l_new_resp_desc , x_return_status => l_return_status , x_msg_count => l_msg_count , x_msg_data => l_msg_data );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After get_resp_id' );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_resp_id    :' || l_new_resp_id );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_appl_id    :' || l_new_appl_id );
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'l_new_resp_desc    :' || l_new_resp_desc );
      
      fnd_user_resp_groups_api.upload_assignment ( user_id                       => ln_fnd_user_id 
                                                 , responsibility_id             => l_new_resp_id 
                                                 , responsibility_application_id => l_new_appl_id 
                                                 , start_date                    => SYSDATE 
                                                 , end_date                      => NULL 
                                                 , description                   => l_new_resp_desc );
                                                 
      XX_EXTERNAL_USERS_DEBUG.log_debug_message(3, 'After upload_assignment' );
  ...
  EXCEPTION
  WHEN OTHERS THEN
    XX_EXTERNAL_USERS_DEBUG.log_debug_message(1, 'Error in provision_fnd_user: ' || SQLERRM);
    --RETURN SQLERRM;
  END provision_fnd_user;
-- NAIT-190252 JIT end

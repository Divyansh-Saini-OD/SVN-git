CREATE OR REPLACE PACKAGE BODY APPS.xx_cdh_omx_dunning_pkg
AS
-- +===========================================================================+
-- |                        Office Depot                                       |
-- +===========================================================================+
...
--|1.3      08-Apr-2015 Abhi K          Updated the webcollect file format name|
--|1.4      05-May-2015 Havish K        Changes done as per Defect # 1239      |
--|1.5      08-Jun-2015 Havish K        MOD5 Changes                           |
-- +===========================================================================+
   ...
   gc_email_file_path      xx_fin_translatevalues.target_value11%TYPE;
   gc_ftp_file_path        xx_fin_translatevalues.target_value2%TYPE;
   ...
   BEGIN
      xx_com_error_log_pub.log_error
                         (p_return_code                 => fnd_api.g_ret_sts_error,
                          ...
                          p_program_name                => 'XX_CDH_OMX_DUNNING_PKG',
                          p_attribute15                 => 'XX_CDH_OMX_DUNNING_PKG',
                          p_program_id                  => NULL,
                          p_module_name                 => 'MOD4A',
...
--------------------------------
   ...
   BEGIN
      log_msg ('Get the AOPS address reference ');
      ...
      SELECT hcas.orig_system_reference
       ...
       WHERE hcas.cust_account_id = hca.cust_account_id
		 ...
		 AND hcas.status = 'A'
         --AND SUBSTR (hps.orig_system_reference,8,INSTR (SUBSTR (hps.orig_system_reference, 8), '-') - 1) = p_omx_bill_consignee  -- Commented as per Defect # 1239  --omx bill_to consignee number
         --AND SUBSTR(hps.orig_system_reference,8,LENGTH(SUBSTR(hps.orig_system_reference,8))-4) = p_omx_bill_consignee -- Added as per Defect # 1239
		 AND SUBSTR(hps.orig_system_reference,8,INSTR(hps.orig_system_reference,'-OMX')-8) = p_omx_bill_consignee -- Added as per Version 1.5, MOD5 Changes
         AND hca.orig_system_reference = LPAD ( (p_aops_cust_number), 8, 0) || '-'|| '00001-A0';

      log_msg (   'AOPS address reference for OMX consignee number'
               || p_omx_bill_consignee
              );
...
----------------------------------------------------------------
      ...
      EXCEPTION
         ...
         THEN
            ...
            log_exception
                       (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.LOG_FILE',
                        p_error_msg           => lc_error_msg
                       );
         WHEN OTHERS
         THEN
      ...
      EXCEPTION
         ...
         THEN
            ...
            log_msg (lc_error_msg);
            log_exception
                     (p_error_location      => 'XXX_CDH_OMX_DUNNING_PKG.LOG_TABLE',
                      p_error_msg           => lc_error_msg
                     );
      END;
   END log_file;
    ...
    /*=====================================================================================+
   | Name       : Update XX_CDH_OMX_DUNNING_STG for each record_id if it is completed |
   |                OR Errored Out.                                                       |
   ...
   EXCEPTION
      ...
      THEN
         ...
         THEN
            p_error_msg :=
               'Error while updating the status UPDATE_DUNNING_STG'
               || SQLERRM;
         END IF;

         log_exception
             (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.UPDATE_DUNNING_STG',
              p_error_msg           => p_error_msg
             );
   END update_dunning_stg;
...
--========================================================================
      xx_ar_wc_utility_pkg.get_interface_settings
                                  (p_process_type           => gc_process_type,
                                   ...
                                   p_delimiter              => gc_email_file_path,
   ...
   BEGIN
      ...
      EXCEPTION
         ...
         THEN
            ...
            log_exception
                      (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.SEND_MAIL',
                       p_error_msg           => lc_error_msg
                      );
         WHEN OTHERS
         THEN
      ...
      THEN
         BEGIN
            SELECT directory_path
              INTO lc_destpath
              FROM all_directories
             WHERE directory_name = gc_email_file_path;
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
               fnd_file.put_line
      ...
      THEN
         ...
         ln_copy_conc_request_id := fnd_request.submit_request 
                                       (application      => 'XXFIN',
...
-- local variable declaration
      ...
      lc_file_name           VARCHAR2 (200)
                                       := 'xxod_omx_dunning_exception_report';
      lc_date                VARCHAR2 (200)
   ...
   BEGIN
      ...
      get_interface_settings (p_error_msg => p_error_msg);
...
-------------------------------------------------------------------------
      lc_file_name :=lc_file_name || lc_date || '_' || gn_request_id || '.csv';
...
--------------------------------------------------------------------------
         ...
         EXCEPTION
            ...
            THEN
               ...
               fnd_file.put_line (fnd_file.LOG, lc_error_msg);
               log_exception
                  (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.GENERATE_EXCEPTION_REPORT',
                   p_error_msg           => lc_error_msg
                  );
         END;
   ...
   PROCEDURE process_dunning_info (
      x_retcode         OUT NOCOPY      NUMBER,
      x_errbuf          OUT NOCOPY      VARCHAR2,
      p_status          IN              VARCHAR2,
      p_return_status   OUT             VARCHAR2,
   ...
   IS
      -- local variable declaration
      lc_filehandle             UTL_FILE.file_type;
      lc_file_name              VARCHAR2 (200)     := 'XXOD_OMX_DUNNING';
      lc_date                   VARCHAR2 (200)
                                      := TO_CHAR (SYSDATE, 'YYYYMMDD_hhmmss');
      lc_message                VARCHAR2 (4000);
      lc_mode                   VARCHAR2 (1)       := 'W';
      ln_fno                    NUMBER                                       := 1;
      lc_source_path            VARCHAR2 (500);
      lc_string                 VARCHAR2 (32000);
      lc_trailer_string         VARCHAR2 (4000);
      ...
      CURSOR cur_dunning
      IS
          SELECT stg.bill_consignee_ref,stg.aops_customer_number,
                hca.account_number customer_number,stg.dunning_flag,record_id,batch_id,hca.account_name
           FROM xx_cdh_omx_dunning_stg stg,hz_cust_accounts hca
          WHERE stg.status = NVL (p_status, 'N')
   ...
   BEGIN
      ...
      THEN
       lc_error_msg :=
                       'Getting Interface Settings from Translation Definition Failed' || ' ' || SQLERRM;
         RAISE e_process_exception;
      END IF;

-------------------------------------------------------------------------
--Set the lc_filename||date||.dat
-------------------------------------------------------------------------
      lc_file_name := lc_file_name ||'_'||lc_instance||'_'||lc_date||'-'||ln_fno||'.dat';
      log_msg ('Opening a utl_file ..');
      fnd_file.put_line (fnd_file.LOG, 'UTLFILE_Path :' || gc_file_path);
      fnd_file.put_line (fnd_file.LOG, 'file_name :' || lc_file_name);
      fnd_file.put_line(fnd_file.LOG,'**********************************************************************************************************************************************************');
...
-------------------------------------------------------------------------
      ...
      FOR cur_dunning_rec IN cur_dunning
      LOOP
         BEGIN
            lc_return_status := NULL;
            lc_error_msg := NULL;
            lc_string := NULL;
            lc_trailer_string := NULL;
			lc_aops_addr_ref := NULL;
            
            log_msg ('Processing record_id ....' || cur_dunning_rec.record_id ||' For Customer Number '|| cur_dunning_rec.customer_number);
            fnd_file.put_line (fnd_file.LOG,
                                  'bill_consignee_ref, :'
                               || cur_dunning_rec.bill_consignee_ref
                              );
            fnd_file.put_line (fnd_file.LOG,
                                  'aops_customer_number, :'
                               || cur_dunning_rec.aops_customer_number
                              );

 -------------------------------------------------------------------------
-- Calling the procedure get_customer_aops_detail
-------------------------------------------------------------------------
            IF cur_dunning_rec.bill_consignee_ref IS NOT NULL
            THEN
               get_aops_addr_reference
                  (p_omx_bill_consignee      => cur_dunning_rec.bill_consignee_ref,
                   p_aops_cust_number        => cur_dunning_rec.aops_customer_number,
...
-------------------------------------------------------------------------
            lc_string :=
                  cur_dunning_rec.customer_number
               || '|~'
               || cur_dunning_rec.account_name
               || '|~'
               || lc_aops_addr_ref
               || '|~'
               || NVL (cur_dunning_rec.dunning_flag, 'N');
            UTL_FILE.put_line (lc_filehandle, lc_string);
-------------------------------------------------------------------------
--Procedure to Updatethe status to 'C'in table xx_cdh_omx_ap_contacts_stg
--------------------------------------------------------------------------
            update_dunning_stg (p_status         => 'C',
                                p_record_id      => cur_dunning_rec.record_id,
                                p_error_msg      => lc_error_msg
                               );
--------------------------------------------------------------------------
...
--------------------------------------------------------------------------
         ...
         EXCEPTION
            ...
            THEN
               ...
               fnd_file.put_line (fnd_file.LOG, lc_error_msg);
               log_exception
                  (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.PROCESS_BILL_CONTACTS',
                   p_error_msg           => lc_error_msg
                  );
...
--------------------------------------------------------------------------
               update_dunning_stg (p_status         => 'E',
                                   p_record_id      => cur_dunning_rec.record_id,
                                   p_error_msg      => lc_error_msg
                                  );
               ln_failed_records := ln_failed_records + 1;
         END;
         lc_batch_id := cur_dunning_rec.batch_id;
    fnd_file.put_line(fnd_file.LOG,'**********************************************************************************************************************************************************');        
      log_msg ('Commit the error log changes ..');
      COMMIT;       
      END LOOP;

            lc_trailer_string := 'Total number of records extracted:'||''||ln_records_processed;
            UTL_FILE.put_line (lc_filehandle, lc_trailer_string);
...
-- destination/archive Directory so we pass p_email_subject NULL
------------------------------------------------------------------
            lc_return_status := NULL;
            lc_error_msg := NULL;
            log_msg ('Calling  procedure send mail to ftp the file');
            send_mail (p_file_name          => lc_file_name,
                       p_email_subject      => NULL,
                       p_email_body         => NULL,
                       p_return_status      => lc_return_status,
                       p_return_msg         => lc_error_msg
                      );
      ...
      THEN
        ...
        --------------------------------------
            log_msg ('Calling  procedure send mail to send Exception OMX Dunning Information Report ');
            send_mail
               (p_file_name          => lc_file_name,
                p_email_subject      => 'Exception OMX Dunning Information Report',
                p_email_body         => 'The Attached File Contains Exceptions Dunning Information.',
                p_return_status      => lc_return_status,
                p_return_msg         => lc_error_msg
               );
         EXCEPTION
...
-- +===================================================================+
   BEGIN
      ...
      process_dunning_info (x_retcode            => x_retcode,
                            x_errbuf             => x_errbuf,
                            p_status             => p_status,
                            p_return_status      => lc_return_status,
   ...
   EXCEPTION
      ...
      ROLLBACK;
         ...
         fnd_file.put_line (fnd_file.LOG, lc_error_msg);
         log_exception (p_error_location      => 'XX_CDH_OMX_DUNNING_PKG.EXTRACT',
                        p_error_msg           => lc_error_msg
                       );
         x_retcode := 2;
       Commit;  
   END EXTRACT;
END xx_cdh_omx_dunning_pkg;
/
SHOW ERRORS;
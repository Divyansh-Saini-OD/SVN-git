create or replace
PACKAGE BODY XX_AP_RB_VENDOR_LOAD_PKG AS
...
-- |    1.1    14-FEB-2012   Paddy Sanjeevi    Deleted Processed and Rejected|
-- |					                                 records in the interface table|
-- |    1.2    26-Jul-2013   Kiran Kumar R     I2170 - Updated to include    |
-- |                                           R12 Retrofit Changes          |
-- |    1.3    07-Jul-2013   P. Lohiya         Defect 28553 SAMBRA DRIVE     |       
-- |                                          - REBATE SUPPLIERS UPLOAD      |
...
CURSOR c_new IS
SELECT UPPER(TRIM(a.vendor_name))   vendor_name
     , TRIM(a.customer_num)         customer_num
     , UPPER(TRIM(a.site_address1)) site_address1
     , UPPER(TRIM(a.site_address2)) site_address2
     , UPPER(TRIM(a.site_address3)) site_address3
     , UPPER(TRIM(a.site_city))     site_city
     , UPPER(TRIM(a.site_state))    site_state
     , UPPER(TRIM(a.site_zip))      site_zip
     , a.rowid                      row_id
  FROM xx_ap_rb_vendor_stg a
 WHERE a.process_flag='N'
   AND a.error_Flag IS NULL
   AND NOT EXISTS ( SELECT 'X'
		      --FROM po_vendors                                     -- commented by kiran on 26-Jul-13 as per R12 Retrofit Changes
		      FROM ap_suppliers                                     -- added by kiran on 26-Jul-13 as per R12 Retrofit Changes
		     WHERE (   vendor_name = UPPER(TRIM(a.vendor_name))
		            OR customer_num = TRIM(a.customer_num)
			   )
		  );

CURSOR c_exists IS
SELECT b.vendor_id
     , UPPER(TRIM(a.vendor_name))   vendor_name
     , TRIM(a.customer_num)         customer_num
     , UPPER(TRIM(a.site_address1)) site_address1
     , UPPER(TRIM(a.site_address2)) site_address2
     , UPPER(TRIM(a.site_address3)) site_address3
     ...
     , UPPER(TRIM(a.site_zip))      site_zip
     , a.rowid                      row_id
  --FROM po_vendors b,                                                    -- commented by kiran on 26-Jul-13 as per R12 Retrofit Changes
  FROM ap_suppliers b,                                                    -- added by kiran on 26-Jul-13 as per R12 Retrofit Changes
       xx_ap_rb_vendor_stg a
 WHERE a.process_flag='N'
   AND a.error_Flag IS NULL
   AND (    b.vendor_name = UPPER(TRIM(a.vendor_name))
         OR b.customer_num = TRIM(a.customer_num)
       );
...
ln_nsupp_count		  NUMBER := 0;
ln_esupp_count            NUMBER := 0;
ln_error_count            NUMBER := 0;
ln_supp_site_count        NUMBER := 0;
ln_esupp_site_count	  NUMBER := 0;
...
ln_ap_supplier_sites_int_s NUMBER;
...
BEGIN
  ...
  SELECT COUNT(1)
   ...
   ln_esupp_count:=ln_sent_count-ln_nsupp_count;
 ...
 WHERE XFTD.translate_id = xftv.translate_id
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      ...
      FND_FILE.PUT_LINE(fnd_file.log,'NO Data Found While deriving SHIP_LOCATION_ID From Translate : '|| 				lc_ship_to_location);
    WHEN OTHERS THEN
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      ...
      FND_FILE.PUT_LINE(fnd_file.log,'NO Data Found While deriving BILL_LOCATION_ID  FROM TRANSLATE : '|| 				lc_bill_to_location);
    WHEN OTHERS THEN
  ...
  FOR r_vendor IN c_vendor LOOP
    ...
    ELSE
       ...
       UPDATE xx_ap_rb_vendor_stg
	  SET site_zip=r_vendor.site_zip
  ...
  END LOOP;
  COMMIT;
  -- Processing of New Vendor and Vendor Sites

  FOR r_vendor IN c_new LOOP
    ...
    IF ln_vendor_iface_count > 0 THEN
       FND_FILE.PUT_LINE(fnd_file.log,'Vendor already exists in PO VENDORS IFACE TABLE : '||r_vendor.vendor_name);
       lc_error_message := 'Vendor already exists in PO VENDORS IFACE TABLE :'||r_vendor.vendor_name;
    END IF;
    ...
    IF lc_error_message IS NULL THEN 
      ...
      SELECT COUNT(*) 
        ...
        -- commented by kiran on 26-Jul-13 as per R12 Retrofit Changes
        /*FROM po_vendor_sites_all s
           , po_vendors          p*/
        -- added by kiran on 26-Jul-13 as per R12 Retrofit Changes
        FROM ap_supplier_sites_all s
           , ap_suppliers          p
        -- ended by kiran on 26-Jul-13 as per R12 Retrofit Changes
       WHERE (   p.vendor_name          = r_vendor.vendor_name
              OR p.customer_num         = r_vendor.customer_num 
	     )
      ...
      SELECT COUNT(*)
       ...
       WHERE SUBSTR(vendor_site_code,1,2) = SUBSTR(TRIM(lc_site_prefix),1,2)
         AND customer_num	    = r_vendor.customer_num
         AND address_line1          = r_vendor.site_address1
         AND NVL(address_line2,-1)  = NVL(r_vendor.site_address2,-1)
         AND NVL(address_line3,-1)  = NVL(r_vendor.site_address3,-1)
         AND city                   = r_vendor.site_city
	...
	BEGIN
	    ...
	    INTO ap_supplier_sites_int
	       (  vendor_interface_id
                ...
                , bill_to_location_id
                , attribute8
                , vendor_site_interface_id -- Added by P Lohiya for Defect 28553
                , last_update_date
                , last_updated_by
                , creation_date
                , created_by
	     ...
	     VALUES
               (  ln_vendor_interface_id
                ...
                , ln_bill_location_id
                , lc_site_category 
                , ap_supplier_sites_int_s.nextval  --Added by P Lohiya for Defect 28553
                , SYSDATE
                , fnd_global.user_id
                , SYSDATE
                , fnd_global.user_id 
    ...
    IF lc_error_message IS NOT NULL THEN
       UPDATE xx_ap_rb_vendor_stg
          SET error_flag       = 'Y'
            , error_message    = error_message||','||lc_error_message
            , process_flag     = 'Y'
    ...
    -- Processing of Adding vendor site to existing vendors


    FOR r_vendor IN c_exists LOOP
      ...
      SELECT COUNT(*)
        ...
        FROM ap_supplier_sites_int
       WHERE SUBSTR(vendor_site_code,1,2) = SUBSTR(TRIM(lc_site_prefix),1,2)
	 AND vendor_id		    = r_vendor.vendor_id 
         AND address_line1          = r_vendor.site_address1
         AND NVL(address_line2,-1)  = NVL(r_vendor.site_address2,-1)
         AND NVL(address_line3,-1)  = NVL(r_vendor.site_address3,-1)
         AND city                   = r_vendor.site_city
...
/*
        ...
        INTO ln_ap_supplier_sites_int_s
        FROM dual;
        */
        FND_FILE.PUT_LINE(fnd_file.log,'AP_SUPPLIER_SITES_INT_S:::'||AP_SUPPLIER_SITES_INT_S.NextVal);
	BEGIN
          INSERT 
	    INTO ap_supplier_sites_int
	       (  vendor_id
	     ...
	     VALUES
            ...
            --    , ap_supplier_sites_int_s.nextval              
             ...
             ln_esupp_site_count := (ln_esupp_site_count + SQL%ROWCOUNT);
        EXCEPTION
          WHEN OTHERS THEN
            dbms_output.put_line('WHEN OTHERS RAISED WHILE INSERTING INTO SUPPLIER_SITE_INT:::'||SQLERRM);
            FND_FILE.PUT_LINE(fnd_file.log,'WHEN OTHERS RAISED WHILE INSERTING INTO SUPPLIER_SITE_INT:::'||SQLERRM);
   ...
   ln_esupp_count:=ln_sent_count-ln_nsupp_count;
    ...
    DBMS_OUTPUT.PUT_LINE('RECEIVED COUNT                          :::'||TO_CHAR(ln_sent_count)); 
    DBMS_OUTPUT.PUT_LINE('New Supplier COUNT                      :::'||TO_CHAR(ln_nsupp_count)); 
    DBMS_OUTPUT.PUT_LINE('successful New SUPPLIER COUNT           :::'||TO_CHAR(ln_supp_count));
    DBMS_OUTPUT.PUT_LINE('successful New SUPPLIER SITE COUNT      :::'||TO_CHAR(ln_supp_site_count));

    DBMS_OUTPUT.PUT_LINE('Existing Supplier COUNT                 :::'||TO_CHAR(ln_esupp_count)); 
    DBMS_OUTPUT.PUT_LINE('successful New SUPPLIER SITE only COUNT :::'||TO_CHAR(ln_esupp_site_count));


    DBMS_OUTPUT.PUT_LINE('Failed New SUPPLIER COUNT               :::'||TO_CHAR((ln_nsupp_count-ln_supp_count)));
    DBMS_OUTPUT.PUT_LINE('Failed New SUPPLIER Site COUNT          :::'||TO_CHAR((ln_nsupp_count-ln_supp_site_count)));

    DBMS_OUTPUT.PUT_LINE('Failed New SUPPLIER Site only COUNT     :::'||TO_CHAR((ln_esupp_count-ln_esupp_site_count)));

    DBMS_OUTPUT.PUT_LINE('END OF PROGRAM');


  FND_FILE.PUT_LINE(fnd_file.output,'RECEIVED COUNT                          : '||TO_CHAR(ln_sent_count)); 
  FND_FILE.PUT_LINE(fnd_file.output,' ');


  FND_FILE.PUT_LINE(fnd_file.output,'New Supplier COUNT                      : '||TO_CHAR(ln_nsupp_count)); 
  FND_FILE.PUT_LINE(fnd_file.output,'successful New SUPPLIER COUNT           : '||TO_CHAR(ln_supp_count));
  FND_FILE.PUT_LINE(fnd_file.output,'successful New SUPPLIER SITE COUNT      : '||TO_CHAR(ln_supp_site_count));
  FND_FILE.PUT_LINE(fnd_file.output,' ');


  FND_FILE.PUT_LINE(fnd_file.output,'Existing Supplier COUNT                 : '||TO_CHAR(ln_esupp_count)); 
  FND_FILE.PUT_LINE(fnd_file.output,'successful New SUPPLIER SITE only COUNT : '||TO_CHAR(ln_esupp_site_count));
  FND_FILE.PUT_LINE(fnd_file.output,' ');

  FND_FILE.PUT_LINE(fnd_file.output,'Failed New SUPPLIER COUNT               : '||TO_CHAR((ln_nsupp_count-ln_supp_count)));
  FND_FILE.PUT_LINE(fnd_file.output,'Failed New SUPPLIER Site COUNT          : '
	||TO_CHAR((ln_nsupp_count-ln_supp_site_count)));

  FND_FILE.PUT_LINE(fnd_file.output,'Failed New SUPPLIER Site only COUNT     : '
	||TO_CHAR((ln_esupp_count-ln_esupp_site_count)));

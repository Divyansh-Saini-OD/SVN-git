...
CREATE OR REPLACE PACKAGE BODY XX_AP_CONSIGN_RTV_UI_PKG
AS
  -- +============================================================================================+
  ...
  -- | 1.2         20/04/2020  Mayur Palsokar    Modified for jira NAIT-106316
  -- +============================================================================================+
...
IS
  CURSOR C3
  IS
    SELECT ASA.VENDOR_SITE_CODE SUPPLIER_SITE,
    ...
    WHERE 1 =1
      ...
      --   AND TO_DATE(TO_CHAR(G_END_DATE) ||' 23:59:59','DD-MON-RR HH24:MI:SS')    -- Commented by Mayur for NAIT-106316
    AND (MMT.TRANSACTION_DATE >= TO_DATE(TO_CHAR(G_START_DATE)
      ||' 00:00:00','DD-MON-YY HH24:MI:SS')
    OR G_START_DATE           IS NULL) -- Added by Mayur for NAIT-106309
    AND (MMT.TRANSACTION_DATE <= TO_DATE(TO_CHAR(G_END_DATE)
      ||' 23:59:59','DD-MON-RR HH24:MI:SS')
    OR G_END_DATE                         IS NULL) -- Added by Mayur for NAIT-106309
...
BEGIN
  ...
  /* Start: Added by Mayur for NAIT-106316 */
  IF (P_RTV                  IS NOT NULL OR P_RGA IS NOT NULL) THEN
    IF L_CONSIGNMENT_DB.COUNT > 0 THEN
      L_CONSIGNMENT_DB.DELETE;
    END IF;
    OPEN C3;
    LOOP
      FETCH C3 BULK COLLECT INTO L_CONSIGNMENT_DB LIMIT 500;
    ...
    CLOSE C3;
  ELSE
    /* End: Added by Mayur for NAIT-106316 */
    ...
    END LOOP;
    CLOSE C3;
  END IF; -- Added by Mayur for NAIT-106316
  RETURN;
EXCEPTION

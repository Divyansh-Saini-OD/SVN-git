  ...
  IF check_duplicate_file(p_file_name) = 'N' THEN
    ...
    LOOP
      ...
      WHEN value_error THEN
        ROLLBACK;
        utl_file.fclose(l_filehandle);
        p_errbuf := 'LOAD_MKT_PLACE_FILES EXCEPTION VALUE ERROR'||' - '||i.process_name||' File - '||p_file_name||' '||SUBSTR(sqlerrm,1,250);
        p_retcode:= '1';
        logit(p_message => p_errbuf);
        ---  XX_CE_MRKTPLC_PRESTG_PKG.INSERT_PRE_STG_EXCPN ( P_PROCESS_NAME , P_REQUEST_ID, SYSDATE, P_ERRBUF , P_FILE_NAME, 'F');
        insert_file_rec( p_process_name => p_process_name, p_file_name => p_file_name,p_err_msg => p_errbuf,p_process_flag=>'E',p_requestid=>p_request_id) ;
      WHEN OTHERS THEN
        ...
        utl_file.fclose(l_filehandle);
        p_retcode:= '1';
        p_errbuf :='LOAD_MKT_PLACE_FILES EXCEPTIONWHEN OTHERS THEN'||' - '||i.process_name||' File - '||p_file_name||' '||SUBSTR(sqlerrm,1,250);
        logit(p_message => p_errbuf);
        --- XX_CE_MRKTPLC_PRESTG_PKG.INSERT_PRE_STG_EXCPN ( P_PROCESS_NAME , P_REQUEST_ID, SYSDATE, P_ERRBUF , P_FILE_NAME, 'F');
        insert_file_rec( p_process_name => p_process_name, p_file_name => p_file_name,p_err_msg => p_errbuf,p_process_flag=>'E',p_requestid=>p_request_id) ;
      END;

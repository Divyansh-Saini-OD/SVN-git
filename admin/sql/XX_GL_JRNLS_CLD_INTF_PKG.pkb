...
AS
  ...
  gn_user_name varchar2(50):='SVC_ESP_FIN';
...
BEGIN
  ...
  logit(p_message =>'Inside file_batch cursor ');
  --==========================================================================================
  -- Default Process Status Flag as N means No Error Exists
  --==========================================================================================
 ...
 OPEN cur_ledger_name;
  ...
  --==========================================================================================
	 gc_error_status_flag:='N';
	  lv_error_message :='User JE Source Name not found';		
						 ...
						 when no_data_found then 
						  ...
						  if gc_error_status_flag='Y' then
							Update XX_GL_JRNLS_CLD_INTF_STG set 				   
								...
								where 1=1
								and file_batch_id=gn_file_batch_id
								and application_name=lv_currency_code_data(idx).currency_code;
								gc_error_status_flag:='N';
						end if;
  ...
  --==========================================================================================	
		...
		commit;
			...
			if gc_error_status_flag='Y' then
				...
				update XX_GL_JRNLS_CLD_INTF_FILES set record_status='V'--Update to E when required. 
				where 1=1
				and file_batch_id=gn_file_batch_id;		
...
AS
  ...
  IS
    SELECT 
      xftv.source_value2 inbound_path,
      xftv.source_value3 archival_path
    FROM xx_fin_translatedefinition xftd,
      xx_fin_translatevalues xftv
...
BEGIN
  ...
  LOOP
	...
	if p_process_name in ('SCM') then
	  ...
	  lc_GL_JRNLS_CLD_INTF_STG.user_je_source_name            					:=l_table(20);
	  ...
	  lc_GL_JRNLS_CLD_INTF_STG.reference10            						    :=l_table(19)							;
	  lc_GL_JRNLS_CLD_INTF_STG.EBS_JOURNAL_source            				    :=l_table(20)							;
...
WHEN utl_file.invalid_operation THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Invalid Operation';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN utl_file.invalid_filehandle THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Invalid File Handle';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN utl_file.read_error THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Read Error';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN utl_file.invalid_path THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Invalid Path';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN utl_file.invalid_mode THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Invalid Mode';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN utl_file.internal_error THEN
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging: Internal Error';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm);
WHEN value_error THEN
  ROLLBACK;
  utl_file.fclose(l_filehandle);
  p_errbuf := 'XX_GL_JRNLS_CLD_INTF_PKG.load_utl_file_staging';
  p_retcode:= '2';
  log_exception (p_program_name => 'XXGLJRNLSCLDINTFLOAD' ,p_error_location => l_error_loc ,p_error_msg => p_errbuf||sqlerrm); 
 WHEN OTHERS THEN
  ROLLBACK;
    ...
    ELSE
      ...
      ELSE
		...
		Update XX_GL_JRNLS_CLD_INTF_STG set record_status='P',
				action='E'
				where file_batch_id=rec.file_batch_id
				and error_description=ERROR_DESCRIPTION||'~OD: GL Interface for Cloud GL Transactions Program did not complete normally.'
				and EBS_JOURNAL_SOURCE=stg_rec.EBS_JOURNAL_SOURCE;
      END IF;
    END IF;
  END;
...
BEGIN
  ...
  ******************************/
  ...
  print_debug_msg(p_message => '+---------------------------------------------------------------------------+' , p_force => true);
  print_debug_msg(p_message => 'Calling VALIDATE_CLD_GL_INTF_AP_FILE Procedure' , p_force => true);
  VAL_CLD_GL_INTF_FILE( p_process_name => p_process_name,p_debug_flag => p_debug_flag,p_errbuf=>x_err_buf,p_retcode=>x_ret_code) ;
  print_debug_msg(P_MESSAGE => 'ExitingVALIDATE_CLD_GL_INTF_AP_FILE Procedure' , p_force => TRUE);
  print_debug_msg(p_message => '+---------------------------------------------------------------------------+' , p_force => true);

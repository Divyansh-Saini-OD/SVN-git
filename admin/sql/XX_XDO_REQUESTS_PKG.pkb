CREATE OR REPLACE PACKAGE BODY APPS.XX_XDO_REQUESTS_PKG AS
-- +============================================================================================+ 
...
IS
  CURSOR c_nextval_1 IS
    SELECT XX_XDO_REQUEST_ID_SEQ.NEXTVAL
      FROM sys.dual;
  
  CURSOR c_new_row 
  ( cp_xdo_request_id         IN   NUMBER )
  IS 
    SELECT ROWID
      FROM XX_XDO_REQUESTS
     WHERE xdo_request_id = cp_xdo_request_id; 
BEGIN
  IF (x_xdo_request_id IS NULL) THEN
    OPEN c_nextval_1;
    FETCH c_nextval_1
     INTO x_xdo_request_id;
    CLOSE c_nextval_1;
  END IF;
  
  INSERT INTO XX_XDO_REQUESTS
  ( xdo_request_id,
    xdo_request_date,
    xdo_request_name,
    xdo_request_group_id,
    language_code,
    source_app_code,
    source_name,
    days_to_keep,
  ...
  OPEN c_new_row
  ( cp_xdo_request_id         => x_xdo_request_id );
  FETCH c_new_row 
   INTO x_rowid;
...
BEGIN
  insert_row
  ( x_rowid                  => x_rowid,
    x_xdo_request_id         => x_insert_row.xdo_request_id,
    p_xdo_request_date       => x_insert_row.xdo_request_date,
    p_xdo_request_name       => x_insert_row.xdo_request_name,
    p_xdo_request_group_id   => x_insert_row.xdo_request_group_id,
    ...
    p_days_to_keep           => x_insert_row.days_to_keep,
...
BEGIN
  ...
  IF (   ( (l_current_row.xdo_request_date = p_xdo_request_date)
           OR ( (l_current_row.xdo_request_date IS NULL) AND (p_xdo_request_date IS NULL) ) )
     AND ( (l_current_row.xdo_request_name = p_xdo_request_name)
           OR ( (l_current_row.xdo_request_name IS NULL) AND (p_xdo_request_name IS NULL) ) )
     AND ( (l_current_row.xdo_request_group_id = p_xdo_request_group_id)
           OR ( (l_current_row.xdo_request_group_id IS NULL) AND (p_xdo_request_group_id IS NULL) ) )
     ...
     AND ( (l_current_row.days_to_keep = p_days_to_keep)
           OR ( (l_current_row.days_to_keep IS NULL) AND (p_days_to_keep IS NULL) ) )
...
BEGIN
  ...
  ( p_xdo_request_id         => x_lock_row.xdo_request_id,
    p_xdo_request_date       => x_lock_row.xdo_request_date,
    p_xdo_request_name       => x_lock_row.xdo_request_name,
    p_xdo_request_group_id   => x_lock_row.xdo_request_group_id,
    p_language_code          => x_lock_row.language_code,
    p_source_app_code        => x_lock_row.source_app_code,
    p_source_name            => x_lock_row.source_name,
    p_days_to_keep           => x_lock_row.days_to_keep,
...
IS
BEGIN
  UPDATE XX_XDO_REQUESTS
     SET xdo_request_date       = p_xdo_request_date,
         xdo_request_name       = p_xdo_request_name,
         xdo_request_group_id   = p_xdo_request_group_id,
...
BEGIN
  ...
  ( p_xdo_request_id         => x_update_row.xdo_request_id,
    p_xdo_request_date       => x_update_row.xdo_request_date,
    p_xdo_request_name       => x_update_row.xdo_request_name,
    p_xdo_request_group_id   => x_update_row.xdo_request_group_id,
    ...
    p_days_to_keep           => x_update_row.days_to_keep,
...
BEGIN
  ...
  x_xdo_request_date       := l_current_row.xdo_request_date;
  x_xdo_request_name       := l_current_row.xdo_request_name;
  x_xdo_request_group_id   := l_current_row.xdo_request_group_id;
  ...
  x_days_to_keep           := l_current_row.days_to_keep;
...
BEGIN
  IF ( row_exists 
       ( p_xdo_request_id         => x_xdo_request_id ) = 'Y' )
  THEN
    update_row
    ( p_xdo_request_id         => x_xdo_request_id,
      p_xdo_request_date       => p_xdo_request_date,
      p_xdo_request_name       => p_xdo_request_name,
      p_xdo_request_group_id   => p_xdo_request_group_id,
      p_language_code          => p_language_code,
      p_source_app_code        => p_source_app_code,
      p_source_name            => p_source_name,
      p_days_to_keep           => p_days_to_keep,
  ...
  ELSE
    insert_row
    ( x_rowid                  => x_rowid,
      x_xdo_request_id         => x_xdo_request_id,
      p_xdo_request_date       => p_xdo_request_date,
      p_xdo_request_name       => p_xdo_request_name,
      p_xdo_request_group_id   => p_xdo_request_group_id,
      p_language_code          => p_language_code,
...
BEGIN
  IF ( row_exists 
       ( p_xdo_request_id => x_load_row.xdo_request_id ) = 'Y' )
  THEN
    update_row

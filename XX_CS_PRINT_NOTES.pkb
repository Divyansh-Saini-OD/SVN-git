...
-- |1.5       25-Jan-16       Manikant Kasu	    	   Removed schema references as per        |
...
END PRINT_SALES_REP;
/****************************************************************************
  -- Missed Promise Date Daily Report
******************************************************************************/
...
CURSOR SR_DET IS
select cb.incident_number sr_no,
       cb.error_code CSR,
       cb.Incident_attribute_1 original_order,
       nvl(cb.incident_attribute_12, cb.incident_attribute_1) active_order,
       cl.meaning Primary_issue,
       cb.Incident_attribute_11 Location_id,
       cb.Creation_date sr_open,
       cb.close_date sr_close,
       cb.inc_responded_by_date response_date,
       nvl(nvl(cb.incident_attribute_13, cb.incident_attribute_6), cb.incident_attribute_2) actual_delivery_date,
       nvl(cb.incident_attribute_6, to_date(cb.incident_attribute_2, 'dd-mon-yyyy')) promise_date
from cs_incidents_all_b cb,
     cs_lookups cl
where cl.lookup_code = cb.problem_code
and   cl.lookup_type = 'REQUEST_PROBLEM_CODE'
and   cb.problem_code in ('LATE DELIVERY', 'RETURN NOT PICKED UP')
and   trunc(cb.last_update_date) = trunc(sysdate)
and   exists (select 'x' from cs_incident_types_tl
    ...
    begin
     ...
     exception
      ...
      open sr_det;
        htp.print('<FONT FACE="Times New Roman" SIZE="+2" COLOR="blue">');
        htp.print('<U>');
        htp.print('Missed Promise ');
        htp.print('</U>');
        htp.print ('</FONT>');
        htp.tableOpen(cattributes => 'BORDER="1" CELLPADDING="0" CELLSPACING="0" WIDTH="120%"');
        htp.TableRowOpen;
        htp.TableRowClose;
        htp.TableRowOpen;
        htp.tableheader('<U>SR Number</U>',calign=>'center');
        htp.tableheader('<U>CSR</U>',calign=>'center');
        htp.tableheader('<U>Original Order</U>',calign=>'center');
        htp.tableheader('<U>Active Order</U>',calign=>'center');
        htp.tableheader('<U>Primary Issue</U>',calign=>'center');
        htp.tableheader('<U>Location Id</U>',calign=>'center');
        htp.tableheader('<U>SR Open</U>',calign=>'center');
        htp.tableheader('<U>SR Close</U>',calign=>'center');
        htp.tableheader('<U>Response Date</U>',calign=>'center');
        htp.tableheader('<U>Promise Date</U>',calign=>'center');
        htp.tableheader('<U>Actual Delivery Date</U>',calign=>'center');
        htp.tableheader('<U>Missed Promise</U>',calign=>'center');
        htp.tableheader('<U>Variance</U>',calign=>'center');
        htp.TableRowClose;
      loop
      fetch sr_det into sr_rec;
      exit when sr_det%NOTFOUND;

        If to_date(sr_rec.actual_delivery_date, 'mm/dd/yyyy') > to_date(sr_rec.promise_date, 'dd-mon-yyyy') then
          
          lc_missed_flag := 'Yes';
          lc_variance  := floor((to_date(sr_rec.actual_delivery_date, 'mm/dd/yyyy') - to_date(sr_rec.promise_date, 'dd-mon-yyyy'))/3600);
          
            htp.print('<TR><TD>');
            htp.tableRowOpen;
            htp.tableData(cvalue => sr_rec.sr_no, calign=>'left');
            htp.tableData(cvalue => sr_rec.csr, calign=>'left');
            htp.tableData(cvalue => sr_rec.original_order, calign=>'left');
            htp.tableData(cvalue => sr_rec.active_order, calign=>'left');
            htp.tableData(cvalue => sr_rec.primary_issue, calign=>'left');
            htp.tableData(cvalue => sr_rec.location_id, calign=>'left');
            htp.tableData(cvalue => sr_rec.sr_open, calign=>'left');
            htp.tableData(cvalue => sr_rec.sr_close, calign=>'left');
            htp.tableData(cvalue => sr_rec.response_date, calign=>'left');
            htp.tableData(cvalue => sr_rec.promise_date, calign=>'left');
            htp.tableData(cvalue => sr_rec.actual_delivery_date, calign=>'left');
            htp.tableData(cvalue => lc_missed_flag, calign=>'left');
            htp.tableData(cvalue => lc_variance, calign=>'left');
            htp.print ('</TD></TR>');
            htp.tableRowClose;

        else
          lc_missed_flag := 'No';
          htp.print('No Missed Promise Requests ');
          htp.nl;
        end If;
      end loop;
        htp.tableClose;
...
i               number := 0;
l_que_ans_id    number;
lc_sku_panel    varchar2(200);

CURSOR panel_cur IS
select distinct ip.panel_name 
...
and    qd.transaction_id = l_que_ans_id;
...
where  ip.panel_id = qp.panel_id
and    qp.question_id = qd.question_id
and    ip.panel_name = lc_sku_panel
and    qd.transaction_id = l_que_ans_id;
...
BEGIN
      ...
      IF L_QUE_ANS_ID IS NOT NULL THEN  
        htp.htmlOpen;
        htp.print; htp.print;
        htp.headOpen;
        htp.Title('Preservice Questions');
        htp.headClose;
        htp.bodyOpen;
        --htp.centerOpen;
        htp.nl;
        ...
        htp.print('<FONT FACE="Times New Roman" SIZE="+2" COLOR="blue">');
        htp.print('<U>');
        htp.print('Preservice Questions');
        htp.print('</U>');
        htp.print ('</FONT>');
        htp.nl;htp.nl;htp.nl;
        
       FOR panel_rec in panel_cur LOOP
             
              lc_sku_panel := panel_rec.panel_name;
              
              htp.tableOpen(cattributes => 'BORDER="1" CELLPADDING="0" CELLSPACING="0" WIDTH="80%"');
              htp.TableRowOpen;
              htp.tableheader('<U>'||panel_rec.panel_name||'</U>',calign=>'left');
              htp.TableRowClose;
              ...
              FOR qa_details_rec IN qa_details LOOP 
                  ...
                  htp.print('<TR><TD>');
                  htp.tableRowOpen;
               --   htp.tableData(cvalue => qa_details_rec.qus, calign=>'left');
                --  htp.tableData(cvalue => qa_details_rec.ans, calign=>'left');
                  htp.tableData(cvalue => qa_details_rec.qus||'  : '||qa_details_rec.ans, calign=>'left');
                  htp.print ('</TD></TR>');
                  htp.tableRowClose;
      ...
      ELSE
          htp.print('No presevice questions for this order ');
          htp.nl;
      END IF;
...
EXCEPTION
  WHEN OTHERS THEN
      htp.print('No preservice questions for this order ');
      htp.nl;
END SERVICE_QA_REC;
/******************************************************************************/
END;

CREATE OR REPLACE PACKAGE BODY XX_PO_XML_DELIVERY_PKG AS
/* $Header: /home/cvs/repository/Office_Depot/SRC/P2P/E0408_PO_XMLG_Modifications/3.\040Source\040Code\040&\040Install\040Files/XX_PO_XML_DELIVERY_PKG_PKG.pkb,v 1.1 2007/09/10 11:06:48 sgaur Exp $ */
...
-- +===================================================================+
-- | Name       : XX_PO_XML_DELIVERY_PKG.pkb                           |
...
x_msg NUMBER;
  ...
  x_progress := 'XX_PO_XML_DELIVERY_PKG.call_txn_delivery : 01';
 ...
 -- else
   --    document_type = 'POCO'
    ...
    BEGIN
     ...
     ELSE
        x_progress :=  'XX_PO_XML_DELIVERY_PKG.: call_txn_delivery:02: POs of type ' || l_doc_type || 'is not supported for XML Delivery';
    wf_core.context('XX_PO_XML_DELIVERY_PKG','call_txn_delivery',x_progress);
        RETURN;
    ...
    EXCEPTION
     WHEN OTHERS THEN
      x_progress :=  'XX_PO_XML_DELIVERY_PKG.: call_txn_delivery:02';
    wf_core.context('XX_PO_XML_DELIVERY_PKG','call_txn_delivery',x_progress);
...
/*  removed ecx_document.send . To avoid unnecessary dependency on ECX. */
 ...
 x_progress :=  'XX_PO_XML_DELIVERY_PKG.call_txn_delivery: 03';
  IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype,itemkey,x_progress);
  END IF;
   EXCEPTION
...
EXCEPTION
WHEN OTHERS THEN
   wf_core.context('XX_PO_XML_DELIVERY_PKG','initialize_wf_parameters',x_progress);
  RAISE;
  --return;
...
x_progress                  VARCHAR2(100) := '000';
  ...
  x_progress := 'XX_PO_XML_DELIVERY_PKG.set_delivery_data : 01';
  IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress);
  END IF;
   ...
   END IF;
    ...
    --replacing APPLICATION_ID item attr with  modified to fix the issue BP to 5442045 or to fix 5532116
     ...
     ELSIF (l_doc_type = 'PO' OR l_doc_type = 'STANDARD')    THEN --for standard POs.
        ...
        WHERE po_header_id= l_doc_id;
     ELSE
        x_progress :=  'XX_PO_XML_DELIVERY_PKG.: set_delivery_data:02: POs of type ' || l_doc_type || 'is not supported for XML Delivery';
    wf_core.context('XX_PO_XML_DELIVERY_PKG', 'set_delivery_data',x_progress);
        RETURN;
     END IF;
    ...
    EXCEPTION
     WHEN OTHERS THEN
      x_progress :=  'XX_PO_XML_DELIVERY_PKG.: set_delivery_data:02';
      wf_core.context('XX_PO_XML_DELIVERY_PKG','set_delivery_data',x_progress);
      RETURN;   --do not raise the exception as that would end the wflow.
    END ;
    ...
    END IF;
     ...
     resultout := wf_engine.eng_completed || ':' ||  'ACTIVITY_PERFORMED';
 x_progress :=  'XX_PO_XML_DELIVERY_PKG.set_delivery_data: 03';
  IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype,itemkey,x_progress);
  END IF;
...
BEGIN
  x_progress := 'XX_PO_XML_DELIVERY_PKG.is_partner_setup : 01';
  IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress);
  END IF;
  ...
  IF (result) THEN
    ...
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_partner_setup : 02';
  ...
  ELSE
     ...
     x_progress := 'XX_PO_XML_DELIVERY_PKG.is_partner_setup : 03';
...
WHEN OTHERS THEN
  x_progress := 'XX_PO_XML_DELIVERY_PKG.is_partner_setup : 04';
  IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress);
  END IF;
  resultout := 'COMPLETE:F';
...
x_progress VARCHAR2(100) := '000';
BEGIN
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 01';
  IF (funcmode <> wf_engine.eng_run) THEN
  ...
  END IF;
    ...
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 02';
   	IF l_doc_type = 'RELEASE' THEN
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 03';
		l_doc_rel_id := l_doc_id;
    ...
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 04';
    	SELECT poh.xml_flag, poh.agent_id INTO l_xml_flag, l_agent_id
        FROM po_headers_all poh
        WHERE po_header_id= l_doc_id;
	END IF;
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 05';
	IF l_xml_flag = 'Y' THEN
		resultout := 'COMPLETE:T';
	END IF;
    x_progress := 'XX_PO_XML_DELIVERY_PKG.is_xml_chosen : 06';
...
x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 01';
...
l_revision_num := TO_NUMBER(PO_WF_UTIL_PKG.GetItemAttrText (itemtype => itemtype,
    ...
    x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 02';
    IF (l_release_id IS NOT NULL OR l_release_id > 0) THEN

       x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 03';
    ...
    ELSE --for standard POs.
       x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 04';
        ...
        l_user_key  := l_ponum || '-' || TO_CHAR(l_revision_num);
    END IF;
    x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 05';


    wf_engine.SetItemUserKey(itemtype => itemtype,
                                itemkey  => itemkey,
                                userkey  => l_user_key);
    x_progress := 'XX_PO_XML_DELIVERY_PKG.setwfUserKey : 06';

    resultout := 'COMPLETE:T';
    wf_core.context('XX_PO_XML_DELIVERY_PKG','setwfUserKey','completed');

EXCEPTION WHEN OTHERS THEN
   wf_engine.SetItemUserKey(itemtype => itemtype,
                            itemkey  => itemkey,
                            userkey  => 'Cannot set item key');
   wf_core.context('XX_PO_XML_DELIVERY_PKG','setwfUserKey',x_progress || ':' || TO_CHAR(l_document_id));
...
BEGIN
   ...
   x_progress := 'XX_PO_XML_DELIVERY_PKG.set_user_context : 001';
   IF (g_po_wf_debug = 'Y') THEN
      /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress);
   END IF;
  ...
  l_appl_id := TO_NUMBER(PO_WF_UTIL_PKG.GetItemAttrText(itemtype=>itemtype, itemkey=>itemkey, aname=>'APPLICATION_ID'));
   ...
   x_progress := 'XX_PO_XML_DELIVERY_PKG.set_user_context : 002';
   IF (g_po_wf_debug = 'Y') THEN
      /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress
   ...
   x_progress := 'XX_PO_XML_DELIVERY_PKG.set_user_context : 003';
   IF (g_po_wf_debug = 'Y') THEN
      /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype, itemkey,x_progress
   ...
   x_progress :=  'XX_PO_XML_DELIVERY_PKG.set_user_context: 004 ' || resultout;
   IF (g_po_wf_debug = 'Y') THEN
     /* DEBUG */  PO_WF_DEBUG_PKG.insert_debug(itemtype,itemkey,x_progress);
   END IF;
...
EXCEPTION
    WHEN OTHERS THEN
    wf_core.context('XX_PO_XML_DELIVERY_PKG','set_user_context',x_progress);

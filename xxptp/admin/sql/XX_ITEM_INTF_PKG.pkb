...
CREATE OR REPLACE PACKAGE BODY XX_INV_ITEM_INTF_PKG
-- +====================================================================================+
...
-- +====================================================================================+
-- | Name        :  ITEM_INTR_ARCH                                                      |
-- | Description :  This procedure is invoked from ESP to archive the processed records |
-- |                                                                                    |
-- | Parameters  :                                                                      |
-- +====================================================================================+
PROCEDURE ITEM_INTF_ARCH(
...
BEGIN
  BEGIN
	...
	( 	 ITEM
		...
		,SHIPPABLE_ITEM_FLAG                  
		,ITEM_PROCESS_FLAG                    
		,INV_CATEGORY_PROCESS_FLAG            
		,ODPB_CATEGORY_PROCESS_FLAG           
		,PO_CATEGORY_PROCESS_FLAG             
		,OD_COUPON_DISC_FLG                   
		,OD_HUB_FLAG                          
		,OD_SELL_RESTRICT_CD                  
		,STANDARD_UOM                         
		,ATP_CATEGORY_PROCESS_FLAG            
		,ATP_CATEGORY_ID                      
		,VALIDATION_ORGS_STATUS_FLAG          
		,MASTER_ITEM_ATTR_PROCESS_FLAG        
		,ERROR_MESSAGE                        
		,API_CHANGE                           
		,TAX_CHANGE                           
		,OLD_INV_CAT_ID                       
    ...
    SELECT 	 ITEM
		...
		,LAST_UPDATE_DATE                     
		,LAST_UPDATED_BY                      
	FROM xx_item_master_intf
     WHERE creation_date < sysdate-16;
  EXCEPTION
    WHEN others THEN
      NULL;	
  END;
  ...
  BEGIN
	...
	(	ITEM                        
		...
		,INVENTORY_ITEM_ID           
		,LOAD_BATCH_ID               
		,LOCATION_PROCESS_FLAG       
		,ORGANIZATION_ID             
		,TEMPLATE_ID                 
		,LOC_ITEM_ATTR_PROCESS_FLAG  
		,SHIPPABLE_ITEM_FLAG         
		,API_CHANGE                  
		,ERROR_MESSAGE               
		,ACTION_TYPE                 
...
BEGIN
 NULL;
/*
  FOR cur IN c_master LOOP
    ln_retcode:=NULL;
    ln_commit:=ln_commit+1;
    ln_retcode:=XX_INV_ITEM_INTF_PKG.UPDATE_RMS_ERROR(cur.process_id,cur.error_message,ln_retcode);
  ...
  END LOOP; 
  COMMIT;
  XX_INV_ITEM_INTF_PKG.ITEM_INTF_ARCH( 
	      	                     x_errbuf     =>lx_errbuf
...
/*
  ...
  SELECT COUNT(1)
   ...
   WHERE concurrent_program_id IN (SELECT concurrent_program_id
					       FROM fnd_concurrent_programs
					      WHERE concurrent_program_name='XX_INV_ITEM_INTF_ERROR_UPD'
						  AND application_id=401
						  AND enabled_flag='Y')
     AND program_application_id=401
     AND phase_code='R';
...
/*
  IF ln_err_run=0 THEN
     ln_request_id := FND_REQUEST.submit_request(
			                             application =>  G_APPLICATION
                  			          ,program     =>  'XX_INV_ITEM_INTF_ERROR_UPD'
			                            ,sub_request =>  FALSE
                                             );
     IF ln_request_id = 0 THEN
        x_errbuf  := 'Unable to submit OD INV Item Interface Error Updates';
        x_retcode :=-1;
     ELSE
        COMMIT;
     END IF;
...
BEGIN
  ...
  IF p_master='Y' THEN
    ...
    UPDATE XX_ITEM_MASTER_INTF XSIM
    ...
    IF SQL%NOTFOUND THEN
	   ...
	   AND  XSIM.load_batch_id                  IS NULL
         ...
         AND    EXISTS ( SELECT 'x'
			  ...
			  WHERE organization_id=ln_master_org
...
BEGIN
  ...
  LOOP
    ...
    ELSE
       SELECT COUNT(1)
         INTO ln_current_count
         FROM XX_ITEM_LOC_INTF XILI
        WHERE XILI.process_flag = 1
    ...
    IF p_master='Y' THEN
	 IF ln_req_count=6 THEN
       ...
       END IF;    
    ELSE
	 IF ln_req_count=gn_threads THEN
          EXIT;
       END IF;    
    END IF;
  END LOOP;
...
TYPE item_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.item_process_flag%TYPE
INDEX BY BINARY_INTEGER;
...
TYPE inv_category_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.inv_category_process_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_inv_categorypf inv_category_pf_tbl_type;

TYPE inv_categoryid_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.inv_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_inv_categoryid inv_categoryid_pf_tbl_type;

TYPE old_inv_cid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.inv_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_old_inv_catid old_inv_cid_tbl_type;

TYPE odpb_category_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.odpb_category_process_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_odpb_categorypf odpb_category_pf_tbl_type;

TYPE odpb_categoryid_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.odpb_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_odpb_categoryid odpb_categoryid_pf_tbl_type;

TYPE old_odpb_cid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.odpb_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_old_odpb_catid old_odpb_cid_tbl_type;

TYPE po_category_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.po_category_process_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_po_categorypf po_category_pf_tbl_type;

TYPE po_categoryid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.po_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_po_categoryid po_categoryid_tbl_type;

TYPE old_po_cid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.po_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_old_po_catid old_po_cid_tbl_type;

TYPE atp_category_pf_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.atp_category_process_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_atp_categorypf atp_category_pf_tbl_type;

TYPE atp_categoryid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.atp_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_atp_categoryid atp_categoryid_tbl_type;

TYPE old_atp_cid_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.atp_category_id%TYPE
INDEX BY BINARY_INTEGER;
lt_old_atp_catid old_atp_cid_tbl_type;

TYPE shippable_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.shippable_item_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_shippable_item_flag shippable_tbl_type;

TYPE od_sku_type_cd_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.od_sku_type_cd%TYPE
INDEX BY BINARY_INTEGER;
lt_od_sku_type_cd od_sku_type_cd_tbl_type;

TYPE error_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.error_message%TYPE
INDEX BY BINARY_INTEGER;
lt_error_tbl  error_tbl_type;

TYPE errflg_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.error_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_errflg_tbl  errflg_tbl_type;


TYPE call_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.api_change%TYPE
INDEX BY BINARY_INTEGER;
lt_call_tbl  call_tbl_type;

TYPE taxc_tbl_type IS TABLE OF XX_ITEM_MASTER_INTF.tax_change%TYPE
INDEX BY BINARY_INTEGER;
...
BEGIN
    ...
    IF lt_itemmaster.COUNT <> 0 THEN
       ...
       ------------------------------------------------------------
       -- Bulk Update XX_ITEM_MASTER_INTF with Process flags and Ids
       ------------------------------------------------------------
...
EXCEPTION
  WHEN EX_MASTER_NO_DATA THEN
       x_retcode := 1;
       x_errbuf  := 'No data found in the staging table XX_ITEM_MASTER_INTF with batch_id - '||p_batch_id;
  WHEN OTHERS THEN
    gc_sqlerrm := SQLERRM;
    gc_sqlcode := SQLCODE;
...
TYPE location_control_id_tbl_type IS TABLE OF XX_ITEM_LOC_INTF.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_location_control_id location_control_id_tbl_type;

TYPE location_pf_tbl_type IS TABLE OF XX_ITEM_LOC_INTF.location_process_flag%TYPE
INDEX BY BINARY_INTEGER;
lt_location_pf location_pf_tbl_type;

TYPE loc_organization_id_tbl_type IS TABLE OF XX_ITEM_LOC_INTF.organization_id%TYPE
INDEX BY BINARY_INTEGER;
lt_org_id loc_organization_id_tbl_type;

TYPE loc_template_id_tbl_type IS TABLE OF XX_ITEM_LOC_INTF.template_id%TYPE
INDEX BY BINARY_INTEGER;
...
TYPE call_tbl_type IS TABLE OF XX_ITEM_LOC_INTF.api_change%TYPE
INDEX BY BINARY_INTEGER;
lt_call_tbl  call_tbl_type;
...
BEGIN
  ...
  IF lt_itemlocation.COUNT <> 0 THEN
       ...
       IF lt_org_id(i) > 0 THEN
          ...
          IF p_process='LC' THEN
             ...
             BEGIN
	           ...
	           FROM apps.xx_inv_item_master_attributes
     ...
     --------------------------------------------------------
     -- Bulk Update XX_ITEM_LOC_INTF with Process flag and Ids
     --------------------------------------------------------
...
WHEN EX_LOCATION_NO_DATA THEN
    x_retcode := 1;
    x_errbuf  := 'No data found in the staging table XX_ITEM_LOC_INTF with batch_id - '||p_batch_id;
WHEN OTHERS THEN
    gc_sqlerrm := SQLERRM;
    gc_sqlcode := SQLCODE;
...
TYPE item_cat_errorid_type IS TABLE OF XX_ITEM_MASTER_INTF.control_id%TYPE
INDEX BY BINARY_INTEGER;
...
TYPE loc_errorid_type IS TABLE OF XX_ITEM_LOC_INTF.control_id%TYPE
INDEX BY BINARY_INTEGER;
...
-- +===================================================================+
-- | Name        :  call_rms_confirm                                   |
-- | Description :  This procedure is invoked to call rms confirm BPEL |
-- |                                                                   |
-- | Parameters  :  p_batch_id                                         |
-- | 		        p_process                                          |
-- |                                                                   |
...
SELECT item,error_message
 ...
 WHERE load_batch_id=p_batch_id
   AND (    item_process_flag <>7
         OR inv_category_process_flag   <> 7
	   OR odpb_category_process_flag  <> 7
	   OR po_category_process_flag    <> 7
	   OR atp_category_process_flag   <> 7
	   OR validation_orgs_status_flag <>'S'
	   OR master_item_attr_process_flag <> 7
	 );

CURSOR C_loc_error 
IS 
SELECT item,loc,error_message
  FROM xx_item_loc_intf
 WHERE load_batch_id=p_batch_id
   AND (    location_process_flag  <> 7
	   OR loc_item_attr_process_flag <> 7
	 );
BEGIN
  IF p_process IN ('MA','MC') THEN

     FOR cur IN c_item_error LOOP
	   NULL;
  ...
  ELSIF p_process IN ('LA','LC') THEN
     ...
     FOR cur IN c_loc_error LOOP
	   NULL;

     END LOOP;
     COMMIT;
  END IF;
END call_rms_confirm;

/*===========================================================================+
 ...
 |      29-Aug-2007   Customized for OD                                       |
 |      27-Sep-2007   Bug fix: For nontrade then intransit type = DIRECT      |
 |      04-Oct-2007   CR: Removed update of attribute6, Bug fix: Count mismatch|
 |      20-Sep-2013   E0351A2 - Modified for R12 Upgrade retrofit changes.    |
 |      12-Nov-2013   E0351A2 - Modified for fix of R12 retrofit defect# 26320|
 +===========================================================================*/
...
import od.oracle.apps.xxptp.rosetta.XxGiShipnetCreationPkg;
import od.oracle.apps.xxptp.rosetta.XxGiShipnetCreationPkg.GShipnetRec;

public class ODShippingNetworks implements JavaConcurrentProgram
{
  public static final String RCS_ID=
    "$Header: CopyLoader.java 120.20 2008/01/17 10:32:31 vssrivat ship $";
  public static final boolean RCS_ID_RECORDED =
    VersionInfo.recordClassVersion( RCS_ID, "oracle.apps.inv.setup.cp.CopyLoader.java" );
...
//myerrams,Bug: 5174575
 ...
 //Bug: 4878810, pgopalar: setter method for the protected variable isRequestStatusError
  ...
  /**
   * Start of OD Custom variables
   */
  ShipNetOut              mCpOutFile;

  /**
   * Concurrent Program Parameters
   */
  String               mReportOnlyMode;
  String               mFromOrgType;
  Number               mFromOrgId;
  /**
   * Other Member variables
   */
  String mSourceOrgType;
  String mTargetOrgType;
  String mSourceOrgNumber;
  String mTargetOrgNumber;
  Number mSourceOrgId;
  Number mErrorCode;
  String mErrorMessage;
  Number mTargetOrgId;
  String mSourceInvOrg;
  String mTargetInvOrg;
  String mShipnetCreate;
  Number mIntransitType;
  Number mFobPoint;
  Number mInterorgTransferCode;
  Number mReceiptRoutingId;
  Number mInternalOrderRequiredFlag;
  String mIntransitInvAccount;
  String mInterorgTransferCrAccount;
  String mInterorgReceivablesAccount;
  String mInterorgPayablesAccount;
  String mInterorgPriceVarAccount;
  String mElementalVisibilityEnabled;
  String mManualReceiptExpense;
  String mSourceOrgCode;
  String mTargetOrgCode;
  //For statistics
  int mNetNonTradeFormed;
  int mNetNonTradeCreated;
  int mNetNonTradeFailed;
  int mNetExceptNonTradeFormed;
  int mNetExceptNonTradeCreated;
  int mNetExceptNonTradeFailed;
  String mFromOrgName;
  boolean isNonTrade = false;
  boolean isExceptNonTrade = false;
   /**
   * End of OD Custom variables
   */ 
...
/* Added by arun andavar
 * OD Functions -- STARTS HERE
 */
    public String getStringParam(String token) throws Exception
    {
  ...
  public Number getNumberParam(String token) throws Exception
  {
 ...
 */
  ...
  public void initialize( )   throws java.lang.Exception
  {
    try
    {
	  ...
	  //modified by Arun Andavar START
      mFromOrgId = getNumberParam("FromOrgId");
      mReportOnlyMode = getStringParam("ReportOnlyMode");
      mFromOrgType = getStringParam("FromOrgType");
      //modified by Arun Andavar END
...
//	  ((NullDBTransactionImpl) mRootAM.getTransaction()).connect( mConcurrentContext.getJDBCConnection() );
      ...
      mRootAM.getTransaction().setLockingMode( Transaction.LOCK_NONE );
		if(mC.get( "ModelOrganization" )!=null)
...
if(mC.get( "ModelOrganization" )!=null)
	  ...
	  mCpFile.writeToLog("End of initialize()");
    }catch(Exception e)
    {
      e.printStackTrace();
      throw e;
...
*/
  ...
  public void runProgram( CpContext pCpContext )
  {
    ...
    try
    {
      initialize();
     /** modified by Arun Andavar START
      * OD Custom Concurrent Program Parameters
      */
      OutFile outFile = mConcurrentContext.getOutFile();
      mCpOutFile = new ShipNetOut(outFile,1);

      ShippingNetworksMain();
      //Statistics of records created/formed
      if("N".equals(mReportOnlyMode))
      {
          if (mFromOrgId != null)
         {
		   OracleCallableStatement st = null;
           try
           {
             String query = "begin select name into :1 from hr_all_organization_units where organization_id = :2; end;";
             st =  (OracleCallableStatement) ((DBTransaction) mRootAM.getTransaction()).createCallableStatement( query, 1 );
             st.registerOutParameter(1,Types.VARCHAR);
             st.setInt (2,mFromOrgId.intValue());
             st.executeUpdate();
             mFromOrgName = st.getString(1);
            }
            catch(SQLException se)
            {
             throw se;
	      ...
	      if (mFromOrgType != null && mFromOrgType.trim() != "")
	      {
	        mFromOrgType = mFromOrgType;
	      }
	      else
	      {
			  mFromOrgType = "";
		  }
         mCpOutFile.writeToOut("===========");
         mCpOutFile.writeToOut("Parameters: ");
         mCpOutFile.writeToOut("===========");
         mCpOutFile.writeToOut(" ");
         mCpOutFile.writeToOut("1. Report only mode  =  N");
         mCpOutFile.writeToOut("2. Source org type   = "+mFromOrgType);
         mCpOutFile.writeToOut("3. From Organization = "+mFromOrgName);
         mCpOutFile.writeToOut(" ");
         mCpOutFile.writeToOut("------------------------------");
         mCpOutFile.writeToOut("Summary from Creation Process:");
         mCpOutFile.writeToOut("------------------------------");
         mCpOutFile.writeToOut("[Note: If any exceptions in Creation process then this summary differ from the validation process summary]");

       if (isExceptNonTrade)
       {
          mCpOutFile.writeToOut("Total Number of shipping networks formed(except NON-TRADE)                   : "+mNetExceptNonTradeFormed);
          mCpOutFile.writeToOut("Total Number of shipping networks got created(except NON-TRADE)              : "+mNetExceptNonTradeCreated);
          mCpOutFile.writeToOut("Total Number of shipping networks not got created(except NON-TRADE)          : "+mNetExceptNonTradeFailed);
       }
       if (isNonTrade)
       {
          mCpOutFile.writeToOut("Total number of NON-TRADE shipping networks formed                    : "+mNetNonTradeFormed);
          mCpOutFile.writeToOut("Total number of NON-TRADE shipping networks got created               : "+mNetNonTradeCreated);
          mCpOutFile.writeToOut("Total number of NON-TRADE shipping networks not got created           : "+mNetNonTradeFailed);
       }
      }

      mCpFile.writeToLog("Shipping networks program completed successfully");
    }
    catch( Exception e )
    {
      //Added by Arun Andavar -- START

      if (mErrorCode != null)
      {
        if (mErrorCode.intValue() == -1)
        {
          lRC.setCompletion( ReqCompletion.ERROR, mErrorMessage  );
          mCpFile.writeToLog("Error occurred in PL/SQL Pre-Build: "+mErrorMessage);
    	 }
        else if (mErrorCode.intValue() == 1)
		 {
          lRC.setCompletion( ReqCompletion.WARNING, mErrorMessage  );
          mCpFile.writeToLog("Warning from PL/SQL Pre-Build: "+mErrorMessage);
         }
	   }//if (mErrorCode != null)
	   else
	   {
       ...
       //  logApiErrors(isRequestStatusError,lRC);
         ...
         lRC.setCompletion( ReqCompletion.ERROR, mErrorMessage);
         mCpFile.writeToLog(mErrorMessage);

	   }
	   //Removed other copy loader specific messages
       //Added by Arun Andavar -- START
...
//      mFilterParamStylesheet.resetParams();
  ...
  //Added by Sarah --START
private void ShippingNetworks() throws Exception
  {
    mCpFile.writeToLog("Start of ShippingNetworks()");
    try
    {
      //--------------Export -----------------------
	  /* Modified for R12 Upgrade Defect # 26320 START*/
      //String xinterOutbound  = exportAPI("ShippingNetworks","oracle.apps.inv.structures.","FROM_ORGANIZATION_CODE");
	  String xinterOutbound  = exportAPI("ShippingNetworks","oracle.apps.inv.structures.");
      //xinterOutbound = "<ShippingNetworksAM ID='BE' MIN_API_VERSION='115.3' MIN_API_PATCH=''><InterorgParametersVO ID='VIEW'><InterorgParametersVO><FromOrganizationCode>XXX</FromOrganizationCode><ToOrganizationCode>XXX</ToOrganizationCode><IntransitType>XXX</IntransitType><MatlInterorgTransferCode>XXX</MatlInterorgTransferCode><FobPoint>XXX</FobPoint><RoutingHeaderId>XXX</RoutingHeaderId><InternalOrderRequiredFlag>XXX</InternalOrderRequiredFlag><ElementalVisibilityEnabled>XXX</ElementalVisibilityEnabled><InterorgReceivablesAccountFlex>XXX</InterorgReceivablesAccountFlex><InterorgTransferCrAccountFlex>XXX</InterorgTransferCrAccountFlex><IntransitInvAccountFlex>XXX</IntransitInvAccountFlex><InterorgPayablesAccountFlex>XXX</InterorgPayablesAccountFlex><InterorgPriceVarAccountFlex>XXX</InterorgPriceVarAccountFlex><ManualReceiptExpense>XXX</ManualReceiptExpense></InterorgParametersVO></InterorgParametersVO></ShippingNetworksAM>";
	  xinterOutbound = "<ShippingNetworksAM ID='BE' MIN_API_VERSION='120.3' MIN_API_PATCH=''><InterorgParametersVO ID='VIEW'><InterorgParametersVO><FromOrganizationCode>XXX</FromOrganizationCode><ToOrganizationCode>XXX</ToOrganizationCode><IntransitType>XXX</IntransitType><MatlInterorgTransferCode>XXX</MatlInterorgTransferCode><FobPoint>XXX</FobPoint><RoutingHeaderId>XXX</RoutingHeaderId><InternalOrderRequiredFlag>XXX</InternalOrderRequiredFlag><ElementalVisibilityEnabled>XXX</ElementalVisibilityEnabled><InterorgReceivablesAccountFlex>XXX</InterorgReceivablesAccountFlex><InterorgTransferCrAccountFlex>XXX</InterorgTransferCrAccountFlex><IntransitInvAccountFlex>XXX</IntransitInvAccountFlex><InterorgPayablesAccountFlex>XXX</InterorgPayablesAccountFlex><InterorgPriceVarAccountFlex>XXX</InterorgPriceVarAccountFlex><ManualReceiptExpense>XXX</ManualReceiptExpense></InterorgParametersVO></InterorgParametersVO></ShippingNetworksAM>";
      mConcurrentContext.getJDBCConnection().commit();	  
	  /* Modified for R12 Upgrade Defect # 26320 END*/

      //--------------Transform -----------------------
      XMLDocumentFragment df1 = changeElementData(xinterOutbound,"FromOrganizationCode",mSourceOrgCode);
      df1 = changeElementData(df1,"ToOrganizationCode",mTargetOrgCode);
      df1 = changeElementData(df1,"IntransitType",mIntransitType.toString());
      df1 = changeElementData(df1,"MatlInterorgTransferCode",mInterorgTransferCode.toString());
      //FOB point will be null in case of intrasit type = direct
      if(mFobPoint != null && mFobPoint.toString().trim()!=""){
            df1 = changeElementData(df1,"FobPoint",mFobPoint.toString());
      }
      else{
         df1 = removeElementFromXML(df1,"FobPoint");
      }
      //Routing id will be null in case of intrasit type = direct
      if(mReceiptRoutingId != null && mReceiptRoutingId.toString().trim()!=""){
            df1 = changeElementData(df1,"RoutingHeaderId",mReceiptRoutingId.toString());
      }
      else{
         df1 = removeElementFromXML(df1,"RoutingHeaderId");
      }
      df1 = changeElementData(df1,"InternalOrderRequiredFlag",mInternalOrderRequiredFlag.toString());
      df1 = changeElementData(df1,"ElementalVisibilityEnabled",mElementalVisibilityEnabled);
      df1 = changeElementData(df1,"InterorgReceivablesAccountFlex",mInterorgReceivablesAccount);
      df1 = changeElementData(df1,"InterorgTransferCrAccountFlex",mInterorgTransferCrAccount);
      //Intransit account will be null in case of intrasit type = direct
      if(mIntransitInvAccount != null && mIntransitInvAccount.trim()!=""){
            df1 = changeElementData(df1,"IntransitInvAccountFlex",mIntransitInvAccount);
      }
      else{
         df1 = removeElementFromXML(df1,"IntransitInvAccountFlex");
      }
      df1 = changeElementData(df1,"InterorgPayablesAccountFlex",mInterorgPayablesAccount);
      //Price variance account will be null for all the avg costing organizations
      if(mInterorgPriceVarAccount != null && mInterorgPriceVarAccount.trim()!=""){
            df1 = changeElementData(df1,"InterorgPriceVarAccountFlex",mInterorgPriceVarAccount);
      }
      else{
         df1 = removeElementFromXML(df1,"InterorgPriceVarAccountFlex");
     }
      df1 = changeElementData(df1,"ManualReceiptExpense",mManualReceiptExpense);

      mCpFile.writeMessage("Input XML for insert after transformation of ShippingNetworks :","[STATEMENT]" );
      mCpFile.writeMessage(getStringFromDocFragment(df1) );
      //--------------Insert -----------------------
      try
      {
         importApi( df1 ,"ShippingNetworks") ;
		    ...
		    OracleCallableStatement st = null;
     /* Added for R12 Upgrade Defect # 26320  START*/			
		String query = "begin select 1 into :1 from mtl_interorg_parameters where from_organization_id = :2 and to_organization_id = :3; exception when no_data_found then null; end;";
		st =  (OracleCallableStatement) ((DBTransaction) mRootAM.getTransaction()).createCallableStatement( query, 1 );
	 /* Added for R12 Upgrade Defect # 26320  END*/		
           try
           {
		    /* commented for R12 Upgrade Defect # 26320*/
             //String query = "begin select 1 into :1 from mtl_interorg_parameters where from_organization_id = :2 and to_organization_id = :3; exception when no_data_found then :1 := 2; end;";
             //st =  (OracleCallableStatement) ((DBTransaction) mRootAM.getTransaction()).createCallableStatement( query, 1 );
             st.registerOutParameter(1,Types.NUMERIC);
             st.setInt (2,mSourceOrgId.intValue());
             st.setInt (3,mTargetOrgId.intValue());
             st.executeUpdate();

             int mExists = st.getInt(1);

             if (mExists == 1)
             {
              //Getting summary count START
              if ( ("NONTRADE".equals(mSourceOrgType)
                   )
                 )
              {
                 mNetNonTradeCreated = mNetNonTradeCreated + 1;
              }
              else if (!("NONTRADE".equals((mSourceOrgType)) && !("NONTRADE".equals((mSourceOrgType))
                                                              )
                      )
                    )
              {
                 mNetExceptNonTradeCreated = mNetExceptNonTradeCreated + 1;
              }
	          //Getting summary count END
		     }
		     //else if (mExists == 2)   //Commented for R12 Upgrade Defect # 26320
			 else
		     {
               if ("NONTRADE".equals(mSourceOrgType))
               {
                  mNetNonTradeFailed = mNetNonTradeFailed + 1;
               }
               else
               {
                  mNetExceptNonTradeFailed = mNetExceptNonTradeFailed + 1;
               }
      ...
      {//If any error while creation shipping networks get the failure count
         if ("NONTRADE".equals(mSourceOrgType))
         {
            mNetNonTradeFailed = mNetNonTradeFailed + 1;
         }
    ...
    {
      mCpFile.writeToLog("Unexpected error in Shipping Networks."+e.getMessage());
    }
    mCpFile.writeToLog("End of ShippingNetworks()");
  }

private void ShippingNetworksMain() throws Exception{
   GShipnetRec[][] shipNetRec = new GShipnetRec[1][];
   Number  [] errorCode       = null;
   String  [] errorMessage    = null;
   //Initializing statistics count variables
   mNetNonTradeFormed 		  = 0;
   mNetNonTradeCreated 		  = 0;
   mNetNonTradeFailed  		  = 0;
   mNetExceptNonTradeFormed   = 0;
   mNetExceptNonTradeCreated  = 0;
   mNetExceptNonTradeFailed   = 0;

  try{
   XxGiShipnetCreationPkg.preBuild((DBTransaction)mRootAM.getTransaction(),
                                    mReportOnlyMode,
                                    mFromOrgType,
                                    mFromOrgId,
                                    shipNetRec,
      ...
      GShipnetRec[] shipNetRecInfo = shipNetRec[0];
      for(int i=0;i<(shipNetRecInfo.length);i++)
      {
		 if (i == 0)
		 {
          mErrorMessage                = shipNetRecInfo[i].getErrorMessage();
          mErrorCode                   = shipNetRecInfo[i].getErrorCode();
           if (mErrorCode != null)
           {
            if (mErrorCode.intValue() == -1)
            {
         ...
         mSourceOrgType               = shipNetRecInfo[i].getSourceOrgType();
         mTargetOrgType               = shipNetRecInfo[i].getTargetOrgType();
         mSourceOrgNumber             = shipNetRecInfo[i].getSourceOrgNumber();
         mTargetOrgNumber             = shipNetRecInfo[i].getTargetOrgNumber();
         mSourceOrgId                 = shipNetRecInfo[i].getSourceOrgId();
         mTargetOrgId                 = shipNetRecInfo[i].getTargetOrgId();
         mSourceInvOrg                = shipNetRecInfo[i].getSourceInvOrg();
         mTargetInvOrg                = shipNetRecInfo[i].getTargetInvOrg();
         mShipnetCreate               = shipNetRecInfo[i].getShipnetCreate();
         mIntransitType               = shipNetRecInfo[i].getIntransitType();
         mFobPoint                    = shipNetRecInfo[i].getFobPoint();
         mInterorgTransferCode        = shipNetRecInfo[i].getInterorgTransferCode();
         mReceiptRoutingId            = shipNetRecInfo[i].getReceiptRoutingId();
         mInternalOrderRequiredFlag   = shipNetRecInfo[i].getInternalOrderRequiredFlag();
         mIntransitInvAccount         = shipNetRecInfo[i].getIntransitInvAccount();
         mInterorgTransferCrAccount   = shipNetRecInfo[i].getInterorgTransferCrAccount();
         mInterorgReceivablesAccount  = shipNetRecInfo[i].getInterorgReceivablesAccount();
         mInterorgPayablesAccount     = shipNetRecInfo[i].getInterorgPayablesAccount();
         mInterorgPriceVarAccount     = shipNetRecInfo[i].getInterorgPriceVarAccount();
         mElementalVisibilityEnabled  = shipNetRecInfo[i].getElementalVisibilityEnabled();
         mManualReceiptExpense        = shipNetRecInfo[i].getManualReceiptExpense();
         mSourceOrgCode               = shipNetRecInfo[i].getSourceOrgCode();
         mTargetOrgCode               = shipNetRecInfo[i].getTargetOrgCode();
         ...
         if("N".equals(mReportOnlyMode))
         {
           if ("NONTRADE".equals(mSourceOrgType))
           {
			   isNonTrade = true;
               //Getting summary count START
    		   mNetNonTradeFormed = mNetNonTradeFormed + 1;
           	   //Getting summary count END

               if("Y".equals(mShipnetCreate))
               {
                  ShippingNetworks();
               }//if("Y".equals(mShipnetCreate))
               else
           ...
           {   isExceptNonTrade = true;
    		   mNetExceptNonTradeFormed = mNetExceptNonTradeFormed + 1;
               if("Y".equals(mShipnetCreate))
               {
                  ShippingNetworks();
               }//if("Y".equals(mShipnetCreate))

...
/**
 * ==========================================================================================================================================================
 *  Name:  TrMaDropDeducNonDedInquirySearchBean.java
 *
 *  Description : This TrMaDropDeducNonDedInquirySearchBean has methods related validations of adjustment criteria jsff for Trade Match chargeback summary report.
 *
...
public class TrMaDropDeducNonDedInquirySearchBean implements Serializable {

    private static ADFLogger logger = ADFLogger.createADFLogger(TrMaDropDeducNonDedInquirySearchBean.class);
    ...
    private RichInputDate glFromDate;
    private RichInputDate glToDate;
    private RichPanelFormLayout dateRegionBind;
    private RichInputListOfValues invoiceNumBind;
    private RichInputListOfValues bindSupNum;
    private RichSelectOneRadio dateTypeBind;


    public TrMaDropDeducNonDedInquirySearchBean() {
    }
    ...
    public void deduInqSearch() {
        ...
        if ((fromDateBind.getValue() == null && toDateBind.getValue() == null) &&
            (glFromDate.getValue() == null && glToDate.getValue() == null) && invoiceNumBind.getValue()==null) {

            errMsg = "Please provide Invoice # or From and To Date Range”";
            FacesMessage errorMsg = new FacesMessage(FacesMessage.SEVERITY_ERROR, errMsg, null);
            ...
            System.err.println("<>>>>>>>>>repOptBind.getValue()"+repOptBind.getValue());


            if ("SUM".equals(repOptBind.getValue())) {

                OperationBinding opBindSearchCharBack = ADFUtils.findOperation("searchTraMatDedInq");
                opBindSearchCharBack.getParamsMap().put("trMaDedInqSearchMap", searchMap);
                opBindSearchCharBack.execute();


                DCIteratorBinding iteratMainValue = ADFUtils.findIterator("XxApDropDeducNonDedInqSumVOIterator");
            ...
            if ("DTL".equals(repOptBind.getValue())) {


                OperationBinding opBindSearchCharBack = ADFUtils.findOperation("searchTraMatDedDtlInq");
                opBindSearchCharBack.getParamsMap().put("trMaDedInqSearchMap", searchMap);
                opBindSearchCharBack.execute();

                DCIteratorBinding iteratSubValue = ADFUtils.findIterator("XxApDropDeducNonDedInqDtlVOIterator");
                iteratSubValue.executeQuery();
                recordCount = iteratSubValue.getRowSetIterator().getRowCount();

                if (recordCount == 1) {

                    if (iteratSubValue.getCurrentRow().getAttribute("SupplierName") == null) {
    ...
    public String clearSearch() {
        ...
        glFromDate.setSubmittedValue(null);
        glToDate.setSubmittedValue(null);
        invoiceNumBind.setSubmittedValue(null);
        bindSupNum.setSubmittedValue(null);
        dateTypeBind.setSubmittedValue(null);
        
          repOptBind.setSubmittedValue(null);
        reporOptSelItemBind.setValue(null);
        DCIteratorBinding orderSearchIterBind = (DCIteratorBinding) getBindings().get("XxApDropDeducNonDedInqSearchVOIterator");
        XxApDropDeducNonDedInqSearchVORowImpl row = (XxApDropDeducNonDedInqSearchVORowImpl) orderSearchIterBind.getCurrentRow();
        row.remove();
        XxApDropDeducNonDedInqSearchVORowImpl newRow =
            (XxApDropDeducNonDedInqSearchVORowImpl) orderSearchIterBind.getRowSetIterator().createRow();
    ...
    public void viewChargeBkDtlActListener(ActionEvent actionEvent) {
        ...
        oracle.jbo.domain.Number supplierSiteId =
            (oracle.jbo.domain.Number) sourceLink.getAttributes().get("VendorSiteIdVal");
    ...
    public void reportOptionValueChange(ValueChangeEvent valueChangeEvent) {
        ...
        if ("SUM".equals(valueChangeEvent.getNewValue())) {


            vendAssMenuItem.setRendered(true);
            venMenuItemBind.setRendered(false);
        }
        if ("DTL".equals(valueChangeEvent.getNewValue())) {
    ...
    public void setGlFromDate(RichInputDate glFromDate) {
        this.glFromDate = glFromDate;
    }

    public RichInputDate getGlFromDate() {
        return glFromDate;
    }

    public void setGlToDate(RichInputDate glToDate) {
        this.glToDate = glToDate;
    }

    public RichInputDate getGlToDate() {
        return glToDate;
    }

    public void dateTypeChangeList(ValueChangeEvent valueChangeEvent) {
        
        
  
        ADFUtils.setBoundAttributeValue("Invdaterangfrom", null);
        ADFUtils.setBoundAttributeValue("Invdaterangto", null);
        ADFUtils.setBoundAttributeValue("Gldaterangfrom", null);
        ADFUtils.setBoundAttributeValue("Gldaterangto", null);
        AdfFacesContext.getCurrentInstance().addPartialTarget(dateRegionBind);
        AdfFacesContext.getCurrentInstance().addPartialTarget(searchAreaBind);
    }

    public void setDateRegionBind(RichPanelFormLayout dateRegionBind) {
        this.dateRegionBind = dateRegionBind;
    }

    public RichPanelFormLayout getDateRegionBind() {
        return dateRegionBind;
    }


    public void setInvoiceNumBind(RichInputListOfValues invoiceNumBind) {
        this.invoiceNumBind = invoiceNumBind;
    }

    public RichInputListOfValues getInvoiceNumBind() {
        return invoiceNumBind;
    }

    public void setBindSupNum(RichInputListOfValues bindSupNum) {
        this.bindSupNum = bindSupNum;
    }

    public RichInputListOfValues getBindSupNum() {
        return bindSupNum;
    }

    public void setDateTypeBind(RichSelectOneRadio dateTypeBind) {
        this.dateTypeBind = dateTypeBind;
    }

    public RichSelectOneRadio getDateTypeBind() {
        return dateTypeBind;
    }
}

...
CURSOR lcu_dept(lc_trx_number VARCHAR2) IS
  SELECT DISTINCT sku_dept
  FROM xx_om_line_attributes_all OLAL,
    ...
    IF(ln_invoice_amount >= lcu_promo_rec.minimum_amount) THEN
       FOR lcu_dept_rec IN lcu_dept(lcu_inv_num_rec.trx_number)
        ...
        BEGIN
            ...
            AND   APD.promo_values IN(lcu_dept_rec.sku_dept)
            AND   APD.promo_column LIKE 'Dep%';
            DBMS_OUTPUT.PUT_LINE('Promo Dept for '|| lcu_dept_rec.sku_dept|| ' is ' || lc_promo_dept);
          EXCEPTION
          WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('No Promo Dept for '|| lcu_dept_rec.sku_dept);
            END;
        END LOOP;

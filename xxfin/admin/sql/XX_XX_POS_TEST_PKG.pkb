create or replace
PACKAGE BODY XX_XX_POS_TEST_PKG AS 
   -- +=====================================================================+
   -- | Name :                                                              |
   -- | Description :                                                       | 
...
--                                            WHERE XRILA.orig_system_bill_customer_id = OBJECTTABLE$(X)
   ...
   BEGIN  
      ...
      ---------------------------------------------
      DELETE xx_ra_int_distributions_all XRIDA
       WHERE EXISTS (SELECT 1
                       FROM xx_ra_int_lines_all XRI
                      WHERE XRI.interface_line_attribute1  = p_trx_number
      ...
      IS
         SELECT COUNT(1)          LINE_COUNT
           ...
           FROM applsys.fnd_concurrent_requests    FCR
               ...
               ,applsys.fnd_concurrent_requests    FCR2
               ,applsys.fnd_concurrent_programs    FCP2
               ,applsys.fnd_concurrent_requests    FCR3
               ,applsys.fnd_concurrent_programs    FCP3
               ,ar.ra_interface_lines_all          RILA
               ,ar.ra_cust_trx_types_all           RCTA
          WHERE FCR.parent_request_id        = gn_request_id
            AND FCR.program_application_id   = FCP.application_id
            AND FCP.concurrent_program_id    = FCR.concurrent_program_id
            AND FCR.request_id               = FCR2.parent_request_id
...
--               ,DECODE(XRID.account_class,'REC',NULL,XRIL.reason_code)                      -- removed for defect 12289
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT xxfin.XX_AR_SUMMARY_TRANS_NUM_S.nextval                    
           INTO ln_trx_number
           FROM dual;
      ...
      IS
         SELECT MIN(X.orig_system_bill_customer_id)  MIN_CUST_ID
          ...
          FROM (SELECT XRILA.orig_system_bill_customer_id, OTC.account_number
                  ...
                  FROM xxfin.xx_ra_int_lines_all    XRILA
                      ,xxfin.xx_ar_intstorecust_otc OTC

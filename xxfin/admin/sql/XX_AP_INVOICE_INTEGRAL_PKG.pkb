...
-- | 4.8         25/02/2021   Anmol Patil         Added check to validate Invoice/Ship Date     |
 --|                                                   NAIT-116748                              |
-- |4.9          06/01/2022   Taruna Baluni       Added Calendar for spin day1 		NAIT-205702	|
-- |                                                                                            |
-- +============================================================================================+
...
-- +============================================================================================+
-- |  Name	 : IsDate                                                                           |
-- |  Description: Function to validate Date ( Created for Version 4.8 )                 |
-- =============================================================================================|
...
-- Version 4.5 End
-- Version 4.8  Start
       		IF (isdate(CONCAT(CONCAT(l_table(8),l_table(7)),l_table(6)),'DDMMYY')=1 AND isdate(CONCAT(CONCAT(l_table(11),l_table(10)),l_table(9)),'DDMMYY')=1)
		THEN NULL;
		ELSE
		gn_bad_rec_flag:= 1;
		END IF;
	...
	-- To get the Email details
        ...
        BEGIN
            conn := xx_pa_pb_mail.begin_mail(sender => lc_email_from,
                                             ...
                                             --subject => lc_email_subject ,   --commented for Jira NAIT-87653
											 subject =>l_instance_name_1 ||': '|| lc_email_subject,    ---Added for NAIT-87653 to include instance name in email subject
		...
		LOOP
		    BEGIN
				...
				THEN
					...
					ELSIF lc_rec_type = '5'
					THEN
						IF (isdate(CONCAT(CONCAT(l_hdr_table(8),l_hdr_table(7)),l_hdr_table(6)),'DDMMYY')=1 AND isdate(CONCAT(CONCAT(l_hdr_table(11),l_hdr_table(10)),l_hdr_table(9)),'DDMMYY')=1)   -- Version 4.8
						THEN
							print_debug_msg ('Insert Header',FALSE);
							...
							ln_count_hdr := ln_count_hdr + 1;
						ELSE
						ln_log := ln_log || v_inv_num || '                     ' || v_ven_num || '                     ' || v_po_num ||'                             ' || gn_count   || '           ' || 'Invalid INVOICE/SHIP DATE'  || chr(10);  -- Version 4.8
						END IF;
					END IF;	-- lc_rec_type

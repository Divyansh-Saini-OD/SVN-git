...
AS
   ...
   -- |  4.2         2-Apr-2015     Mark Schmit          make payables_cost = ln_cost for defect #1011  |
   -- |  4.3         30-Oct-2015    Madhu Bolli          122 Retrofit - Remove schema     |
   -- +===================================================================================+
   -- +===================================================================================+
   -- | Name        :GET_DEFAULTS                                                         |
            ...
            LOOP
               BEGIN                             -- RULE ASSETPR18
                  ...
                  UPDATE XX_FA_MASS_ADDITIONS_STG -- ADDED 25-Mar-2015 from defect 976
                   ...
                   and   asset_number = lcu_dpis.asset_number
                         AND company_code = lcu_dpis.company_code;
                  COMMIT;
               EXCEPTION
                  WHEN OTHERS
      ...
      THEN
         ...
         LOOP
          ...
          -- DBMS_OUTPUT.PUT_LINE ('Step J: ');
           ...
           --   DBMS_OUTPUT.PUT_LINE ('Step L');                 -- RULE ASSETPR15
               ...
               THEN
                  SELECT PWC_INITIAL_TAX_COST
                   ...
                   WHERE     pwc_asset_nbr = lcu_process_records.attribute10
                         AND book = 'BK1';
           --        DBMS_OUTPUT.PUT_LINE ('Step M LT ' || ln_attribute7);                 -- RULE ASSETPR15
               ELSE
                  ln_attribute7 := lcu_process_records.fixed_assets_cost;
            --      DBMS_OUTPUT.PUT_LINE ('Step M GT' || ln_attribute7);                 -- RULE ASSETPR15
               END IF;
            EXCEPTION
               WHEN NO_DATA_FOUND
               THEN
            ...
            --  DBMS_OUTPUT.PUT_LINE ('Step N ' || to_date(lcu_process_records.date_placed_in_service, 'DD-MON-RR'));                 -- RULE ASSETPR15
            ...
            BEGIN
               ...
               THEN
                  SELECT TO_DATE (pwc_in_service_date, 'MM/DD/RRRR')
                   ...
                   WHERE     pwc_asset_nbr = lcu_process_records.attribute10
                         AND book = 'BK1';
               --     DBMS_OUTPUT.PUT_LINE ('Step N2 ' || ln_attribute8);                 -- RULE ASSETPR15
               ELSE
                  ln_attribute8 := lcu_process_records.date_placed_in_service;
              --  DBMS_OUTPUT.PUT_LINE ('Step N3 ' || ln_attribute8);            -- RULE ASSETPR15
               END IF;
            EXCEPTION
               WHEN NO_DATA_FOUND
               THEN
           ...
           -- DBMS_OUTPUT.PUT_LINE ('Step OO: ');
               ...
               UPDATE xx_fa_mass_additions_stg
                  SET calc_amortization_start_date =
                      ...
                      payables_cost = ln_cost,         -- RULE ASSETPR20

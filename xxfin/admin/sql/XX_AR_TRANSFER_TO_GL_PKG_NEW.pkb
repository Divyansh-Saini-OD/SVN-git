...
CREATE OR REPLACE PACKAGE BODY XX_AR_TRANSFER_TO_GL_PKG_NEW
-- +===================================================================+
...
BEGIN
        ...
        LOOP
          ...
          ELSE
            ...
            UPDATE  /*+ index(gl_interface xx_gl_interface_n1) */ gl_interface
               SET  reference1            = ln_parent_request_id||'~'||group_id
  ...
  --we are not submitting the Journal import program as the child program.--
     ...
     FND_FILE.PUT_LINE(FND_FILE.OUTPUT,            '*OFFICE DEPOT INC                                                                            Report Date: '||TO_CHAR(SYSDATE,'DD-MON-YYYY HH:MI')||'*');
     FND_FILE.PUT_LINE(FND_FILE.OUTPUT,            'Multithread Request ID                      : '||FND_GLOBAL.CONC_REQUEST_ID);
     FND_FILE.PUT_LINE(FND_FILE.OUTPUT,            'General Ledger Transfer Program Request ID  : '||p_req_id);
     ...
     FND_FILE.PUT_LINE(FND_FILE.OUTPUT,            'Set of Books Name                           : '||lc_set_of_books_name);
     FND_FILE.PUT_LINE(FND_FILE.OUTPUT, chr(10) || '****************************************************************************************************************************' || chr(10));

...
CURSOR lcu_dept(lc_trx_number VARCHAR2) IS
   SELECT DISTINCT MIS.secondary_inventory
...
CURSOR lcu_location(lc_trx_number VARCHAR2) IS
   SELECT DISTINCT RCTL.attribute6
   FROM  ra_customer_trx_lines_all RCTL
        ,ra_customer_trx_all RCT
   WHERE RCTL.customer_trx_id = RCT.customer_trx_id
   AND RCT.trx_number = lc_trx_number
   AND RCTL.attribute6 IS NOT NULL;
   ...
   FOR lcu_card_num_rec IN lcu_card_num(lc_rec_num)
    LOOP
      ...
      WHEN NO_DATA_FOUND THEN
        ...
        lc_error_loc  := 'When Obtaining the Invoice Amount';
        fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
        dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
      END;
    IF(ln_invoice_amount >= lcu_promo_rec.minimum_amount) THEN
        ...
        LOOP
          BEGIN
            ...
            AND   APD.promo_values IN(lcu_dept_rec.secondary_inventory)
            AND   APD.promo_column LIKE 'Dep%';
            DBMS_OUTPUT.PUT_LINE('Promo Dept for '|| lcu_dept_rec.secondary_inventory|| ' is ' || lc_promo_dept);
          EXCEPTION
          WHEN NO_DATA_FOUND THEN
            lc_error_code := SQLCODE;
            lc_error_msg  := SQLERRM;
            lc_error_loc  := 'When fetching the promo departments';
            fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
         END LOOP;
         ...
         BEGIN
          ...
          WHEN NO_DATA_FOUND THEN
            lc_error_code := SQLCODE;
            lc_error_msg  := SQLERRM;
            lc_error_loc  := 'When fetching the promo skus';
            fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
          END;
          END LOOP;
          ...
          LOOP
            BEGIN
              ...
              WHERE  APH.promo_id = APD.promo_id
               ...
               AND   APD.promo_values IN(lcu_location_rec.attribute6);
               DBMS_OUTPUT.PUT_LINE('Promo Location for '|| lcu_location_rec.attribute6 || 'is' || lc_promo_location);
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
              lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the promo location';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            END;
            BEGIN
              ...
              WHERE  FFVN.flex_value_set_id = (SELECT FFVL.flex_value_set_id
                                               ...
                                               WHERE  FFVL.flex_value = lcu_location_rec.attribute6)
              AND    FFVN.child_flex_value_low = lcu_location_rec.attribute6
              AND    FFVN.range_attribute = 'C';
              DBMS_OUTPUT.PUT_LINE('District for '|| lcu_location_rec.attribute6 || 'is' || lc_district);
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
              lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the district';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            END;
            BEGIN
              ...
              DBMS_OUTPUT.PUT_LINE('Region for '|| lcu_location_rec.attribute6 || 'is' || lc_region);
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
              lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the region';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            END;
            BEGIN
            ...
            WHEN NO_DATA_FOUND THEN
              lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the promo district';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            END;
            BEGIN
              ...
              WHERE  APH.promo_id = APD.promo_id
               ...
               DBMS_OUTPUT.PUT_LINE('Promo Region for '|| lcu_location_rec.attribute6 ||'is' || lc_promo_region);
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
              lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the promo region';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
            END;
          END LOOP;
...
WHEN OTHERS THEN
   lc_error_code := SQLCODE;
              lc_error_msg  := SQLERRM;
              lc_error_loc  := 'When fetching the promo code';
              fnd_file.put_line(fnd_file.log, 'Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);
              dbms_output.put_line('Oracle Error '||lc_error_msg||' detected at '||lc_error_loc);

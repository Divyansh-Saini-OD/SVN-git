CREATE OR REPLACE
PACKAGE BODY XX_OD_AR_LBX_WRAPPER_PKG
AS
gRequestId   NUMBER    DEFAULT FND_GLOBAL.CONC_REQUEST_ID;
PROCEDURE XX_OD_AR_LBX_WRAPPER_PRC(
...
l_mail_subject VARCHAR2(40) := 'MEC Update LBX Wrapper Program';
l_sender_address VARCHAR2(30) :='noreply@officedepot.com';
l_mail_body   VARCHAR2(100) := 'Hi, '||
'Please find the attached report of Updated LBX Wrapper';
...
BEGIN
		...
		fnd_file.put_line(fnd_file.output,rpad('OD MEC Update LBX Wrapper Program',50,' '));
		...
		fnd_file.put_line(fnd_file.output,rpad('+-',99,'-')||'+');
		fnd_file.put_line(fnd_file.output,' ');
		fnd_file.put_line(fnd_file.output,rpad('Updated Table',49,' ')||': '||'XXFIN.XX_AR_LBX_WRAPPER_TEMP');
...
END XX_OD_AR_LBX_WRAPPER_PRC;

END XX_OD_AR_LBX_WRAPPER_PKG;
/
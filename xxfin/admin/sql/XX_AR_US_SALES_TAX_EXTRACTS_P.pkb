...
create or replace
PACKAGE BODY XX_AR_US_SALES_TAX_EXTRACT_P AS
-- +===================================================================+
...
-- |Version 1.6 14-MAY-09   Ranjith         Perf fix POC               | 
-- +===================================================================+
-- +===================================================================+
-- | Name : TAX_EXTRACT                                                |
  ...
  IS
         SELECT c_company_seg
            ...
            --   ,c_ship_from_location            -- Commented for defect 13446
               ...
               ,SUM(c_exempt_tax_amount)                         c_exempt_tax_amount
         FROM (
                       SELECT /*+ LEADING(TYPES TRX DIST) NO_EXPAND FULL(TRX) FULL(TYPES) FULL(DIST) FULL(TAXDIST) FULL(GLTAX) FULL(TAX) FULL(LINE) FULL(SU) FULL(ACCT_SITE) FULL(PARTY_SITE) FULL(LOC) */  -- Hint added for POC
...
ln_limit_size := NVL(p_limit_size ,10000);
  ...
  lc_trx_id_to   := lt_trx_id(lt_trx_id.last);
   ...
   ln_request_id         := FND_REQUEST.SUBMIT_REQUEST(application      => 'XXFIN'
                                                      ,program          => 'XX_AR_US_SALES_TAX_EXT_P'
                                                      ,description      => ''
                                                      ,sub_request      => TRUE
                                                      ,argument1     => p_company

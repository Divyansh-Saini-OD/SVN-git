...
BEGIN
  ...
  /* Populate GL_INTERFACE_NA_STG table from global temporary table */
  fnd_file.put_line(FND_FILE.LOG, ' ');
  fnd_file.put_line(FND_FILE.LOG, '------ Populate GL_INTERFACE_NA_STG table from global temporary table ------');
        INSERT INTO xxfin.xx_gl_interface_na_stg    --gl_interface(
           (status,
            date_created,
            created_by,
            actual_flag,
        ...
        fnd_file.put_line(FND_FILE.LOG, 'GL_INTERFACE_NA_STG: Rows inserted = '||to_char(sql%rowcount));

CREATE OR REPLACE PACKAGE BODY APPS.XX_AP_ESCHEAT_PKG
AS
  -- +==================================================================================================+
  ...
  AS
    ...
    BEGIN
      ...
      END IF;
       ...
       INSERT INTO xxfin.XX_AP_ESCHEAT_PAR_PMT_HIS
          (ORG_DOC_NUM
          ,ORG_CHECK_ID
          ,ORG_PMT_DATE
          ,ORG_PAY_AMT
       ...
       ------------------------------------------------------------------------
       INSERT INTO xxfin.XX_AP_ESCHEAT_PAR_INV_HIS
          (ORG_CHECK_ID
          ,ORG_INV_PAY_TERMS
          ,ORG_INV_PAY_GROUP
          ,INVOICE_TYPE
    ...
    BEGIN
        ...
        OPEN  UPDATE_CLR_INFO;
          ...
          EXIT WHEN UPDATE_CLR_INFO%NOTFOUND;
             ...
             -----------------------------------------------------------
             lc_current_step  := 'UPDATE XXFIN.XX_AP_ESCHEAT_PAR_PMT_HIS';
             -----------------------------------------------------------
             UPDATE XXFIN.XX_AP_ESCHEAT_PAR_PMT_HIS
...
-- +===================================================================+
   ...
   END SEND_EMAIL_OUTPUT;
    ...
    FOR void_type IN (SELECT DISTINCT PH.VOID_TYPE
                      FROM xxfin.XX_AP_ESCHEAT_PAR_PMT_HIS PH
                          ,xxfin.XX_AP_ESCHEAT_PAR_INV_HIS IH
                          ,apps.AP_INVOICES AI
                          ,apps.AP_INVOICE_PAYMENTS IP
                          ,apps.AP_CHECKS_V AC
          ...
          IF ((lc_dev_phase1 = 'COMPLETE')
               ...
               IF ln_conc_id1 > 0  THEN
                  ...
                  IF ln_conc_id2 > 0  THEN
                      ...
                      lb_prt_result := apps.FND_REQUEST.SET_PRINT_OPTIONS
                                              ('XPTR'         --printer name
                                               ,'LANDSCAPE'   --style
                                               ,1
                                               ,TRUE
             ...
             ---- Commented By Sravanthi on 8/7/2013 as part of R12 Retrofit             -----------
                     /* -------------------------------------------------------
                      ...
                      --------------------------------------------------------
                      ln_pub_request_id := apps.fnd_request.submit_request

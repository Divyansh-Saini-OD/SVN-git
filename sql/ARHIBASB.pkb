 ...
 * FILE       ARHIBASB.pls
 *
 * PACKAGE    HZ_BATCH_IMPORT_PKG
 *
 * DESCRIPTION
 *     This package contains non-public API for batch import 
 *
 * RELATED PACKAGES
 *
 * PUBLIC VARIABLES
 ...
 *   19-JAN-2005    V.Ravichandran      o Bug 4079902. changes made for 
 *                                        SSM-SST Integration. Modified the 
 *                                        procedure Import_batch to
 *                                        comment code which validates
 *                                        the value for orig_system from 
 *                                        lookup and instead validated it 
 *                                        from the table HZ_ORIG_SYSTEMS_B.
 *   13-MAR-2006    V.Ravichandran      o Bug 5091668. 
 *   19-APR-2006    V.Ravichandran      o Bug 5171854. Called the DQM Sync program
...
CREATE OR REPLACE PACKAGE BODY HZ_BATCH_IMPORT_PKG AS
/*$Header: ARHIBASB.pls 115.23.115104.3 2006/04/19 12:34:28 vravicha noship $ */
...
BEGIN
    ...
    IF p_import_run_option = 'COMPLETE' OR p_import_run_option = 'CONTINUE' THEN
      ...
      IF NVL(r_batch_info.registry_dedup_flag,'N') = 'Y' THEN
        ...
        */
        ...
        -- determine whether the dedup rule specify automerge
        ...
        ------------------------------------------
        IF nvl(l_automerge_flag,'N') = 'Y' THEN
    ...
    IF l_req_data = 'DATA_LOAD' THEN
      ...
      */
      IF NVL(r_batch_info.registry_dedup_flag,'N') = 'Y' THEN
        ...
        INTO   l_automerge_flag
        FROM   hz_match_rules_b
        WHERE  match_rule_id = r_batch_info.registry_dedup_match_rule_id;
  ...
  IF l_req_data = 'POST_PROCESS' THEN
   ...
   /* Call DQM Sync concurrent program to synchronize indexes. */
...
if (NVL(FND_PROFILE.VALUE('XX_CDH_CONV_ENABLE_DQM_IN_POSTPROCESSING'), 'N') = 'Y') then
    ...
    l_dqm_sync_request :=
        ...
        IF l_dqm_sync_request = 0 THEN
          fnd_file.put_line(FND_FILE.LOG, 'UIC***** Error submitting DQM sync ' || l_num_of_workers);
        ELSE
          fnd_file.put_line(FND_FILE.LOG, 'UIC***** Submitted request for DQM Sync' || l_num_of_workers);
        END IF;

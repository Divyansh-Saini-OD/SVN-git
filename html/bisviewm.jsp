 <%! public static final String RCS_ID = "$Header: bisviewm.jsp 115.208 2007/04/16 10:02:35 ashgarg noship $"; %>F
 <%!
 /**
  +===========================================================================+
  ...
  |                                                                           |
  |  DESCRIPTION                                                              |
  |            Main jsp to render reports/portlets                            |
  |                                                                           |
  |  HISTORY                                                                  |
  |   Date          Developer     Description                                 |
  |   19/11/2002     nbarik       Bug Fix 2439795 Hide copyright for PRINTABLE|
  |   November 29, 2002    ansingh   Error Logging Enhancement                |
  |   December 12, 2002    nkishore  Bug Fix 2693443--ApplyLivePortlet fails  |
  |   December 23, 2002    nkishore  Portlet Customizations ApplyLivePortlet  |
  |                                    handled for db portlets also           |
  |   February 11, 2003    nkishore  Related Links New UI BugFix 2784178      |
  |   March 10, 2003       ansingh				 QueryBuilder Enhancement            |
  |   03/24/03       nbarik      Bug Fix 2809833 - Show global buttons        |
  |   04/21/03       nkishore    Bug Fix 2953393 -- added  PMVUtil.getPageId  |
  |   06/27/03       rcmuthuk    Bug Fix 3019530 -- moved saveParamBean.init  |
  |                              above setProperty tag                        |
  |   07/07/03       rcmuthuk    Bug Fix 3038609 -- moved handling PERSONALIZE|
  |                              above header, BugFix: 3038618
  |   05/21/03       ksadagop    Export to PDF Enhancement#2944720            |
  |   06/17/03       ksadagop    BugFix 3050837 - Dynamic Header/Footer Report|
  |   07/28/03       rcmuthuk    Added Debug Messages
  |   08/04/03       nbarik      Bug Fix 3081564                              |
  |   08/21/03       nbarik      Bug Fix 3099831 - delay function access check|
  |   09/01/03       nbarik      Bug Fix 3122691 - Enable SQL Trace for ERROR |
  |   12/09/03       nbarik     Enh 3252288                                   |
  |   12/19/03       nkishore    BugFix 3314027                               |
  |   01/13/04       nbarik      Bug Fix 3267597 Include bispmvft.jsp         |
  |   01/21/04       nkishore    BugFix--3387377                              |
  |   ppalpart  22/01/04          Bug Fix: 3356159 Lov BLAF compliance         |
  |   04/13/04       nbarik      Bug Fix 3549494 - No security access for Lov report|
  |   05/13/04       ashgarg     Bug Fix 3620033 - OA Compliance MAC Key Implementation |
  |   06/18/04       ashgarg     Bug Fix 3690658 - fromPersonalize check      |
  |   06/23/04       ansingh     KPIPortletCustomization                      |
  |   07/05/04       ppalpart    Call ServletWrapper.getRequestURL instead of |
  |                              HTTPUtils.getRequestURL                      |
  |   07/22/04       ksadagop    BugFix#3645793 Custom Lov Fix                |
  |   08/13/04       ashgarg     Bug Fix:3823820                              |
  |   09/02/04       ksadagop    BugFix#3862069                               |
  |   09/08/04       ppalpart    Bug Fix: 3875740                             |
  |   10/13/04       ugodavar    Enh.3946492 - pMaxResultSetSize & pOutputFormat  |
  |   10/18/04       ppalpart    Bug Fix 3949416                              |
  |   01/18/05       smargand    Enh #4031435  - Report XML Definition        |
  |   08/29/05       ugodavar    Bug #4576358  - iFrame not getting displayed |
  |   10/09/05       ugodavar    Bug #4141392  BOOKMARKING SHOULD NOT RELY ON |
  |                                 SERVER SIDE REDIRECT                      |
  |  04/16/07    ashgarg  Bug#5471847                                         |
  +===========================================================================+
  **/
  %><%@
   page  import="oracle.apps.fnd.common.*"
   import="java.sql.*"
  import="od.oracle.apps.xxcrm.bis.pmv.report.ODReportPageBean"
 ...
 %><%
   ...
   try
   {
     ...
     if (userSession.isSONAR()) {
       ODReportPageBean reportPageBean = new ODReportPageBean(userSession);
       ...
       ODReportPageBean reportPageBean = new ODReportPageBean();
   ...
   if (userSession.isExportMode()) {
         ...
         if(!StringUtil.emptyString(pOutputFormat) && !"RELATED".equals(pMode) && !"DRILL".equals(pMode))
         {
           ...
           if (!useWebADI) // the usual way of handling Excel download
           {
			 ...
			 try 
		{
       		flagprof = webAppsContext.getProfileStore().setProfile("VO_MAX_FETCH_SIZE",webAppsContext.getProfileStore().getProfile("XXBI_MAX_FETCH_SIZE"));
		}
		catch(Exception _ex) { }
    ...
    <%   }
       ...
       <P>This product requires use of a browser that supports JavaScript
        ...
        <%
        	...
        	String isFirstTime = request.getParameter("pFirstTime");
        	if("0".equals(isFirstTime)){
        	if((selectedTab == null) || ("".equals(selectedTab)))
        	   if(pageContext.getSession().getValue("smjSelectedTab")!=null)
        	        selectedTab = pageContext.getSession().getValue("smjSelectedTab").toString();
        	   else
        			selectedTab = "subTabId0";
			}
			if((selectedTab == null) || ("".equals(selectedTab)))
				selectedTab = "subTabId0";
        	pageContext.getSession().putValue("smjSelectedTab",selectedTab);

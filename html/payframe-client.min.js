...
var configFromMerchant;var payframeIsReady=false;var eventHandler=function(d){if(d.origin!=="https://request.securepaypage-litle.com"){return
}var b=JSON.parse(d.data);if(b.ready){payframeIsReady=true;var a=document.getElementById("vantiv-payframe").contentWindow;
configFromMerchant.action="configure";a.postMessage(JSON.stringify(configFromMerchant),"https://request.securepaypage-litle.com")
}else{if(b.height){var c=document.getElementById("vantiv-payframe");c.height=b.height}else{configFromMerchant.callback(b)
}}};if(window.addEventListener){window.addEventListener("message",eventHandler,false)}else{if(window.attachEvent){window.attachEvent("onmessage",eventHandler)
}}var LitlePayframeClient=function(f){if(f.htmlTimeout){setTimeout(function(){if(payframeIsReady===false){var g={response:"884",message:"PayFrame HTML failed to load"};
configFromMerchant.callback(g)}},f.htmlTimeout)}configFromMerchant=f;var e=configFromMerchant.div;var d=configFromMerchant.paypageId;
var b=configFromMerchant.style;var a=configFromMerchant.height;var c="api3";jQuery("#"+e).html("<iframe id='vantiv-payframe' name='vantiv-payframe' frameborder='0' src='https://request.securepaypage-litle.com/LitlePayPage/payframe_"+d+"_"+b+"_"+c+".html' height='"+a+"' width='100%'></iframe>");
return{getPaypageRegistrationId:function(h){h.action="getPaypageRegistrationId";h.paypageId=configFromMerchant.paypageId;
h.reportGroup=configFromMerchant.reportGroup;h.timeout=configFromMerchant.timeout;var g=document.getElementById("vantiv-payframe").contentWindow;
g.postMessage(JSON.stringify(h),"https://request.securepaypage-litle.com")},autoAdjustHeight:function(){var h={action:"getDocHeight"};
var g=document.getElementById("vantiv-payframe").contentWindow;g.postMessage(JSON.stringify(h),"https://request.securepaypage-litle.com")
}}};
/*json2.js*/

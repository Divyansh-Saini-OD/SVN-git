<!-- $Header: bispmvhd.jsp 115.57 2006/09/11 08:09:30 msaran noship $ -->
<%@
...
<HEAD>
<% if (!browserTitle.equals("header")) { %>
<TITLE><%=ODPMVUtil.getMessage(browserTitle,webAppsContext)%>
<% if(!StringUtil.emptyString(viewName))
{ %>
<%=viewName%>
<% } %>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" summary="">
  ...
  <% if(StringUtil.emptyString(globalTitle))
   { %>
  <%= Util.escapeGTLTHTML(browserTitle, "")%>
  <% if(!StringUtil.emptyString(viewName))
  { %>
  <%=viewName%>
  <% } } else {%>
  <%= Util.escapeGTLTHTML(globalTitle, "")%>
  <% } %>
  </span>
  <!--ksadagop Enh.4240831 - SaveAs Feature-->

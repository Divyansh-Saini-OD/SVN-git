...
<a href ="<%=IBUUtil.getURL("IBU_SR_DETAILS_NEW")+"&srID="+requestNumber%>"><img src="../XXCRM_HTML/images/bRequnkdE_Store.gif" border="0" align="absmiddle" width="116" height="18"></a>
<%
}
  }
%>
...
<%
        ...
        {
            itemInfo = nestedRegionItems.getItembyCode("IBU_CF_SR_DTL_OVERVIEW_TAB_RG", true);
            if(itemInfo != null){
               regionKey = 
               new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
            }else{
            ...
            nestedRegionItems   = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);
            out.println("<!--IBU_CF_SR_DTL_OVERVIEW_TAB_RG:"+(itemInfo == null ? "":itemInfo.getSubRegionCode())+"-->");
            out.println(RendererUtil.blankCell(4, null));
%>
            <%@ include file="ibuSRDetailsOverview.jsp"%>
<%
        }
        else if ("IBU_CF_SR_DTL_CONTACTS_TAB_RG".equals(sCurrentTab))
        {
            itemInfo = nestedRegionItems.getItembyCode("IBU_CF_SR_DTL_CONTACTS_TAB_RG", true);
            if(itemInfo != null){
            regionKey          
                = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
            }else{
            ...
            out.println("<!--IBU_CF_SR_DTL_CONTACTS_TAB_RG:" + (itemInfo ==null?"":itemInfo.getSubRegionCode())+"-->"); 
            nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);
            while(nestedRegionItems2.hasMoreItem()){
              itemInfo = nestedRegionItems2.getNextItem();
              if(!itemInfo.getShowFlag())
                 continue;
              if (itemInfo.getAttrCode().equals("IBU_CF_SR_ADDRESS_RG")){
                 regionKey  = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
                 out.println(RendererUtil.blankCell(4, null));
                 pageContext.setAttribute("IncludeAddressPage", regionKey, PageContext.REQUEST_SCOPE);
%>
             <jsp:include page="ibuSRDetailsAddresses.jsp" flush="true" />
<%
             continue;
            }
            if(itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_CONTACTS_RG")){
                 regionKey  = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
                 out.println(RendererUtil.blankCell(4, null));
                 pageContext.setAttribute("IncludeContactPage", regionKey, PageContext.REQUEST_SCOPE);
            
%>
            <jsp:include page="ibuSRDetailsContacts.jsp" flush="true" />
<%
             continue;
            }
           }
...
<%
        ...
        {
            itemInfo = nestedRegionItems.getItembyCode("IBU_CF_SR_DTL_RESOLN_TAB_RG", true);
            if(itemInfo != null){
              regionKey = 
                new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
            }else{
            ...
            out.println("<!--IBU_CF_SR_DTL_RESOLN_TAB_RG:"+(itemInfo==null?"":itemInfo.getSubRegionCode())+"-->");
            out.println(RendererUtil.blankCell(4, null));
            while (nestedRegionItems.hasMoreItem())
            {
                itemInfo = nestedRegionItems.getNextItem();
                ...
                if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_SOLN_RG"))
                {
                    regionKey           = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
                    nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);
                    out.println("<!--IBU_CF_SR_DTL_SOLN_RG:"+itemInfo.getSubRegionCode()+"-->");
                    out.println(RendererUtil.blankCell(4, null));
%>
                    <%@ include file="ibuSRDetailsSolutions.jsp"%>
<%
                }
                if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_ACTS_RG"))
                {
                    regionKey           = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
                  //  nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);

                    out.println(RendererUtil.blankCell(4, null));
                    pageContext.setAttribute("IncludeResActionsPage", regionKey, PageContext.REQUEST_SCOPE);

%>
                <jsp:include page="ibuSRDetailsResActions.jsp" flush="true" /> 
<%
                }
                if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_RET_SHIP_RG"))
                {
                    regionKey           = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
               //     nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);

                    out.println(RendererUtil.blankCell(4, null));
                    pageContext.setAttribute("IncludeRetShipPage", regionKey, PageContext.REQUEST_SCOPE);
%>
                <jsp:include page="ibuSRDetailsRetShip.jsp" flush="true" /> 

<%
                }
                if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_REP_RG"))
                {
                    regionKey = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
                 //   nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);

                  out.println(RendererUtil.blankCell(4, null));
                  pageContext.setAttribute("IncludeRepairsPage", regionKey, PageContext.REQUEST_SCOPE);
%>
               <jsp:include page="ibuSRDetailsRepairs.jsp" flush="true" /> 
<%
                }
            }
        }
        ...
        {
          itemInfo = nestedRegionItems.getItembyCode("IBU_CF_SR_DTL_CHARGES_TAB_RG", true);
          if(itemInfo != null){
           regionKey = 
            new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
          }else{
		...
		out.println(RendererUtil.blankCell(4, null));
          while (nestedRegionItems.hasMoreItem())
          {
            itemInfo = nestedRegionItems.getNextItem();
            if (!itemInfo.getShowFlag()) continue;
            if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_A_CHRG_RG"))
            {
              regionKey           = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
             // nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);

              out.println(RendererUtil.blankCell(4, null));
              pageContext.setAttribute("IncludeActChargesPage",regionKey, PageContext.REQUEST_SCOPE);
%>
             <jsp:include page="ibuSRDetailsActCharges.jsp" flush="true" />
<%
            }
            if (itemInfo.getAttrCode().equals("IBU_CF_SR_DTL_E_CHRG_RG"))
            {
              regionKey           = new ConfigRegionKey(itemInfo.getSubRegionCode(), itemInfo.getSubRegionAppID());
            //  nestedRegionItems2  = ConfigDataLoader.fetchRegionItemsByRegion(_connection, regionKey);

              out.println(RendererUtil.blankCell(4, null));
              pageContext.setAttribute("IncludeEstChargesPage", regionKey, PageContext.REQUEST_SCOPE);
%>
              <jsp:include page="ibuSRDetailsEstCharges.jsp" flush="true" />
<%
            }
          }
       }
...
<% }else{ %>
<p class="errorMessage"><%=oracle.apps.jtf.base.resources.AOLMessageManager.getMessageSt("IBU_R_NO_VIEW_PERMISSION")%>
<% } %>

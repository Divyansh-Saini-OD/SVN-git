...
***************************************************************************************************/
  ...
  , resultout OUT NOCOPY    VARCHAR2
    ...
    lc_contact_name     varchar2(200);
    lc_contact_type     varchar2(100);
    lc_type_name        varchar2(250);
    lc_summary          varchar2(250);
    lc_from_value       varchar2(200);
    ln_request_id       number;
    ln_request_number   number;
    ln_user_id          number;
    lc_fun_url          varchar2(250)  := fnd_profile.value('XX_CS_SOP_LINK_URL');
  ...
  BEGIN
    IF funcmode = 'RUN' THEN
      ...
      BEGIN
        SELECT CS.INCIDENT_ID,
          ...
          FROM  JTF_TASKS_VL TL,
                ...
                HZ_PARTIES HZ,
                JTF_RS_GROUPS_VL GL, 
                CS_INCIDENT_TYPES_TL TL
          WHERE GL.GROUP_ID = CS.OWNER_GROUP_ID
           AND   HZ.PARTY_ID = CS.CUSTOMER_ID
           AND   TL.SOURCE_OBJECT_ID = CS.INCIDENT_ID
           AND   CS.INCIDENT_TYPE_ID = TL.INCIDENT_TYPE_ID
      ...
      IF LC_CUST_NAME IS NOT NULL THEN
        ...
        IF lc_cust_name = 'Office Depot Internal' then
           begin
             SELECT FN.DESCRIPTION
             INTO LC_CUST_INFO
             FROM CS_HZ_SR_CONTACT_POINTS CS,
                  FND_USER FN
             WHERE CS.PARTY_ID = FN.EMPLOYEE_ID
             AND   FN.USER_ID = LN_USER_ID
             AND   CS.INCIDENT_ID = LN_REQUEST_ID;
          EXCEPTION
           WHEN OTHERS THEN
              LC_CUST_INFO := NULL;
          end;
        ...
        IF lc_cust_name = 'Office Depot Internal' then
            IF lc_summary IS NOT NULL THEN
              set_text_attr(
                   p_itemtype   => itemtype
                   , p_itemkey    => itemkey
                   , p_attr_name  => 'TASK_DESC'
                   , p_attr_value => lc_summary
               );
            END IF;
        
         lc_fun_url  := lc_fun_url||ln_request_number;
         
          set_text_attr(
               p_itemtype   => itemtype
               , p_itemkey    => itemkey
               , p_attr_name  => 'XX_REQUEST_LINK'
               , p_attr_value => lc_fun_url
           );

...
--| 1.7       18-Jun-12   Raj Jagarlamudi  Coffee Subscription prob    |
-- |1.8       24-Sep-13   Arun Gannarapu   Made changes to pass        |
-- |                                       p_auto_assign to "Y"        |
-- |1.9       31-JAN-13   Arun Gannarapu   reset the record in create_Sr method
-- |2.0       23-May-14   Arun Gannarapu   Made changes to fix the defect 30185
-- |2.1       22-JAN-16   Vasu Raparla     Removed schema References   |
-- |                                       for R.12.2                  |
-- |2.2       09-SEP-16   arun Gannarapu   Defect 39255 for 12.2 retrofit
-- |2.3       16-SEP-16   Arun Gannarapu   Defect  39120 (to fix latency) |
-- |2.4       28-OCT-16   Suresh Naragam   Made Changes to fix the defect#38961 |
-- +===================================================================+
...
IS
      ...
      l_link_rec_pub               CS_INCIDENTLINKS_PUB.CS_INCIDENT_LINK_REC_TYPE;
...
Procedure Update_ServiceRequest(p_sr_number               IN varchar2,
                                p_sr_service_request_rec  IN CS_ServiceRequest_PUB.service_request_rec_type,
...
BEGIN
    ...
    *******************************************************************/
    ...
    lr_service_request_rec.type_id := p_sr_service_request_rec.type_id;
...
ln_amazon_cust_id         number := fnd_profile.value('XX_CS_AMAZON_CUST_ID');
...
*****************************************************************************/
-- Changed the query on 12-SEP-2016 to fix the defect#38961
  /*BEGIN
    select 'Y'
    into  lc_sr_flag
    from  cs_incidents_all_b
  ...
  BEGIN
    ...
    and   incident_status_id not in (
          ...
          and   def.translation_name='XX_CS_INCIDENT_STATUS'
...
IF (nvl(x_return_status,'S') = 'S') then -- (1)
     ...
     IF p_sr_req_rec.customer_id is null then
      IF p_sr_req_rec.problem_code = 'AMAZON' THEN
         p_sr_req_rec.customer_id := ln_amazon_cust_id;
       END IF;
     END IF;
    ...
    If p_sr_req_rec.customer_id is not null then
      ...
      *******************************************************************************/
      ...
      exception
       when no_data_found then
          x_return_status := 'F';
          x_msg_data := 'Customer not exists in EBS ';
        when others then
          x_return_status := 'F';
    ...
    IF ln_party_id is not null then -- (1) Customer check
       BEGIN
         select s1.party_site_id,
          ...
          and   s1.orig_system_reference = lpad(to_char(p_sr_req_rec.customer_id),8,0)||'-00001-A0' ---- Added for Defect #39120 (to fix latency)
...
IF lc_request_type in ('Stocked Products','Stock - Non Order')  then
    ...
    IF p_sr_req_rec.problem_code IN ('LATE DELIVERY', 'RETURN NOT PICKED UP') then
        lr_service_request_rec.request_context := nvl(lc_item_catagory,'ORDER');
    end if;
  -- Preventing future delivery issues
   IF p_sr_req_rec.problem_code = 'LATE DELIVERY'  THEN 
          begin
              ld_delivery_Date := xx_cs_sr_utils_pkg.res_rev_time_cal
                                          (p_date => SYSDATE,
                                          p_hours => ln_delivery_time,
                                          p_cal_id => 'OD ST CAL',
                                          p_time_id => ln_time_id);
            exception
              when others then
                  ld_delivery_Date  :=  sysdate;
            end;
     --dbms_output.put_line('dates '||p_sr_req_rec.ship_date||' '||ld_delivery_date);
     
    IF trunc(p_sr_req_rec.ship_date) >= trunc(ld_delivery_Date) THEN
      x_return_status := 'F';
      x_msg_data := 'This order is scheduled for delivery between the hours of 8:30 and 5:00.  A Late Delivery service request cannot be submitted until after the scheduled delivery time has passed';
    END IF;
   END IF;
...
IF (nvl(x_return_status,'S') = 'S') then  -- (2)
  /*******************************************************************
  -- Urgency Id
  ******************************************************************/
  begin
...
IF lc_request_type = 'Stocked Products' then
    ...
    begin
      ...
      and   mtlb.segment1 = to_char(p_sr_req_rec.warehouse_id)
      and   mtlb.segment1 in (select attribute1 from hr_all_organization_units
    ...
    IF lr_service_request_rec.category_id is null 
        AND p_sr_req_rec.problem_code = 'PRINT ON DEMAND' then
             begin
                ...
                where mtlb.structure_id = mtls.structure_id
                and   mtls.category_set_name = 'CS Warehouses'
                and   mtlb.segment1 = p_sr_req_rec.csc_location
                and   mtlb.segment1 in (select attribute1 from hr_all_organization_units
                                where  substr(type,1,2) = 'WH');
              exception
                when others then
    ...
    IF p_sr_req_rec.problem_code in ('BIN CHECK','EMERGENCY ORDER/WILL CALL','MSDS REQUEST','TOTE - OPT OUT',
                                      'COFFEE-SUBSCRIPTION','AMAZON') then
...
*****************************************************************************/
--IF p_sr_req_rec.problem_code in ('MSDS REQUEST','TOTE - OPT OUT','COFFEE-SUBSCRIPTION','AMAZON') then
IF NVL(LC_MAIL_FLAG,'N') = 'Y' AND LN_OBLIGATION_TIME = 0 THEN  
        ...
        loop
           IF I = 1 then
              ...
              IF lc_request_type like 'VW%' THEN
                ...
                lr_service_request_rec.resolution_summary  := substr(p_order_tbl(i).attribute1, instr(p_order_tbl(i).attribute1,'-')+1);
              END IF;
           else
...
lt_notes_table(1).note_type   := 'GENERAL';
     ...
     /************************************************************************
          -- Get Resources
     *************************************************************************/
          -- Commented by Arun G as part of R12 Retrofit ..
    ...
    IF (lx_return_status <> FND_API.G_RET_STS_SUCCESS) then
      ...
      -- Update the service request type for Non Code SRs

     IF lx_return_status = 'S' AND lx_request_number IS NOT NULL AND lc_update_Sr ='Y'
     THEN 

       lr_service_request_rec.type_id := p_sr_req_rec.type_id;

       Update_ServiceRequest(p_sr_number               => lx_request_number,
                             p_sr_service_request_rec  => lr_service_request_rec,
                             x_return_status           => lx_return_status1,
                             x_msg_data                => lx_msg_data);

     END IF; -- lx_return_status 
...
and   cs.lookup_code        = alb.problem_code
and   alb.incident_type_id  = tlb.incident_type_id
AND   alb.customer_id = ln_customer_id
--and   nvl(alb.incident_attribute_9,'x') = nvl(lpad(to_char(p_sr_req_rec.customer_id),8,0),nvl(alb.incident_attribute_9,'x'))
and   tlb.incident_type_id = nvl(p_sr_req_rec.type_id,tlb.incident_type_id)
and   alb.incident_status_id = nvl(to_number(p_sr_req_rec.status_name),alb.incident_status_id)
and   nvl(alb.problem_code,'x') = decode(p_sr_req_rec.problem_code,null,nvl(alb.problem_code,'x'),p_sr_req_rec.problem_code)
      ...
      ELSE
        IF p_sr_req_rec.customer_id is not null then
          BEGIN 
            ...
            where orig_system_reference = lpad(to_char(p_sr_req_rec.customer_id),8,0)||'-'||'00001-A0';
          EXCEPTION
            WHEN OTHERS THEN
              x_return_status := 'E';
              x_msg_data := 'Customer id should pass for bulk search';
          END;
        END IF;
...
ln_incident_number  cs_incidents_all_b.incident_number%TYPE;
...
and   alb.incident_type_id = tlb.incident_type_id
and   alb.incident_number = ln_incident_number
--and   alb.incident_id = p_sr_req_rec.request_id
and   cs.lookup_type = 'REQUEST_PROBLEM_CODE'
and   cs.enabled_flag = 'Y'
and   st.incident_subtype = 'INC'
and   st.language = userenv('LANG')
and   tlb.language = userenv('LANG')
and   not exists (select 'x' from xx_cs_tds_parts where request_number = alb.incident_number);
...
cursor c2 is
select  jtf.note_type_meaning,
        jtf.notes,
        ext_user.source_name||' ('||fnd.user_name||')' user_name,
        jtf.creation_date
from  jtf_notes_vl jtf, fnd_user fnd,
      jtf_rs_resource_extns ext_user
where source_object_code='SR'
and   source_object_id = p_request_id
and   ext_user.user_id(+) = fnd.user_id
and   jtf.last_updated_by = fnd.user_id
and   jtf.note_status <> 'P' 
and   jtf.note_type_meaning = 'General Note';

cursor c3 is
select 'Device' meaning,
       qp.node_name||':'||qd.freeform_string notes
...
and    qd.transaction_id = ln_script_id;

cursor c4 is
select 'Services' meaning,
       qp.node_name||':'||qd.freeform_string notes
from   ies_question_data qd,
       ies_questions qp,
       ies_panels ip
...
begin
      BEGIN
        select b.name, a.tier
        into lc_type, ln_script_id
    ...
    IF NVL(LC_TYPE,'X') like 'TDS%' THEN
      begin
        open c2;
        loop
          fetch c2 into c2_rec;
          exit when c2%notfound;
             ...
             BEGIN
               p_sr_notes_tbl(i).notes          := c2_rec.note_type_meaning;
               p_sr_notes_tbl(i).note_details   := c2_rec.notes;
               p_sr_notes_tbl(i).created_by     := c2_rec.user_name;
               p_sr_notes_tbl(i).creation_date  := c2_rec.creation_date;
        ...
        loop
          ...
          exit when c3%notfound;
            ...
            p_sr_notes_tbl.extend;
             ...
             BEGIN
               p_sr_notes_tbl(i).notes          := c3_rec.meaning;
               p_sr_notes_tbl(i).note_details   := c3_rec.notes;
               p_sr_notes_tbl(i).created_by     := null;
               p_sr_notes_tbl(i).creation_date  := sysdate;
        ...
        open c4;
         loop
          ...
          exit when c4%notfound;
             ...
             BEGIN
               p_sr_notes_tbl(i).notes          := c4_rec.meaning;
               p_sr_notes_tbl(i).note_details   := c4_rec.notes; 
...
and   ext_user.user_id(+) = fnd.user_id
and   jtf.last_updated_by = fnd.user_id
and   jtf.note_status = 'E' 
and   jtf.note_type_meaning = 'General Note'
order by jtf.creation_date;

cursor c3 is
select 'Device' meaning,
       qp.node_name,
       qd.freeform_string notes,
       qd.creation_date
from   ies_question_data qd,
       ies_questions qp,
       ies_panels ip
...
begin
      ...
      I                        := 1;
      P_SR_NOTES_TBL           := XX_CS_TDS_NOTES_TBL();
      ...
      begin
        ...
        loop
          ...
          exit when c2%notfound;
             p_sr_notes_tbl.extend;
             p_sr_notes_tbl(i) := xx_cs_tds_notes_rec(null,null,null,null,null);
      
             BEGIN
               p_sr_notes_tbl(i).note_type      := c2_rec.note_type_meaning;
               p_sr_notes_tbl(i).notes          := c2_rec.notes;
               p_sr_notes_tbl(i).note_details   := c2_rec.notes_detail;
               p_sr_notes_tbl(i).created_by     := c2_rec.user_name;
               p_sr_notes_tbl(i).creation_date  := c2_rec.creation_date;
        ...
        loop
          ...
          exit when c3%notfound;
            ...
            p_sr_notes_tbl.extend;
             ...
             BEGIN
               p_sr_notes_tbl(i).note_type      := c3_rec.meaning;
               p_sr_notes_tbl(i).notes          := c3_rec.node_name;
               p_sr_notes_tbl(i).note_details   := c3_rec.notes;
               p_sr_notes_tbl(i).created_by     := null;
               p_sr_notes_tbl(i).creation_date  := c3_rec.creation_date;
        ...
        open c4;
         loop
          ...
          exit when c4%notfound;
             ...
             BEGIN
               p_sr_notes_tbl(i).note_type      := c4_rec.meaning;
               p_sr_notes_tbl(i).notes          := c4_rec.node_name;
               p_sr_notes_tbl(i).note_details   := c4_rec.notes;
               p_sr_notes_tbl(i).creation_date  := c4_rec.creation_date;

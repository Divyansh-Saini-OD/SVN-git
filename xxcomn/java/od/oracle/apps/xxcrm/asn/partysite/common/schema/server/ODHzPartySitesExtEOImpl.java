...
public class ODHzPartySitesExtEOImpl extends OAEntityImpl 
{
  ...
  protected static final int RELATIONSHIPID = 28;
  ...
  protected static final int STARTDATE = 48;
  protected static final int ENDDATE = 49;
  ...
  protected static final int SOURCELANG = 58;
  protected static final int LANGUAGE = 59;
   ...
   */
  public ODHzPartySitesExtEOImpl()
  {
  }
  ...
  public static synchronized EntityDefImpl getDefinitionObject()
  {
    ...
    {
      mDefinitionObject = (oracle.apps.fnd.framework.server.OAEntityDefImpl)EntityDefImpl.findDefObject("od.oracle.apps.xxcrm.asn.partysite.common.schema.server.ODHzPartySitesExtEO");
    }
  ...
  public void remove()
  {
    Diagnostic.println("HzPartySitesExtEOImpl - Inside remove !!! ");

    // get sysdate
    Date sysdate = 
      ((OADBTransaction)(this.getDBTransaction())).getCurrentDBDate();
    Diagnostic.println("sysdate - " + sysdate.toString() );

    // get truncated sysdate
    java.sql.Date trunc_sysdate = sysdate.dateValue();    
    Diagnostic.println("trunc_sysdate - " + trunc_sysdate.toString() );      

    // get end date
    Date endDate = getEndDate();
    Diagnostic.println("endDate - " + (endDate == null? null : endDate.toString()));

    // get truncated end date
    java.sql.Date trunc_endDate = 
      (endDate == null ? (new oracle.jbo.domain.Date("4712-12-31")).dateValue() : endDate.dateValue());
    Diagnostic.println("trunc_endDate - " + (trunc_endDate == null? null : trunc_endDate.toString()));

    // get start date
    Date startDate = getStartDate();
    Diagnostic.println("startDate - " + (startDate == null? null : startDate.toString()));

    // get truncated start date
    java.sql.Date trunc_startDate = (startDate == null? null : startDate.dateValue());
    Diagnostic.println("trunc_startDate - " + (trunc_startDate == null? null : trunc_startDate.toString()));

    // get status
    String status = getStatus();

    // current: active + (today or date in future)
    // remove current: 1) set status to I
    //                 2) set end-date to sysdate if start-date <= sysdate
    if (("A".equals(status)) && (trunc_endDate.equals(trunc_sysdate) || 
                                 trunc_endDate.after(trunc_sysdate)))
    {
      if (trunc_startDate.before(trunc_sysdate) ||
          trunc_startDate.equals(trunc_sysdate))
      {
        setEndDate(sysdate);
      }
      setStatus("I");                 

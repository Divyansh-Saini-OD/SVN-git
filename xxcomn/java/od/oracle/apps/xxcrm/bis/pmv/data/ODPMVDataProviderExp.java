...
// Source File Name:   ODPMVDataProviderExp.java
...
public class ODPMVDataProviderExp
{

    public ODPMVDataProviderExp(java.util.Vector vector)
    {
        _env = "";
        _reportDataObject = new PMVReportDataObject(vector);
    }

    public ODPMVDataProviderExp(oracle.apps.bis.pmv.session.UserSession usersession, oracle.apps.bis.pmv.parameters.ParameterHelper parameterhelper, java.util.Vector vector)
        throws oracle.apps.bis.pmv.PMVException
    {
        ...
        try
        {
            ...
            int ai[] = getLowerUpperBound(j, k, s, l, vector.size(), flag2, 60000);
            j = ai[0];
            k = ai[1];
            try
            {

...
// Source File Name:   ODPMVProvider.java
...
import od.oracle.apps.xxcrm.bis.pmv.data.ODPMVDataProvider;

// Referenced classes of package oracle.apps.bis.pmv.data:
//            ODPMVDataProvider, PMVMetadataProvider

public class ODPMVProvider
    implements oracle.apps.bis.data.DataProvider
{

    public ODPMVProvider(oracle.apps.bis.pmv.session.UserSession usersession, oracle.apps.bis.pmv.parameters.ParameterHelper parameterhelper, java.util.Vector vector, boolean flag)
        throws oracle.apps.bis.pmv.PMVException
    {
        _metadataProvider = new PMVMetadataProvider(usersession, parameterhelper);
        if(flag)
        {
            _dataProvider = new ODPMVDataProvider(vector);
            return;
        } else
        {
            _dataProvider = new ODPMVDataProvider(usersession, parameterhelper, vector);
            return;
        }
    }

    public ODPMVProvider(oracle.apps.bis.pmv.session.UserSession usersession, oracle.apps.bis.pmv.parameters.ParameterHelper parameterhelper, java.util.Vector vector)
        throws oracle.apps.bis.pmv.PMVException
    {
    ...
    od.oracle.apps.xxcrm.bis.pmv.data.ODPMVDataProvider _dataProvider;

package od.oracle.apps.xxcrm.errorhandler.lov.server;
import oracle.apps.fnd.framework.server.OAViewRowImpl;
import oracle.jbo.server.AttributeDefImpl;
//  ---------------------------------------------------------------
//  ---    File generated by Oracle Business Components for Java.
//  ---------------------------------------------------------------

public class ProgramTypeVORowImpl extends OAViewRowImpl 
{
  protected static final int PROGRAMTYPE = 0;


  protected static final int MODULENAME = 1;
  /**
   * 
   * This is the default constructor (do not remove)
   */
  public ProgramTypeVORowImpl()
  {
  }

  /**
   * 
   * Gets the attribute value for the calculated attribute ProgramType
   */
  public String getProgramType()
  {
    return (String)getAttributeInternal(PROGRAMTYPE);
  }

  /**
   * 
   * Sets <code>value</code> as the attribute value for the calculated attribute ProgramType
   */
  public void setProgramType(String value)
  {
    setAttributeInternal(PROGRAMTYPE, value);
  }

  /**
   * 
   * Gets the attribute value for the calculated attribute ModuleName
   */
  public String getModuleName()
  {
    return (String)getAttributeInternal(MODULENAME);
  }

  /**
   * 
   * Sets <code>value</code> as the attribute value for the calculated attribute ModuleName
   */
  public void setModuleName(String value)
  {
    setAttributeInternal(MODULENAME, value);
  }
  //  Generated method. Do not modify.

  protected Object getAttrInvokeAccessor(int index, AttributeDefImpl attrDef) throws Exception
  {
    switch (index)
      {
      case PROGRAMTYPE:
        return getProgramType();
      case MODULENAME:
        return getModuleName();
      default:
        return super.getAttrInvokeAccessor(index, attrDef);
      }
  }
  //  Generated method. Do not modify.

  protected void setAttrInvokeAccessor(int index, Object value, AttributeDefImpl attrDef) throws Exception
  {
    switch (index)
      {
      case PROGRAMTYPE:
        setProgramType((String)value);
        return;
      case MODULENAME:
        setModuleName((String)value);
        return;
      default:
        super.setAttrInvokeAccessor(index, value, attrDef);
        return;
      }
  }
}
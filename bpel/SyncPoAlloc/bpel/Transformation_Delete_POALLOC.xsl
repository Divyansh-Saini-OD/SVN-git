...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.1.0(build 061009.0802) AT [THU JUN 21 11:55:43 EDT 2007]. -->
?>
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <ns1:PoallocCollection>
      <ns1:Poalloc>
        <ns1:poNbr>
          <xsl:value-of select="number(/APPS:XX_PO_ALLOCATION_T/PO)"/>
        </ns1:poNbr>
        <ns1:locId>
          <xsl:value-of select="number(/APPS:XX_PO_ALLOCATION_T/SHIP_TO)"/>
        </ns1:locId>
        <ns1:sku>
          <xsl:value-of select="number(/APPS:XX_PO_ALLOCATION_T/SKU)"/>
        </ns1:sku>
        <ns1:allocLocId>
          <xsl:value-of select="number(/APPS:XX_PO_ALLOCATION_T/ALLOC_LOC)"/>
        </ns1:allocLocId>
      </ns1:Poalloc>
    </ns1:PoallocCollection>
  </xsl:template>

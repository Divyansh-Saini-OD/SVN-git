...
begin
  ...
  if(q_present <> 1 ) 
  then
	dbms_aqadm.create_queue('WF_BPEL_Q', 'WF_BPEL_QTAB');
  ...
  select guid into event_guid from wf_events where name = 'xx.oracle.apps.om.DPSCancelLines.out' ;
 
  select count(*) into subscription_present from wf_event_subscriptions where event_filter_guid = event_guid;

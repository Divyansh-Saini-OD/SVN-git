...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.3.0(build 070615.0525) AT [TUE SEP 20 05:29:24 EDT 2011]. -->
?>
<xsl:stylesheet version="1.0"
                ...
                xmlns:tns="http://TargetNamespace.com/Pcard_term2"
  ...
  <xsl:template match="/">
    <ns0:ProcessInvoice>
      <ns0:DataArea>
        <xsl:if test="/tns:container/tns:InvoiceHeader">
          <xsl:for-each select="/tns:container/tns:InvoiceHeader">
            <ns0:Invoice>
              <ns0:InvoiceHeader>
                <ns0:Description>
                  <xsl:value-of select="concat(tns:RunDate,tns:RunTime,tns:CompanyID,tns:MapperName)"/>
                </ns0:Description>
              ...
              <xsl:for-each select="tns:Lines/tns:Line-Invoice">
                <ns0:InvoiceLine>
                  <xsl:if test='tns:TransactionAmount > "0"'>
                    <ns0:TotalAmount>

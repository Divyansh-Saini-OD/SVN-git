...
import oracle.apps.ego.extfwk.user.data.server.EgoExtAttributeDataEOImpl;
...
public class ODAccountEOImpl extends EgoExtAttributeDataEOImpl
{
  ...
  protected static final int NEXTATTR11 = 40;
  protected static final int NEXTATTR12 = 41;
  protected static final int NEXTATTR13 = 42;
  protected static final int NEXTATTR14 = 43;
  protected static final int NEXTATTR15 = 44;
  protected static final int NEXTATTR16 = 45;
  protected static final int NEXTATTR17 = 46;
  protected static final int NEXTATTR18 = 47;
  protected static final int NEXTATTR19 = 48;
  protected static final int NEXTATTR20 = 49;
  protected static final int DEXTATTR1 = 50;
  protected static final int DEXTATTR2 = 51;
  protected static final int DEXTATTR3 = 52;
  protected static final int DEXTATTR4 = 53;
  protected static final int DEXTATTR5 = 54;
  ...
  protected static final int DELETEATTR = 80;
  ...
  public static synchronized EntityDefImpl getDefinitionObject()
  {
    ...
    {
      mDefinitionObject = (oracle.apps.fnd.framework.server.OAEntityDefImpl)EntityDefImpl.findDefObject("od.oracle.apps.xxcrm.addattr.account.server.ODAccountEO");
    }
  ...
  public void create(AttributeList attributeList)
  {
    ...
    OADBTransactionImpl txn=(OADBTransactionImpl)this.getOADBTransaction();
    Number seqId = txn.getSequenceValue("XX_CDH_CUST_DOC_ID_S");
    this.setNExtAttr2(seqId);
    txn.putValue("CustDocSeqId",seqId);
  }
  ...
  /**
   *
   * Gets the attribute value for DeleteAttr, using the alias name DeleteAttr
   */
  ...
  /**
   *
   * Sets <code>value</code> as the attribute value for DeleteAttr
   */
  ...
  if (dml == DML_INSERT || dml == DML_UPDATE || dml == DML_DELETE) {
    ...
    String pageName = "";
      ...
      pageName = this.getOADBTransaction().getTransientValue("pageName").toString();
      ...
      String validateQry = " BEGIN XX_CDH_CUST_ACHID.VALIDATE_RESP_ACCESS(:1, :2, :3); END;";
      ...
      try
      {
        ...
        callableStatement.setString(1, pageName);
        ...
        errMsg   = callableStatement.getString(2);
        errstatus = callableStatement.getString(3);

        this.getOADBTransaction().writeDiagnostics("doDML: ", "Errorcheck for Responsiblity- Errstatus: "+errstatus + ", errMsg: " + errMsg + ", pageName" + pageName,OAFwkConstants.STATEMENT);
        this.getOADBTransaction().writeDiagnostics("doDML: ", "inside dml:" + this.getOADBTransaction().getTransientValue("achPageId"), OAFwkConstants.STATEMENT);
        if ("E".equals(errstatus))
        {
          ...
          exceptions.add(
                          new OAAttrValException (
                                                   ...
                                                   pageName, //Page Name
                                                   "Not Authorized", //Bad Value
                                                   "XXCRM", //Message Application Short Code
                                                   errMsg
                                                  )
                         );
    ...
    if(this.getOADBTransaction().getTransientValue("BillDocsPageId")!=null){
      pageId = this.getOADBTransaction().getTransientValue("BillDocsPageId").toString();
    }
    if("Y".equals(pageId))
      {
      businessRules( dml);
      }
    if(this.getOADBTransaction().getTransientValue("achPageId")!=null){
      achPageId = this.getOADBTransaction().getTransientValue("achPageId").toString();
    }
    if("Y".equals(achPageId))
      {
      achIdBusinessRules( dml);
      }
  }
  }

  public void achIdBusinessRules(int dml){

	  this.getOADBTransaction().writeDiagnostics("achIdBusinessRules: ", "inside businessrules:" + this.getCustAccountId(), OAFwkConstants.STATEMENT);
	  this.getOADBTransaction().writeDiagnostics("achIdBusinessRules: ", "inside businessrules2:" + this.getOADBTransaction().getTransientValue("paccountId"), OAFwkConstants.STATEMENT);

      String errMsg = "";
      String errstatus  = "";

      //Call to the procedure to validate data
      String validateQry = " BEGIN XX_CDH_CUST_ACHID.VALIDATE_ACH_ID(:1, :2, :3, :4); END;";
      OracleCallableStatement callableStatement = null;
      Connection conn;
      ...
      try
      {
	    ...
	    //if (this.getCustAccountId() != null) { commented by ivarada 14-jun-2011

        if (this.getOADBTransaction().getTransientValue("paccountId") != null) {

        this.getOADBTransaction().writeDiagnostics("achIdBusinessRules: ", "CustAcctId: " + this.getOADBTransaction().getTransientValue("paccountId"), OAFwkConstants.STATEMENT);
        callableStatement.setInt(1, Integer.parseInt(String.valueOf(this.getOADBTransaction().getTransientValue("paccountId"))));
        callableStatement.setString(2, this.getCExtAttr1());
        callableStatement.registerOutParameter(3, OracleTypes.VARCHAR, 0, 3000);
        callableStatement.registerOutParameter(4, OracleTypes.VARCHAR, 0, 3000);
        callableStatement.execute();
        errMsg   = callableStatement.getString(3);
        errstatus = callableStatement.getString(4);
        this.getOADBTransaction().writeDiagnostics("achIdBusinessRules: ", "Errorcheck for Responsiblity- Errstatus: "+errstatus + ", errMsg: " + errMsg ,OAFwkConstants.STATEMENT);
        if ("E".equals(errstatus))
        {
          exceptions.add(
                          new OAAttrValException (
  ...
  public void businessRules(int dml){
    ...
    {
      String docType = this.getCExtAttr1().toString();
      String docID = this.getNExtAttr1().toString();
      if(("Invoice".equals(docType)) && (!docID.startsWith("1")))
      {
   		       exceptions.add(
   		                      new OAAttrValException (
   		 	                    OAException.TYP_ENTITY_OBJECT, // Entity attribute exception.
   			                    getEntityDef().getFullName(), //Entity full def name
   			                    getPrimaryKey(), //Row primary key
   			                    "NExtAttr1", //Attribute Name
   			                    this.getNExtAttr1().toString(), //Bad Value
   			                    "IMC", //Message Application Short Code
   			                    "XX_CDH_INVOICE_1XXX_SERIES")); //Message Code
      }
      else if(("Consolidated Bill".equals(docType)) && (!docID.startsWith("2")))
      {
   		       exceptions.add(
   		                      new OAAttrValException (
   		 	                    OAException.TYP_ENTITY_OBJECT, // Entity attribute exception.
   			                    ...
   			                    this.getNExtAttr1().toString(), //Bad Value
   			                    "IMC", //Message Application Short Code
   			                    "XX_CDH_CONSBILL_2XXX_SERIES")); //Message Code
      }
    if ( dml == DML_INSERT)
    {
      Number oldCustDocID = new Number(Integer.parseInt(txn.getValue("CustDocSeqId").toString()));
      if(!oldCustDocID.equals(this.getNExtAttr2().toString()))
        {
            this.setNExtAttr2(oldCustDocID);
   		       exceptions.add(
   		                      new OAAttrValException (
   		 	                    OAException.TYP_ENTITY_OBJECT, // Entity attribute exception.
   			                    getEntityDef().getFullName(), //Entity full def name
   			                    getPrimaryKey(), //Row primary key
   			                    "NExtAttr2", //Attribute Name
   			                    this.getNExtAttr2().toString(), //Bad Value
   			                    "IMC", //Message Application Short Code
   			                    "XX_CDH_CUST_DOC_ID_NOT_EDITABL")); //Message Code
        }
    }
    if (dml == DML_UPDATE) {
        Number oldCustDocID = (Number)getPostedAttribute(getAttributeIndexOf("NExtAttr2"));
        this.getOADBTransaction().writeDiagnostics("SMJ:","oldCustDocID"+oldCustDocID.toString(),OAFwkConstants.STATEMENT);
        this.getOADBTransaction().writeDiagnostics("SMJ:","newCustDocID"+this.getNExtAttr2().toString(),OAFwkConstants.STATEMENT);
        if(!oldCustDocID.equals(this.getNExtAttr2().toString()))
        {
            this.setNExtAttr2(oldCustDocID);
   		       exceptions.add(

...
BEGIN
  ...
  END IF;
  --Begin Request
  --fnd_file.put_line(fnd_file.log, 'before URL ' );--ID / Password - SVC-BIZBOXWS /svc4bizboxws
  ...
  --FND_FILE.PUT_LINE(FND_FILE.LOG, 'line 128' );
  ...
  --l_username     := 'SVC-EBSWS';                                                 -- MS Temp--SVC-BIZBOXWS/svc4bizboxws
  --l_vps_password := 'svcebs4uat';                                                -- MS Temp
  --FND_FILE.PUT_LINE(FND_FILE.LOG,'id/pwd: '|| l_username||':'||l_vps_password ); -- MS Temp
  ...
  --  fnd_file.put_line(fnd_file.log,'in auth ');
  --  utl_http.set_header(req, 'Authorization', 'Basic ' || UTL_RAW.cast_to_varchar2(UTL_ENCODE.base64_encode(UTL_RAW.cast_to_raw(l_username||':'||l_vps_password))));
  --  END IF;
  --fnd_file.put_line(fnd_file.log,'CLOB Length is '||dbms_lob.getlength( payloadcontent));
  LOOP
    EXIT
  ...
  END LOOP;
  --  fnd_file.put_line(fnd_file.log,'last Tag position is  '||xx_ap_last_tag_position(dbms_lob.substr(payloadcontent,10000, 1),l_next_pos));
  --utl_http.write_text(req, payloadcontent);
  ...
  WHEN utl_http.end_of_body THEN
    utl_http.end_response(res);
    --     fnd_file.put_line(fnd_file.log,'ERROR IS IN PUBLISH INVLICE STATUS '||sqlerrm);
  END;
  p_response_code:=res.status_code;
  --EXCEPTION
  --when others then
  --fnd_file.put_line(fnd_file.log,'g_vendor_site_id'||g_vendor_site_id);
  BEGIN
...
ORDER BY 2,3;
--Commented below query for NAIT-64664
/*    SELECT key_value_1,
address_type,
seq_no,
primary_addr_ind,
...
FROM xx_ap_sup_vendor_contact vend_cont,
xx_ap_sup_address_type addr_type
WHERE vend_cont.key_value_1=v_vendor_site_id--
AND vend_cont.addr_type_id =addr_type.addr_type_id
AND vend_cont.enable_flag  ='Y'
AND addr_type.enable_flag  ='Y'
ORDER BY addr_type.addr_type_id,
seq_no;-- and addr_type IN ('01','03');*/
/* UNION ALL-- added by Sunil NAIT-64664 as suggested by Antonio
SELECT
a.vendor_site_id,
CASE WHEN purchasing_site_flag = 'N' AND pay_site_flag = 'N' THEN 0 -- RTV
ELSE CASE WHEN purchasing_site_flag = 'Y' AND pay_site_flag = 'N' THEN 2 -- PR
ELSE CASE WHEN purchasing_site_flag = 'N' AND pay_site_flag = 'Y' THEN 3 -- PY
ELSE CASE WHEN purchasing_site_flag = 'Y' AND pay_site_flag = 'Y' THEN 4 -- PR/PY
END
end
END
end address_type,
...
a.zip,
NVL(a.country, 'US'),
b.first_name||' '||b.last_name contact_name,
replace(a.area_code||a.phone,'-') phone,
replace(b.fax_area_code||b.fax,'-') fax,
b.email_address,
0 od_phone_nbr_ext,
NULL od_phone_800_nbr,
NULL od_comment1,
...
'N' email_ind_flg,
NULL od_ship_from_addr_id
FROM apps.ap_supplier_sites_all a, -- V4.00 po_vendor_sites_all a,
apps.po_vendor_contacts b
where a.vendor_site_id = b.vendor_site_id(+)
...
BEGIN
  ...
  --   fnd_file.put_line(fnd_file.log,'Root Node'||l_domdoc);
  ...
  --
  --Hard coded VendorTin as discussed with Antonio--commented as req by Antonio
  -- l_v_vendor_tin_n := dbms_xmldom.appendchild( l_supp_header_node--l_supplier_node
  --, dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:vendorTin' )) );
  --l_v_vendor_tin_tn := dbms_xmldom.appendchild( l_v_vendor_tin_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '9')) );
  ...
  --
  --   fnd_file.put_line(fnd_file.log,xml_output);
  --    fnd_file.put_line(fnd_file.log,'Error getting minority_cd_desc'||sqlerrm);
  --      fnd_file.put_line(fnd_file.log,'Error getting minority_cd_desc'||sqlerrm);
  --   utl_http.write_text(p_req, payloadcontent);
  ...
  IF v_site_purchaddr1 IS NOT NULL THEN
    ...
    --
    --
    --   fnd_file.put_line(fnd_file.log,'purch address');
    ---
    l_addr_element := dbms_xmldom.createelement(l_domdoc, 'sup:address' );
    l_addr_node    := dbms_xmldom.appendchild( l_addr_list_node--l_supplier_node
    , dbms_xmldom.makenode( l_addr_element) );
    ...
    -- , dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:isPrimaryAddress' )) );
    -- l_v_site_pur_isprimaryaddr_tn := dbms_xmldom.appendchild( l_v_site_pur_isprimaryaddr_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '' )) );
    ...
    --
    --    fnd_file.put_line(fnd_file.log,'purch address end');
  END IF;
  ---Pay Address
  IF v_site_payaddr1 IS NOT NULL THEN
    ...
    --                                             );
    -- fnd_file.put_line(fnd_file.log,'pay address');
    --
    l_addr_element := dbms_xmldom.createelement(l_domdoc, 'sup:address' );
    l_addr_node    := dbms_xmldom.appendchild( l_addr_list_node--l_supplier_node
    , dbms_xmldom.makenode( l_addr_element) );
    ...
    --   , dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:isPrimaryAddress' )) );
    --  l_v_site_pay_isprimaryaddr_tn := dbms_xmldom.appendchild( l_v_site_pay_isprimaryaddr_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '' )) );
  ...
  IF v_site_ppaddr1 IS NOT NULL THEN
    ...
    --                                             );
    --  fnd_file.put_line(fnd_file.log,'purch and pay address');
    --
    l_addr_element := dbms_xmldom.createelement(l_domdoc, 'sup:address' );
    l_addr_node    := dbms_xmldom.appendchild( l_addr_list_node--l_supplier_node
    , dbms_xmldom.makenode( l_addr_element) );
    ...
    -- , dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:isPrimaryAddress' )) );
    -- l_v_site_pp_isprimaryaddr_tn := dbms_xmldom.appendchild( l_v_site_pp_isprimaryaddr_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '' )) );
  ...
  IF v_site_rtvaddr1 IS NOT NULL THEN
    ...
    --                                            );
    -- fnd_file.put_line(fnd_file.log,'rtv address');
    --
    l_addr_element := dbms_xmldom.createelement(l_domdoc, 'sup:address' );
    l_addr_node    := dbms_xmldom.appendchild( l_addr_list_node--l_supplier_node
    , dbms_xmldom.makenode( l_addr_element) );
    ...
    -- , dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:isPrimaryAddress' )) );
    -- l_v_site_rtv_isprimaryaddr_tn := dbms_xmldom.appendchild( l_v_site_rtv_isprimaryaddr_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '' )) );
  ...
  BEGIN
    ...
    --                                            );
    --  fnd_file.put_line(fnd_file.log, 'Before address data loop line 3592 ' );
    ...
    LOOP
      ...
      -- , dbms_xmldom.makenode(dbms_xmldom.createelement(l_domdoc, 'sup:isPrimaryAddress' )) );
      --   l_v_cust_add_isprimaryaddr_tn := dbms_xmldom.appendchild( l_v_cust_add_isprimaryaddr_n , dbms_xmldom.makenode(dbms_xmldom.createtextnode(l_domdoc, '' )) );
  ...
  -- fnd_file.put_line (fnd_file.output,xml_output );
  /*   LOOP
  EXIT
  WHEN l_offset > dbms_lob.getlength(xml_output);
  fnd_file.put_line(fnd_file.log,dbms_lob.substr(xml_output, 10000, l_offset )) ;
  l_offset := l_offset + 10000;
  END LOOP;*/
  ...
  --p_request_id:=fnd_profile.VALUE('CONCURRENT_REQUEST_ID');
  ...
  --fnd_file.put_line (fnd_file.LOG, 'Response Code' ||p_response_code);
...
--=========
BEGIN
  -- fnd_global.apps_initialize(3817336,50660,20043);---Check
  ...
  IF v_exit_flag = 'N' THEN
    ...
    WHEN NOT mainsupplupdate_cur % found;
      ...
      */
        ...
        BEGIN
          IF v_site_terms IS NOT NULL THEN
            ...
            BEGIN
              ...
              --OLd query commented by Sunil for NAIT-64249
              /*              SELECT name,
              description
              INTO v_site_terms_name,
              v_site_terms_name_desc
            ...
            --    v_site_terms_name_desc:=x_source_value3;
        ...
        BEGIN
          BEGIN
            SELECT k.lead_time,
              ...
              --        j.od_contract_signature,--additional attributes from custom table
              --       j.od_contract_title,--additional attributes from custom table
              ...
              k.rtv_related_site,
              --    j.od_ven_sig_name,--additional attributes from custom table
              --  j.od_ven_sig_title,--additional attributes from custom table
            ...
            --    AND  k.vendor_site_id = j.supplier;
          ...
          */
          ...
          IF(v_od_date_signed IS NOT NULL) THEN
            BEGIN
              --   v_od_date_signed := to_char(to_date(substr(v_od_date_signed, 1, 10), 'YYYY/MM/DD'), 'YYYY-MM-DD');--commented by sunil for
      ...
      --      v_globalvendor_id := xx_po_global_vendor_pkg.f_get_outbound(v_vendor_site_id);--commented by sunil
      ...
      END IF;*/
      -- create_data_line;--commented by sunil
      /*IF(v_gss_flag = 'Y') THEN
      ...
      IF(v_rms_flag = 'Y') THEN
        ...
        dbms_output.put_line(' RMS Data '||v_file_data1 || v_file_data2|| v_file_data3 || v_file_data4 || v_file_data5 || v_file_data6 || v_attribute10 || v_attribute11 || v_attribute12 || v_attribute13 || c_separator || v_attribute15 || c_separator || v_minority_class --   3.10

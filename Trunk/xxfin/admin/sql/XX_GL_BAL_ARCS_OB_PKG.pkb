...
-- |  Description: This procedure retrieves data from Gl arcs and  writes data to the outbound file,
---this program is to extract data for three ledger ('CA_CAD_P','US_USD_P','UK_GBP_P') in USD currency |
-- =============================================================================================|
PROCEDURE populate_gl_out_file_rc(
  ...
  IF LC_GL_DATA.COUNT > 0 THEN
    ...
    LOOP
      IF LC_GL_DATA(l_rec).YTD_BEGINNING_BAL<>0 OR LC_GL_DATA(l_rec).PTD_NET_DR<>0 OR LC_GL_DATA(l_rec).PTD_NET_CR<>0 OR LC_GL_DATA(l_rec).YTD_BALANCE <>0 THEN
        UTL_FILE.put_line(lc_file_handle, LC_GL_DATA(l_rec).COMPANY||','|| LC_GL_DATA(l_rec).COST_CENTER||','|| LC_GL_DATA(l_rec).ACCOUNT||','|| LC_GL_DATA(l_rec).LOCATION||','|| LC_GL_DATA(l_rec).INTERCOMPANY||','|| LC_GL_DATA(l_rec).LOB||','|| LC_GL_DATA(l_rec).FUTURE||','|| LC_GL_DATA(l_rec).YTD_BEGINNING_BAL||','|| ''||','|| ''||','|| LC_GL_DATA(l_rec).YTD_BALANCE||','|| LC_GL_DATA(l_rec).CURRENCY_CODE||','|| LC_GL_DATA(l_rec).LEDGER_ID||','|| LC_GL_DATA(l_rec).PERIOD_NAME||','|| LC_GL_DATA(l_rec).CODE_COMBINATION_ID||','|| LC_GL_DATA(l_rec).BALANCE_TYPE||','|| LC_GL_DATA(l_rec).ACCOUNT_DESCRIPTION );
      END IF;
    END LOOP;
...
WHEN OTHERS THEN
  ...
  print_debug_msg ('End - populate_gl_out_file_rc - '||lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
  p_errbuf  := SUBSTR (SQLERRM, 1, 150);
  P_RETCODE := 2;
END populate_gl_out_file_rc;
-- +============================================================================================+
-- |  Name   : populate_gl_out_file                                                         |
-- |  Description: This procedure retrieves data from gl_balances for all code combination and  writes data to the outbound file|
-- =============================================================================================|
...
IS
  ...
  FROM GL_LOOKUPS GLLookups,
    ...
    gl_balances gb
  WHERE 1 = 1
    --and gcc.code_combination_id=81316
  AND gb.period_name          = c_period_name
  AND gb.actual_flag          = 'A'
  AND gb.template_id         IS NULL
...
WHEN UTL_FILE.access_denied THEN
  ...
  p_retcode := 2;
WHEN UTL_FILE.delete_failed THEN
  lc_errormsg := ( 'ARCS GL Balances Outbound Program Errored :- ' || ' delete_failed :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  print_debug_msg (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
  ...
  p_retcode := 2;
WHEN UTL_FILE.file_open THEN
  lc_errormsg := ( 'ARCS GL Balances Outbound Program Errored :- ' || ' file_open :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  print_debug_msg (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
  ...
  p_retcode := 2;
WHEN UTL_FILE.internal_error THEN
  lc_errormsg := ( 'ARCS GL Balances Outbound Program Errored :- ' || ' internal_error :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  print_debug_msg (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
...
WHEN UTL_FILE.invalid_filehandle THEN
  ...
  p_retcode := 2;
WHEN UTL_FILE.invalid_filename THEN
  lc_errormsg := ( 'ARCS GL Balances Outbound Program Errored :- ' || ' invalid_filename :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  print_debug_msg (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
  ...
  p_retcode := 2;
WHEN UTL_FILE.invalid_maxlinesize THEN
  lc_errormsg := ( 'ARCS GL Balances Outbound Program Errored :- ' || ' invalid_maxlinesize :: ' || SUBSTR (SQLERRM, 1, 3800) || SQLCODE );
  print_debug_msg (lc_errormsg, TRUE);
  UTL_FILE.fclose_all;
  lc_file_handle := UTL_FILE.fopen (l_file_path, l_file_name, 'W', 32767);
  UTL_FILE.fclose(lc_file_handle);
...
-- |  Description: This procedure generates the GL Balances Outbound file for ARCS and ARCS for Reporting currency              |
-- |                                                                                            |
...
AS
  ...
  lc_error_loc            VARCHAR2(100)  := 'XX_GL_BAL_ARCS_OB_PKG.process_gl_balances_ob';
...
BEGIN
  ...
  print_out_msg('                                                  OD: GL Balances ARCS Extract');
  print_out_msg('                                                 -------------------------------');
  print_out_msg('');
  print_out_msg('');
  ...
  ELSE
    POPULATE_GL_OUT_FILE_RC(P_ERRBUF => LC_ERROR_MSG ,P_RETCODE => LN_IRETCODE ,P_PERIOD_NAME => P_PERIOD_NAME ,P_DEBUG => P_DEBUG);
  END IF;
  ...
  IF ln_iretcode = 0 THEN
    ...
    print_debug_msg('Completed GL Balances to ARCS Outbound Interface......:: '||TO_CHAR(SYSDATE,'MM-DD-YYYY HH:Mi:SS'), TRUE);
  ELSIF ln_iretcode = 1 THEN
    p_retcode      := 1;
    p_errbuf       := lc_error_msg;
    print_debug_msg('In Warning, GL Balances to ARCS Outbound Interface......:: '||lc_error_msg, TRUE);
  ELSIF ln_iretcode = 2 THEN
    p_retcode      := 2;
    p_errbuf       := lc_error_msg;
    print_debug_msg('In Error, GL Balances to ARCS Outbound Interface......:: '||lc_error_msg, TRUE);
  END IF;

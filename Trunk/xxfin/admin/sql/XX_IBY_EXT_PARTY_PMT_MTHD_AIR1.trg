...
CREATE OR REPLACE TRIGGER apps.XX_IBY_EXT_PARTY_PMT_MTHD_AIR1 AFTER
INSERT ON apps.iby_ext_party_pmt_mthds FOR EACH ROW WHEN (new.primary_flag = 'Y' and ((new.inactive_date IS NULL) or (new.inactive_date > sysdate)))
...
-- PRAGMA AUTONOMOUS_TRANSACTION;
   ...
   CURSOR c2(l_vendor_site_id NUMBER) IS
     	SELECT payment_method_lookup_code
      	  FROM xx_po_vendor_sites_all_aud
...
BEGIN
   ...
   IF r1.vendor_site_id IS NOT NULL THEN
      ...
      OPEN c2(r1.vendor_site_id);
      FETCH c2 INTO l_payment_method_code;
      CLOSE c2;

      IF NVL(l_payment_method_code,'X') <> NVL(:new.payment_method_code,'X') THEN

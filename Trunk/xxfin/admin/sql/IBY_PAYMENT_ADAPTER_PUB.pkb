create or replace
PACKAGE BODY IBY_PAYMENT_ADAPTER_PUB AS
/* $Header: ibyppadb.pls 115.124 2007/12/26 07:02:16 lmallick ship $ */



-- 7/21/2010 - Office Depot modification for I0349 defect 4180 - populating ODRetCode and ODPS2000 in OraPmtReq out parameter x_reqresp_rec
...
--------------------------------------------------------------------------------------------
 ...
 BEGIN
	...
	iby_utility_pvt.get_property(iby_security_pkg.C_SHARED_WALLET_LOC_PROP_NAME,l_wallet_location);
                 ...
                 /*
 	          * Fix for bug 6690528(FP of 6003652):
 	          *
	...
	*/
	...
	FOR i IN 1..l_names.COUNT LOOP
	    ...
	    ELSIF l_names(i) = 'OapfNlsLang' THEN
	       ...
	       iby_debug_pub.add(debug_msg => 'Response nls_lang=' || x_reqresp_rec.Response.NLS_LANG,
                debug_level => FND_LOG.LEVEL_STATEMENT,
                module => G_DEBUG_MODULE || '.OraPmtReq');

-- Bushrod start of addition for I0349 Defect #4180
      ELSIF l_names(i) = 'OapfODRetCode' THEN
         x_reqresp_rec.ODRetCode := l_values(i);
         iby_debug_pub.add(debug_msg => 'Response ODRetCode=' || x_reqresp_rec.ODRetCode,
                debug_level => FND_LOG.LEVEL_STATEMENT,
                module => G_DEBUG_MODULE || '.OraPmtReq');
      ELSIF l_names(i) = 'OapfODPS2000' THEN
         x_reqresp_rec.ODPS2000 := l_values(i);
         iby_debug_pub.add(debug_msg => 'Response ODPS2000=' || x_reqresp_rec.ODPS2000,
                debug_level => FND_LOG.LEVEL_STATEMENT,
                module => G_DEBUG_MODULE || '.OraPmtReq');
-- Bushrod end of addition for I0349 Defect #4180

create or replace
PACKAGE BODY      xx_ce_cc_stmt_match_pkg
AS
-- +=================================================================================+
...
-- |  1.3     2014-05-28   Pravendra Lohiya   Defect 29889 Discover bank lines in    |
-- |                                          bank account 2090002608366 are matching| 
-- |                                           but not reconciling                   |
-- |  1.4     2014-12-16   Kirubha Samuel     Added logic to update ajb999 tables    |
-- |                                          for QC Defect #32826                   |
-- |  1.5     2015-06-10   Deepak V           Defect 32945 - Performance fix done by |
-- |                                          adding a new translation               |
-- |                                          OD_AJB_STATEMENT_MATCH   		     |
-- |  1.6     2015-07-23   Avinash            Defect 34808 populate trx_id in 	     |
-- |                                          ajb996,998,998			     |
-- +=================================================================================+
-- -------------------------------------------
-- Global Variables
...
--------------------------------------------------------------------------------
   BEGIN
      SELECT TRX_CODE
       ...
       AND   bank_account_id = p_bank_account_id; -- Added by Pravendra for Defect no 29889 
    EXCEPTION
      When NO_DATA_FOUND
      Then
        fnd_file.put_line (fnd_file.LOG, ' ');
...
-- +=================================================================================+
   ...
   IS
      ...
      IS
         SELECT   processor_id, bank_rec_id, recon_date
            ...
            WHERE 1 = 1
			  ...
			  -- AND recon_date <= p_trx_date  Commented for defect 32945
              AND recon_date BETWEEN (p_trx_date - p_recon_days) AND p_trx_date -- Added for defect 32945
      ...
      CURSOR ajb_multi_match_cur (p_provider IN VARCHAR2, p_trx_date IN DATE,p_recon_days IN   NUMBER)
      IS
         SELECT   processor_id, bank_rec_id, recon_date
                , SUM (net_amount) net_amount
             FROM xx_ce_ajb_net_amounts_v xnet
	  ...
	  lc_recon_days        NUMBER; -- Added for defect 32945
	  v_start date;
   BEGIN
...
-- ------------------------------------------------
	...
	/* Code added for 32945 to  get translation value for recon days beyond which system will not look at */
	  ...
	  BEGIN
		  ...
		  WHERE xftv.translate_id = xftd.translate_id
			...
			AND xftd.translation_name = 'OD_AJB_STATEMENT_MATCH'
			AND xftv.enabled_flag = 'Y'
			AND xftd.enabled_flag = 'Y';
			
			fnd_file.put_line (fnd_file.log,'Recon days history from translation OD_AJB_STATEMENT_MATCH is ' || lc_recon_days);
	  EXCEPTION
		WHEN OTHERS THEN
			fnd_file.put_line (fnd_file.log,'Error while fetching translation value for Recon Date ' || sqlerrm);
			lc_recon_days := 100;
	  END;
	   v_start := sysdate;
               ...
               BEGIN
                  ...
                  BEGIN                             --Single match process start
                     ...
                     OPEN ajb_single_match_cur (ccdeptab (i).provider_code
                                              , ccdeptab (i).trx_date
                                              , ccdeptab (i).amount
											  , lc_recon_days  -- Added for defect 32945
                                               );
...
/* Start of changes for update statement for Defect 2610*/
                        ...
                        ln_rows_updated := 0;
						/* INDEX(XCA8 XX_CE_AJB998_N13)*/  --Commented for Defect 32945
                       ...
                       -- AND xca8.recon_date <= ccdeptab (i).trx_date  Commented for Defect 32945
						AND xca8.recon_date BETWEEN (ccdeptab (i).trx_date-lc_recon_days) AND ccdeptab (i).trx_date -- Added for defect 32945
						...
						--AND xca6.recon_date <= ccdeptab (i).trx_date Commented for Defect 32945
                        AND xca6.recon_date BETWEEN (ccdeptab (i).trx_date - lc_recon_days) AND ccdeptab (i).trx_date -- Added for defect 32945
                       ...
                       /* AND EXISTS (                      -- TEST FOR CHARGEBACK TYPE AND PROVIDER
						...
						--Changes starts for defect #32826
                        ln_rows_updated := 0;
                        UPDATE xx_ce_ajb999 xca9
                        SET xca9.status_1310 = 'PROCESSED'
                          , trx_id = ln_trx_id
                          , last_update_date = SYSDATE
                          , last_updated_by = gn_user_id
                        WHERE xca9.bank_rec_id = lc_bank_rec_id
                        AND xca9.processor_id =  lc_processor_id
                        AND xca9.recon_date <= ccdeptab (i).trx_date
                        AND xca9.org_id = gn_org_id
                        AND xca9.status_1310 = 'NEW'
                        AND xca9.status IN ('PREPROCESSED', 'MATCHED_AR');
						ln_rows_updated := SQL%ROWCOUNT;
						fnd_file.put_line (fnd_file.log,ln_rows_updated || ' rows updated (Single-match) in xx_ce_ajb999, processor_id '
                                                                        ||lc_processor_id
                                                                        || ', bank rec id '
                                                                        ||lc_bank_rec_id);
						--Changes starts for defect #32826
                                       ...
                                       fnd_file.put_line (fnd_file.log,ln_rows_updated || ' rows updated (Multi-match) in xx_ce_ajb996, processor_id '
                                        ...
                                        UPDATE xx_ce_ajb999 xca9
                                          SET xca9.status_1310 = 'PROCESSED'
                                            ...
                                            , last_update_date = SYSDATE
                                            , last_updated_by = gn_user_id
                                        --WHERE xca9.bank_rec_id = lc_bank_rec_id  --commented for defect 9249
                                        --  AND xca9.processor_id =  lc_processor_id  --commented for defect 9249
                                        WHERE xca9.bank_rec_id = multimatchtab (k).bank_rec_id  --added for defect 9249
                                          AND xca9.processor_id =  multimatchtab (k).processor_id  --added for defect 9249
                                          AND xca9.recon_date <= ccdeptab (i).trx_date
                                          AND xca9.org_id = gn_org_id
                                          AND xca9.status_1310 = 'NEW'
                                          AND xca9.status IN ('PREPROCESSED', 'MATCHED_AR');
										  ln_rows_updated := SQL%ROWCOUNT;
										fnd_file.put_line (fnd_file.log,ln_rows_updated || ' rows updated (Multi-match) in xx_ce_ajb999, processor_id '
                                                                                       ||multimatchtab (k).processor_id
                                                                                       || ', bank rec id '
                                                                                       ||multimatchtab (k).bank_rec_id);
										--Changes ends for defect #32826

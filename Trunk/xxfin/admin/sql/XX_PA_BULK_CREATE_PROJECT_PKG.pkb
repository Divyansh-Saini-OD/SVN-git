...
END PUBLISH_REPORT;
-- +============================================================================================+
-- |  Name: XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR                                            |
-- |  Description: This pkg.procedure will remove special characters          - |
-- |               XX_PA_BULK_CREATE_PRJ_STG.                                                   |
-- =============================================================================================|
FUNCTION REMOVE_SPCL_CHAR(
    p_data_element IN VARCHAR2)
  RETURN VARCHAR2
IS
  LC_DATA_ELEMENT VARCHAR2(1000);
BEGIN
  lc_data_element:=trim(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(P_DATA_ELEMENT, CHR(13), ''), CHR(10), ''),'  ',' '),CHR(38),''),'- ','-'),' -','-'));
  RETURN LC_DATA_ELEMENT;
END REMOVE_SPCL_CHAR;
-- +============================================================================================+
...
AS
    lc_exc_flag           BOOLEAN := FALSE;
    lv_err_msg            VARCHAR2(1000) :=NULL;
    ln_proj_no_count      NUMBER :=0;
    ln_proj_long_name_count NUMBER :=0;
    ln_proj_desc_count NUMBER :=0;
BEGIN
  SELECT COUNT (1)
  INTO ln_proj_no_count
  FROM
    (SELECT SUBSTR(P_PROJECT_NAME, REGEXP_INSTR(P_PROJECT_NAME,'[^a-z|A-Z|0-9 ]'),1) special_char
    FROM Dual
    WHERE (REGEXP_LIKE(P_PROJECT_NAME, '[[:space:]]{2}')
    OR REGEXP_LIKE (P_PROJECT_NAME, '[^A-Z|a-z|0-9| ]', 'i'))
    ) Temp
  GROUP BY Special_Char;
    SELECT COUNT (1)
  INTO ln_proj_long_name_count
  FROM
    (SELECT SUBSTR(P_PROJECT_LONG_NAME, REGEXP_INSTR(P_PROJECT_LONG_NAME,'[^a-z|A-Z|0-9 ]'),1) special_char
    FROM Dual
    WHERE (REGEXP_LIKE(P_PROJECT_LONG_NAME, '[[:space:]]{2}')
    OR REGEXP_LIKE (P_PROJECT_LONG_NAME, '[^A-Z|a-z|0-9| ]', 'i'))
    ) Temp
  GROUP BY Special_Char;
    SELECT COUNT (1)
  INTO ln_proj_desc_count
  FROM
    (SELECT SUBSTR(P_PROJECT_DESCRIPTION, REGEXP_INSTR(P_PROJECT_DESCRIPTION,'[^a-z|A-Z|0-9 ]'),1) special_char
    FROM Dual
    WHERE (REGEXP_LIKE(P_PROJECT_DESCRIPTION, '[[:space:]]{2}')
    OR REGEXP_LIKE (P_PROJECT_DESCRIPTION, '[^A-Z|a-z|0-9| ]', 'i'))
    ) Temp
  Group By Special_Char;
 
  IF LENGTH(P_PROJECT_EXTENSION) != 8 THEN
    lv_err_msg := 'Invalid Project Extension. Must be eight characters.';
    FND_MESSAGE.SET_NAME('BNE','Validation_ERROR');
    FND_MESSAGE.SET_TOKEN('Validation_ERROR',LV_ERR_MSG);
  ELSIF ln_proj_no_count > 0 THEN
    lv_err_msg               := 'PROJECT_NAME column contains extra spaces or special characters. Please correct.';
    fnd_message.set_name('BNE','Validation_ERROR');
    fnd_message.set_token('Validation_ERROR',lv_err_msg);
  ELSIF ln_proj_long_name_count > 0 THEN
    lv_err_msg               := 'PROJECT_LONG_NAME column contains extra spaces or special characters. Please correct.';
    fnd_message.set_name('BNE','Validation_ERROR');
    fnd_message.set_token('Validation_ERROR',lv_err_msg);
  ELSIF ln_proj_desc_count > 0 THEN
    lv_err_msg               := 'PROJECT_DESCRIPTION column contains extra spaces or special characters. Please correct.';
    fnd_message.set_name('BNE','Validation_ERROR');
    fnd_message.set_token('Validation_ERROR',lv_err_msg);
  ELSE
    INSERT
    INTO XX_PA_BULK_CREATE_PRJ_STG
      ...
      VALUES
      (
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PAN),
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PROJECT_EXTENSION),
        P_PROJECT_START_DATE,
        P_PROJECT_END_DATE,
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PROJECT_NAME),
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PROJECT_LONG_NAME),
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PROJECT_DESCRIPTION),
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_ADD_TO_DESC),
        P_PROJECT_MANAGER,
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_TEMPLATE_NAME),
        P_PROJECT_LOCATION,
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_PROJECT_ORG),
        P_CAPITAL_BUDGET,
        P_EXPENSE_BUDGET,
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_COUNTRY),
        XX_PA_BULK_CREATE_PROJECT_PKG.REMOVE_SPCL_CHAR(P_CURRENCY_CODE),
        p_project_number,
        'I',
        P_STATUS_CODE,
        P_CONC_REQ_ID,
...
WHEN OTHERS THEN
  ...
  P_Flag       := 1;
  FND_FILE.PUT_LINE(FND_FILE.OUTPUT,'The Request : '||gn_request_id||' completed in error. Please refer to log file for more details. '||P_error_mesg);

...
-- +======================================================================+
    ...
    AS
        ...
        IS
			...
			--AND RCT.TRX_NUMBER                = '1218210030'
			AND Ara.Applied_Customer_Trx_Id   = Rct.Customer_Trx_Id
			AND Ara.creation_date between p_start_date and p_end_date
        ...
        lc_netting_file_name             VARCHAR2(250)              := TO_CHAR(SYSDATE,'yyyymmddhhmmss')||'_NETTING';
        ...
        BEGIN
			...
			AND period_num    =
			  (	SELECT EXTRACT (MONTH FROM (ADD_MONTHS (end_date, -1))) xx_temp
				FROM GL_PERIODS gp
				WHERE 1=1
				AND SYSDATE-17 BETWEEN START_DATE AND END_DATE
				);
        ...
        THEN
            BEGIN
                lf_netting_file := UTL_FILE.fopen('XXFIN_OUTBOUND',
                                               ...
                                               'w',
                                               ln_chunk_size);
                lc_netting_file_header :=
                       'AR Customer Receipt NUMBER'
                    || lc_delimeter
                    || 'Invoice NUMBER'
                    || lc_delimeter
                    || 'Invoice DATE';
                ...
                LOOP
                    lc_netting_file_content :=
                        ...
                        || lc_delimeter
                        || l_netting_extract_tab(i).Invoice_DATE;
                    UTL_FILE.put_line(lf_netting_file,
                                      lc_netting_file_content);
                END LOOP;

...
BEGIN
   ...
   LOOP
		...
		DBMS_OUTPUT.PUT_LINE('Total number of Records :'||ln_total_records_processed);
		
		FOR indx IN l_trx_dtls_tab.FIRST..l_trx_dtls_tab.LAST 
        LOOP
		DBMS_OUTPUT.PUT_LINE('                          ');
		DBMS_OUTPUT.PUT_LINE('Processing Trx Number :'||l_trx_dtls_tab(indx).trx_number);
        BEGIN 
		   ...
		   DBMS_OUTPUT.PUT_LINE('Cust account ID :'||ln_cust_account_id);
		   DBMS_OUTPUT.PUT_LINE('Cust account Number :'||lc_account_number);
		   DBMS_OUTPUT.PUT_LINE('Cycle Name :'||lc_cycle_name);
		   DBMS_OUTPUT.PUT_LINE('Billing Cycle ID :'||ln_billing_cycle_id);
		   
		   ----------------------------------------------------------------------------
		   /* Step 2: To get the last bill date */
		   ----------------------------------------------------------------------------
		   DBMS_OUTPUT.PUT_LINE('Cust account Site use ID :'||l_trx_dtls_tab(indx).bill_to_site_use_id);
		   BEGIN
		   ...
		   END;
		   DBMS_OUTPUT.PUT_LINE('Billing Date :'||ld_billing_date);
		   ----------------------------------------------------------------------------
		...
		THEN
			...
			DBMS_OUTPUT.PUT_LINE('Failed Records :'||ln_failed_records); 
			DBMS_OUTPUT.PUT_LINE('Error Message :'||l_trx_dtls_tab(indx).error_message); 
        END;

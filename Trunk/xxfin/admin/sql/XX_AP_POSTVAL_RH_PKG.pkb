...
IS
  ...
  indx NUMBER;
  CURSOR ap_lines_cur(p_po_header_id NUMBER, p_item_id NUMBER,p_org_id NUMBER)
  IS
    SELECT l.invoice_id,
      l.quantity_invoiced,
...
IS
  ...
  CURSOR ap_lines_qty_ord_cur(p_po_header_id NUMBER, p_item_id NUMBER,p_org_id NUMBER)
  IS
    SELECT l.invoice_id,
      l.quantity_invoiced,
      l.po_line_location_id
...
BEGIN
  ...
  ln_unbilled_qty:=0;
    BEGIN
      ...
      OPEN ap_lines_cur(l_po_items_tab(indx).po_header_id,l_po_items_tab(indx).inventory_item_id,ln_org_id);
        ...
        ELSE
         --changes start for NAIT-50192, to handle release hold for previous invoice(FIFO) if it is within tolerance
          ...
          EXCEPTION WHEN OTHERS THEN
            ...
            print_debug_msg ('Exception caught while calculating qty percent for Po_header_id:'||TO_CHAR(l_po_items_tab(indx).po_header_id)|| ' ItemId:'||TO_CHAR(l_po_items_tab(indx).inventory_item_id),TRUE);
      ...
      OPEN ap_lines_qty_ord_cur(l_po_items_tab(indx).po_header_id,l_po_items_tab(indx).inventory_item_id,ln_org_id);
      ...
      LOOP
      ln_qty_diff_pt:=0;
      print_debug_msg('For Qty ORD Hold --Processing invoice_id /ln_qtyord_unbilled_qty '||l_ap_lines_qty_tab(l_qindx).invoice_id||' / '||ln_qtyord_unbilled_qty);

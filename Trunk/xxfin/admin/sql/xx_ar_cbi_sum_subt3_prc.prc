CREATE OR REPLACE PROCEDURE xx_ar_cbi_sum_subt3 
                                                (
...
begin
 ...
 lv_sql_stmnt1 :='SELECT  '||lv_col||', SUM(TOTAL_AMOUNT) TOTAL_AMOUNT'
  ...
  -- ================   
  -- First Insert for the third field in the sort order where the value is blank...
  -- ================  
BEGIN
...
END;                    
  ...
  -- ================   
  -- Second Insert for the third field in the sort order where the value is not blank...
  -- ================  
BEGIN  
  source_cur := dbms_sql.open_cursor;
...
END;
 ...
 ELSE
   lv_sql_stmnt7 :='SELECT SUM(TOTAL_AMOUNT) TOTAL_AMOUNT'
                 ||lv_enter
                 ||'FROM xx_ar_cbi_softheaders_hdr_v '
                 ||'WHERE cons_inv_id ='||p_cons_inv_id;
                 
  -- ================   
  -- FourthInsert for the first field in the sort order where the value is not blank...
  -- ================  
BEGIN  
  source_cur := dbms_sql.open_cursor;

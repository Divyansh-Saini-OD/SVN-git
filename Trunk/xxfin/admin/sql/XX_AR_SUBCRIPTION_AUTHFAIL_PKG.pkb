  ...
  *********************************************************************/
  ...
  AS
    ...
    IS
      ...
      AND    APS.segment1        = XFTV.target_value1;
  ...
  BEGIN
    ...
    LOOP
      ...
      ************************************************************/
      logit(p_message => '-----------------------------------------------',
            p_debug   => TRUE);
      logit(p_message => 'Submitting request for adding layout for the program : ' || lc_description_in || ' for vendor #: ' || subscr_vendors_rec.segment1,
            p_debug   => TRUE);
      logit(p_message      => '-----------------------------------------------',
            p_debug   => TRUE);
      /************************************************
      ...
      *************************************************/
      ...
      logit(p_message      => '-----------------------------------------------',
            p_debug   => TRUE);
      logit(p_message => 'Submitting program: ' || lc_description_in || ' for vendor #: ' || subscr_vendors_rec.segment1,
            p_debug   => TRUE);
      logit(p_message      => '-----------------------------------------------',
            p_debug   => TRUE);
     /************************************************************************************************
      ...
      ************************************************************************************************/
      ln_request_id := fnd_request.submit_request(application => lc_appln_shortname_in,
                                                  ...
                                                  argument2   => subscr_vendors_rec.segment1,
      ...
      THEN
        logit(p_message      => '-----------------------------------------------',
              p_debug   => TRUE);
        logit(p_message => 'ERROR in program: ' || lc_description_in || ' for vendor #: ' || subscr_vendors_rec.segment1||' SQLCODE: ' || SQLCODE || ' SQLERRM: ' || SQLERRM,
              p_debug   => TRUE);
        logit(p_message      => '-----------------------------------------------',
              p_debug   => TRUE);
        ...
        ELSE
           ...
           *************************************************************************************************************************/
           ...
           THEN
             logit(p_message      => '-----------------------------------------------',
                   p_debug   => TRUE);
             logit(p_message => 'Program :' || lc_description_in || ' for vendor: ' ||subscr_vendors_rec.segment1 || ' with request id: ' || ln_request_id || ' completed successfully ',
                   p_debug   => TRUE);
             logit(p_message      => '-----------------------------------------------',
                   p_debug   => TRUE);
              BEGIN
              ...
              logit(p_message => '-----------------------------------------------',
                    p_debug   => TRUE);
              logit(p_message => 'Program : XML Report Bursting  for vendor: ' ||subscr_vendors_rec.segment1 || ' submitted with request id: ' || ln_xmlburst_reqid,
                    p_debug   => TRUE);
              logit(p_message      => '-----------------------------------------------',
                    p_debug   => TRUE);
              COMMIT;
              ...
              THEN
                logit(p_message => '-----------------------------------------------',
                      p_debug   => TRUE);
                logit(p_message => 'ERROR in XML Report Bursting program: for vendor #: '|| subscr_vendors_rec.segment1||' SQLCODE: ' || SQLCODE || ' SQLERRM: ' || SQLERRM,
                      p_debug   => TRUE);
                logit(p_message      => '-----------------------------------------------',
                      p_debug   => TRUE);

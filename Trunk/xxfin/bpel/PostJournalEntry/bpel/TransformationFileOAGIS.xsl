...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.3.0(build 070615.0525) AT [WED JAN 23 22:43:02 EST 2008]. -->
?>
    ...
    <ns0:PostJournalEntry>
      <ns0:DataArea>
        <xsl:for-each select="/tns:GLJournalInvoice/tns:Header">
          ...
          <xsl:variable name="numFollowingDetails"
                        select="count(following-sibling::tns:Detail)"/>
          <xsl:variable name="lastDetailInHeader"
                        select="$numFollowingDetails - count(following-sibling::tns:Header[1]/following-sibling::tns:Detail)"/>
            <ns0:JournalEntry>
          ...
          <xsl:for-each select="following-sibling::tns:Detail[position() &lt;= $lastDetailInHeader]">
            <xsl:variable name="detail" select="."/>              

...
--Initialize Variables
        ...
        indx              NUMBER;
        p_req_id          NUMBER;
        v_date            VARCHAR2(11);--:= TO_DATE(p_date,'DD-MON-YYYY'); 
...
-- cursor to collect all Open Trade POs
        ...
        outtable          recordtype;
    BEGIN
    select FND_PROFILE.value('OD_TDM_LAST_RUN_PO') INTO v_date from dual; --TO_DATE(p_date,'DD-MON-YYYY');
    fnd_file.put_line(fnd_file.log,v_date); 
...
--        mo_global.init('PO');
        ...
        ELSIF (p_num_days IS NOT NULL) THEN
            OPEN tdm FOR SELECT
                ...
                --AND to_char(poh.creation_date, 'dd-mon-rrrr') > to_char(p_num_days, 'dd-mon-rrrr');
                AND poh.creation_date > TO_DATE((SYSDATE-p_num_days),'dd-mon-rrrr');
        ...
        ELSE
            OPEN tdm FOR SELECT
                ...
                supa.vendor_site_code_alt,
                poh.attribute_category,
                to_char(to_char(poh.creation_date, 'dd-mon-rrrr hh24:mi:ss')) po_creation_date
                         FROM
                po_headers_all poh,
                ap_supplier_sites_all supa
                         WHERE
                ...
                --AND to_char(poh.creation_date, 'dd-mon-rrrr') > to_char(p_po_date, 'dd-mon-rrrr');
                --AND to_char(poh.creation_date, 'dd-mon-rrrr') > ( to_char((SYSDATE- p_num_days), 'dd-mon-rrrr')  );
        ...
        IF ( p_email IS NOT NULL AND v_one_off = 'N' )
        THEN
            BEGIN
            ...
            END;
        ELSIF (p_send_to_tdm = 'YES' AND p_po_number IS NULL AND p_one_off = 'NO') THEN
            BEGIN
                -- Call file copy 
                p_req_id := fnd_request.submit_request(
                application => 'xxfin',

package com.od.external.view.servlet;
...
public class HomeServlet extends HttpServlet {
    ...
    private static final String SQL = "select CREATION_DATE,LAST_LOGON_DATE from FND_USER " + "where USER_ID =:1";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
        ...
        wrappedRequest.getRequestDispatcher("/faces/ExternalHome").forward(wrappedRequest, response);
    }
    ...
    private void manageAttributes(AppsRequestWrapper wrappedRequest,
        ...
        System.err.println("currentUser>>>>>>>in Home Servlet " + currentUser);
        ...
        Timestamp lastLogon = (Timestamp) httpSession.getAttribute("LAST_LOGON");
        Timestamp createdOn = (Timestamp) httpSession.getAttribute("CREATED_ON");
        if (lastLogon == null || createdOn == null) { // we are going query EBS
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    logger.info("DB query returned CREATION_DATE=" + createdOn + " ; LAST_LOGON_DATE =" + lastLogon);
                ...
                httpSession.setAttribute("FORMATTED_CREATION_DATE", formattedDate);
                // Now we process lastLogon
                longTime = lastLogon.getTime();
                date = new Date(longTime);
                String formattedLogonDate = session.getDateFormat().format(date);
                httpSession.setAttribute("FORMATTED_LOGON_DATE", formattedLogonDate);
                ...
                String formattedTime = new StringBuilder().append(hour)
                                                          .append(":")
                                                          .append(min)
                                                          .append(":")
                                                          .append(sec)
                                                          .toString();
                httpSession.setAttribute("FORMATTED_LOGON_TIME", formattedTime);

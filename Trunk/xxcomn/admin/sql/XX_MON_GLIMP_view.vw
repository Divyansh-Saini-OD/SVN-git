...
-- +===========================================================================================+
-- | Name        : XX_MON_GLIMP                                                                |
-- | Description : View to select details of the AR HV Journal Import Program                  |
-- |Change Record:                                                                             |
-- |===============                                                                            |
-- |Version   Date          Author              Remarks                                        |
-- |=======   ==========   ===========      ===================================================|
-- |   1     2010/07/21     Bhuvaneswary S  Modified to fetch org_id,cycle_date,event,user_name|
-- |   2     2010/09/03     Ritch Hartman   Defect 7765 - GL Archive GL.* - schema name remove |
-- +===========================================================================================+

CREATE OR REPLACE VIEW APPS.XX_MON_GLIMP("REQUEST_ID","PROGRAM_NAME","START_DATE","END_DATE","VOLUME","PROCESSING_TIME_IN_SEC",
"THROUGHPUT","ORG_ID","CYCLE_DATE","EVENT","USER_NAME")
AS 
  SELECT FCR.request_id                                                         REQUEST_ID   -- XXARJRNLIMPPRG Request Id
        ,'GL Imp-ARGLTP-ALL'                                                    PROGRAM_NAME -- GL Journal for ALL types
    ...
    FROM fnd_concurrent_requests FCR
        ...
        --GL Base Tables--
        ,gl_je_batches           GJB
  WHERE FCP.concurrent_program_name   = 'XXARJRNLIMPPRG'   -- OD: AR HV Journal Import
    AND FCP.application_id            = 20043              -- XXFIN application id 
    AND FCR.concurrent_program_id     = FCP.concurrent_program_id
    AND FCR.phase_code                = 'C'
    AND FPO.profile_option_id         = (SELECT profile_option_id FROM apps.fnd_profile_options WHERE profile_option_name = 'ORG_ID')
...
--    AND FCR.request_date                <= (SYSDATE)
    ...
    AND GJB.name                      LIKE 'AR%'||GCFCR.request_id||'%'

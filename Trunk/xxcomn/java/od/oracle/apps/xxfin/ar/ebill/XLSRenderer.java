...
/*
  ...
  -- Component Id: E2059
  -- Script Location:
             $CUSTOM_JAVA_TOP/od/oracle/apps/xxfin/ar/ebill
  -- Description: This is used for eXLS Rendering
  -- Package Usage       :
  -- Name                  Type         Purpose
  -- --------------------  -----------  ------------------------------------------
  -- RenderXLSFile         void         This method has logic for eXLS Rendering
  -- outputAndClearSubTotals int        This is for rendering Subtotal
  -- Notes:
   -- History:
   -- Name            Date         Version    Description
   -- -----           -----        -------    -----------
   -- Sridevi K       9-Sep-2015  1.0         Modified for MOD 4B R2 eXLS Rendering
   -- Sridevi K       18-NOV-2015 1.1         Modified for Defect 1884 for applying case-insensitive sort
   -- Sridevi K       8-JAN-2016  2.0         Modified for Defect 36049 - maximum open cursors exceeded
   -- Hanmanth J      17-Oct-2016 2.1         Removed closing the jdbc connection for defect 38745
  */
...
public class XLSRenderer implements JavaConcurrentProgram {
    ...
    abstract class Aggregation {
        ...
        public Aggregation() {
            shouldDisplayOnNewLine_ = true;
            hasOutline_ = false;
            label_ = "";
            colDistinct_ = -1;
            isGrand_ = false;
            isOverSubtotals_ = false;
        } // don't call directly; use aggConstructor
        ...
        public void add(String value, String distinct) {
            if (!valueSet_.contains(distinct)) {
                valueSet_.add(distinct);
                add(value);
            }
        }
        ...
        public boolean isOverSubtotals() {
            return isOverSubtotals_;
        }
        ...
        public boolean shouldDisplayOnNewLine() {
            return shouldDisplayOnNewLine_;
        }

        public int colDistinct() {
            return colDistinct_;
        }

        public int colDisplayLabel() {
            return colDisplayLabel_;
        }

        public int colDisplayValue() {
            return colDisplayValue_;
        }
        ...
        public int colAggregate() {
            return colAggregate_;
        }
        ...
        public String headerForLabelCol() {
            return headerForLabelCol_;
        }

        public String headerForValueCol() {
            return headerForValueCol_;
        }

        public DataType aggrDataType() {
            return aggrDataType_;
        }

        public String aggrFormat() {
            return aggrFormat_;
        }

        public CellStyle cellStyleForValue() {
            return cellStyleForValue_;
        }
        ...
        public void setIsGrand(boolean isGrand) {
            isGrand_ = isGrand;
        }

        public void setIsOverSubtotals(boolean isOverSubtotals) {
            isOverSubtotals_ = isOverSubtotals;
        }

        public void setHasOutline(boolean has) {
            hasOutline_ = has;
        }

        public void setShouldDisplayOnNewLine(boolean should) {
            shouldDisplayOnNewLine_ = should;
        }

        public void setColGroup(int col) {
            colGroup_ = col;
        }

        public void setColAggregate(int col) {
            colAggregate_ = col;
        }

        public void setColDistinct(int col) {
            colDistinct_ = col;
            valueSet_ = new HashSet();
        }

        public void setColDisplayLabel(int col) {
            colDisplayLabel_ = col;
        }

        public void setColDisplayValue(int col) {
            colDisplayValue_ = col;
        }

        public void setHeaderForLabelCol(String header) {
            headerForLabelCol_ = header;
        }

        public void setHeaderForValueCol(String header) {
            headerForValueCol_ = header;
        }

        private void setAggrDataType(String dataType) { // set through aggConstructor
            if (dataType == null || dataType.equals("VARCHAR2"))
                aggrDataType_ = DataType.VARCHAR2;
            else if (dataType.equals("NUMERIC"))
                aggrDataType_ = DataType.NUMERIC;
            else if (dataType.equals("DATE"))
                aggrDataType_ = DataType.DATE;
            else
                aggrDataType_ = DataType.VARCHAR2;
        }
        ...
        public void setAggrFormat(String aggrFormat) {
            aggrFormat_ = aggrFormat;
        }

        public void setCellStyleForValue(CellStyle cellStyle) {
            cellStyleForValue_ = cellStyle;
        }

        public ArrayList grandAggs() {
            return grandAggs_;
        }
            ...
            if (grandAggs_ != null) {
                for (int i = 0; i < grandAggs_.size(); i++) {
                    ...
                    if (grandAgg.key().equals("AVG")) {
                        AggregationAverage grandAvg = 
                            (AggregationAverage)grandAgg;
                        ...
                        AggregationCount grandCount = 
                            (AggregationCount)grandAgg;
                        if (grandCount.isOverSubtotals() || 
                            !key().equals("COUNT") || 
                            grandCount.aggrDataType() != DataType.NUMERIC)
                            grandCount.add(toString());
                        else
                            grandCount.add(toDouble());
                    } else if (grandAgg.aggrDataType() != DataType.NUMERIC)
                        grandAgg.add(toString());
                    else
                        grandAgg.add(toDouble());
                }
            }
        }
        ...
        public void setLabel(String label) {
            ...
            if (!isGrand()) {
                ...
                if (label == null)
                    ...
                    label += "||GRANDAUTO";
            } // Add Grand Total automatically, unless suppressed with NOGRAND
            if (label != null) {
                ...
                for (int i = 0; i < saAggDef.length; i++) {
                    ...
                    if (!isGrand() && saAggDef[i].startsWith("GRAND")) {
                        if (!saAggDef[i].startsWith("GRANDAUTO") || 
                            bAddGrandTotal) {
                            if (grandAggs_ == null)
                                grandAggs_ = new ArrayList();
                            Aggregation agg = 
                                aggConstructor(key(), aggrDataType());
                            if (agg != null) {
                                ...
                                String sNewLabel = 
                                    saAggDef[i].replace(saAggAtt[0], "");
                                if (sNewLabel.startsWith("|"))
                                    sNewLabel = sNewLabel.substring(1);
                        ...
                        if (grandAggs_ == null)
                            grandAggs_ = new ArrayList();
                        Aggregation agg = 
                            aggConstructor(saAggAtt[0].replace("SUB", ""), 
                                           aggrDataType());
                        if (agg != null) {
                            agg.setIsGrand(true);
                        ...
                        for (int j = 0; j < saAggAtt.length; j++) {
                            if (saAggAtt[j] != null) {
                                if (saAggAtt[j].startsWith("NOGRAND"))
                                    bAddGrandTotal = false;
                                else if (saAggAtt[j].startsWith("OUTLINE"))
                                    setHasOutline(true);
                                else if (saAggAtt[j].startsWith("LABELCOL")) {
                                    setColDisplayLabel(Integer.parseInt(saAggAtt[j].replace("LABELCOL", 
                                                                                            "").replace("=", 
                                                                                                        "").trim()));
                                    bValueColSet = true;
                                } else if (saAggAtt[j].startsWith("VALUECOL"))
                                    setColDisplayValue(Integer.parseInt(saAggAtt[j].replace("VALUECOL", 
                                                                                            "").replace("=", 
                                                                                                        "").trim()));
                                else if (saAggAtt[j].startsWith("DISTINCTCOL"))
                                    setColDistinct(Integer.parseInt(saAggAtt[j].replace("DISTINCTCOL", 
                                                                                        "").replace("=", 
                                                                                                    "").trim()));
                                else if (saAggAtt[j].startsWith("VALUEHEADER"))
                                    setHeaderForValueCol(saAggAtt[j].replace("VALUEHEADER", 
                                                                             "").replace("=", 
                                                                                         "").trim());
                                else if (saAggAtt[j].startsWith("LABELHEADER"))
                                    setHeaderForLabelCol(saAggAtt[j].replace("LABELHEADER", 
                                                                             "").replace("=", 
                                                                                         "").trim());
                                else if (saAggAtt[j].startsWith("FORMAT"))
                                    setAggrFormat(saAggAtt[j].replace("FORMAT", 
                                                                      "").replace("=", 
                                                                                  "").trim());
                                else if (saAggAtt[j].startsWith("NONEWROW"))
                                    setShouldDisplayOnNewLine(false);
                                else if (j == saAggAtt.length - 1) {
                                    label_ = saAggAtt[j];
                                    if (!bValueColSet && 
                                        label_.indexOf("&VALUE") >= 0 && 
                                        colDisplayLabel_ != colDisplayValue_)
                                        colDisplayValue_ = colDisplayLabel_;
                                }
                            }
                        }
                    }
        ...
        public String displayLabel(String group) {
            ...
            if (sDisplay == null || sDisplay.equals("")) {
                sDisplay = group + " " + name();
                if (colDisplayLabel() == colDisplayValue())
                    sDisplay += " " + toString();
            } else {
                sDisplay = sDisplay.replace("&GROUP", group);
                if (sDisplay.indexOf("&VALUE") >= 0)
                    sDisplay = sDisplay.replace("&VALUE", toString());
                else if (colDisplayLabel() == colDisplayValue())
                    sDisplay = sDisplay + " " + toString();
            }
            return sDisplay;
        }

        private Aggregation aggConstructor(String aggFunction, 
                                           DataType aggDataType) {
            Aggregation agg = null;
            if (aggFunction.equals(AggregationSum.key)) {
                if (aggDataType == DataType.NUMERIC)
                    agg = new AggregationSum();
            } else if (aggFunction.equals(AggregationCount.key))
                agg = new AggregationCount();
            else if (aggFunction.equals(AggregationAverage.key)) {
                if (aggDataType == DataType.NUMERIC)
                    agg = new AggregationAverage();
            } else if (aggFunction.equals(AggregationMax.key))
                agg = new AggregationMax();
            else if (aggFunction.equals(AggregationMin.key))
                agg = new AggregationMin();
            if (agg != null)
                agg.setAggrDataType(aggDataType);
            return agg;
        }
    }
    ...
    class AggregationSum extends Aggregation {
        ...
        private static final double PRECISION_MULTIPLIER = 
            10000.0; // 4 decimal places; //money doesn't work well in doubles because of rounding issues, so we store sums in long data //types as hundredths of pennies
        //private Long sum_;  //Defect 10388

        BigDecimal sum_ = new BigDecimal("0.00"); //Defect 10388

        public AggregationSum() {
            setKey(key);
            sum_ = new BigDecimal("0.00");
        }

        public String name() {
            if (isOverSubtotals())
                return "Sum of " + parentName() + "s";
            else
                return "Sum";
        }
        ...
        public boolean validFunctionValue() {
            return true;
        }

        public Double toDouble() {
            return sum_.doubleValue();
        } //Defect 10388

        public String toString() {
            return sum_.toString();
        }
    }
    ...
    class AggregationAverage extends Aggregation {
        ...
        public AggregationAverage() {
            setKey(key);
            count_ = new AggregationCount();
            sum_ = new AggregationSum();
        }

        public String name() {
            if (isOverSubtotals())
                return "Average of " + parentName() + "s";
            else
                return "Average";
        }
        ...
        public Double count() {
            return count_.toDouble();
        }

        public Double sum() {
            return sum_.toDouble();
        }
        ...
        public void add(String sValue) {
            ...
            if (aggrDataType() == DataType.NUMERIC) {
                ...
                if (bNoValue) {
                    bNoValue = false;
                    dMin_ = dValue;
                } else if (dValue < dMin_)
                    dMin_ = dValue;
            } else { // Dates are staged in canonical form "yyyy-MM-dd", so straight text comparison should work
                if (bNoValue) {
                    bNoValue = false;
                    sMin_ = sValue;
                } else if (sMin_.compareToIgnoreCase(sValue) > 0)
                    sMin_ = sValue;
            }
        }
    ...
    class AggregationGroup {
        ...
        public void addAggregation(Aggregation agg) {
            if (aggregations_ == null)
                aggregations_ = new ArrayList();
            aggregations_.add(agg);
        }
    }

    private Connection connection; // Database Connection Object
    double m_nPrecisionMultiplier = 
        10000.0; // 5 decimal places; money doesn't work well in doubles because of rounding issues, so we store sums in long data types as hundredths of pennies
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //ods.setURL("jdbc:oracle:thin:apps@//chvsdb06d.na.odcorp.net:1541/GSIDEV02");
    ...
    public void runProgram(CpContext cpcontext) {
        ...
        try {
            ...
            ex.printStackTrace();
            System.out.println("\nXLS file rendering thread failed" + 
                               ex.toString());
        }
    ...
    private void RenderXLSFiles(int nThreadID, 
                                int nThreadCount) throws Exception {
        CallableStatement csFiles = null;
        ResultSet rsFiles = null;
        try {
            csFiles = 
                    connection.prepareCall("BEGIN XX_AR_EBL_RENDER_XLS_PKG.XLS_FILES_TO_RENDER(?,?,?); END;"); // Get the file rows that need to be rendered
            ...
            while (rsFiles.next()) {
                ...
                try {
                    ...
                    System.out.println("eBill file_id #" + nFileID + 
                                       " rendered");
                    ...
                    try {
                        ...
                        pstmt = 
                                connection.prepareStatement("update XX_AR_EBL_FILE SET file_data=?, status='RENDER_ERROR', status_detail=?, last_updated_by=fnd_global.user_id, last_update_date=SYSDATE, last_update_login=fnd_global.login_id WHERE file_id = ?");
                        ...
                        System.out.println("Error setting XX_AR_EBLFILE status to RENDER_ERROR for file_id=" + 
                                           nFileID + 
                                           "; XX_AR_EBL_COMMON_UTIL_PKG.UPDATE_DATA_EXTRACT_STATUS not called; XLS render for this file_id will be retried automatically on next run, if no action is taken.).  ERROR message: " + 
                                           updateEx.toString());
                    } finally {
                        if (pstmt != null)
                            pstmt.close();
                    ...
                    if (pStatusUpdateSuccessful) {
                        ...
                        try {
                            cstmt = 
                                    connection.prepareCall("BEGIN XX_AR_EBL_COMMON_UTIL_PKG.UPDATE_DATA_EXTRACT_STATUS(?,?); END;");
                            cstmt.setInt(1, nFileID);
                            cstmt.setString(2, sInvoiceType);
                            cstmt.execute();
                        } catch (Exception ex) { // need to proceed, but this will write to log
                            System.out.println("ERROR: XLS rendering failed for file_id " + 
                                               nFileID + 
                                               " and XX_AR_EBL_FILE status was set to RENDER_ERROR.  However, XX_AR_EBL_COMMON_UTIL_PKG.UPDATE_DATA_EXTRACT_STATUS failed: " + 
                                               ex.toString());
                        } finally {
                            if (cstmt != null)
                                cstmt.close();
    ...
    private void RenderXLSFile(int nFileID, 
                               String sInvoiceType) throws SQLException, 
                                                           IOException, 
                                                           Exception {
        ...
        SimpleDateFormat canonicalDateFormat = 
        ...
        DataType[] dtaColumnDataType = 
            new DataType[256]; // 256 is current max cols for the Excel versions we are generating
        AggregationGroup[] aAggregationGroups;
        boolean bIncludeHeader = 
            false; // this means show the column labels; the other header attributes (e.g., billing_for, billing_period, etc) are hidden if the logo path is null
        Row r = null;
        Cell c = null;
        ...
        cstmt.registerOutParameter(3, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_description", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(4, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_cell_cons_bill_number", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(5, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_cell_billing_period", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(6, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_cell_pay_terms", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(7, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_cell_due_date", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(8, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_billing_for", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(9, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_billing_id", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(10, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_aops_id", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(11, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_include_header", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(12, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_logo_hyperlink_url", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(13, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_logo_alt_text", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(14, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_logo_path", OracleTypes.VARCHAR);
        //Added to get additional columns as part of Module 4B Release 1 Start
        cstmt.registerOutParameter(15, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_merchadise_label", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(16, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_merchadise_amt", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(17, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_salestax_label", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(18, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_salestax_amt", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(19, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_misc_label", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(20, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_misc_amt", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(21, 
                                   OracleTypes.VARCHAR); // cstmt.registerOutParameter("x_total_gift_card_label", OracleTypes.VARCHAR);
        cstmt.registerOutParameter(22, 
        ...
        String sLogoPath = 
            cstmt.getString(14); // cstmt.getString("x_logo_path");
        ...
        if (sLogoPath != null) {
            if (gsXXFIN_TOP != null)
                sLogoPath = sLogoPath.replace("$XXFIN_TOP", gsXXFIN_TOP);
            ...
            c.setCellValue(cstmt.getString(15)); // c.setCellValue(cstmt.getString("x_total_merchadise_label"));  //Module 4B Release 1
            c = r.createCell(5); //Module 4B Release 1
            //c.setCellValue(cstmt.getString(16)); // c.setCellValue(cstmt.getString("x_total_merchadise_amt"));  //Module 4B Release 1

            double merchandise_amt = Double.parseDouble(cstmt.getString(16));
            c.setCellValue(merchandise_amt);
            /*
            CellStyle merchandise_amt = wb.createCellStyle();
            DataFormat df1 = wb.createDataFormat();
            merchandise_amt.setDataFormat(df.getFormat("#,##0.0")); //or cs.setDataFormat((short)7);
            c.setCellValue(cstmt.getString(16));
            c.setCellStyle(merchandise_amt);
            */
            ...
            c = r.createCell(4); //Module 4B Release 1
            c.setCellValue(cstmt.getString(17)); // c.setCellValue(cstmt.getString("x_total_salestax_label"));    //Module 4B Release 1
            c = r.createCell(5); //Module 4B Release 1
            //c.setCellValue(cstmt.getString(18)); // c.setCellValue(cstmt.getString("x_total_salestax_amt"));    //Module 4B Release 1

            double salestax_amt = Double.parseDouble(cstmt.getString(18));
            c.setCellValue(salestax_amt);
            ...
            c = r.createCell(4); //Module 4B Release 1
            c.setCellValue(cstmt.getString(19)); // c.setCellValue(cstmt.getString("x_total_misc_label"));    //Module 4B Release 1
            c = r.createCell(5); //Module 4B Release 1
            //c.setCellValue(cstmt.getString(20)); // c.setCellValue(cstmt.getString("x_total_misc_amt"));    //Module 4B Release 1

            double misc_amt = Double.parseDouble(cstmt.getString(20));
            c.setCellValue(misc_amt);
            ...
            if (cstmt.getString(22) != null) {
                double gift_card_amt = Double.parseDouble(cstmt.getString(22));
                c.setCellValue(gift_card_amt);
            }
        ...
        String sIncludeHeader = 
            cstmt.getString(11); // String sIncludeHeader = cstmt.getString("x_include_header");
        //sIncludeHeader = "Y"; // for debugging
        if (sIncludeHeader != null && sIncludeHeader.equals("Y")) {
            bIncludeHeader = true;
        }

        String sSplitTabsBy = cstmt.getString(23);

        String sEnableXLSubTotal = cstmt.getString(24);
        ...
        try {
            cstmt = 
                    connection.prepareCall("BEGIN XX_AR_EBL_RENDER_XLS_PKG.XLS_FILE_COLS(?,?); END;");
            cstmt.setInt(1, nFileID);
           ...
           // }
            while (rset.next()) {
                ...
                if (bIncludeHeader) {
                    ...
                    if (sDataType != null && 
                        (sDataType.equals("NUMERIC") || sDataType.equals("DATE")))
                ...
                if (sCellFormat != null)
                    cellStyle.setDataFormat(df.getFormat(sCellFormat));
                /*
...
*/
                if (sDataType != null && 
                    ...
                    if (sDataType.equals("DATE"))
                        dtaColumnDataType[nColumnCount] = DataType.DATE;
                    else
                        dtaColumnDataType[nColumnCount] = DataType.NUMERIC;
                } else {
        ...
        try { // get group by columns
            cstmt = 
                    connection.prepareCall("BEGIN XX_AR_EBL_RENDER_XLS_PKG.XLS_FILE_AGGS(?,?); END;");
            cstmt.setInt(1, nFileID);
            cstmt.registerOutParameter(2, OracleTypes.CURSOR);
            cstmt.execute();
            rset = (ResultSet)cstmt.getObject(2);
            ...
            while (rset.next()) {
                try {
                    ...
                    Aggregation agg = 
                        aggConstructor(sAggregationFunction, sAggrDataType);
                    if (agg == null) {
                        if (sWarnings.length() > 0)
                            sWarnings += "\n";
                        String sMsg = 
                            "Aggregation function " + sAggregationFunction + 
                            " unknown or not allowed on field data type " + 
                            sAggrDataType;
                        sWarnings += sMsg;
                    ...
                    if (sAggrFormat != null && 
                        agg.colDisplayValue() != agg.colDisplayLabel() && 
                        !sAggregationFunction.equals("COUNT") && 
                        (agg.aggrDataType() == DataType.NUMERIC || 
                         agg.aggrDataType() == DataType.DATE)) {
                        cellStyle = wb.createCellStyle();
                        if (agg.aggrFormat() != null)
                            cellStyle.setDataFormat(df.getFormat(sAggrFormat));
                    ...
                    for (int i = 0; grandAggs != null && i < grandAggs.size(); 
                         i++) {
                        Aggregation grandAgg = (Aggregation)grandAggs.get(i);
                        if (grandAgg.aggrFormat() != null && 
                            grandAgg.colDisplayValue() != 
                            grandAgg.colDisplayLabel() && 
                            !grandAgg.key().equals("COUNT") && 
                            (grandAgg.aggrDataType() == DataType.NUMERIC || 
                             grandAgg.aggrDataType() == DataType.DATE)) {
                            if (cellStyle != null && 
                                sAggrFormat.equals(grandAgg.aggrFormat()))
        ...
        aAggregationGroups = 
                new AggregationGroup[alAggregationGroups.size()]; // now compress and order the sparse array
        int nMaxCol = 
            nColumnCount; // nMaxCol is how many columns to autosize (in case aggregations are displayed to the right of staged fields)
        for (int i = 0; i < alAggregationGroups.size(); i++) {
            ...
            for (int j = 0; j < alAggs.size(); j++) {
                ...
                nMaxCol = 
                        Math.max(nMaxCol, setAggColHeaders(agg, rHeaderRow, nColumnCount, 
                                                           headerCellStyle));
                ArrayList grandAggs = agg.grandAggs();
                if (grandAggs != null)
                    for (int k = 0; k < grandAggs.size(); k++)
                        nMaxCol = 
                                Math.max(nMaxCol, setAggColHeaders((Aggregation)grandAggs.get(k), 
                                                                   rHeaderRow, 
                                                                   nColumnCount, 
                                                                   headerCellStyle));
            }
        ...
        try { // get sort order
            cstmt = 
                    connection.prepareCall("BEGIN XX_AR_EBL_RENDER_XLS_PKG.XLS_FILE_SORT_COLS(?,?); END;");
            cstmt.setInt(1, nFileID);
            cstmt.registerOutParameter(2, OracleTypes.CURSOR);
            cstmt.execute();
            rset = (ResultSet)cstmt.getObject(2);
            ...
            while (rset.next()) {
                ...
                if (sDataType != null && sDataType.equals("NUMERIC")) {
                    ...
                    sbSortColumns.append(",LOWER(COLUMN");
                    sbSortColumns.append(rset.getInt("column_number"));
                    sbSortColumns.append(")");
                }
                sSortType = rset.getString("sort_type");
                if (sSortType != null && sSortType.equals("DESC"))
                    sbSortColumns.append(" DESC");
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        ...
        //Start - Add logic for Mod 4B R2
        //This is for getting Tab names and creating tabs
        CallableStatement csTabsInfo = null;
        ResultSet rsTabsInfo = null;
        String custDocId = null;
        int nMaxTabs = 0;

        try {
            csTabsInfo = 
                    connection.prepareCall("BEGIN XX_AR_EBL_RENDER_XLS_PKG.GET_XL_TABS_INFO(?,?,?,?); END;", 
                                           ResultSet.TYPE_SCROLL_INSENSITIVE, 
                                           ResultSet.CONCUR_READ_ONLY);

            csTabsInfo.setInt(1, nFileID);
            csTabsInfo.setString(2, custDocId); // 75945472);
            csTabsInfo.registerOutParameter(3, OracleTypes.CURSOR);
            csTabsInfo.registerOutParameter(4, OracleTypes.INTEGER);
            csTabsInfo.execute();


            nMaxTabs = csTabsInfo.getInt(4);
            //System.out.println("nMaxTabs " + nMaxTabs);


            rsTabsInfo = (ResultSet)csTabsInfo.getObject(3);


            ArrayList tabName = new ArrayList();
            ArrayList tabNum = new ArrayList();

            while (rsTabsInfo.next()) {
                tabName.add(rsTabsInfo.getString("tab_name"));
                tabNum.add(rsTabsInfo.getInt("tab_num"));
            }

            int nTotalTabs = tabNum.size();


            //System.out.println("total no of tabs " + nTotalTabs);

            if ((nMaxTabs != 0) && (nTotalTabs > nMaxTabs)) {

                if (sWarnings.length() > 0)
                    sWarnings += "\n";
                sWarnings += 
                        "Split by tabs were not created. " + nTotalTabs + " tabs are required to be created, which exceeds Max Tabs profile value " + 
                        nMaxTabs + ".";
                ...
                for (int i = 0; i < tabNum.size(); i++) {
                    int nTabNum = (Integer)tabNum.get(i);


                    String sTabName = (String)tabName.get(i);
                    //System.out.println("Rendering XLS Tab " + sInvoiceType);
                    Sheet newSheet = wb.cloneSheet(0);
                    int newIndex = wb.getSheetIndex(newSheet);
                    //System.out.println("Nex Index value " + newIndex);
                    wb.setSheetName((newIndex), sTabName);
                    //commented on 10/22/2015 
                    if (sLogoPath != null){ 
                      for (int k = 0; k < 5; k++) {
                          Row row = newSheet.getRow(k);
                          if (row != null) {
            ...
            ex.printStackTrace();
            throw new Exception("Error in while rendering tabs\n" + 
                                ex.toString());
        } finally {
            if (rsTabsInfo != null)
                rsTabsInfo.close();
        ...
        //System.out.println("alAggregationGroups.size()" + 
        //                   alAggregationGroups.size());
        if (alAggregationGroups.size() == 0) {
            //System.out.println("sEnableXLSubTotal set to N as no aggregation setup");
            sEnableXLSubTotal = "N";
        }
        //End - Add logic for Mod 4B R2

        //added the DISTINCT Clause in the SELECT statement for Defect# to remove duplicate from EXCEL Sheet output
        StringBuilder sbSQL = new StringBuilder("select  * from ( SELECT ");
        for (int i = 1; i <= nColumnCount; i++) {
            sbSQL.append("COLUMN");
            sbSQL.append(i);
            sbSQL.append(",");
        }
        //  "  AND CONSOLIDATED_BILL_NUMBER=20865401 " + 
        // "   AND COLUMN3=\'735NE125THST\'  "+
        sbSQL.append("rec_type FROM XX_AR_EBL_XLS_STG WHERE file_id=? " + 
                     "AND rec_order<>0 ORDER BY ");
        ...
        System.out.println("SQL Query :" + sbSQL);
        ...
        try { // get staged bill data
            ...
            while (rset.next()) {
                ...
                if (nRowNumber == 1) {
                    for (int i = 0; i < aAggregationGroups.length; i++) {
                        sColVal = 
                                rset.getString(aAggregationGroups[i].getCol());
                        if (sColVal != null)
                            sColVal = sColVal.trim();
                        aAggregationGroups[i].setVal(sColVal);
                    }
                } else {
                    int nFirstChangedGroup = 
                        firstChangedGroup(rset, aAggregationGroups); // highest precedence group change forces change to all lower groups
                    if (nFirstChangedGroup >= 0) {
                        nNextDataRow = 
                                outputAndClearSubtotals(s, nNextDataRow, nColumnCount, 
                                                        aAggregationGroups, 
                                                        nFirstChangedGroup, 
                                                        aggLabelStyle, 
                                                        canonicalDateFormat, 
                        ...
                        for (int i = nFirstChangedGroup; 
                             i < aAggregationGroups.length; i++) {
                            sColVal = 
                                    rset.getString(aAggregationGroups[i].getCol());
                            if (sColVal != null)
                                sColVal = sColVal.trim();
                ...
                for (int i = 0; i < aAggregationGroups.length; i++) {
                    ...
                    for (int j = 0; j < alAggregations.size(); j++) {
                        ...
                        if (colDistinct > 0)
                            agg.add(sAggVal, 
                                    rset.getString(colDistinct)); // means only add if the "distinct col" val hasn't been seen before for current group
                        else
                            agg.add(sAggVal);
                    }
                }
            ...
            nNextDataRow = 
                    outputAndClearSubtotals(s, nNextDataRow, nColumnCount, 
                                            aAggregationGroups, 0, 
                                            aggLabelStyle, canonicalDateFormat, 
                                            sEnableXLSubTotal);


            int nLastRow = nNextDataRow;
            if ("Y".equals(sEnableXLSubTotal)) {
	      if (sLogoPath != null) 
                s.groupRow(6, nLastRow - 1);
              else
	         s.groupRow(1, nLastRow - 1);
            }
            outputGrandTotals(s, nNextDataRow, nColumnCount, 
                              aAggregationGroups, aggLabelStyle, 
                              canonicalDateFormat);
        } catch (Exception ex) {
            ex.printStackTrace();
        ...
        //Start - Add logic for Mod 4B R2
        //This is for getting Tab data
        for (int j = 1; j < wb.getNumberOfSheets(); j++) {
            Sheet tabSheet = wb.getSheetAt(j);
            int lastIndex = tabSheet.getLastRowNum();
            //Added if condition on 10/22/2015
            //System.out.println("Last Index value is"+lastIndex );
            if (lastIndex != 0) {              
              try{
                    tabSheet.shiftRows(5, lastIndex, -5);
                 } catch (IllegalArgumentException iae) {
                    tabSheet.removeRow(tabSheet.getRow(0));
                 }
            }
        }

        PreparedStatement pTabstmt = null;
        // CallableStatement csTabsData = null;
        ResultSet rsTabsData = null;
        int nTabsNextDataRow = 1;

        for (int j = 1; j < wb.getNumberOfSheets(); j++) {
            nTabsNextDataRow = 1;

            for (int m = 0; m < aAggregationGroups.length; m++) {
                aAggregationGroups[m].setStartRow(1);
            ...
            // nTabRowNumber = 0;
            int tabIndex = wb.getSheetIndex(tabSheet);
            //System.out.println("tabIndex  :" + tabIndex);

            StringBuilder sbTabSQL = 
                new StringBuilder("select  * from ( SELECT ");
            for (int i = 1; i < nColumnCount; i++) {
                // System.out.println("nColumnCount :" +nColumnCount+" i:"+i);
                sbTabSQL.append("COLUMN");
                sbTabSQL.append(i);
                sbTabSQL.append(",");
            }
            sbTabSQL.append("COLUMN" + nColumnCount);

            //sbSQL.append(" "rec_type FROM XX_AR_EBL_XL_TAB_SPLIT_DTL WHERE file_id=? " + 
            sbTabSQL.append(" FROM XX_AR_EBL_XL_TAB_SPLIT_DTL WHERE file_id=? " + 
                            " AND tab_num=? ORDER BY ");
            String sTabSortColumns = sbSortColumns.toString();
            if (sTabSortColumns.equals(""))
                sbTabSQL.append("consolidated_bill_number, customer_trx_id, rec_order, trx_line_number");
            else {
                sbTabSQL.append(sSortColumns.substring(1));
                sbTabSQL.append(", rec_order, trx_line_number");
            }

            sbTabSQL.append(sTabSortColumns);
            sbTabSQL.append(')');

            System.out.println("SQL Tab Query :" + sbTabSQL);

            try { // get staged bill data
                pTabstmt = connection.prepareStatement(sbTabSQL.toString());
                pTabstmt.setInt(1, nFileID);
                pTabstmt.setInt(2, tabIndex);
                rsTabsData = pTabstmt.executeQuery();
                String sTabColVal;
                //       int nTabRowNumber = 0;


                //   Sheet s1 = wb.getSheetAt(j);

                int nTabRowNumber = 0;
                while (rsTabsData.next()) {
                    //System.out.println("tab******************************************************nTabRowNumber" + 
                     //                  nTabRowNumber);
                    nTabRowNumber++;
                    if (nTabRowNumber == 1) {
                        for (int i = 0; i < aAggregationGroups.length; i++) {
                            sTabColVal = 
                                    rsTabsData.getString(aAggregationGroups[i].getCol());
                            if (sTabColVal != null)
                                sTabColVal = sTabColVal.trim();
                            aAggregationGroups[i].setVal(sTabColVal);

                            //System.out.println("rsTabsData.getString(aAggregationGroups[i].getCol())" + 
                            //                   rsTabsData.getString(aAggregationGroups[i].getCol()));
                            //System.out.println(aAggregationGroups[i].getVal());
                        }
                    } else {
                        int nFirstChangedGroup = 
                            firstChangedGroup(rsTabsData, aAggregationGroups);

                        //System.out.println("nFirstChangedGroup" + 
                        //                   nFirstChangedGroup);
                        if (nFirstChangedGroup >= 0) {
                            nTabsNextDataRow = 
                                    outputAndClearSubtotals(tabSheet, 
                                                            nTabsNextDataRow, 
                                                            nColumnCount, 
                                                            aAggregationGroups, 
                                                            nFirstChangedGroup, 
                    ...
                    //System.out.println("aAggregationGroups.length" + 
                    //                   aAggregationGroups.length);

                    for (int i = 0; i < aAggregationGroups.length; i++) {
                        AggregationGroup ag = aAggregationGroups[i];
                        ArrayList alAggregations = ag.getAggregations();

                        //System.out.println("alAggregations.size()" + 
                        //                   alAggregations.size());

                        for (int k = 0; k < alAggregations.size(); k++) {
                            Aggregation agg = 
                                (Aggregation)alAggregations.get(k);
                            String sAggVal = 
                                rsTabsData.getString(agg.colAggregate());
                            //System.out.println("k" + k + "  " + 
                            //                   agg.toString());
                            //System.out.println("sAggVal:" + sAggVal);


                            int colDistinct = agg.colDistinct();
                            if (colDistinct > 0)
                                agg.add(sAggVal, 
                                        rsTabsData.getString(colDistinct)); // means only add if the "distinct col" val hasn't been seen before for current group
                            else
                                agg.add(sAggVal);
                        }
                    }
                    ...
                    //                   nTabsNextDataRow);
                    r = tabSheet.createRow(nTabsNextDataRow++);
                    for (int i = 0; i < nColumnCount; i++) {
                        c = r.createCell(i);
                        c.setCellStyle((CellStyle)alsCellStyle.get(i));
                        sTabColVal = rsTabsData.getString(i + 1);

                        //System.out.println("i :" + i + " sColVal  :" + 
                        //                   sTabColVal + 
                        //                   " dtaColumnDataType[i]:" + 
                        //                   dtaColumnDataType[i]);
                        ...
                        if (dtaColumnDataType[i] == DataType.DATE) {
                            try {
                                c.setCellValue(canonicalDateFormat.parse(sTabColVal));

                            } catch (Exception e) {
                                try {
                                    c.setCellValue(Double.parseDouble(sTabColVal));
                                } catch (Exception e2) {
                                    c.setCellValue(sTabColVal);
                                }
                            }
                        } else if (dtaColumnDataType[i] == DataType.NUMERIC) {
                            try {
                ...
                nTabsNextDataRow = 
                        outputAndClearSubtotals(tabSheet, nTabsNextDataRow, 
                                                nColumnCount, 
                                                aAggregationGroups, 0, 
                                                aggLabelStyle, 
                                                canonicalDateFormat, 
                                                sEnableXLSubTotal);
               int nLastRow = nTabsNextDataRow;
               if ("Y".equals(sEnableXLSubTotal)) {
                   tabSheet.groupRow(1, nLastRow - 1);
               }
  
              outputGrandTotals(tabSheet, nTabsNextDataRow, nColumnCount, 
                              aAggregationGroups, aggLabelStyle, 
                              canonicalDateFormat);
        ...
        System.setProperty("java.awt.headless", 
                           "true"); // only need to do this if graphical environment is not available

        //Start - Modified logic for Mod 4B R2
        for (int i = 0; i <= nMaxCol; i++) {
            ...
            for (int j = 0; j < wb.getNumberOfSheets(); j++) {
                Sheet s1 = wb.getSheetAt(j);
                s1.autoSizeColumn(i);
                Double w = new Double(s1.getColumnWidth(i));
                //System.out.println("column " + i + " width = " + w);
                w = 
  w * 1.24; // without the graphical environment, autosize on server is undersizing the columns
                s1.setColumnWidth(i, w.intValue());
            }

            //End - Modified logic for Mod 4B R2
            //         else if (w>0)
        ...
        if (sLogoPath != null) {
            try {
                ...
                insertGraphic(wb, s, sLogoPath, sLogoHyperlink, 
                              sLogoHyperlinkLabel); // "brand.png" // "/home/u250648/brand.png"
                ...
                System.out.println("Warning: Unable to insertGraphic for eBill file_id " + 
                                   nFileID + '\n' + 
...
*/
        ...
        try {
            ...
            throw new Exception("Error writing output to ByteArrayOutputStream for file_id " + 
                                nFileID + '\n' + ex.toString());
        }
...
*/
        ...
        BLOB blob = 
            BLOB.createTemporary(connection, true, BLOB.DURATION_SESSION);
        ...
        try {
            pstmt = 
                    connection.prepareStatement("UPDATE XX_AR_EBL_FILE SET file_data=?, status=?, status_detail=?, last_updated_by=fnd_global.user_id, last_update_date=SYSDATE, last_update_login=fnd_global.login_id  WHERE file_id = ?");
            ...
            if (nUpdateCount < 1)
                throw new SQLException("Unable to update blob for file_id " + 
                                       nFileID);
            else if (nUpdateCount > 1)
                throw new SQLException("Too many rows updated for file_id " + 
                                       nFileID);
            else
                System.out.println("Blob update successful");
        } catch (Exception ex) {
            throw new Exception("Error updating XX_AR_EBL_FILE for file_id " + 
                                nFileID + ": " + ex.toString());
        } finally {
            if (pstmt != null)
                pstmt.close();
        ...
        try {
            cstmt = 
                    connection.prepareCall("BEGIN XX_AR_EBL_COMMON_UTIL_PKG.UPDATE_BILL_STATUS_eXLS(?,?); END;");
            cstmt.setInt(1, nFileID);
            cstmt.setString(2, sInvoiceType);
            cstmt.execute();
            ...
            String sMsg = 
                "ERROR: XLS rendering was successful for file_id " + nFileID + 
                " and file_data blob was updated.  However, XX_AR_EBL_COMMON_UTIL_PKG.UPDATE_BILL_STATUS_eXLS failed: " + 
                ex.toString();
    ...
    private int setAggColHeaders(Aggregation agg, Row rowHeader, 
                                 int nDisplayCols, CellStyle headerCellStyle) {
        int nMaxCol = nDisplayCols;
        if (agg.shouldDisplayOnNewLine() == false && 
            (agg.colDisplayLabel() <= nDisplayCols || 
             agg.colDisplayValue() <= nDisplayCols))
        ...
        if (rowHeader != null) {
            ...
            if (nValueCol >= nDisplayCols) {
                ...
                if (c == null)
                    c = rowHeader.createCell(nValueCol);
                c.setCellStyle(headerCellStyle);
            ...
            if (nLabelCol != nValueCol && nLabelCol >= nDisplayCols) {
                ...
                if (c == null)
                    c = rowHeader.createCell(nLabelCol);
                c.setCellStyle(headerCellStyle);
                c.setCellValue(agg.headerForLabelCol());

                if (nLabelCol > nMaxCol)
                    nMaxCol = nLabelCol;
            }
        }
        return nMaxCol;
    ...
    private int firstChangedGroup(ResultSet rset, 
                                  AggregationGroup[] aAggregationGroups) throws Exception {
        int nFirstChangedGroup = -1;
        for (int i = 0; 
             i < aAggregationGroups.length && nFirstChangedGroup < 0; i++) {
            AggregationGroup ag = aAggregationGroups[i];
            String v1 = rset.getString(ag.getCol());
            String v2 = ag.getVal();
            if ((v1 != null && v2 != null && v2.equals(v1.trim())) || 
                (v1 == null && v2 == null))
                ; // do nothing (logic is easier to consider this way)
            else
                nFirstChangedGroup = 
                        i; // this will be set when the values are different, including the case when only one is null
        }
        return nFirstChangedGroup;
    }


    //Modified logic for Mod 4B R2

    private int outputAndClearSubtotals(Sheet sheet, int nStartDataOnRow, 
                                        int nDisplayColumns, 
                                        AggregationGroup[] aAggregationGroups, 
                                        int nFirstChangedGroup, 
                                        CellStyle aggLabelStyle, 
                                        SimpleDateFormat canonicalDateFormat, 
        ...
        //System.out.println("outputAndClearSubtotals**:" + enableXLSubTotal);
        while (i >= nFirstChangedGroup) {
            ...
            for (int j = 0; j < alAggregations.size(); j++) {
                ...
                if ("Y".equals(enableXLSubTotal)) {
                    //System.out.println("Grouping:startrow:" + 
                    //                   oAggregationGroup.getStartRow() + 
                    //                   "*****lastDataRow-1:" + 
                    //                   nStartDataOnRow);
                    sheet.groupRow(oAggregationGroup.getStartRow(), 
                                   nStartDataOnRow);
                }
                if (agg.shouldDisplayOnNewLine()) {
                    ...
                    for (int nCol = 0; 
                         nCol < Math.max(nDisplayColumns, Math.max(agg.colDisplayLabel(), 
                                                                   agg.colDisplayValue())); 
                         nCol++) {
                        c = r.createCell(nCol);
                ...
                if (c == null)
                    c = r.createCell(agg.colDisplayLabel() - 1);
                c.setCellValue(agg.displayLabel(oAggregationGroup.getVal()));
                c.setCellStyle(aggLabelStyle);

                /*
                 * System.out.println("agg.displayLabel(oAggregationGroup.getVal())::" + 
                                   agg.displayLabel(oAggregationGroup.getVal()));
                System.out.println("oAggregationGroup.getVal()::" + 
                                   oAggregationGroup.getVal());
                System.out.println("agg.toString()" + agg.toString());
                */
                if (agg.colDisplayLabel() != agg.colDisplayValue()) {
                    c = r.getCell(agg.colDisplayValue() - 1);
                    if (c == null)
                        c = r.createCell(agg.colDisplayValue() - 1);
                    if (agg.cellStyleForValue() == null)
                        c.setCellStyle(aggLabelStyle);
                    else
                        c.setCellStyle(agg.cellStyleForValue());

                    if (agg.aggrDataType() == DataType.NUMERIC)
                        c.setCellValue(Math.round(agg.toDouble() * 100.0) / 
                                       100.0); // Show max of 2 decimals per defect 7311
        ...
        for (i = aAggregationGroups.length - 1; i >= nFirstChangedGroup; i--) {
            ...
            //System.out.println("OutputAndClearSubtotals:i" + i + 
            //                   " nStartDataOnRow" + nStartDataOnRow);
        }

        //System.out.println("END outputAndClearSubtotals");
        return nStartDataOnRow;
    }


    private void outputGrandTotals(Sheet sheet, int nStartDataOnRow, 
                                   int nDisplayColumns, 
                                   AggregationGroup[] aAggregationGroups, 
                                   CellStyle aggLabelStyle, 
                                   SimpleDateFormat canonicalDateFormat) {
            ...
            for (int j = 0; j < alAggregations.size(); j++) {
                ...
                if (grandAggs != null) {
                    for (int k = 0; k < grandAggs.size(); k++) {
                        ...
                        for (int nCol = 0; 
                             nCol < Math.max(nDisplayColumns, Math.max(grandAgg.colDisplayLabel(), 
                                                                       grandAgg.colDisplayValue())); 
                             nCol++) {
                            c = r.createCell(nCol);
                            c.setCellStyle(aggLabelStyle);
                        ...
                        if (c == null)
                            c = r.createCell(grandAgg.colDisplayLabel() - 1);
                        if (grandAgg.isOverSubtotals())
                            c.setCellValue(grandAgg.displayLabel("ALL GROUPS"));
                        else
                            c.setCellValue(grandAgg.displayLabel("ALL VALUES"));

                        if (grandAgg.colDisplayLabel() != 
                            grandAgg.colDisplayValue()) {
                            c = r.getCell(grandAgg.colDisplayValue() - 1);
                            if (c == null)
                                c = 
  r.createCell(grandAgg.colDisplayValue() - 1);
                            if (grandAgg.cellStyleForValue() == null)
                                c.setCellStyle(aggLabelStyle);
                            else
                                c.setCellStyle(grandAgg.cellStyleForValue());

                            if (grandAgg.aggrDataType() == DataType.NUMERIC)
                                c.setCellValue(Math.round(grandAgg.toDouble() * 
                                                          100.0) / 
                                               100.0); // Show max of 2 decimals per defect 7311
                            else if (grandAgg.aggrDataType() == 
                                     DataType.DATE) {
                                try {
    ...
    private void insertGraphic(Workbook oWB, Sheet oSheet, String sFilename, 
                               String sHyperlink, 
                               String sLabel) throws Exception {
        int pictureIdx = 0;
        ...
        if (sLowerFilename.endsWith(".png"))
            ...
            is.close();
            throw new Exception("File extension unsupported for image " + 
                                sFilename);
        }
        is.close();
...
*/
    ...
    private Aggregation aggConstructor(String aggFunction, 
        ...
        Aggregation agg = null;
        if (aggFunction.equals(AggregationSum.key)) {
            if (aggDataType != null && aggDataType.equals("NUMERIC"))
                agg = new AggregationSum();
        } else if (aggFunction.equals(AggregationCount.key))
            agg = new AggregationCount();
        else if (aggFunction.equals(AggregationAverage.key)) {

package od.oracle.apps.xxcrm.addattr.account.stmts.schema.server;
...
import java.sql.Types;
 ...
 /*
  ...
  -- Script Location:
             $CUSTOM_JAVA_TOP/od/oracle/apps/xxcrm/addattr/account/stmts/schema/server
  -- Description: EO java file for XX_CDH_CUST_ACCT_EXT_B table. Used for Statements at Ship to Flag
  ...
  --
  */
public class ODStatementsShiptoEOImpl extends OAEntityImpl {
     ...
     */
    public ODStatementsShiptoEOImpl() {
    }
    ...
    public static synchronized EntityDefImpl getDefinitionObject() {
        if (mDefinitionObject == null) {
            mDefinitionObject = 
                    (OAEntityDefImpl)EntityDefImpl.findDefObject("od.oracle.apps.xxcrm.addattr.account.stmts.schema.server.ODStatementsShiptoEO");
        }
    ...
    protected void doDML(int operation, TransactionEvent e) {
         ...
         this.getOADBTransaction().writeDiagnostics("XXOD:Statements At Shipto EO:doDML","**********************Start****************"+this.getAttrGroupId(),
    ...
    if (operation == DML_INSERT) 
     {
         ...
         {
          this.getOADBTransaction().writeDiagnostics("XXOD: Statements At Shipto EO:doDML","INSERT",
          OAFwkConstants.STATEMENT);
        }
    }
    if(operation == DML_UPDATE)
    {
        if (this.getOADBTransaction().isLoggingEnabled(OAFwkConstants.STATEMENT))
        {
        this.getOADBTransaction().writeDiagnostics("XXOD: Statements At Shipto EO:doDML","UPDATE",
          OAFwkConstants.STATEMENT);
        }
    }
      if(operation == DML_DELETE)
     {
        if (this.getOADBTransaction().isLoggingEnabled(OAFwkConstants.STATEMENT))
         {
          this.getOADBTransaction().writeDiagnostics("XXOD: Statements At Shipto EO:doDML","DELETE",
          OAFwkConstants.STATEMENT);
         }
      }
      ...
      this.getOADBTransaction().writeDiagnostics("ODStatementsShiptoEOImpl:doDML: ", "getInitialCExtAttr1(): "+initialCExtAttr1,OAFwkConstants.STATEMENT);
      this.getOADBTransaction().writeDiagnostics("ODStatementsShiptoEOImpl:doDML: ", "getCExtAttr1(): "+EOCExtAttr1,OAFwkConstants.STATEMENT);

      this.getOADBTransaction().writeDiagnostics("ODStatementsShiptoEOImpl:doDML: ", 
                                                   "this.rowcount " + 
                                                   this.rowcount, 
                                                   OAFwkConstants.STATEMENT);
 ...
 protected void prepareForDML(int dml, TransactionEvent evt) {
        ...
        this.getOADBTransaction().writeDiagnostics("XXOD: EO:prepareForDML", 
		...
		if (dml == DML_INSERT) {
            ...
            this.getOADBTransaction().writeDiagnostics("XXOD:Statement At Shipto Control EO:prepareForDML", 
                                                       "Step 10.10", 
                                                       OAFwkConstants.STATEMENT);
            ...
            try {
                ...
                this.getOADBTransaction().writeDiagnostics("XXOD: Statement At Shipto  EO:prepareForDML", 
                                                           "Step 10.20", 
                                                           OAFwkConstants.STATEMENT);

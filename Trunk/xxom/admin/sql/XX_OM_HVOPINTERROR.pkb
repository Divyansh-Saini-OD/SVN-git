...
and h.orig_sys_document_ref = oep.original_sys_document_ref
and h.order_source_id = oep.order_source_id
...
and substr(message_text,1,8) not in('10000033','10000027','10000014','10000009');
...
If substr(error_cur_rec.message_text,1,4) = '1000' then
  ...
  -- Checking the Finance errors
   if substr(error_cur_rec.message_text,1,8) = '10000007'
   ...
   if substr(error_cur_rec.message_text,1,8) =     '10000010'
  ...
  -- Checking of Order Management Errors
    if substr(error_cur_rec.message_text,1,8) = '10000030'
 ...
 ELSE IF substr(error_cur_rec.message_text,1,4) = 'ORA-' THEN
      if substr(error_cur_rec.message_text,1,4) = 'ORA-14403' then	
       ln_gtss_count := ln_gtss_count + 1;
       ln_gtss_amount := ln_om_amount + error_cur_rec.order_total;
      else if substr(error_cur_rec.message_text,1,4) = 'ORA-03233' then	
      ln_gtss_count := ln_gtss_count + 1;
       ln_gtss_amount := ln_om_amount + error_cur_rec.order_total;
     else
      ln_om_count := ln_om_count + 1;

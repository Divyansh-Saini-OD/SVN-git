...
-- |=======   ==========  =============    ============================|
-- |1.0       15-Aug-07   Raj Jagarlamudi  Initial draft version       |
-- |1.1       13-Jun-08   Raj Jagarlamudi  Qual Attribute changed      |
-- +===================================================================+
...
BEGIN
 ...
 /**************************************************************************/
  ...
  p_seed_qual_rec.LAST_UPDATED_BY         :=  UID;
  p_seed_qual_rec.CREATION_DATE           :=  sysdate;
  p_seed_qual_rec.CREATED_BY              :=  UID;
...
p_qual_usgs_rec.LAST_UPDATED_BY :=  UID;
p_qual_usgs_rec.CREATION_DATE := sysdate;
p_qual_usgs_rec.CREATED_BY := UID;
...
p_qual_usgs_rec.RULE1 := 'SELECT  /*+ INDEX (jtv jtf_terr_values_n8) (jtdr jtf_terr_denorm_rules_n2) (jtq jtf_terr_qual_n2) */' || chr(10) || ' DISTINCT jtdr.terr_id, jtdr.absolute_rank, jtdr.related_terr_id, jtdr.top_level_terr_id, jtdr.num_winners ' || chr(10) || ' FROM jtf_terr_values_all jtv, jtf_terr_denorm_rules_all jtdr, jtf_terr_qual_all jtq ' || chr(10) || ' WHERE jtv.COMPARISON_OPERATOR = ''='' AND jtv.low_value_char = p_rec.incident_attribute_8(i) AND jtv.terr_qual_id = jtq.terr_qual_id ' || chr(10) || 'AND jtdr.source_id = -1002 AND jtdr.qual_type_id = lp_qual_type_id AND ' || chr(10) || 'jtdr.related_terr_id = jtq.terr_id AND jtq.qual_usg_id = ' || l_sequence_id;
p_qual_usgs_rec.RULE2 := 'SELECT  /*+ INDEX (jtv jtf_terr_values_n8) (jtdr jtf_terr_denorm_rules_n2) (jtq jtf_terr_qual_n2) */' || chr(10) || ' DISTINCT jtdr.terr_id, jtdr.absolute_rank, jtdr.related_terr_id, jtdr.top_level_terr_id, jtdr.num_winners ' || chr(10) || ' FROM jtf_terr_values_all jtv, jtf_terr_denorm_rules_all jtdr, jtf_terr_qual_all jtq ' || chr(10) || ' WHERE jtv.COMPARISON_OPERATOR = ''<>'' AND ( jtv.low_value_char <> p_rec.incident_attribute_8(i) OR p_rec.SQUAL_CHAR25(i) is null ) ' || chr(10) || 'AND jtv.terr_qual_id = jtq.terr_qual_id AND jtdr.source_id = -1002 AND ' || chr(10) || 'jtdr.qual_type_id = lp_qual_type_id AND jtdr.related_terr_id = jtq.terr_id AND jtq.qual_usg_id = ' || l_sequence_id;
...
p_qual_usgs_rec.DISPLAY_SQL3 := null;
p_qual_usgs_rec.ORG_ID := l_org_id;
p_qual_usgs_rec.RULE1 := 'SELECT /*+ INDEX (jtv jtf_terr_values_n9) (jtdr tf_terr_denorm_rules_n2)(jtq jtf_terr_qual_n2)*/ ' || chr(10) || ' DISTINCT jtdr.terr_id , jtdr.absolute_rank , jtdr.related_terr_id , jtdr.top_level_terr_id , jtdr.num_winners ' || chr(10) || ' FROM jtf_terr_values_all jtv, jtf_terr_denorm_rules_all jtdr, jtf_terr_qual_all jtq ' || chr(10) || ' WHERE ((p_rec.incident_attribute_11(i) = jtv.low_value_char_id AND jtv.COMPARISON_OPERATOR = ''='' )) ' || chr(10) || ' AND jtv.terr_qual_id = jtq.terr_qual_id AND jtdr.source_id = -1002 AND jtdr.related_terr_id = jtq.terr_id ' || chr(10) || ' AND jtq.qual_usg_id = ' || l_sequence_id;
p_qual_usgs_rec.RULE2 := 'SELECT /*+ INDEX (jtv jtf_terr_values_n9) (jtdr tf_terr_denorm_rules_n2)(jtq jtf_terr_qual_n2)*/ ' || chr(10) || ' DISTINCT jtdr.terr_id , jtdr.absolute_rank , jtdr.related_terr_id , jtdr.top_level_terr_id , jtdr.num_winners ' || chr(10) || ' FROM jtf_terr_values_all jtv, jtf_terr_denorm_rules_all jtdr, jtf_terr_qual_all jtq ' || chr(10) || ' WHERE (((p_rec.incident_attribute_11(i) <> jtv.low_value_char_id OR p_rec.squal_num39(i) IS NULL) AND jtv.COMPARISON_OPERATOR = ''<>'' )) ' || chr(10) || ' AND jtv.terr_qual_id = jtq.terr_qual_id AND jtdr.source_id = -1002 ' || chr(10) || ' AND jtdr.related_terr_id = jtq.terr_id AND jtq.qual_usg_id = ' ||l_sequence_id;
p_qual_usgs_rec.DISPLAY_SEQUENCE := null;
p_qual_usgs_rec.DISPLAY_LENGTH := null;
p_qual_usgs_rec.JSP_LOV_SQL := null;
p_qual_usgs_rec.use_in_lookup_flag := null;

...
-- +===========================================================================+
-- | Name        :    XX_C2T_CC_OE_PMT_NULL_CC_PHASE_2_1.sql                   |
...
BEGIN
		...
		------------------------------------------------------------------------
		-- STEP #1 - Drop task XX_C2T_NULL_CC_OE_PMT                          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #1 - Drop task XX_C2T_NULL_CC_OE_PMT - '||SYSDATE);   
			
		BEGIN
			gc_error_loc := 'Executing dbms_parallel_execute.drop_task.';
			DBMS_PARALLEL_EXECUTE.DROP_TASK('XX_C2T_NULL_CC_OE_PMT');
			
			DBMS_OUTPUT.PUT_LINE('XX_C2T_NULL_CC_OE_PMT was dropped from previous run.');
    EXCEPTION
			WHEN OTHERS 
      THEN 
			   gc_error_loc := 'XX_C2T_NULL_CC_OE_PMT did not exist.Drop was not required.';
			   DBMS_OUTPUT.PUT_LINE(gc_error_loc);
		END;
			
		------------------------------------------------------------------------
    -- STEP #2 - Create task XX_C2T_NULL_CC_OE_PMT.                       --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #2 - Create task XX_C2T_NULL_CC_OE_PMT. - '||SYSDATE); 
			
		gc_error_loc := 'Executing dbms_parallel_execute.create_task.';
		DBMS_PARALLEL_EXECUTE.CREATE_TASK(TASK_NAME => 'XX_C2T_NULL_CC_OE_PMT');
			   
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_task.');
			
		------------------------------------------------------------------------
		-- STEP #3 - Create Chunks for task XX_C2T_NULL_CC_OE_PMT.            --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #3 - Create Chunks for task XX_C2T_NULL_CC_OE_PMT. - '||SYSDATE); 
		...
		DBMS_PARALLEL_EXECUTE.CREATE_CHUNKS_BY_SQL('XX_C2T_NULL_CC_OE_PMT', gc_chunk_sql, FALSE);
			
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_chunks_by_sql.');
			
		------------------------------------------------------------------------
		-- STEP #4 - Run Task XX_C2T_NULL_CC_OE_PMT.                          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #4 - Run Task XX_C2T_NULL_CC_OE_PMT. - '||SYSDATE); 
			
    gc_error_loc := 'Generating SQL Statement.';
		gc_sql_stmt  :=  'UPDATE oe_payments 
		...
		DBMS_PARALLEL_EXECUTE.RUN_TASK(task_name => 'XX_C2T_NULL_CC_OE_PMT',
    ...
    ------------------------------------------------------------------------
    -- STEP #5 - Verify Status of Chunks XX_C2T_NULL_CC_OE_PMT            --
    ------------------------------------------------------------------------
		gc_error_loc := 'Getting total number of chunks';
		SELECT COUNT(1)
			INTO gc_total_chunks_cnt
			FROM dba_parallel_execute_chunks
		 WHERE task_name = 'XX_C2T_NULL_CC_OE_PMT';
    ...
    THEN
			 ...
			 DBMS_PARALLEL_EXECUTE.RESUME_TASK ('XX_C2T_NULL_CC_OE_PMT');
		...
		DBMS_OUTPUT.PUT_LINE('XX_C2T_NULL_CC_OE_PMT has been completed - '||SYSDATE);

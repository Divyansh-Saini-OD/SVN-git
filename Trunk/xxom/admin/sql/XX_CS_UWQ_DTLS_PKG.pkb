...
gc_group_name        VARCHAR2(100);
gc_return_status     VARCHAR2(50);
gn_msg_count         NUMBER;
gc_msg_data          VARCHAR2(1000);
GC_PKG_NAME          VARCHAR2(100) := 'XX_CS_UWQ_DTLS_PKG'; 
gc_api_name          VARCHAR2(100) := 'GET_START_END';
  ...
  EXCEPTION
    ...
    /*  gc_err_code := 'XX_OM_0001_UNKNOWN_ERROR';
      ...
      gt_err_report_type :=
          ...
          gc_return_status := FND_API.G_RET_STS_UNEXP_ERROR ;
           IF FND_MSG_PUB.Check_Msg_Level ( FND_MSG_PUB.G_MSG_LVL_UNEXP_ERROR ) THEN
              FND_MSG_PUB.Add_Exc_Msg( GC_PKG_NAME,gc_api_name);
           END IF;
           FND_MSG_PUB.Count_And_Get
           ( p_count         =>      gn_msg_count,
             p_data          =>      gc_msg_data
           );
 END;
...
FUNCTION GET_FIRST(p_row_no NUMBER,
                      ...
                      p_group_name VARCHAR2 ) RETURN NUMBER AS
  BEGIN
    IF P_GROUP_NAME NOT LIKE 'VW%' THEN
      IF p_row_no = 1 THEN
         gc_group_name   := p_group_name;
         gc_first_inc_id := p_incident_id;
      END IF;
      IF P_GROUP_NAME = GC_GROUP_NAME THEN
          gc_first_inc_id := gc_first_inc_id;
      ELSE
          gc_group_name   := p_group_name;
          gc_first_inc_id := p_incident_id;
      END IF;

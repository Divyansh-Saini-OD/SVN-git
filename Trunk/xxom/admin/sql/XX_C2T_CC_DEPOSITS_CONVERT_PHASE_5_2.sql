...
-- +===========================================================================+
-- | Name        :    XX_C2T_CC_DEPOSITS_CONVERT_PHASE_5_2.sql                 |
-- | Description :    Convert Deposits Credit cards to Tokens                  |
-- | Rice ID     :    C0705                                                    |
-- |Change Record:                                                             |
-- |===============                                                            |
...
BEGIN
		...
		------------------------------------------------------------------------
		-- STEP #1 - Drop task XX_C2T_CONVERT_DEPOSITS                        --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #1 - Drop task XX_C2T_CONVERT_DEPOSITS - '||SYSDATE);   
			
		BEGIN
			gc_error_loc := 'Executing dbms_parallel_execute.drop_task.';
			DBMS_PARALLEL_EXECUTE.DROP_TASK('XX_C2T_CONVERT_DEPOSITS');
			
			DBMS_OUTPUT.PUT_LINE('XX_C2T_CONVERT_DEPOSITS was dropped from previous run.');
    EXCEPTION
			WHEN OTHERS 
      THEN 
			   gc_error_loc := 'XX_C2T_CONVERT_DEPOSITS did not exist.Drop was not required.';
			   DBMS_OUTPUT.PUT_LINE(gc_error_loc);
		END;
			
		------------------------------------------------------------------------
    -- STEP #2 - Create task XX_C2T_CONVERT_DEPOSITS.                     --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #2 - Create task XX_C2T_CONVERT_DEPOSITS. - '||SYSDATE); 
			
		gc_error_loc := 'Executing dbms_parallel_execute.create_task.';
		DBMS_PARALLEL_EXECUTE.CREATE_TASK(TASK_NAME => 'XX_C2T_CONVERT_DEPOSITS');
			   
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_task.');
			
		------------------------------------------------------------------------
		-- STEP #3 - Create Chunks for task XX_C2T_CONVERT_DEPOSITS.          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #3 - Create Chunks for task XX_C2T_CONVERT_DEPOSITS. - '||SYSDATE); 
		...
		DBMS_PARALLEL_EXECUTE.CREATE_CHUNKS_BY_SQL('XX_C2T_CONVERT_DEPOSITS', gc_chunk_sql, FALSE);
			
		DBMS_OUTPUT.PUT_LINE('Completed dbms_parallel_execute.create_chunks_by_sql.');
			
		------------------------------------------------------------------------
		-- STEP #4 - Run Task XX_C2T_CONVERT_DEPOSITS.                          --
		------------------------------------------------------------------------
		DBMS_OUTPUT.PUT_LINE('STEP #4 - Run Task XX_C2T_CONVERT_DEPOSITS. - '||SYSDATE); 
			
    gc_error_loc := 'Generating SQL Statement.';
		gc_sql_stmt  := 'MERGE /*+ LEADING(STG) */ 
                      INTO xx_om_legacy_deposits DEP
                     USING (SELECT /*+ LEADING(XOR) USE_NL(XOR XCV) INDEX(XCV XX_C2T_CC_TOKEN_CRYPTOVAULT_N1) */
                                   XOR.transaction_number
								                  ,XOR.order_source_id
								                  ,XOR.payment_number
                                  ,XCV.token_number_new 
                                  ,XCV.token_key_label_new
                              FROM xx_c2t_cc_token_stg_deposits  XOR
                                  ,xx_c2t_cc_token_crypto_vault  XCV
                             WHERE XOR.deposit_id BETWEEN :start_id AND :end_id
                               AND XOR.re_encrypt_status  = ''C''
                               AND XOR.convert_status    <> ''C''
                               AND XOR.credit_card_number_new = XCV.credit_card_number_new ) STG
                        ON (    DEP.transaction_number = STG.transaction_number
                            AND DEP.order_source_id = STG.order_source_id
                            AND DEP.payment_number = STG.payment_number)
                      WHEN MATCHED THEN UPDATE 
                                           SET DEP.credit_card_number = STG.token_number_new
                                              ,DEP.identifier         = STG.token_key_label_new
                                              ,DEP.token_flag         = ''Y''  ';
			
		gc_error_loc := 'Executing dbms_parallel_execute.run_task using 64 parallel threads';
		DBMS_PARALLEL_EXECUTE.RUN_TASK(task_name => 'XX_C2T_CONVERT_DEPOSITS',
    ...
    ------------------------------------------------------------------------
    -- STEP #5 - Verify Status of Chunks XX_C2T_CONVERT_DEPOSITS            --
    ------------------------------------------------------------------------
		gc_error_loc := 'Getting total number of chunks';
		SELECT COUNT(1)
			INTO gc_total_chunks_cnt
			FROM dba_parallel_execute_chunks
		 WHERE task_name = 'XX_C2T_CONVERT_DEPOSITS';
    ...
    THEN
			 ...
			 DBMS_PARALLEL_EXECUTE.RESUME_TASK ('XX_C2T_CONVERT_DEPOSITS');
		...
		DBMS_OUTPUT.PUT_LINE('XX_C2T_CONVERT_DEPOSITS has been completed - '||SYSDATE);

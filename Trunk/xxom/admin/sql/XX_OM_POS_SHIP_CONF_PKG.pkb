...
CREATE OR REPLACE PACKAGE BODY XX_OM_POS_SHIP_CONF_PKG
-- +===================================================================+
...
-- |=======    ==========    =============    ======================== |
-- |DRAFT 1A   16-Jan-2007   Shashi Kumar     Initial draft version    |
-- |                                                                   |
...
PROCEDURE Validate_Ord_Lines_Proc(
                                  p_header_id            IN NUMBER   
                                 ,p_order_lines_tbl      IN OUT XX_OM_ORDER_LINES_TBL
                                 ,x_status               OUT VARCHAR2
                                 ,x_message              OUT VARCHAR2
                                 ,x_ack_ord_lines_tbl    OUT XX_OM_ACK_ORD_LINES_TBL
                                 ) 
AS
                ...
                IF ln_organization_id IS NOT NULL THEN
                    ...
                    EXCEPTION
                        ...
                        WHEN NO_DATA_FOUND THEN
                            ...
                            FND_MESSAGE.SET_NAME('XXOM','ODP_OM_NO_DATA_FOUND_ERR');
                            FND_MESSAGE.SET_TOKEN('ERROR_CODE',SQLCODE); 
                            FND_MESSAGE.SET_TOKEN('ERROR_MESSAGE',SQLERRM);

                            g_error_description:= FND_MESSAGE.GET;
                            g_error_code       := 'ODP_OM_NO_DATA_FOUND_ERR';
...
-- Commented By Shashi on 11-May-07 so that the BPEL ack table type gets success condition also
--            IF lc_status = 'E' THEN                
...
AS
    ...
    lt_order_payments_tbl   XX_OM_ORDER_PAYMENTS_TBL;
    lt_order_tbl            XX_OM_ORDER_LINES_TBL;
...
BEGIN
    ...
    lt_order_tbl          := p_order_lines_tbl;
    ...
    IF lt_order_payments_tbl.COUNT > 0 THEN
        ...
        LOOP
            ...
            --  Added by shashi on 16-may-07 , If the Order is a cancel order then there is no need to process the payments  --          
            IF lt_order_tbl(i).line_status = 'Cancel' THEN
                RETURN;
            END IF;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        g_entity_ref        := 'HEADER_ID';
        g_entity_ref_id     := p_source_header_id;
...
AS
    ...
    lc_commit                   VARCHAR2(30) := FND_API.G_TRUE;
...
BEGIN
    ...
    IF lc_status = WSH_UTIL_CORE.G_RET_STS_SUCCESS THEN
        ...
        x_status     := 'S';
    ELSIF  lc_status = WSH_UTIL_CORE.G_RET_STS_ERROR THEN
        x_status     := 'E';

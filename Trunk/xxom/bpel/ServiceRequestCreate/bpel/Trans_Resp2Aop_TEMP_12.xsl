...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.3.0(build 070615.0525) AT [TUE MAY 04 18:21:30 EDT 2010]. -->
?>
  ...
  <xsl:template match="/">
    <tns:Root-Element>
      <tns:RESPONSE_MQ>
        ...
        <xsl:if test="/db:OutputParameters/db:X_ORDER_NUM">
          <xsl:choose>
            <xsl:when test="/db:OutputParameters/db:X_ORDER_NUM">
              <tns:request_number/>
            </xsl:when>
            <xsl:otherwise>
              <tns:request_number>

...
BEGIN
   ...
   XX_COM_CONV_ELEMENTS_PKG.log_exceptions_proc(
                                               ...
                                               p_package_name          => 'XX_INV_MERCHIER_VAL_CONV_PKG',
...
BEGIN
   ...
   WHERE XQQS.process_flag <>7;

   COMMIT;

EXCEPTION
   WHEN OTHERS THEN
       display_log('Error in update_master_batch_id : '||SQLERRM);
...
BEGIN
   ...
   AND    XQLL.orig_sys_header_ref IN (SELECT orig_sys_header_ref
   ...
   AND    XQPA.orig_sys_header_ref IN (SELECT orig_sys_header_ref
                                     FROM   xx_qp_list_headers_stg XQPL
                                     WHERE  XQPL.load_batch_id = ln_seq
                                     AND    XQPL.process_flag = 2
                                    )
   AND    XQPA.orig_sys_line_ref IN  (SELECT  orig_sys_line_ref
   ...
   AND    XQQS.orig_sys_header_ref IN (SELECT orig_sys_header_ref
                                     FROM   xx_qp_list_headers_stg XQPL
                                     WHERE  XQPL.load_batch_id = ln_seq
                                     AND    XQPL.process_flag = 2
                                    )
   AND    XQQS.orig_sys_line_ref   IN (SELECT orig_sys_line_ref
                                     FROM   xx_qp_list_lines_stg XQLL
                                     WHERE  XQLL.load_batch_id = ln_seq
                                     AND    XQLL.process_flag = 2
                                    );
...
BEGIN
   ...
   IF lc_return_status = 'S' THEN
      ...
      IF  lc_launch = 'N' THEN
         ...
         display_out('No Data Found in the Table XX_QP_LIST_HEADERS_STG');
         x_retcode := 2;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
       x_retcode := 2;
       display_log('Error in submit_sub_requests - '||SQLERRM);
...
PROCEDURE validate_arithmetic_operator(
                                       ...
                                       x_err_arithmetic_operator OUT VARCHAR2,
                                       x_status                  OUT NOCOPY VARCHAR2
                                      )
IS
...
PROCEDURE validate_comparision_operator(
                                        ...
                                        x_err_operator_code         OUT VARCHAR2,
                                        x_status                    OUT NOCOPY VARCHAR2
                                       )
IS
...
BEGIN
   ...
   CASE
      WHEN p_modifier_level_code IS NULL THEN

          BEGIN
               SELECT QPP.pricing_phase_id
               INTO   x_phase_id
               FROM   qp_pricing_phases QPP
               WHERE  QPP.modifier_level_code IS NULL
...
lc_status             VARCHAR2(01);
lc_err_flag           VARCHAR2(01);
...
BEGIN
   ...
   IF ln_line_no <> 0 THEN
     ...
     WHERE  XQLL.orig_sys_header_ref = p_orig_sys_header_ref
     AND    XQLL.load_batch_id = p_batch_id
     AND    XQLL.attribute9    = 'R';

     IF ln_num_of_r = 1 THEN
...
lc_err_line_code           VARCHAR2(03);
lc_err_mod_code            VARCHAR2(03);
lc_err_inv_item            VARCHAR2(03);
...
lc_err_record              VARCHAR2(03);
lc_err_comments_attrib12   VARCHAR2(03);
...
-- -------------------------------------------------
TYPE header_tbl_type IS TABLE OF xx_qp_list_headers_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
TYPE line_tbl_type IS TABLE OF xx_qp_list_lines_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
TYPE pric_attrib_tbl_type IS TABLE OF xx_qp_pricing_attributes_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
-- -----------------------------------------------------
TYPE qualifiers_tbl_type IS TABLE OF xx_qp_qualifiers_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
lt_qualifiers qualifiers_tbl_type;

-- ----------------------------------------------------------
-- Table Type for holding control_id of header staging table
-- ----------------------------------------------------------
TYPE hdr_control_id_tbl_type IS TABLE OF xx_qp_list_headers_stg.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_hdr_control_id hdr_control_id_tbl_type;
...
-- --------------------------------------------------------
-- Table Type for holding control_id of line staging table
-- --------------------------------------------------------
TYPE line_control_id_tbl_type IS TABLE OF xx_qp_list_lines_stg.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_line_control_id line_control_id_tbl_type;
...
-- ----------------------------------------------------------------------
-- Table Type for holding control_id of pricing attributes staging table
-- ----------------------------------------------------------------------
TYPE pric_cntrl_id_tbl_type IS TABLE OF xx_qp_pricing_attributes_stg.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_pric_cntrl_id pric_cntrl_id_tbl_type;
...
-- -------------------------------------------------------------
-- Table Type for holding control_id of qualfiers staging table
-- -------------------------------------------------------------

TYPE qualifiers_cntrl_id_tbl_type IS TABLE OF xx_qp_qualifiers_stg.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_qualifiers_cntrl_id qualifiers_cntrl_id_tbl_type;
...
AND    XQQS.orig_sys_line_ref   = p_line_ref
      ...
      WHEN lc_conv_return_status = 'S' THEN
            ...
            IF  lt_header.count <> 0 THEN
               ...
               LOOP
                   ...
                   lt_hdr_control_id(ln_hdr_index) :=   lt_header(i).control_id;
                   ...
                   -- --------------------------------------------------
                   -- Validate the comments and attribute12 of a header
                   -- --------------------------------------------------
                   validate_comments_attrib12(
                                              p_attribute12           => lt_header(i).attribute12,
                                              p_comments              => lt_header(i).comments,
                                              x_err_comments_attrib12 => lc_err_msg
                                             );

                   CASE
                       WHEN lc_err_msg = 'Y' THEN
                           ...
                           -- ----------------------------------------------------------------------------------------
                           CASE
                               WHEN ((lt_header(i).attribute12 IN ('2A','2B')) AND (lt_header(i).attribute10 > 1)) THEN
                               ...
                               ELSE
                                   NULL;
                           END CASE;
                       WHEN lc_err_msg = 'N' THEN
                            log_procedure(
                                          p_control_id           => lt_header(i).control_id,
                                          p_source_system_code   => lt_header(i).source_system_code,
                                          p_procedure_name       => 'VALIDATE_COMMENTS_ATTRIB12',
                                          p_staging_table_name   => 'XX_QP_LIST_HEADERS_STG',
                                          p_staging_column_name  => 'ATTRIBUTE12/COMMENTS',
                                          p_staging_column_value => lt_header(i).attribute12,
                                          p_source_system_ref    => lt_header(i).source_system_ref,
                                          p_batch_id             => p_batch_id,
                                          p_exception_log        => 'Coupon and Desciption Mismatch',
                                          p_oracle_error_code    => NULL,
                                          p_oracle_error_msg     => NULL
                                         );
                            lc_err_hdr_msg := 'E';
                     ...
                     LOOP
                        ...
                        IF lt_line.COUNT <> 0 THEN
                           ...
                           LOOP
                                ...
                                lt_line_control_id(ln_line_index) := lt_line(j).control_id;
                                   ...
                                   CASE
                                       WHEN lc_err_msg = 'E' THEN
                                           log_procedure(
                                                         ...
                                                         p_source_system_ref    => lt_line(j).source_system_ref,
                                                         p_batch_id             => p_batch_id,
                                                         p_exception_log        => 'Phase Id does not exists when modifier_level_code IS NULL',
                                                         p_oracle_error_code    => NULL,
                                                         p_oracle_error_msg     => NULL
                                                        );
                                           lc_err_line_msg := 'E';
                                       ...
                                       ELSE
                                           log_procedure(
                                                         ...
                                                         p_source_system_ref    => lt_line(j).source_system_ref,
                                                         p_batch_id             => p_batch_id,
                                                         p_exception_log        => 'Error when modifier_level_code IS NULL',
                                                         p_oracle_error_code    => NULL,
                                                         p_oracle_error_msg     => lc_err_msg
                                                        );
                                            lc_err_line_msg := 'E';
                                ...
                                ELSE
                                   ...
                                   CASE
                                       ...
                                       ELSE
                                           CASE
                                               WHEN (lt_line(j).modifier_level_code IN ('LINE','ORDER')) THEN
                                   ...
                                   END CASE; --lt_line(j).modifier_level_code IN ('LINE','ORDER')
                                ...
                                LOOP
                                   ...
                                   IF lt_pric_attrib.COUNT <> 0 THEN
                                      ...
                                      LOOP
                                          ...
                                          lt_pric_cntrl_id(ln_pric_attrib_index) := lt_pric_attrib(k).control_id;
                                          ...
                                          IF lt_pric_attrib(k).product_attr_value NOT IN ('ALL','ITEM_CATEGORY') THEN
                                             ...
                                             CASE
                                                 WHEN lc_err_msg = 'NE' THEN
                                                     CASE
                                                         WHEN (lt_line(j).rltd_modifier_grp_type IN ('QUALIFIER','BENEFIT')) THEN

                                                               derive_benefit_list_line_id(
                                                                                           p_inv_item_id  => ln_inv_item,
                                                                ...
                                                                CASE
                                                                    WHEN lc_err_int_msg = 'E' THEN
                                                                             log_procedure(
                                                                                            ...
                                                                                            p_procedure_name       => 'DERIVE_BENEFIT_LIST_LINE_ID',
                                                                                            ...
                                                                                            p_batch_id             => p_batch_id,
                                                                                            p_exception_log        => 'Inventory Item Not set-up in ZONE_74',
                                                                                            p_oracle_error_code    => NULL,
                                                                                            p_oracle_error_msg     => NULL
                                                                                           );
                                                                         ...
                                                                         ELSE
                                                                             log_procedure(
                                                                                            ...
                                                                                            p_exception_log        => NULL,
                                                                                            p_oracle_error_code    => NULL,
                                                                                            p_oracle_error_msg     => lc_err_int_msg
                                                                                           );
                                                                             lc_err_pric_attrib := 'E';
                                                                END CASE;
                                      ...
                                      FORALL ln_pric_attrib_index IN lt_pric_cntrl_id.FIRST .. lt_pric_cntrl_id.LAST
                                      UPDATE xx_qp_pricing_attributes_stg XQPA
                                      SET    XQPA.process_flag = lt_pric_process_flag(ln_pric_attrib_index)
                                      WHERE  XQPA.control_id = lt_pric_cntrl_id(ln_pric_attrib_index);
                                ...
                                LOOP
                                    ...
                                    IF lt_qualifiers.COUNT <> 0 THEN
                                       ...
                                       LOOP
                                          ...
                                          lt_qualifiers_cntrl_id(ln_qual_index) := lt_qualifiers(l).control_id;
                                          ...
                                          ELSE
                                              ...
                                              lt_qualifiers_flag(ln_qual_index)   := 3;
                                               ...
                                               -- ----------------------------------------------------
                                               -- Assign the process_flag of line
                                               -- -----------------------------------------------------
                                              lt_line_process_flag(ln_line_index) := 3;
                                       ...
                                       FORALL ln_qual_index IN lt_qualifiers_cntrl_id.FIRST .. lt_qualifiers_cntrl_id.LAST
                                    UPDATE xx_qp_qualifiers_stg XQQS
                                    SET    XQQS.process_flag = lt_qualifiers_flag(ln_qual_index)
                                    WHERE  XQQS.control_id = lt_qualifiers_cntrl_id(ln_qual_index);
                           ...
                           FORALL ln_line_index IN lt_line_control_id.FIRST .. lt_line_control_id.LAST
                           UPDATE xx_qp_list_lines_stg XQLL
                           SET    XQLL.process_flag = lt_line_process_flag(ln_line_index)
                           WHERE  XQLL.control_id = lt_line_control_id(ln_line_index);
               ...
               -- process_flag
               -- ------------------------------------------------------------------------
               FORALL ln_hdr_index IN  lt_hdr_control_id.FIRST .. lt_hdr_control_id.LAST
               UPDATE xx_qp_list_headers_stg XQPL
               SET    XQPL.process_flag = lt_hdr_process_flag(ln_hdr_index)
               WHERE  XQPL.control_id = lt_hdr_control_id(ln_hdr_index);
...
ln_pro_hdr_index     NUMBER := 0;
ln_tbl_line_index    NUMBER := 0;
ln_tbl_pa_index      NUMBER := 0;
ln_tbl_qua_index     NUMBER := 0;
...
-- Table Type for holding header staging table data
-- -------------------------------------------------
TYPE pro_header_tbl_type IS TABLE OF xx_qp_list_headers_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
TYPE pro_hdr_ctl_id_tbl_type IS TABLE OF xx_qp_list_headers_stg.control_id%TYPE
INDEX BY BINARY_INTEGER;
lt_pro_hdr_ctl_id pro_hdr_ctl_id_tbl_type;
...
-- Table Type for holding line staging table data
-- -------------------------------------------------
TYPE pro_line_tbl_type IS TABLE OF xx_qp_list_lines_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
TYPE pro_pric_attrib_tbl_type IS TABLE OF xx_qp_pricing_attributes_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
TYPE pro_qual_tbl_type IS TABLE OF xx_qp_qualifiers_stg%ROWTYPE
INDEX BY BINARY_INTEGER;
...
AND    XQQS.orig_sys_header_ref = p_header_orig
AND    XQQS.orig_sys_line_ref   = p_line_orig
AND    XQQS.load_batch_id = p_batch_id
ORDER BY XQQS.control_id;
      ...
      WHEN gc_pro_flag = 'S' THEN
            ...
            IF lt_pro_header.COUNT <> 0 THEN
              ...
              BEGIN
                   ...
                   lt_pro_hdr_ctl_id(ln_pro_hdr_index) := lt_pro_header(i).control_id;

                   -- ------------------------------------------------------------------
                   -- Setting up the modifier list record type
                   -- ------------------------------------------------------------------
                   l_modifier_list_rec.automatic_flag           := lt_pro_header(i).automatic_flag;
                   ...
                   l_modifier_list_rec.end_date_active          := TO_DATE(TO_CHAR(lt_pro_header(i).end_date_active,'DD-MON-YYYY'),'DD-MON-YYYY');
                   ...
                   l_modifier_list_rec.program_application_id   := lt_pro_header(i).program_application_id;
                   l_modifier_list_rec.program_id               := lt_pro_header(i).program_id;
                   l_modifier_list_rec.program_update_date      := lt_pro_header(i).program_update_date;
                   l_modifier_list_rec.prorate_flag             := NVL(lt_pro_header(i).prorate_flag,FND_API.G_MISS_CHAR); --lt_pro_header(i).prorate_flag;
                   l_modifier_list_rec.request_id               := lt_pro_header(i).request_id;
                   ...
                   l_modifier_list_rec.start_date_active        := TO_DATE(TO_CHAR(lt_pro_header(i).start_date_active,'DD-MON-YYYY'),'DD-MON-YYYY');
                   ...
                   l_modifier_list_rec.ask_for_flag             := NVL(lt_pro_header(i).ask_for_flag,FND_API.G_MISS_NUM);
                   l_modifier_list_rec.return_status            := lt_pro_header(i).return_status;
                   l_modifier_list_rec.db_flag                  := lt_pro_header(i).db_flag;
                   ...
                   -- ------------------------------------------------------------------
                   IF lt_pro_header(i).attribute12 IN ('2A','2B') AND lt_pro_header(i).attribute10 > 1 THEN
                      get_operand(
                   ...
                   IF lt_pro_line.COUNT <> 0 THEN
                      ...
                      LOOP
                          ...
                          -- ------------------------------------------------------------------
                              ...
                              IF lt_pro_header(i).attribute12 IN ('2A','2B') AND lt_pro_header(i).attribute10 > 1 AND lt_pro_line(j).attribute9 = 'R' THEN
                                 l_modifiers_tbl(ln_tbl_line_index).operand  := lt_pro_line(j).operand;
                              ELSIF lt_pro_header(i).attribute12 IN ('2A','2B') AND lt_pro_header(i).attribute10 > 1 AND lt_pro_line(j).attribute9 = 'Q' THEN
                              ...
                              l_modifiers_tbl(ln_tbl_line_index).automatic_flag            := lt_pro_line(j).automatic_flag;
                              ...
                              l_modifiers_tbl(ln_tbl_line_index).start_date_active         := lt_pro_line(j).start_date_active;
                              l_modifiers_tbl(ln_tbl_line_index).end_date_active           := lt_pro_line(j).end_date_active;
                              ...
                              l_modifiers_tbl(ln_tbl_line_index).modifier_parent_index     := ln_pro_hdr_index;
                              l_modifiers_tbl(ln_tbl_line_index).operation                 := QP_GLOBALS.G_OPR_CREATE;
                              l_modifiers_tbl(ln_tbl_line_index).attribute11               := NVL(lt_pro_line(j).attribute11,FND_API.G_MISS_NUM);
                              ...
                              l_modifiers_tbl(ln_tbl_line_index).context                   := 'CouponWiz';
                              ...
                              IF (lt_pro_line(j).list_line_type_code = 'DIS') THEN
                              ...
                              -- ------------------------------------------------------------------
                              -- Derive the phase_id for the modifier_level_code of a line
                              -- ------------------------------------------------------------------
                              IF UPPER(lt_pro_line(j).modifier_level_code) IN ('LINE','ORDER','NULL') THEN
                                 derive_phase_id(
                                                 p_modifier_level_code => lt_pro_line(j).modifier_level_code,
                                                 x_phase_id => ln_phase_id,
                                                 x_status   => lc_status
                                                );
                                 ln_pricing_phase_id := ln_phase_id;
                              ELSE
                                  ln_pricing_phase_id := lt_pro_line(j).pricing_phase_id;
                              END IF;

                              l_modifiers_tbl(ln_tbl_line_index).pricing_phase_id             := ln_pricing_phase_id;
                          ...
                          IF lt_pro_pric_attrib.COUNT <> 0 THEN
                             ...
                             LOOP
                                 ...
                                 l_pricing_attr_tbl(ln_tbl_pa_index).pricing_phase_id           := NVL(NVL(ln_pricing_phase_id,lt_pro_line(j).pricing_phase_id),lt_pro_pric_attrib(k).pricing_phase_id);
                                 ...
                                 IF lt_pro_line(j).modifier_level_code IN ('LINE') THEN

                                    l_pricing_attr_tbl(ln_tbl_pa_index).product_attr_value := To_Char(ln_inventory_item_id);

                                 ELSIF lt_pro_line(j).modifier_level_code IN ('LINEGROUP') THEN
                                       ...
                                       END IF;
                                 ELSE
                                     IF lt_pro_header(i).attribute12 = '7' THEN
                                        IF l_pricing_attr_tbl(ln_tbl_pa_index).product_attr_value = 'ITEM_CATEGORY' THEN
                   ...
                   EXCEPTION
                     WHEN FND_API.G_EXC_UNEXPECTED_ERROR THEN
                          ...
                          FOR k IN 1 .. l_count
                          LOOP
                              l_data := oe_msg_pub.get( p_msg_index => k,
                                                       p_encoded   => 'F'
                                                      );
                              log_procedure(
              ...
              FORALL ln_pro_hdr_index IN  lt_pro_header.FIRST .. lt_pro_header.LAST
              UPDATE xx_qp_list_headers_stg XQLS
              SET    XQLS.process_flag = lt_hdr_pro_flag(ln_pro_hdr_index)
              WHERE  XQLS.control_id = lt_pro_hdr_ctl_id(ln_pro_hdr_index);
...
PROCEDURE update_err_child_batch_id(p_batch_id NUMBER)
IS
BEGIN
   ...
   UPDATE xx_qp_list_headers_stg XQPL
   SET    XQPL.process_flag = 2
   WHERE  XQPL.process_flag IN (3,6)
   AND    XQPL.load_batch_id = p_batch_id;
   ...
   UPDATE xx_qp_list_lines_stg XQLL
   SET    XQLL.process_flag = 2
   WHERE  XQLL.process_flag IN (3,6)
   AND    XQLL.load_batch_id = p_batch_id
   AND    XQLL.orig_sys_header_ref IN (SELECT orig_sys_header_ref
                                    FROM   xx_qp_list_headers_stg XQPL
   ...
   UPDATE xx_qp_pricing_attributes_stg XQPA
   SET    XQPA.process_flag = 2
   WHERE  XQPA.process_flag IN (3,6)
   AND    XQPA.load_batch_id = p_batch_id
   AND    XQPA.orig_sys_header_ref IN (SELECT orig_sys_header_ref
                                     FROM   xx_qp_list_headers_stg XQPL
   ...
   UPDATE xx_qp_qualifiers_stg XQQS
   SET    XQQS.process_flag = 2
   WHERE  XQQS.process_flag IN (3,6)
   AND    XQQS.load_batch_id = p_batch_id
   AND    XQQS.orig_sys_header_ref IN (SELECT orig_sys_header_ref
                                     FROM   xx_qp_list_headers_stg XQPL
                                     WHERE  XQPL.process_flag =2
...
BEGIN
   ...
   BEGIN
      ...
      IF NVL(p_reset_status_flag,'N') = 'Y' THEN
         update_err_child_batch_id(p_batch_id);
      END IF;
...
EXCEPTION
   WHEN OTHERS THEN
       x_retcode := 2;
       display_log('Error in child_main');
